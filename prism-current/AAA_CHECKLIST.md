...
## Current playable-water milestone

- [x] WM-01. Add the optional first-person expedition from the existing game menu, retaining every rhythm mode and score namespace.
- [x] WM-02. Implement auxiliary power, draining basin, opened floodgate, submerged prism recovery and return-to-console completion in connected 3D spaces.
- [x] WM-03. Add walk/wade/swim movement, dive clearance, surface air recovery, checkpoint rescue, torch and validated local stage saves.
- [x] WM-04. Implement controller gameplay/menu navigation plus keyboard/mouse and touch controls; no browser alert is required to close a mission dialog.
- [x] WM-05. Add planar water reflections, wave normals, bounded ripples, analytic floor refraction, stylized tile caustics and underwater fog with Light/still-water settings.
- [ ] WM-HW. Physical Xbox/touch usability, sustained named-GPU performance and human route/readability testing remain open. This is not an immersive locomotion release.

See [WATER_MISSION.md](WATER_MISSION.md). PR #141 holds actual source and public receipts. This requested content milestone does not close A-03 save export/recovery or any earlier hardware gate.

## Previous shader milestone
...
See [SPECTRAL_NOTES.md](SPECTRAL_NOTES.md) for the rendering design, controls, resource bounds and acceptance scope. [PR #127](https://github.com/v5ma/v5ma.github.io/pull/127) records its publication receipts.
...
D-01 Jewelbox art remains implemented. The spectral release implemented three shader-theme directions within D-02 and expanded D-06's screenshot coverage. It did not complete all D-02 stage/art-direction and track-specific-lighting work, nor D-06's full desktop/VR/AR/accessibility matrix. D-03 original controller/hand assets remain open. Floodgate Recovery adds a playable environment within the same ongoing visual-production effort.
...
The release PR records F-02/F-03/F-04 outcomes for this candidate; every future update must repeat them. Historical CR-HW, CR-XR, TB-06, PL-HW, FS-HUMAN and FS-XR remain open under their original definitions. Next: resolve any regression, then implement A-03 save recovery while human water, shader, lesson and music reviews continue separately.
