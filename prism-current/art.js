...
(function(root){'use strict';
 function create(T,scene){
  ...
  function ring(parent,r,tube,material,x,y,z){const o=mesh(parent,own(new T.TorusGeometry(r,tube,r<.05?6:8,r<.05?32:96)),material,x,y,z);return o;}
  ...
  const glyphs=[0,1].map(h=>Array.from({length:PrismCore.dirs.length},(_,d)=>glyph(h,d)));
...
#include <colorspace_fragment>}`});basic.set('sparkles',sparkleMaterial);
  ...
  function jewel(parent,hand,dir=0,withGlyph=true){const g=new T.Group();parent.add(g);const body=mesh(g,geometry.gem,fx.jewel(hand));body.name='64-triangle cushion-cut crystal';const bezel=mesh(g,geometry.bezel,hand?fx.gold:fx.silver);bezel.name='Reflective precious-metal setting';
   ...
   const face=mesh(g,plane,glyphs[hand][dir],0,0,.12,.29,.29,1);face.visible=withGlyph;face.renderOrder=3;
...
#include <colorspace_fragment>}`}));basic.set('sky',sky.material);
  ...
  let previewKey='',previewNotes=PrismCore.chart('first-light').notes;let quality='cinematic',intensity=.72,reduced=matchMedia('(prefers-reduced-motion: reduce)').matches;
  ...
  function update(s,time,dt,menu){if(disposed)return;menuNow=menu;if(menu){const game=scene.components?.['prism-game'],key=(game?.track||'first-light')+'/'+(game?.difficulty||'flow');if(key!==previewKey){previewKey=key;previewNotes=PrismCore.chart(game?.track||'first-light',game?.difficulty||'flow').notes;}}if(warming){menu=false;s=warming;time=warming.song.notes[0].time-.8;}const immersive=settings();const t=menu?(reduced?10:7+time%14):time;fx.update(time,((t*(s?.song.bpm||104)/60)%1)/1,intensity,reduced);sparkleMaterial.uniforms.uTime.value=reduced?0:time;sparkleMaterial.uniforms.uStrength.value=intensity*(state.ar?.45:1);
  ...
  const art={T,group,studio,lane,notes,hands,update,burst,blade,textPanel,colors,mesh,mat,neon,prepare,dispose,graphics:state,fx};art.spectral=root.PrismSpectral?.install(art,scene);art.poolStage=root.PrismPoolStage?.install(art,scene);return art;
 }
