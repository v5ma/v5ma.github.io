...
(function(root){'use strict';
 ...
 function install(g){const T=g.T,scene=g.el,stage=g.art.stage,prior=new Map(),sources={},menu=g.art.panel(1.68,1.14),hud=g.art.panel(1.75,.28),ray=new T.Raycaster();
  ...
  const cursors=[0,1].map(h=>{const m=new T.Mesh(new T.SphereGeometry(.024,10,6),new T.MeshBasicMaterial({color:h?0xffb4d3:0x9affdf,depthTest:false,depthWrite:false}));m.name='river-xr-cursor-'+h;m.renderOrder=21;m.frustumCulled=false;scene.object3D.add(m);m.visible=false;return m;});
