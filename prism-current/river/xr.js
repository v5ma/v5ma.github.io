...
(function(root){'use strict';
 const VERSION='0.10.1',WIDTH=1200,HEIGHT=814;
 ...
 function install(g){const T=g.T,scene=g.el,stage=g.art.stage,prior=new Map(),sources={},menu=g.art.panel(1.68,1.14),hud=g.art.panel(1.75,.28),ray=new T.Raycaster();
  ...
  const rays=[0,1].map(h=>{const geo=new T.BufferGeometry().setFromPoints([new T.Vector3(),new T.Vector3(0,0,-2)]),m=new T.Line(geo,new T.LineBasicMaterial({color:h?0xffb4d3:0x9affdf,depthTest:false,depthWrite:false}));m.name='river-xr-ray-'+h;m.renderOrder=20;m.frustumCulled=false;scene.object3D.add(m);m.visible=false;return m;});
  const cursors=[0,1].map(h=>{const m=new T.Mesh(new T.SphereGeometry(.014,10,6),new T.MeshBasicMaterial({color:h?0xffb4d3:0x9affdf,depthTest:false,depthWrite:false}));m.name='river-xr-cursor-'+h;m.renderOrder=21;m.frustumCulled=false;scene.object3D.add(m);m.visible=false;return m;});
  ...
  function action(i){if(!session||session.visibilityState!=='visible'||performance.now()-lastAction<120)return;if(i<0||i>7)return;lastAction=performance.now();actions++;
  ...
  function entered(){session=scene.renderer.xr.getSession();g.cancel();g.immersive=true;calibrated=false;centeredOnce=false;lastPhase='';lastAction=-Infinity;actions=0;clear();g.clearInputs();document.body.classList.add('immersive');scene.renderer.setClearColor(0x000000,scene.is('ar-mode')?0:1);
  ...
  function exited(){detach();session=null;g.immersive=false;calibrated=false;viewer=null;clear();g.cancel();stage.position.set(0,0,0);stage.rotation.set(0,0,0);stage.updateMatrixWorld(true);document.body.classList.remove('immersive');scene.renderer.setClearColor(0x0a182c,1);menu.mesh.visible=hud.mesh.visible=false;g.notice('Headset ended. Choose a fresh run.');}
  ...
  function pointAt(aim,h){const r=rays[h],cursor=cursors[h];r.visible=true;ray.set(aim.origin,aim.direction);menu.mesh.updateWorldMatrix(true,false);const hit=ray.intersectObject(menu.mesh,false)[0],index=hit?.uv?actionAtUV(hit.uv.x,hit.uv.y):-1;
  ...
  function collect(t){const output={hands:[null,null],body:[0,1.45,0]};menu.mesh.visible=!!session&&g.phase!=='playing';hud.mesh.visible=!!session;if(!session)return output;
   ...
   for(const src of input){const h=src.handedness==='right'?1:0,old=prior.get(src),aim=src.targetRaySpace&&frame.getPose(src.targetRaySpace,space),pose=src.gripSpace&&frame.getPose(src.gripSpace,space),btn=list(src.gamepad?.buttons).map(x=>!!x.pressed||x.value>.55),was=old?.btn||btn;let pinch=false;
    ...
    if(g.phase!=='playing'){
     if(dir!=='0,0'&&old&&(dir!==oldDir||stamp>=repeatAt)){focus=navigate(focus,...direction);focusMode='stick';repeatAt=stamp+(dir!==oldDir?380:160);}
     if(btn[5]&&!was[5])requested=0;
    ...
    if(!src.hand&&pose&&!pose.emulatedPosition&&['left','right'].includes(src.handedness)){
     ...
     output.hands[h]={pose:{a:a.toArray(),b:b.toArray()},origin:b.toArray(),direction:normal.toArray(),fire:!!btn[0]&&fireArmed[h],shield:{active:!!btn[1],center:a.clone().addScaledVector(normal,.32).toArray(),normal:normal.toArray(),raised,hand:h}};
  ...
  return {collect,recenter,haptic,get session(){return session},get calibrated(){return calibrated},get diagnostics(){return {version:VERSION,menuVisible:menu.mesh.visible,focus,focusMode,hover:[...hover],actions,inputCount:list(session?.inputSources).length,trackedControllers:tracked.size};},dispose(){detach();scene.removeEventListener('enter-vr',entered);scene.removeEventListener('exit-vr',exited);clear();for(const r of [...rays,...cursors]){r.removeFromParent();r.geometry.dispose();r.material.dispose();}}};
 }
