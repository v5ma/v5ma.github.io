/* Prism AR Archipelago 0.1.0. Explicit adapter over the full Friendly Current game.
...
(function(root){'use strict';
 ...
 function install(T,scene,art){
  ...
  let islands,forest,clouds,optics,disposed=false,ready=false,pending=null,currentMode='islands',shown=false;
  ...
  function warm(renderer,camera){
   ...
   for(const group of [islands.group,clouds.group])for(const m of group.children){const proxy=new T.Mesh(m.geometry,m.material);proxy.frustumCulled=false;scratch.add(proxy);}
  ...
  art.reset=function(){if(disposed)return;reset.call(art);forest.reset();clouds.reset();};
  ...
  Object.defineProperty(art,'stats',{configurable:true,get:()=>({...getStats.call(art),arScenery:{version:VERSION,mode:currentMode,visible:shown&&!disposed,prepared:ready,islands:islands.stats,trees:forest.stats,clouds:clouds.stats,optics:optics.stats}})});
  return art;
 }
