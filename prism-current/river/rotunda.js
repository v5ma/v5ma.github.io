...
(function(root){'use strict';
 ...
 function install(g,menu,hud){const T=g.T,scene=g.el,stage=g.art.stage;let prefs=preferences(null);try{prefs=preferences(localStorage.getItem(KEY));}catch{}
  ...
  function draw(f=focus,hov=[]){if(g.phase==='playing')return;focus=f;const s=g.state,key=[page,g.phase,g.chapter,g.message,g.audio.volume,g.audio.effectsVolume,g.quiet,g.cruise,g.quality,JSON.stringify(prefs),focus,...hov,s?.score,s?.health,s?.mode,support.ar,support.vr].join('|');if(key===lastPaint)return;lastPaint=key;
   ...
   if(page==='help')detail='Swing or drag fruit arrows. Triggers shoot. Grips shield. B/Y pauses.';
   if(page==='layout')detail='Panel stays planted. Stick click resets placement. Your camera never moves.';
  ...
  function key(e){if(g.immersive)return;if(e.code==='F2'){e.preventDefault();e.stopImmediatePropagation();textControls=!textControls;document.body.classList.toggle('text-controls',textControls);if(!textControls){anchor();wrap.focus();}return;}if(textControls||g.phase==='playing')return;
  ...
  function update(input,viewer){if(disposed)return;const now=performance.now(),dt=Math.min(.05,(now-lastStamp)/1000||0);lastStamp=now;
   ...
   if(now-lastHud>130){lastHud=now;const c=hud.context,w=hud.canvas.width,h=hud.canvas.height;c.clearRect(0,0,w,h);c.fillStyle='#112d40f5';c.fillRect(0,0,w,h);c.fillStyle='#b6ffe0';c.font='700 53px system-ui';c.fillText(`${s?.score||0} POINTS   ${s?.health??100} HULL   ${s?.combo||0}x`,25,64);c.font='30px system-ui';c.fillStyle='#ffffff';c.fillText(now-noticeAt<1600?notice:(s?RiverCore.phase(g.chapter,s.time).name:'River Prism'),25,117);c.font='23px system-ui';c.fillText('B / Y: rotunda     Trigger: laser     Grip: shield',25,163);hud.texture.needsUpdate=true;}
