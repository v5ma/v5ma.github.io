...
(function(){'use strict';const $=id=>document.getElementById(id),C=RiverCore,PREF='prism-current.river.settings.v1';
 AFRAME.registerComponent('river-game',{
  init(){this.T=AFRAME.THREE;this.art=RiverArt.build(this.T,this.el);this.audio=new PrismAudio();this.state=null;this.chapter='duck-armada';this.phase='menu';this.immersive=false;this.message='Water + Fire 0.11.2 / Cut fruit. Shoot engines. Grip shields.';this.cruise=false;this.quiet=matchMedia('(prefers-reduced-motion: reduce)').matches;this.quality='balanced';this.serial=0;this.busy=false;this.aim=[0,0];this.playerX=0;this.crouch=0;this.activeHand=0;this.keys=new Set();this.captures=new Map();this.previous=[null,null];this.animations=[null,null];this.shields=[null,null];this.mouse=null;this.mouseFire=false;this.touchFire=false;this.touchShield=false;this.padId=null;this.padOld=[];this.seedPad=true;this.padDir=0;this.repeatAt=0;this.lastTime=0;this.lastFx=0;this.runMode='desktop';this.records={};this.cut=[0,-1];
  ...
  ui(){for(const [id,chapter]of[['duck','duck-armada'],['space','mothership']])$(id).onclick=()=>{this.cancel();this.chapter=chapter;this.sync();};$('play').onclick=()=>this.phase==='paused'?this.resume():this.start();$('pause').onclick=()=>this.pauseRun('Paused');$('resume').onclick=()=>this.resume();$('retry').onclick=()=>this.start();$('back').onclick=()=>this.cancel();$('again').onclick=()=>this.start();$('next').onclick=()=>{this.cancel();this.chapter=this.chapter==='duck-armada'?'mothership':'duck-armada';this.sync();this.start();};$('results-back').onclick=()=>this.cancel();
   $('volume').oninput=e=>{this.audio.level(+e.target.value/100);};$('effects').oninput=e=>this.audio.effectLevel(+e.target.value/100);$('quiet').onchange=e=>{this.quiet=e.target.checked;this.sync();};$('cruise').onchange=e=>{this.cruise=e.target.checked;this.sync();};$('quality').onchange=e=>{this.quality=e.target.value;this.sync();};$('hand').onclick=()=>{this.activeHand=1-this.activeHand;this.sync();};
  ...
  sync(){document.body.dataset.phase=this.phase;$('menu').inert=!['menu','loading'].includes(this.phase);$('pause-panel').inert=this.phase!=='paused';$('results').inert=!['complete','failed','escaped'].includes(this.phase);$('duck').setAttribute('aria-pressed',this.chapter==='duck-armada');$('space').setAttribute('aria-pressed',this.chapter==='mothership');$('play').textContent=this.busy?'Preparing soundtrack...':'Play '+(this.chapter==='mothership'?'Mothership Channel':'Duck Armada');$('play').disabled=this.busy;$('volume').value=Math.round(this.audio.volume*100);$('effects').value=Math.round(this.audio.effectsVolume*100);$('quiet').checked=this.quiet;$('cruise').checked=this.cruise;$('quality').value=this.quality;$('hand').textContent=this.activeHand?'R blade / F':'L blade / F';
   try{localStorage.setItem(PREF,JSON.stringify({quiet:this.quiet,quality:this.quality,cruise:this.cruise}));}catch{}
  ...
  tick(now,delta){const dt=Math.min(.05,(delta||0)/1000);let pad=this.poll(dt,now);if(!this.immersive){if(this.phase==='playing'){const dx=(this.keys.has('KeyD')?1:0)-(this.keys.has('KeyA')?1:0);this.playerX=Math.max(-.8,Math.min(.8,this.playerX+dx*dt*1.3));if(this.padId===null)this.crouch+=((this.keys.has('Space')||this.keys.has('KeyS')?.46:0)-this.crouch)*Math.min(1,dt*12);const dx2=(this.keys.has('ArrowRight')?1:0)-(this.keys.has('ArrowLeft')?1:0),dy2=(this.keys.has('ArrowUp')?1:0)-(this.keys.has('ArrowDown')?1:0);if(dx2||dy2){const n=Math.hypot(dx2,dy2);this.cut=[dx2/n,dy2/n];}}this.el.camera.position.set(this.playerX,1.65-this.crouch,0);this.el.camera.updateMatrixWorld();}
   ...
   if(this.state){const s=this.state,p=C.phase(this.chapter,s.time);$('score').textContent=s.score.toLocaleString();$('combo').textContent=s.combo+'x';$('health').value=s.health;$('health-text').textContent=s.health+' hull';$('section').textContent=s.bossDefeated?'VICTORY LAP':p.name;$('cue').textContent=s.bossDefeated?'Boss down. Finish the song and gather the remaining fruit.':p.cue;$('clock').textContent=Math.ceil(Math.max(0,s.duration-s.time))+'s';$('progress').value=s.time/s.duration;const boss=s.entities.find(n=>n.type==='boss');$('boss').hidden=!boss;if(boss){$('boss-label').textContent=(C.open(s,boss)?'CORE OPEN / ':'SHIELDED / ')+(this.chapter==='mothership'?'MOTHERSHIP':'ADMIRAL QUACK');$('boss-health').max=boss.maxHP;$('boss-health').value=boss.hp;}
    for(const e of s.events){if(e.id<=this.lastFx)continue;this.lastFx=e.id;if(['block','destroy','damage'].includes(e.type)){this.audio.hit(e.hand||0,e.type==='damage'?.4:1);this.xr.haptic(e.hand||0);}}
