...
(function(root){'use strict';
 ...
 function build(T,scene){const C=RiverCore,stage=new T.Group(),environment=new T.Group(),actors=new T.Group(),fx=new T.Group();stage.add(environment,actors,fx);scene.object3D.add(stage);
  ...
  // A stable symbol accompanies each color. Texture creation only at construction.
  function symbol(a,h,x,y,size){a.save();a.strokeStyle='#123345';a.lineWidth=5;a.beginPath();if(h){a.moveTo(x,y-size);a.lineTo(x+size,y);a.lineTo(x,y+size);a.lineTo(x-size,y);a.closePath();}else a.arc(x,y,size,0,Math.PI*2);a.stroke();a.restore();}
  const badgeTextures=[],badgeMaterials=handColors.map((_,h)=>{const c=document.createElement('canvas');c.width=c.height=64;const a=c.getContext('2d');a.fillStyle=h?'#ffbfd8':'#afffe4';a.fillRect(0,0,64,64);symbol(a,h,32,32,19);const t=new T.CanvasTexture(c);t.colorSpace=T.SRGBColorSpace;t.generateMipmaps=false;t.minFilter=T.LinearFilter;textures.push(t);badgeTextures.push(t);return mat(new T.MeshBasicMaterial({map:t,side:T.DoubleSide,toneMapped:false}));});
  const glyphs=[];for(let h=0;h<2;h++){glyphs[h]=[];for(let d=0;d<8;d++){const c=document.createElement('canvas');c.width=c.height=128;const a=c.getContext('2d');a.fillStyle=h?'#ffbfd8':'#afffe4';a.beginPath();a.arc(64,64,61,0,Math.PI*2);a.fill();a.save();a.translate(64,50);a.scale(.72,.72);const v=C.DIRS[d];a.rotate(Math.atan2(-v[1],v[0]));a.fillStyle='#123345';a.beginPath();a.moveTo(38,0);a.lineTo(0,-29);a.lineTo(0,-10);a.lineTo(-30,-10);a.lineTo(-30,10);a.lineTo(0,10);a.lineTo(0,29);a.closePath();a.fill();a.restore();symbol(a,h,64,99,12);const t=new T.CanvasTexture(c);t.colorSpace=T.SRGBColorSpace;textures.push(t);glyphs[h][d]=mat(new T.MeshBasicMaterial({map:t,transparent:true,depthWrite:false}));}}
...
#include <colorspace_fragment>
  ...
  const weapons=[0,1].map(h=>{const g=new T.Group();g.name='prism-saber-'+h;const grip=new T.Mesh(cylinder,mat(new T.MeshStandardMaterial({color:0x314152,metalness:.8,roughness:.25})));grip.scale.set(.024,.18,.024);grip.rotation.x=Math.PI/2;g.add(grip);const blade=new T.Mesh(box,handMats[h]);blade.position.z=-.43;blade.scale.set(.023,.028,.68);g.add(blade);const badge=new T.Mesh(box,badgeMaterials[h]);badge.name='prism-saber-badge-'+h;badge.position.set(0,.030,-.07);badge.scale.set(.075,.023,.075);g.add(badge);stage.add(g);const shield=new T.Group(),disc=new T.Mesh(shieldGeo,shieldMaterial[h]),rim=new T.Mesh(torus,handMats[h]);rim.scale.set(.50,.50,.18);shield.add(disc,rim);stage.add(shield);g.visible=shield.visible=false;blade.name='prism-saber-blade-'+h;return {g,blade,shield,badge,color:h};});
