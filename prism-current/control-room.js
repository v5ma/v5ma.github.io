...
(function(root){'use strict';
 ...
 function install(g){const $=id=>document.getElementById(id);let padIndex=null,padId='',old=[],direction=0,nextRepeat=0,seed=true,connected=false,phase='',returnFocus=null;let noticeTime=-Infinity;
  ...
  function visible(el){if(el.disabled||el.closest('[inert]')||!el.getClientRects().length)return false;const details=el.closest('details');return !details||details.open||el.tagName==='SUMMARY';}
  ...
  function closeMixer(){if(popup.hidden)return;popup.hidden=true;$('menu').inert=!['menu','loading'].includes(g.phase);$('pause-panel').inert=g.phase!=='paused';$('results').inert=g.phase!=='complete';const target=visible(returnFocus||popup)?returnFocus:null;focus(target||items()[0]);}
  function openMixer(){if(g.immersive)return;returnFocus=document.activeElement;g.pauseRun('Mixer open. The track is paused.');popup.hidden=false;for(const id of ['menu','pause-panel','results'])$(id).inert=true;syncAudio();focus($('mix-music'));}
  ...
  const key=e=>{if(e.code==='Tab'&&!popup.hidden){e.preventDefault();e.stopImmediatePropagation();move(e.shiftKey?-1:1);return;}if(e.code==='Escape'&&!popup.hidden){e.preventDefault();e.stopImmediatePropagation();closeMixer();}};
  ...
  sync();return {tick,sync,syncAudio,textAllowed,openMixer,closeMixer,get connected(){return connected},dispose(){document.removeEventListener('keydown',key,true);window.removeEventListener('blur',blur);window.removeEventListener('gamepaddisconnected',disconnect);}};
 }
