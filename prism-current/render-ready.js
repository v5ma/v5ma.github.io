...
(function(root){'use strict';
 function install(art,scene){const T=art.T,original=art.prepare.bind(art),dispose=art.dispose.bind(art);let target=null;
  art.prepare=async function(song){
   ...
   // Reuse real material references, so linked programs and textures are kept.
   // All temporary objects live outside the displayed scene and tracked hands.
   ...
   try{
    ...
    // Hold the real, empty count-in layout while the normal renderer warms.
    // This avoids switching back to a large menu sculpture after preparation.
    ...
    try{
     ...
     while(stable<8&&performance.now()<deadline){const now=await new Promise(resolve=>requestAnimationFrame(resolve));stable=now-last<120?stable+1:0;last=now;}
