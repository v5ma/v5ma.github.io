...
window.runEffectsPolishFixture=async function(){
 ...
 function measure(a){let visible=0,energy=0,hot=0;for(let i=0;i<a.length;i+=4){if(a[i+3]>4)visible++;energy+=a[i]+a[i+1]+a[i+2];if(a[i+3]>10&&a[i]>a[i+2]*1.6)hot++;}return {visible,energy,hot};}
 ...
 check(metrics.smoke.visible>100&&metrics.smoke.energy<metrics.burst.energy,'Cooling smoke remains visible with lower energy than the initial burst');
