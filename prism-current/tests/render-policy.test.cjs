...
test('Runtime retains the interruption threshold, physical-input slice path and record identity',()=>{A.match(source,/t-this.lastTime>\.35/);A.match(source,/C\.slice\(this.state,h,this.previous\[h\].pose,hand.pose/);A.match(source,/this.state.chapter,this.runMode,this.state.difficulty,this.state.cruise/);});
