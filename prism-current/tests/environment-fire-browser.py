...
with sync_playwright() as pw:
    ...
    try:
        ...
        p.goto(URL,wait_until='domcontentloaded');p.wait_for_function('window.River?.snapshot().ready&&River.snapshot().stats.fire?.version==="0.1.1"')
        ...
        check(p.evaluate('AFRAME.scenes[0].renderer.info.programs.some(p=>p.name==="Currentworks Fire 0.1.1")'),'Host loading precompiles fire before the first destruction')
