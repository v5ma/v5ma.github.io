"""Native pointer correctness at a small software-renderer drawing buffer."""
...
with sync_playwright() as pw:
 ...
 try:
  ...
  (OUT/'report.json').write_text(json.dumps({'passed':len(checks),'checks':checks,'errors':errors,'scope':'Native HTTP A-Frame at one-eighth pixel ratio; actual pointerdown plus timed DOM pointer moves through the real handler. No clock, note, score or player-state writes. Correctness check, not a human accuracy or GPU performance benchmark.'},indent=2))
