...
with sync_playwright() as pw:
 ...
 try:
  p.goto(BASE+'/prism-current/rhythm.html',wait_until='domcontentloaded');p.wait_for_function('window.Prism?.snapshot().ready');p.evaluate((ROOT/'prism-current/tests/input-driver.js').read_text());p.locator('[data-track="first-light"]').click();p.locator('#difficulty').select_option('flow');p.locator('#start').click();p.wait_for_function('Prism.snapshot().phase==="playing"&&Prism.snapshot().time>1')
