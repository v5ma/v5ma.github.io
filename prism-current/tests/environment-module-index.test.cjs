...
for(const name of Object.keys(index.modules))test(name+': reusable module index matches the saved source and module facade',async()=>{
 ...
 A.equal(entry.version,api.VERSION);A.equal(esm.default,api);A.equal(globalThis[entry.global],api);A.equal(typeof api[entry.factory||'create'],'function');
 if(entry.integration)A.ok(fs.statSync(path.resolve(dir,entry.integration)).isFile());
 else {A.match(entry.status,/library-only/);A.ok(fs.statSync(path.join(dir,entry.api)).isFile());}
