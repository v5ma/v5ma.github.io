...
window.startRiverDriver=()=>{
 ...
 window.riverDriver=setInterval(()=>{const s=River.snapshot();if(s.phase!=='playing'){for(const code of [...held])key(code,false);active=null;return;}const imminent=s.entities.some(n=>['bomb','bolt'].includes(n.type)&&n.position[2]>-2.5);key('KeyQ',imminent);key('KeyE',imminent);
  ...
  const boss=s.entities.find(n=>n.type==='boss'&&n.open),enemy=boss||s.entities.find(n=>['plane','fighter','boat'].includes(n.type))||s.entities.find(n=>n.type==='catapult'&&n.position[2]>-8)||s.entities.find(n=>n.type==='bomb');
