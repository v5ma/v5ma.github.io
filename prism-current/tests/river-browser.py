"""Main-entry production playthrough. Synthetic inputs, never direct score/state writes.
...
with sync_playwright() as pw:
 ...
 try:
  ...
  check(p.locator('#play').is_visible(),'The main Play button is visible without a side-mode detour')
  ...
  p.reload(wait_until='domcontentloaded');p.wait_for_function('window.River?.snapshot().ready');check(len(p.evaluate('River.snapshot().records'))==2,'Both chapter clears persist in the new isolated namespace')
  ...
  p.goto(URL,wait_until='domcontentloaded');p.wait_for_function('window.River?.snapshot().ready&&River.snapshot().controller');p.bring_to_front();p.keyboard.press('Shift')
  ...
  p.goto(URL,wait_until='domcontentloaded');p.wait_for_function('window.River?.snapshot().ready&&!document.getElementById("enter-vr").disabled');p.locator('#enter-vr').click();p.wait_for_function('River.snapshot().immersive&&River.snapshot().calibrated')
  def button(hand,i,on):p.evaluate('([h,i,on])=>TestXR.button(h,i,on)',[hand,i,on])
  p.evaluate('TestXR.pose("left",[-.4,1.53,-.4])');button('left',0,False);p.wait_for_timeout(120);button('left',0,True);p.wait_for_timeout(150);button('left',0,False);p.wait_for_function("River.snapshot().phase==='playing'")
  ...
  p.evaluate('TestXR.state.handMode=true;TestXR.pose("left",[.4,1.53,-.4]);TestXR.state.pinch=false');p.wait_for_timeout(200);p.evaluate('TestXR.state.pinch=true');p.wait_for_timeout(250)
