...
with sync_playwright() as pw:
    ...
    try:
        ...
        check(snap()['rotunda']['preferences']['arScenery']=='minimal' and not snap()['stats']['arScenery']['grass']['visible'],'Point-and-trigger Minimal scenery actually hides islands and grass')
        ...
        check(snap()['stats']['arScenery']['prepared'],'Island, tree, cloud and grass buffers are prepared before gameplay')
        check(p.evaluate('AFRAME.scenes[0].renderer.info.programs.some(p=>p.name==="Currentworks Grass 0.1.0")'),'The actual grass shader compiles in the existing AR renderer')
        ...
        check(before['time']==after['time'] and before['stats']['arScenery']['clouds']['time']==after['stats']['arScenery']['clouds']['time'] and before['stats']['arScenery']['grass']==after['stats']['arScenery']['grass'],'Pausing freezes the host, cloud and grass animation')
