...
with sync_playwright() as pw:
 ...
 try:
  ...
  weak=p.evaluate('PrismPhrases.summary(Prism.component.state).weakest')
  press(13);press(13);check(p.locator('#practice-weakest').evaluate('(e)=>e===document.activeElement'),'Controller reaches the weakest-section shortcut')
  press(0);p.wait_for_function("Prism.snapshot().phase==='playing'")
  check(p.evaluate('Prism.snapshot().practice.active.section')==weak,'The result shortcut starts the actual weakest section')
  press(9)
  for _ in range(3):press(13)
  press(0);check(p.evaluate('Prism.snapshot().phase')=='menu','Controller exits the unfinished targeted practice without a mouse')
  check(p.evaluate('Object.keys(Prism.snapshot().practice.records).length')==0,'An aborted targeted practice creates no completed practice record')
