...
(function(root){'use strict';
 const Tidal=root.PrismTidal||(typeof require==='function'?require('./tidal-bloom.js'):null);
 const Undertow=root.PrismUndertow||(typeof require==='function'?require('./undertow.js'):null);
 const VERSION='0.9.0',WINDOW=.17,SPEED=3.4,PLANE=-1.05,SIZE=.17;
 ...
 const dirs=[[0,-1],[0,1],[1,0],[-1,0],[.707,-.707],[-.707,-.707],[0,0],[.707,.707],[-.707,.707]];
 ...
 if(Tidal)TRACKS.push(Tidal.TRACK);if(Undertow)TRACKS.push(Undertow.TRACK);
 function chart(id='first-light',difficulty='flow'){
  const song=TRACKS.find(s=>s.id===id);if(!song||!['flow','pulse'].includes(difficulty))throw Error('Unknown track or chart');if(id==='undertow')return Undertow.chart(difficulty);if(id==='tidal-bloom')return Tidal.chart(difficulty);const notes=[];
 ...
 function create(song,options={}){if(!song?.notes?.length)throw Error('A chart is required');return {song,mode:options.mode||'slice',reach:clamp(Number(options.reach)||1,.7,1.1),state:'ready',time:0,judged:{},judgmentDetails:{},hits:0,misses:0,bad:0,combo:0,best:0,score:0,quality:0,last:null};}
 ...
 function finish(s,n,type,quality=0,hitTime=null){if(s.judged[n.id])return null;s.judged[n.id]=type;s.judgmentDetails[n.id]={type,quality,offsetMs:Number.isFinite(hitTime)?(hitTime-n.time)*1000:null};s.last={id:n.id,type,quality,time:s.time};if(type==='hit'){s.hits++;s.combo++;s.best=Math.max(s.best,s.combo);s.quality+=quality;const multiplier=Math.min(8,2**Math.floor(s.combo/8));s.score+=Math.round(100*quality)*multiplier;}else{s.combo=0;s[type==='miss'?'misses':'bad']++;}return s.last;}
 ...
 function slice(s,hand,previous,current,t0,t1){
  ...
  for(const n of s.song.notes){if(s.judged[n.id]||t0>n.time+WINDOW||t1<n.time-WINDOW)continue;
   ...
   const quality=clamp(.45+.3*(1-Math.abs(hitTime-n.time)/WINDOW)+.25*(1-closest/SIZE),.45,1);events.push(finish(s,n,'hit',quality,hitTime));
