...
export function createSpatialXR({scene,stage,view,getState,release,openOptions}){
 ...
 const basis=createHeadingLatch();
 ...
 function draw(renderer,camera){const mode=effective();mini.visible=mode==='diorama';headWorld.visible=mode==='first-person';blocker.visible=aim.visible=false;if(mode!=='first-person')headBlocked=false;if(mode==='theatre')return false;
  const g=geometry.root,parent=g.parent,position=g.position.clone(),rotation=g.quaternion.clone(),scale=g.scale.clone(),s=getState();
  ...
  }finally{parent?.add(g);g.position.copy(position);g.quaternion.copy(rotation);g.scale.copy(scale);g.updateWorldMatrix(true,true);geometry.setPresentation('desktop');}
 }
