...
export function createSpatialXR({scene,stage,view,getState,release,openOptions}){
 ...
 function setMode(v){if(!['theatre','diorama','first-person'].includes(v)||v!=='theatre'&&!geometry.available()||v==='first-person'&&sessionMode==='immersive-ar')return false;desired=v;fpOrigin=null;headBlocked=false;snapArmed=false;release();basis.reset();return true;}
 async function begin(value,newSession,reference,type='immersive-vr'){desired=value;session=newSession;sessionMode=type;headBlocked=false;viewer=fpOrigin=null;hitPose=null;placement='manual';basis.reset();snapArmed=false;applyMini();hitSource?.cancel?.();hitSource=null;
