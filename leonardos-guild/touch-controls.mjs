...
export function createTouchControls({stick,surface,active,onPress,onHold,show}){
 ...
 function reset(){const id=axis.pointer,camId=camera?.id;axis.reset();camera=null;if(id!==null)release(stick,id);if(camId!==undefined)release(surface,camId);lookX=lookY=0;for(const [pointer,{button,code}]of [...presses]){presses.delete(pointer);if(code)onHold(code,false);button.classList.remove('held');release(button,pointer);}paint();}
