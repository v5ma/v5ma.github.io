import {environmentOptions} from './environment.mjs';
import {createVaultUI} from './vault-ui.mjs';
...
import {reviewRoadAction,commitRoadReview} from './road-core.mjs';
import {bindGraphicsRecovery} from './xr-startup-recovery.mjs';
...
import {attachQuarter,inQuarter} from './quarter-core.mjs';
...
import {chooseTool,reloadSling,toggleCover,specialAbility,fireTool} from './resonance-core.mjs';
...
let preferences={touch:!!isTouch,quality:isTouch?'balanced':'high',distance:1,environment:environmentOptions()};
try{const v=JSON.parse(localStorage.getItem(preferenceKey));if(v){preferences.environment=environmentOptions(v.environment);if(typeof v.touch==='boolean')preferences.touch=v.touch;if(['high','balanced','low'].includes(v.quality))preferences.quality=v.quality;if([.8,1,1.4].includes(v.distance))preferences.distance=v.distance;}}catch{}
...
let thumb=null,lastMode=null,lifeUI=null,streetUI=null,cityUI=null,doorsUI=null,consoleUI=null,frontierUI=null,pad=null,xr=null,quarterUI=null,spatialUI=null,roadUI=null,vaultUI=null;function showTouch(){document.body.classList.toggle('touch',preferences.touch);}showTouch();
let storage=true,saved=null,initialRaw=null;
try{initialRaw=localStorage.getItem(SAVE_KEY);saved=readSave(initialRaw,world);}catch{storage=false;}
let graphics=null;
let state=sessionState(saved),view,playing=false,paused=false,keys=new Set(),touch=new Set(),accumulator=0,last=0,lastSave='',lastUI=0,jump=false,look=0,drag=null,audio=null,flash='';
audio=createResonanceAudio({getState:()=>state,world});
const durable=createRoadStorage({storage:{getItem:key=>localStorage.getItem(key),setItem:(key,value)=>localStorage.setItem(key,value)},key:SAVE_KEY,initialRaw,serialize:s=>JSON.stringify(saveData(s)),onConflict:()=>{clearInput();if(playing)pause();}});
function persistState(candidate){durable.write(candidate);lastSave=JSON.stringify(saveData(candidate));storage=true;$('save-status').textContent='Progress saved on this device';return true;}
function save(){try{const value=JSON.stringify(saveData(state));if(value===lastSave)return !durable.inspect().conflict;return persistState(state);}catch(e){storage=false;$('save-status').textContent='Not saved: '+e.message;return false;}}
window.addEventListener('storage',e=>{if(durable.changed(e))$('save-status').textContent='Another tab changed this adventure. Reload to continue; this tab will not overwrite it.';});
...
$('reset').onclick=()=>resetDialog.showModal();$('reset-cancel').onclick=()=>resetDialog.close();$('reset-accept').onclick=()=>{state=sessionState();lastSave='';save();resetDialog.close();$('pause-dialog').close();view?.update(0,state,{snap:true});};resetDialog.addEventListener('close',()=>setPause(false));
$('menu-return').onclick=()=>{playing=false;setPause(true);$('pause-dialog').close();$('hud').hidden=true;$('menu').hidden=false;$('menu').style.display='flex';$('start').textContent='Continue on screen';};
...
window.addEventListener('keydown',e=>{if(/INPUT|TEXTAREA|SELECT/.test(e.target.tagName)||!playing)return;
 if((e.code==='KeyP'||e.code==='Escape')&&!e.repeat){if(resetDialog.open)resetDialog.close();else if(vaultUI?.close()){}else if(roadUI?.close()){}else if(spatialUI?.close()){}else if(quarterUI?.close()){}else if(frontierUI?.close()){}else if(consoleUI?.close()){}else if(doorsUI?.close()){}else if(cityUI?.close()){}else if(streetUI?.close()){}else if(lifeUI?.close()){}else if($('settings-dialog').open)$('settings-dialog').close();else if($('shop-dialog').open)$('shop-dialog').close();else if($('map-dialog').open)$('map-dialog').close();else if($('pause-dialog').open)$('pause-dialog').close();else pause();e.preventDefault();return;}
 if(paused||consoleUI?.wheelActive())return;if(handled.has(e.code))e.preventDefault();keys.add(e.code);if(e.repeat)return;
 if(['KeyG','KeyI'].includes(e.code)&&vaultUI?.interact())return;
 if(['KeyG','KeyI'].includes(e.code)&&roadUI?.interact())return;
 if(inQuarter(state)&&['KeyG','KeyN'].includes(e.code)){quarterUI.open();return;}
 if(inQuarter(state)&&['KeyG','KeyI','KeyT','KeyY','KeyV','KeyN','KeyE','KeyX'].includes(e.code)){quarterUI.interact();return;}if(e.code==='KeyI'&&quarterUI.interact())return;
 if(inBadlands(state)&&['KeyG','KeyI','KeyT','KeyY','KeyV','KeyN'].includes(e.code)){frontierUI.open();return;}if(e.code==='KeyI'&&frontierUI.interact())return;if(e.code==='KeyU'){consoleUI.dispatch();return;}if(e.code==='Tab'){consoleUI.openWheel('tools');return;}if(e.code==='KeyG'){doorsUI.open();return;}if(e.code==='KeyO'){dodgeDoor(state);return;}if(e.code==='KeyI'){cityUI.open();return;}if(e.code==='KeyV'){streetUI.open();return;}if(e.code==='KeyY'){streetUI.interact();return;}if(e.code==='KeyN'){lifeUI.note();return;}if(e.code==='KeyT'){lifeUI.talk();return;}if(e.code==='KeyR'){cast(state);return;}
...
consoleUI=createResonanceUI({getState:()=>state,world,audio,setPause,save,active:()=>playing&&!paused,playing:()=>playing,actions:{road:()=>roadUI.open('story'),pack:()=>roadUI.open('pack'),nearby:()=>{if(!quarterUI.interact()&&!frontierUI.interact())cityUI.open();},guide:()=>inQuarter(state)?quarterUI.open():inBadlands(state)?frontierUI.open('contracts'):doorsUI.open('adventures'),journal:()=>inQuarter(state)?quarterUI.open():inBadlands(state)?frontierUI.open('pack'):lifeUI.note(),expeditions:()=>inQuarter(state)?quarterUI.open():frontierUI.open(),map}});
function consoleJump(){
 if(inQuarter(state)||inBadlands(state)){jump=true;return;}
...
const quickTools=createQuickTools(state.resonance.tool);
function quickTool(){quickTools.observe(state.resonance.tool);const id=quickTools.target();if(chooseTool(state,id,state.resonance.variants[id]||0)){quickTools.observe(id);save();}}
const gameActions={objective:()=>({title:missionText(state).title,hint:state.toastT>0?state.toast:district(state)}),quickTool,presentation:()=>spatialUI?.open(),
 start,pause,map,jump:consoleJump,dodge:()=>dodgeDoor(state),interact:()=>{if(!vaultUI?.interact()&&!roadUI?.interact()&&!quarterUI.interact()&&!frontierUI.interact())doorsUI.interact();},vehicle:()=>enterExit(state,world),
 throwLeft:()=>{if(!state.doors.level&&!state.life.inside)throwPaper(state,world,1);},throwRight:()=>{if(!state.doors.level&&!state.life.inside)throwPaper(state,world,-1);},attack:()=>attack(state,world),recenter:()=>view?.recenter(),guide:()=>inQuarter(state)?quarterUI.open():inBadlands(state)?frontierUI.open():doorsUI.open('adventures'),journal:()=>inQuarter(state)?quarterUI.open():inBadlands(state)?frontierUI.open('pack'):lifeUI.note(),magic:()=>cast(state),scan:()=>scan(state),
...
// Same commands on the screen and scene desk; nested Back retains the parent.
for(const [id,label,run]of [['map','Map and next goal',map],['missions','Adventures and missions',gameActions.guide],['inventory','Inventory and original notebook',()=>roadUI.open('pack')],['audio','Sound and controls',()=>consoleUI.openAudio()]]){const b=document.createElement('button');b.id='desk-'+id;b.textContent=label;b.onclick=run;$('resume').before(b);}
const journey=createRoadTravel({save:()=>{setPause(true);return persistState(state);},getSession:()=>view?.renderer.xr.getSession(),navigate:url=>location.assign(url),baseURL:location.href});
roadUI=createRoadUI({getState:()=>state,playing:()=>playing,setPause,persist:persistState,onChange:text=>{tell(state,text);view?.update(0,state,{snap:true});updateUI();},travel:journey,legacyJournal:gameActions.journal,legacyFrontier:()=>frontierUI.open(),survey:()=>vaultUI.open()});
vaultUI=createVaultUI({getState:()=>state,playing:()=>playing,setPause,persist:persistState,onChange:text=>{tell(state,text);view?.update(0,state,{snap:true});updateUI();},frontier:()=>frontierUI.open()});
const vaultTitle=document.createElement('button');vaultTitle.id='vault-start';vaultTitle.disabled=true;vaultTitle.textContent='Play Lantern Vault survey / keep my adventure';$('start').before(vaultTitle);vaultTitle.onclick=()=>{state.vault.tracking=true;state.road.tracking=false;start();save();updateUI();};
const vaultDesk=document.createElement('button');vaultDesk.id='desk-vault';vaultDesk.textContent='Lantern Vault / tools and field notes';vaultDesk.onclick=()=>vaultUI.open();$('resume').before(vaultDesk);
const storyTitle=document.createElement('button');storyTitle.id='road-start';storyTitle.disabled=true;storyTitle.textContent='Play The Road Beyond the Lanterns';$('start').before(storyTitle);storyTitle.onclick=()=>{state.vault.tracking=false;state.road.tracking=true;state.road.selected='';start();save();updateUI();};
const storyDesk=document.createElement('button');storyDesk.id='desk-road';storyDesk.textContent='Lantern road / story, shops and frontier';storyDesk.onclick=()=>roadUI.open('story');$('resume').before(storyDesk);
if(new URLSearchParams(location.search).get('journey')==='return'&&state.road.away){const q=reviewRoadAction(state,'returned');const r=commitRoadReview(state,q,persistState);if(!r.ok)$('save-status').textContent=r.error;}
pad=createGamepad({getState:()=>state,playing:()=>playing,active:()=>playing&&!paused,getPreferences:()=>consoleUI.preferences,actions:gameActions});
audio.setHaptics(kind=>pad.rumble(kind));
$('dispatch-button').onclick=()=>consoleUI.dispatch();
$('adventures-button').onclick=()=>inBadlands(state)?frontierUI.open():doorsUI.open();
for(const suffix of ['title','pause']){const block=document.createElement('div');block.className='pad-supplement';block.innerHTML=`<button id="${suffix}-settings">Controls and graphics</button><button id="${suffix}-sound">Audio and controller</button>`;$(suffix==='title'?'xr-launcher':'xr-pause-launcher').after(block);$(suffix+'-settings').onclick=()=>{setPause(true);$('settings-dialog').showModal();};$(suffix+'-sound').onclick=()=>consoleUI.openAudio();}

thumb=createTouchControls({stick:$('move-stick'),surface:$('world'),active:()=>playing&&!paused,show:()=>{},onPress:touchAction,onHold:(code,down)=>{if(!down){touch.delete(code);return;}if(!playing||paused)return;if(code==='hack'&&vaultUI?.interact())return;if(code==='hack'&&distance(state,world.shop)<7&&Math.abs(state.speed)<1.7){openShop();return;}touch.add(code);if(code==='q')throwPaper(state,world,1);if(code==='c')throwPaper(state,world,-1);if(code==='hack')scan(state);}});
const environmentMotion=$('environment-motion'),environmentOpacity=$('environment-opacity');
function paintEnvironment(){environmentMotion.textContent='Water and foliage motion: '+(preferences.environment.quiet?'Still':'Flowing');environmentOpacity.textContent='AR water opacity: '+Math.round(preferences.environment.arWaterOpacity*100)+'%';}
environmentMotion.onclick=()=>{preferences.environment.quiet=!preferences.environment.quiet;view?.configureEnvironment(preferences.environment);paintEnvironment();savePreferences();};
environmentOpacity.onclick=()=>{const values=[0,.25,.45,.65,.85,1],value=preferences.environment.arWaterOpacity;preferences.environment.arWaterOpacity=values.find(v=>v>value+.001)??0;view?.configureEnvironment(preferences.environment);paintEnvironment();savePreferences();};
paintEnvironment();$('show-joystick').checked=preferences.touch;$('graphics-quality').value=preferences.quality;$('camera-distance').value=preferences.distance;
...
function updateUI(){if(state.mode!==lastMode){thumb?.reset();touch.clear();lastMode=state.mode;document.querySelectorAll('.foot-action').forEach(b=>b.hidden=state.mode!=='foot');}const p=missionText(state),set=(id,text)=>{if($(id).textContent!==text)$(id).textContent=text;};set('mission-tag',p.tag);set('mission-title',p.title);set('mission-description',p.text);set('target-distance',Math.round(distance(state,activeTarget(state,world)))+' m');set('district',district(state).toUpperCase());set('mode-name',state.mode==='bike'?(state.life.bike==='standard'?'LEONARDO’S BICYCLE':state.life.bike.toUpperCase()+' BICYCLE'):state.mode==='car'?'PEDAL CARRIAGE':'ON FOOT / GUILD APPRENTICE');set('speed',String(Math.round(Math.abs(state.speed)*3.6)).padStart(2,'0'));set('papers',String(state.papers));set('score',String(state.score));set('credits',String(state.credits));$('health').style.width=Math.min(100,state.health/stats(state).maxHealth*100)+'%';set('trace-status',state.trace>.1?'LOCAL COMMOTION '+Math.round(state.trace*100)+'%':'FIELD DESK / '+district(state).toUpperCase());$('toast').classList.toggle('visible',state.toastT>0);set('toast',state.toast);flash=state.toast;
 ...
 if(distance(state,world.depot)<8)hint='Leonardo’s workshop · hold H to report / replenish';if(distance(state,world.shop)<7)hint='MARKET STALL · Stop, then F or B to trade';if(distance(state,world.bandit)<14&&!state.defeated)hint=state.mode==='foot'?'J: STAFF STRIKE · Hold K: BRACE · F: remount':'GUARD AHEAD · Stop and press F to dismount';if(distance(state,world.newsroom)<8&&state.defeated&&!state.folio)hint='H: Recover Leonardo’s stolen folio';if(preferences.touch){hint=state.mode==='foot'?'Joystick walks / Drag to look / Ride to mount':'Joystick steers / Boost to pedal / Brake to stop';if(node)hint='Stop nearby / hold Inspect to operate';if(distance(state,world.shop)<7)hint='ARTISANS / Stop, then tap Inspect to trade';if(distance(state,world.bandit)<14&&!state.defeated)hint=state.mode==='foot'?'STAFF to strike / Hold BRACE during the windup':'Guard ahead / Stop and tap Ride to dismount';if(distance(state,world.depot)<8)hint='LEONARDO / Hold Inspect to report or replenish';}if(safeTown(state)&&distance(state,world.bandit)<14&&!state.defeated)hint='X / speak to the watchman. Vinci is safe.';set('context',roadUI?.hint()||hint);$('duel').hidden=state.defeated||distance(state,world.bandit)>16;$('duel-health').value=state.banditHP;$('duel-warning').textContent=state.banditPhase==='windup'?'Guard winding up — hold K to brace':'J strikes · K braces · You may retreat';$('hack').style.display=state.hackProgress>0?'block':'none';$('hack').querySelector('i').style.transform=`scaleX(${state.hackProgress})`;drawMap($('minimap'));if($('map-dialog').open)drawMap($('city-map'),true);lifeUI?.update();streetUI?.update();cityUI?.update();doorsUI?.update();consoleUI?.update();frontierUI?.update();quarterUI?.update();if(vaultUI?.hint())set('context',vaultUI.hint());
}
function frame(now,xrFrame){if(graphics?.isLost()){last=now;return;}const dt=last?Math.min(.1,(now-last)/1000):0;last=now;quickTools.observe(state.resonance.tool);pad?.poll(now,dt);xr?.poll(now,dt,xrFrame);const input=controls(dt),hidden=gamePageHidden(document.hidden,xr?.visible?.());
 if(playing&&!paused&&!hidden){accumulator=Math.min(.18,accumulator+dt*consoleUI.timeScale());while(accumulator>=1/60){step(state,world,input,1/60);input.jump=false;jump=false;accumulator-=1/60;}if(now-lastUI>80){updateUI();lastUI=now;}save();}
 else {accumulator=0;consoleUI.update();}
 xr?.beforeGame();view.update(playing&&!paused?dt*consoleUI.timeScale():0,state,{immersive:!!xr?.presenting(),immersiveAR:['alpha-blend','additive'].includes(view.renderer.xr.getSession()?.environmentBlendMode),consoleCamera:input.consoleCamera,steer:input.steer,look:playing&&!paused?input.look:0,lookY:playing&&!paused?input.lookY:0});xr?.afterGame();look=0;
 audio.update(state,dt,{playing,paused,hidden,cameraYaw:view.heading()});
}
try{
 const quality=preferences.quality;view=createScene($('world'),world,state,quality);view.setDistance(preferences.distance);view.configureEnvironment(preferences.environment);await view.prepareEnvironment();xr=createGuildXR({renderer:view.renderer,view,getState:()=>state,playing:()=>playing,active:()=>playing&&!paused,actions:gameActions,ui:pad.ui,consoleUI,clearInput});spatialUI=createSpatialOptions({getXR:()=>xr,setPause,getState:()=>state});const viewButton=document.createElement('button');viewButton.id='guild-spatial-button';viewButton.textContent='XR view and diorama settings';viewButton.onclick=()=>spatialUI.open();$('xr-pause-launcher').after(viewButton);window.addEventListener('resize',()=>view.resize());graphics=bindGraphicsRecovery({canvas:$('world'),failure:$('failure'),detail:$('failure-detail'),pause,release:clearInput,restore:()=>{last=0;view.update(0,state,{snap:true});}});
 $('start').disabled=false;storyTitle.disabled=false;vaultTitle.disabled=false;$('start').textContent=inQuarter(state)?'Continue Quarter on screen':saved?'Continue Vinci on screen':'Play full Vinci on screen';if(saved)$('resume-info').textContent='Your saved deliveries and story progress are ready.';if(!storage)$('resume-info').textContent='Storage is blocked. Play is available, but progress will not be saved.';
 Object.defineProperty(window,'LeonardoGuild',{value:Object.freeze({version:VERSION,inspect:()=>({build:PLAYTEST_BUILD,version:VERSION,running:playing&&!paused,paused,mode:state.mode,x:state.x,z:state.z,yaw:state.yaw,speed:state.speed,lift:state.lift,steps:state.steps,mission:state.mission,relay:state.relay,completed:state.completed,deliveries:[...state.deliveries],papers:state.papers,credits:state.credits,score:state.score,health:state.health,scan:state.scan,trace:state.trace,events:state.events.map(e=>({...e})),vehicles:JSON.parse(JSON.stringify(state.vehicle)),target:{...activeTarget(state,world)},folio:state.folio,defeated:state.defeated,banditHP:state.banditHP,upgraded:state.upgraded,guarding:state.guarding,saveKey:SAVE_KEY,vault:JSON.parse(JSON.stringify(state.vault)),vaultUI:vaultUI.inspect(),road:JSON.parse(JSON.stringify(state.road)),roadUI:roadUI.inspect(),saveConflict:durable.inspect().conflict,controller:pad.inspect(),quickTools:quickTools.inspect(),xr:xr?.inspect(),graphics:graphics?.inspect(),audio:audio.inspect(),console:consoleUI.inspect(),resonance:JSON.parse(JSON.stringify(state.resonance)),quarter:JSON.parse(JSON.stringify(state.quarter)),quarterUI:quarterUI.inspect(),frontier:JSON.parse(JSON.stringify(state.frontier)),doors:JSON.parse(JSON.stringify(state.doors)),cycle:JSON.parse(JSON.stringify(state.cycle)),life:JSON.parse(JSON.stringify(state.life)),city:JSON.parse(JSON.stringify(state.city)),street:JSON.parse(JSON.stringify(state.street)),attributes:stats(state),townSize:{...world.limits},input:thumb.inspect(),touchEnabled:preferences.touch,render:view.inspect()})})});
 updateUI();view.renderer.setAnimationLoop(frame);
}catch(e){console.error(e);$('failure-detail').textContent=String(e.message||e);$('failure').hidden=false;$('start').textContent='Graphics unavailable';document.querySelectorAll('[data-xr-entry-status]').forEach(n=>n.textContent='The game renderer could not start: '+String(e.message||e));}
