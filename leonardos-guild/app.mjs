...
function sessionState(saved=null){return attachFrontier(newState(saved),saved?.frontier);}
...
let thumb=null,lastMode=null,lifeUI=null,streetUI=null,cityUI=null,doorsUI=null,consoleUI=null,frontierUI=null,pad=null,xr=null;function showTouch(){document.body.classList.toggle('touch',preferences.touch);}showTouch();
...
window.addEventListener('keydown',e=>{if(/INPUT|TEXTAREA|SELECT/.test(e.target.tagName)||!playing)return;
 if((e.code==='KeyP'||e.code==='Escape')&&!e.repeat){if(resetDialog.open)resetDialog.close();else if(frontierUI?.close()){}else if(consoleUI?.close()){}else if(doorsUI?.close()){}else if(cityUI?.close()){}else if(streetUI?.close()){}else if(lifeUI?.close()){}else if($('settings-dialog').open)$('settings-dialog').close();else if($('shop-dialog').open)$('shop-dialog').close();else if($('map-dialog').open)$('map-dialog').close();else if($('pause-dialog').open)$('pause-dialog').close();else pause();e.preventDefault();return;}
...
consoleUI=createResonanceUI({getState:()=>state,world,audio,setPause,save,active:()=>playing&&!paused,playing:()=>playing,actions:{nearby:()=>{if(!frontierUI.interact())cityUI.open();},guide:()=>inBadlands(state)?frontierUI.open('contracts'):doorsUI.open('adventures'),journal:()=>inBadlands(state)?frontierUI.open('pack'):lifeUI.note(),expeditions:()=>frontierUI.open(),map}});
function consoleJump(){
 if(inBadlands(state)){jump=true;return;}
...
const gameActions={quickTool,
 start,pause,map,jump:consoleJump,dodge:()=>dodgeDoor(state),interact:()=>{if(!frontierUI.interact())doorsUI.interact();},vehicle:()=>enterExit(state,world),
 throwLeft:()=>{if(!state.doors.level&&!state.life.inside)throwPaper(state,world,1);},throwRight:()=>{if(!state.doors.level&&!state.life.inside)throwPaper(state,world,-1);},attack:()=>attack(state,world),recenter:()=>view?.recenter(),guide:()=>inBadlands(state)?frontierUI.open():doorsUI.open('adventures'),journal:()=>inBadlands(state)?frontierUI.open('pack'):lifeUI.note(),magic:()=>cast(state),scan:()=>scan(state),
