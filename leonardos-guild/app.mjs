...
function sessionState(saved=null){return createCampaignState(saved,location.search);}
...
let thumb=null,lastMode=null,lifeUI=null,streetUI=null,cityUI=null,doorsUI=null,consoleUI=null,frontierUI=null,pad=null,xr=null,quarterUI=null,spatialUI=null;function showTouch(){document.body.classList.toggle('touch',preferences.touch);}showTouch();
...
window.addEventListener('keydown',e=>{if(/INPUT|TEXTAREA|SELECT/.test(e.target.tagName)||!playing)return;
 if((e.code==='KeyP'||e.code==='Escape')&&!e.repeat){if(resetDialog.open)resetDialog.close();else if(spatialUI?.close()){}else if(quarterUI?.close()){}else if(frontierUI?.close()){}else if(consoleUI?.close()){}else if(doorsUI?.close()){}else if(cityUI?.close()){}else if(streetUI?.close()){}else if(lifeUI?.close()){}else if($('settings-dialog').open)$('settings-dialog').close();else if($('shop-dialog').open)$('shop-dialog').close();else if($('map-dialog').open)$('map-dialog').close();else if($('pause-dialog').open)$('pause-dialog').close();else pause();e.preventDefault();return;}
...
const gameActions={objective:()=>({title:missionText(state).title,hint:state.toastT>0?state.toast:district(state)}),quickTool,presentation:()=>spatialUI?.open(),
 start,pause,map,jump:consoleJump,dodge:()=>dodgeDoor(state),interact:()=>{if(!quarterUI.interact()&&!frontierUI.interact())doorsUI.interact();},vehicle:()=>enterExit(state,world),
...
for(const [id,label,run]of [['map','Map and next goal',map],['missions','Adventures and missions',gameActions.guide],['inventory','Journal and inventory',gameActions.journal],['audio','Sound and controls',()=>consoleUI.openAudio()]]){const b=document.createElement('button');b.id='desk-'+id;b.textContent=label;b.onclick=run;$('resume').before(b);}
