...
function sessionState(saved=null){return attachQuarter(attachFrontier(newState(saved),saved?.frontier),saved?.quarter,{fresh:!saved,legacy:new URLSearchParams(location.search).get('district')==='legacy'});}
...
const gameActions={quickTool,presentation:()=>spatialUI?.open(),
...
try{
 ...
 $('start').disabled=false;$('start').textContent=saved?'Continue on screen':'Play on screen';if(saved)$('resume-info').textContent='Your saved deliveries and story progress are ready.';if(!storage)$('resume-info').textContent='Storage is blocked. Play is available, but progress will not be saved.';
