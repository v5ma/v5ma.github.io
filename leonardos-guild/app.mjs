...
function frame(now,xrFrame){const dt=last?Math.min(.1,(now-last)/1000):0;last=now;quickTools.observe(state.resonance.tool);pad?.poll(now,dt);xr?.poll(now,dt,xrFrame);const input=controls(dt),hidden=gamePageHidden(document.hidden,xr?.visible?.());
