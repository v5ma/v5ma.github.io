...
function sessionState(saved=null){const s=createCampaignState(saved,location.search);if((!saved&&!new URLSearchParams(location.search).has('district'))||new URLSearchParams(location.search).get('chapter')==='lantern-road')s.road.tracking=true;return s;}
...
let thumb=null,lastMode=null,lifeUI=null,streetUI=null,cityUI=null,doorsUI=null,consoleUI=null,frontierUI=null,pad=null,xr=null,quarterUI=null,spatialUI=null,roadUI=null;function showTouch(){document.body.classList.toggle('touch',preferences.touch);}showTouch();
...
window.addEventListener('keydown',e=>{if(/INPUT|TEXTAREA|SELECT/.test(e.target.tagName)||!playing)return;
 if((e.code==='KeyP'||e.code==='Escape')&&!e.repeat){if(resetDialog.open)resetDialog.close();else if(roadUI?.close()){}else if(spatialUI?.close()){}else if(quarterUI?.close()){}else if(frontierUI?.close()){}else if(consoleUI?.close()){}else if(doorsUI?.close()){}else if(cityUI?.close()){}else if(streetUI?.close()){}else if(lifeUI?.close()){}else if($('settings-dialog').open)$('settings-dialog').close();else if($('shop-dialog').open)$('shop-dialog').close();else if($('map-dialog').open)$('map-dialog').close();else if($('pause-dialog').open)$('pause-dialog').close();else pause();e.preventDefault();return;}
...
const gameActions={objective:()=>({title:missionText(state).title,hint:state.toastT>0?state.toast:district(state)}),quickTool,presentation:()=>spatialUI?.open(),
 start,pause,map,jump:consoleJump,dodge:()=>dodgeDoor(state),interact:()=>{if(!roadUI?.interact()&&!quarterUI.interact()&&!frontierUI.interact())doorsUI.interact();},vehicle:()=>enterExit(state,world),
...
roadUI=createRoadUI({getState:()=>state,playing:()=>playing,setPause,persist:persistState,onChange:text=>{tell(state,text);view?.update(0,state,{snap:true});updateUI();},travel:journey,legacyJournal:gameActions.journal,legacyFrontier:()=>frontierUI.open()});
const storyTitle=document.createElement('button');storyTitle.id='road-start';storyTitle.disabled=true;storyTitle.textContent='Play The Road Beyond the Lanterns';$('start').before(storyTitle);storyTitle.onclick=()=>{state.road.tracking=true;state.road.selected='';start();save();updateUI();};
...
function drawMap(canvas,full=false){if(quarterUI?.drawMap(canvas,full))return;if(frontierUI?.drawMap(canvas,full))return;const g=canvas.getContext('2d'),w=canvas.width,h=canvas.height,scale=full?Math.min(w/330,h/650):1.55,ox=full?w/2:w/2-state.x*scale,oy=full?55:-state.z*scale+h*.52;
