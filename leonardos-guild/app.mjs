...
import {inBadlands,safeTown,nearbyFrontier} from './frontier-core.mjs';
...
let thumb=null,lastMode=null,lifeUI=null,streetUI=null,cityUI=null,doorsUI=null,frontierUI=null,consoleUI=null,pad=null;function showTouch(){document.body.classList.toggle('touch',preferences.touch);}showTouch();
...
window.addEventListener('keydown',e=>{if(/INPUT|TEXTAREA|SELECT/.test(e.target.tagName)||!playing)return;
 if((e.code==='KeyP'||e.code==='Escape')&&!e.repeat){if(resetDialog.open)resetDialog.close();else if(consoleUI?.close()){}else if(frontierUI?.close()){}else if(doorsUI?.close()){}else if(cityUI?.close()){}else if(streetUI?.close()){}else if(lifeUI?.close()){}else if($('settings-dialog').open)$('settings-dialog').close();else if($('shop-dialog').open)$('shop-dialog').close();else if($('map-dialog').open)$('map-dialog').close();else if($('pause-dialog').open)$('pause-dialog').close();else pause();e.preventDefault();return;}
 if(paused||consoleUI?.wheelActive())return;if(handled.has(e.code))e.preventDefault();keys.add(e.code);if(e.repeat)return;
 if(e.code==='KeyU'){consoleUI.dispatch();return;}if(e.code==='Tab'){consoleUI.openWheel('tools');return;}if(e.code==='KeyG'){if(inBadlands(state)||nearbyFrontier(state,world).length)frontierUI.open();else doorsUI.open();return;}if(e.code==='KeyO'){dodgeDoor(state);return;}if(e.code==='KeyI'){inBadlands(state)?frontierUI.open():cityUI.open();return;}if(e.code==='KeyV'){inBadlands(state)?frontierUI.open():streetUI.open();return;}if(e.code==='KeyY'){inBadlands(state)?frontierUI.open():streetUI.interact();return;}if(e.code==='KeyN'){inBadlands(state)?frontierUI.open('contracts'):lifeUI.note();return;}if(e.code==='KeyT'){inBadlands(state)?frontierUI.open():lifeUI.talk();return;}if(e.code==='KeyR'){cast(state);return;}
...
function touchAction(action){if(!playing||paused)return;switch(action){case'adventures':inBadlands(state)?frontierUI.open('contracts'):doorsUI.open();break;case'nearby':inBadlands(state)?frontierUI.open():cityUI.open();break;case'work':inBadlands(state)?frontierUI.open():streetUI.interact();break;case'social':inBadlands(state)?frontierUI.open():lifeUI.talk();break;case'magic':cast(state);break;case'vehicle':if(!inBadlands(state)&&distance(state,world.shop)<7&&Math.abs(state.speed)<1.7)openShop();else enterExit(state,world);break;case'attack':fireTool(state,world,{attack,throwPaper,cast});break;case'dispatch':consoleUI.dispatch();break;case'reload':reloadSling(state);break;case'jump':jump=true;break;case'recenter':view?.recenter();break;}}
...
frontierUI=createFrontierUI({getState:()=>state,world,setPause,save,active:()=>playing&&!paused,onTransition:()=>view?.update(0,state,{snap:true}),legacyNearby:()=>cityUI.open()});
consoleUI=createResonanceUI({getState:()=>state,world,audio,setPause,save,active:()=>playing&&!paused,playing:()=>playing,actions:{regions:()=>frontierUI.open(),nearby:()=>inBadlands(state)?frontierUI.open():cityUI.open(),guide:()=>inBadlands(state)?frontierUI.open('contracts'):doorsUI.open('adventures'),journal:()=>inBadlands(state)?frontierUI.open('contracts'):lifeUI.note(),map}});
...
pad=createGamepad({getState:()=>state,playing:()=>playing,active:()=>playing&&!paused,getPreferences:()=>consoleUI.preferences,actions:{
 start,pause,map,jump:consoleJump,dodge:()=>dodgeDoor(state),interact:()=>frontierUI.interact()||doorsUI.interact(),vehicle:()=>enterExit(state,world),
 throwLeft:()=>{if(!inBadlands(state)&&!state.doors.level&&!state.life.inside)throwPaper(state,world,1);},throwRight:()=>{if(!inBadlands(state)&&!state.doors.level&&!state.life.inside)throwPaper(state,world,-1);},attack:()=>attack(state,world),recenter:()=>view?.recenter(),guide:()=>inBadlands(state)?frontierUI.open('contracts'):doorsUI.open('adventures'),journal:()=>inBadlands(state)?frontierUI.open('contracts'):lifeUI.note(),magic:()=>cast(state),scan:()=>scan(state),
...
$('regions-button').onclick=()=>frontierUI.open();
$('adventures-button').onclick=()=>inBadlands(state)?frontierUI.open('contracts'):doorsUI.open();
...
function drawMap(canvas,full=false){if(frontierUI?.drawMap(canvas,full))return;const g=canvas.getContext('2d'),w=canvas.width,h=canvas.height,scale=full?Math.min(w/330,h/650):1.55,ox=full?w/2:w/2-state.x*scale,oy=full?55:-state.z*scale+h*.52;
...
function updateUI(){if(state.mode!==lastMode){thumb?.reset();touch.clear();lastMode=state.mode;document.querySelectorAll('.foot-action').forEach(b=>b.hidden=state.mode!=='foot');}const p=missionText(state),set=(id,text)=>{if($(id).textContent!==text)$(id).textContent=text;};set('mission-tag',p.tag);set('mission-title',p.title);set('mission-description',p.text);set('target-distance',Math.round(distance(state,activeTarget(state,world)))+' m');set('district',district(state).toUpperCase());set('mode-name',inBadlands(state)?'ON FOOT / EXPEDITION':state.mode==='bike'?(state.life.bike==='standard'?'LEONARDO’S BICYCLE':state.life.bike.toUpperCase()+' BICYCLE'):state.mode==='car'?'PEDAL CARRIAGE':'ON FOOT / GUILD APPRENTICE');set('speed',String(Math.round(Math.abs(state.speed)*3.6)).padStart(2,'0'));set('papers',String(state.papers));set('score',String(state.score));set('credits',String(state.credits));$('health').style.width=Math.min(100,state.health/stats(state).maxHealth*100)+'%';set('trace-status',state.trace>.1?'LOCAL COMMOTION '+Math.round(state.trace*100)+'%':'V0.11 / BORDERLANDS');$('toast').classList.toggle('visible',state.toastT>0);set('toast',state.toast);flash=state.toast;
 ...
 if(!inBadlands(state)&&node)hint=state.scan>0?((state.relay&&node.type==='relay')?'Waterwheel restored. The bridge passage is open.':'Stop and hold H: '+node.name):'X: inspect '+node.name;
 if(!inBadlands(state)&&distance(state,world.depot)<8)hint='Leonardo’s workshop · hold H to report / replenish';if(!inBadlands(state)&&distance(state,world.shop)<7)hint='MARKET STALL · Stop, then F or B to trade';if(!inBadlands(state)&&distance(state,world.bandit)<14&&!state.defeated)hint=state.mode==='foot'?'J: STAFF STRIKE · Hold K: BRACE · F: remount':'GUARD AHEAD · Stop and press F to dismount';if(!inBadlands(state)&&distance(state,world.newsroom)<8&&state.defeated&&!state.folio)hint='H: Recover Leonardo’s stolen folio';if(preferences.touch){hint=state.mode==='foot'?'Joystick walks / Drag to look / Ride to mount':'Joystick steers / Boost to pedal / Brake to stop';if(node)hint='Stop nearby / hold Inspect to operate';if(!inBadlands(state)&&distance(state,world.shop)<7)hint='ARTISANS / Stop, then tap Inspect to trade';if(!inBadlands(state)&&distance(state,world.bandit)<14&&!state.defeated)hint=state.mode==='foot'?'STAFF to strike / Hold BRACE during the windup':'Guard ahead / Stop and tap Ride to dismount';if(distance(state,world.depot)<8)hint='LEONARDO / Hold Inspect to report or replenish';}set('context',hint);$('duel').hidden=safeTown(state)||inBadlands(state)||state.defeated||distance(state,world.bandit)>16;$('duel-health').value=state.banditHP;$('duel-warning').textContent=state.banditPhase==='windup'?'Guard winding up — hold K to brace':'J strikes · K braces · You may retreat';$('hack').style.display=state.hackProgress>0?'block':'none';$('hack').querySelector('i').style.transform=`scaleX(${state.hackProgress})`;drawMap($('minimap'));if($('map-dialog').open)drawMap($('city-map'),true);lifeUI?.update();streetUI?.update();cityUI?.update();doorsUI?.update();frontierUI?.update();consoleUI?.update();
}
