...
import {attachFrontier,targetFrontier,stillwaterDestination} from './frontier-core.mjs';
import {attachQuarter} from './quarter-core.mjs';
export const PLAYTEST_BUILD='guild-currentworks-wayfinding-20260922';
...
export function createCampaignState(saved=null,search=''){
 ...
 const state=attachQuarter(attachFrontier(newState(saved),saved?.frontier),record,{fresh:!saved,legacy:!quarter});
 if(!quarter&&new URLSearchParams(search).get('chapter')==='stillwater')targetFrontier(state,stillwaterDestination(state));
 return state;
}
