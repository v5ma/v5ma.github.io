...
export function createFrontierArt({scene,camera,renderer,m,heightAt}){
 ...
 const geo=new T.PlaneGeometry(196,200,40,40);geo.rotateX(-Math.PI/2);geo.translate(300,0,100);const v=geo.attributes.position;for(let i=0;i<v.count;i++)v.setY(i,frontierHeight(v.getX(i),v.getZ(i))-.09);geo.computeVertexNormals();carvePoolTerrain(geo);root.add(new T.Mesh(geo,new T.MeshStandardMaterial({color:'#53615e',roughness:1})));
