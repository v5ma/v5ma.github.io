...
with sync_playwright() as p:
 ...
 try:
  ...
  check(read(page)['mode']=='car','The fictional pedal carriage retains the vehicle-entering mechanics')
  ...
  drive(page,80,329,1.2);hold(page,['KeyK']);page.wait_for_timeout(2400)
  check(read(page)['guarding'],'The brace control is active during the guard encounter')
  for i in range(3):
   page.keyboard.press('KeyJ');page.wait_for_timeout(850)
  hold(page,[]);page.wait_for_function('LeonardoGuild.inspect().defeated',timeout=15000)
  check(any(e['type']=='duel-won' for e in read(page)['events']),'Real staff strikes make the folio guard yield')
  ...
  check(read(page)['mission']==4,'The complete commission returns physically to Leonardo’s workshop')
  page.screenshot(path=str(OUT/'06-guild-commission-complete.png'))
