...
export function createTownLifeVisuals({scene,root,w,m,rider,bike,camera}){
 ...
 function update(s,dt,room){current=room?.id||null;const below=!!s.life.inside;cellars.visible=below;scene.background=below?belowBackground:baseBackground;scene.fog=below?belowFog:baseFog;
  ...
  for(const {p,model} of people){const point=personAt(p,s);model.root.visible=!s.doors?.level&&(point.inside||null)===(s.life.inside||null);model.root.position.set(point.x,heightAt(point.x,point.z)+(p.inside?-5:0),point.z);model.root.rotation.y=Math.hypot(s.x-point.x,s.z-point.z)<8?Math.atan2(s.x-point.x,s.z-point.z):Math.PI;model.root.rotation.x=p.id==='rocco'&&s.life.flags.rocco?.25:0;model.root.rotation.z=p.id==='rocco'&&s.life.attackPending?-.17:0;animatePerson(model,s.time,{motion:p.id==='rocco'&&s.life.attackPending?'guard':Math.hypot(s.x-point.x,s.z-point.z)<5?'listen':['ada','bartolo','neri','sofia'].includes(p.id)?'work':'idle'});}
