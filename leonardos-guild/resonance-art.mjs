...
export function createResonanceArt({scene,rider,world}){
 ...
 const stringMat=new T.LineBasicMaterial({color:'#e1c08b'}),string=new T.Line(new T.BufferGeometry().setFromPoints([new T.Vector3(-.07,0,0),new T.Vector3(0,.4,.1),new T.Vector3(.07,0,0)]),stringMat);sling.add(string);(rider.handSockets?.[0]||rider.root).add(sling);
 const feedback=createSlingFeedback({root:scene,world,elevation:(s,x,z)=>heightAt(x,z)+doorElevation(s)+1.3});
 ...
 function update(s){const c=s.resonance;if(!c)return;const y=heightAt(s.x,s.z)+doorElevation(s)+s.lift;
  sling.visible=s.mode==='foot'&&c.tool==='sling';if(rider.handSockets)sling.position.set(0,0,.08);else sling.position.set(-.34,1.15,.32);sling.rotation.z=c.aim?-.7:0;sling.rotation.x=s.attackT>0?Math.sin(s.attackT*30):0;
  feedback.update(s);
 ...
 return {update,inspect:()=>({...snapshot,feedback:feedback.inspect()})};
}
