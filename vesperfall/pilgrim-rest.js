...
(function(root){'use strict';
 function install(g){
  ...
  panel.innerHTML='<p class="eyebrow">PILGRIM\'S REST / LOCAL CHECKPOINT</p><p id="checkpoint-summary" aria-live="polite"></p><div class="rest-actions"><button id="continue-expedition" hidden>Continue saved expedition</button><button id="save-expedition" hidden>Save checkpoint now</button><button id="suspend-expedition" hidden>Save and return to title</button><button id="discard-expedition" hidden>Discard saved expedition</button><button id="reload-expedition" hidden>Reload to use the newer save</button></div><p id="checkpoint-status" role="status"></p><small>Autosaves every 15 seconds, on pause and when hidden. Local to this browser, not a cloud save. Practice, AR Sanctuary and the inspection table do not replace an expedition. AR expedition retains normal mission progress. A restored run waits paused for you.</small>';
  $('resume').after(panel);
