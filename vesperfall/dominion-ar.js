...
(function(root){'use strict';
 function install(g,ui){
  const T=g.T,C=VesperCore,$=id=>document.getElementById(id);let suspended=null,wave=0,explore=null;
  ...
  function transparent(){if(!g.arMode&&!g.arExpedition)return;g.scene.object3D.background=null;g.scene.object3D.fog=null;g.scene.renderer.setClearColor(0x000000,0);if(g.jewelglass){g.jewelglass.sky.visible=false;if(g.arMode)g.jewelglass.worldFX.visible=false;}if(g.arsenal.dust)g.arsenal.dust.visible=false;}
  ...
  g.enterXR=function(){const ar=g.scene.is('ar-mode');$('dominion-dialog').hidden=true;
   if(ar&&g.requestedMode==='ar-expedition'){
    explore={background:g.scene.object3D.background,fog:g.scene.object3D.fog};g.arExpedition=true;g.arMode=false;enter();transparent();
    ui.state.notice='AR expedition keeps your current mission, progress and virtual movement. Stay in your clear real play space. Virtual floors are not physical surfaces. No furniture detection or room scanning. Use golden arrows or configured stick movement, not real walking into virtual rooms.';ui.setScreen('notice');g.pendingPanel=true;return;
   }
   if(ar){suspended={game:g.game,running:g.running,practice:g.practice,training:g.training,banked:g.banked,lastPhase:g.lastPhase,yaw:g.yaw,pitch:g.pitch,rig:g.rig.position.clone(),rotation:g.rig.rotation.clone(),background:g.scene.object3D.background,fog:g.scene.object3D.fog};g.arMode=true;g.rig.position.set(0,0,0);g.rig.rotation.set(0,0,0);}enter();if(ar){arena(false,null,true);ui.state.notice='AR Sanctuary: stay inside a clear play space. The real world remains visible, but this game does not detect furniture. Your expedition is suspended until AR ends.';ui.setScreen('main');g.pendingPanel=true;}};
  g.exitXR=function(){if(g.arExpedition){const view=explore;g.arExpedition=false;explore=null;exit();if(view){g.scene.object3D.background=view.background;g.scene.object3D.fog=view.fog;}g.scene.renderer.setClearAlpha(1);if(g.jewelglass)g.jewelglass.sky.visible=true;g.requestedMode=null;g.setPaused(true);g.toast('AR expedition paused. Your exploration and collected supplies are retained.');return;}const old=suspended;g.arMode=false;suspended=null;exit();if(old){Object.assign(g,{game:old.game,running:old.running,practice:old.practice,training:old.training,banked:old.banked,lastPhase:old.lastPhase,yaw:old.yaw,pitch:old.pitch});g.scene.object3D.background=old.background;g.scene.object3D.fog=old.fog;g.scene.renderer.setClearAlpha(1);if(g.jewelglass)g.jewelglass.sky.visible=true;g.build();g.rig.position.copy(old.rig);g.rig.rotation.copy(old.rotation);g.head.components['look-controls'].yawObject.rotation.y=old.yaw;g.head.components['look-controls'].pitchObject.rotation.x=old.pitch;g.scene.object3D.updateMatrixWorld(true);if(g.jewelglass)g.jewelglass.worldFX.visible=true;if(g.arsenal.dust)g.arsenal.dust.visible=true;g.setPaused(true);g.toast('Your expedition has been restored and is paused.');}g.requestedMode=null;};
  g.start=practice=>g.arMode?arena(practice):start(practice);g.startTraining=kind=>g.arMode?arena(false,kind):training(kind);
  g.build=function(){build();if(g.arMode)g.rig.position.set(0,0,0);if(g.arMode||g.arExpedition)transparent();};
