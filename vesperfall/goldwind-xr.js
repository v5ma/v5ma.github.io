...
(function(root){'use strict';
 function install(g){
  ...
  controls.innerHTML='<legend>Goldwind / physical bow</legend><button id="goldwind-enable">Use Goldwind physical bow controls</button><label>Quest bow controls <select id="xr-bow-controls"><option value="goldwind">Goldwind / golden arrow, thrown disk, reach quiver</option><option value="classic">Classic / optional familiar bindings</option></select></label><label>Goldwind shield <select id="goldwind-shield"><option value="trigger">Bow-hand trigger / grip interacts</option><option value="grip">Bow-hand grip / trigger interacts</option></select></label><label>Goldwind teleport aiming aid <select id="goldwind-assist"><option value="off">Off / golden arrow only</option><option value="draw">Optional gold path while drawing</option></select></label><p>Goldwind bow: draw trigger shoots the selected combat arrow. Hold either draw-hand face button near the string, pull and release for a golden teleport arrow. Grip a nearby supply to take it, or aim and hold grip to pull. Grip on empty space holds a disk; throw and release for short relocation. Hold either bow-hand face button to reveal arrows, then reach with the other controller and squeeze its trigger to select. Click the draw stick to pause; click the bow stick to switch weapon. Crossbow uses its preserved Classic controls. Xbox is unchanged.</p>';
