...
test('Front balustrade blocks falling off the gallery and lower route remains open',()=>{
 const s=C.create('BELL-01');walk(s,0,4.6);walk(s,-4.65,4.6);walk(s,-4.65,-4.85);walk(s,2,-4.85);for(let i=0;i<150;i++)C.move(s,0,.04);assert.equal(s.p[1],3.2);assert.ok(s.p[2]<-4.0);
});
