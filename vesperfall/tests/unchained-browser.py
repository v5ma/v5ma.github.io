...
with sync_playwright() as pw:
 ...
 try:
  ...
  if MODE=='desktop':
   ...
   page.evaluate("TestXR.button('right',1,true)");page.wait_for_function('!!Vesperfall.state.shield');check(True,'The tracked free-hand grip raises a real directional shield')
   page.screenshot(path=str(OUT/'xr-shield.png'));page.evaluate("TestXR.missing('right',true)");page.wait_for_function('!Vesperfall.state.shield');check(True,'Losing the free controller removes its shield rather than leaving invisible protection')
   page.evaluate("TestXR.button('right',1,false);TestXR.missing('right',false)");page.wait_for_function('!!Vesperfall.component.hands.left')
   ...
   page.wait_for_function('Vesperfall.component.arsenal.state.xrArmed');page.evaluate("TestXR.button('left',0,true)");page.wait_for_function('Vesperfall.state.shots===1');page.wait_for_timeout(250);check(state(page)['shots']==1,'A held XR trigger fires only the loaded crossbow bolt')
   page.evaluate("TestXR.button('left',0,false);TestXR.button('right',3,true)");page.wait_for_function('Vesperfall.state.crossbow.reload>0');page.evaluate("TestXR.button('right',3,false)");page.wait_for_function('Vesperfall.state.crossbow.loaded');check(True,'Controller reload follows the real timed magazine state')
   page.evaluate("TestXR.missing('right',true);TestXR.button('left',0,true)");page.wait_for_function('!Vesperfall.component.arsenal.state.xrArmed');page.evaluate("TestXR.missing('right',false)");page.wait_for_function('!!Vesperfall.component.hands.right');page.wait_for_timeout(400)
   check(state(page)['shots']==1,'A held trigger does not fire after tracking returns')
   page.evaluate("TestXR.button('left',0,false)");page.wait_for_function('Vesperfall.component.arsenal.state.xrArmed');page.evaluate("TestXR.button('left',0,true)");page.wait_for_function('Vesperfall.state.shots===2');page.evaluate("TestXR.button('left',0,false)");check(True,'A deliberate new trigger press fires after reconnect neutral arming')
   page.evaluate("TestXR.button('left',1,true)");page.wait_for_function('Vesperfall.state.shardsUsed===1');page.evaluate("TestXR.button('left',1,false)");check(True,'The weapon-hand grip performs one collision-checked shard step')
