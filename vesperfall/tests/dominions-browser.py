...
with sync_playwright() as p:
 ...
 try:
  ...
  xr_set('right',1,True);wait('!!Vesperfall.state.shield');xr_set('right',1,False);wait('!Vesperfall.state.shield');check(True,'Quest free-hand grip controls the physical shield')
  ...
  xrpress('left',5);wait('Vesperfall.component.paused');xraction('Missions');xraction('More / page');xraction('Expedition options');xraction('Sparring');xraction('More / page');xraction('Thorn Duelist');wait("Vesperfall.component.training==='duelist'&&!Vesperfall.component.paused");check(page.evaluate("Vesperfall.component.enemyMeshes[0].userData.dominion!==undefined"),'Quest can start a new enemy trial using only spatial menus')
  ...
  pos=page.evaluate('[...Vesperfall.state.p]');xrpress('left',1);check(page.evaluate('(p)=>JSON.stringify(Vesperfall.state.p)===JSON.stringify(p)',pos),'AR refuses artificial shard locomotion')
