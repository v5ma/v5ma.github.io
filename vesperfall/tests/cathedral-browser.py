...
def capture(p,name):
 ...
 if MODE=='routes':
  p.keyboard.press('KeyP');p.wait_for_function('Vesperfall.component.paused');p.locator('#cathedral-shadows').check();p.locator('#jewel-quality').select_option('balanced');p.locator('#resume').click();p.locator('a-scene canvas').focus()
 ...
 if MODE=='routes':
  p.set_viewport_size({'width':640,'height':480});p.locator('a-scene canvas').focus();p.keyboard.press('KeyV');p.keyboard.press('KeyP');p.wait_for_function('Vesperfall.component.paused');p.locator('#cathedral-shadows').uncheck();p.locator('#jewel-quality').select_option('classic');p.locator('#resume').click();p.locator('a-scene canvas').focus()
...
with sync_playwright() as pw:
 ...
 try:
  ...
  if MODE=='visual':
   ...
   p.keyboard.press('KeyP');p.wait_for_function('Vesperfall.component.paused');p.locator('#jewel-settings summary').click();p.locator('#audio').uncheck();p.locator('#cathedral-shadows').uncheck();p.locator('#jewel-quality').select_option('classic');p.locator('#resume').click();p.locator('a-scene canvas').focus();br=w['architecture']['routes'][0];rooms_to(p,br['from']);walk(p,[br['entry'],[br['entry'][0],3.2,br['a'][2]],br['a']]);check(abs(snap(p)['player'][1]-3.2)<.02,'Normal movement ascends the first room stair to the high-route entrance')
