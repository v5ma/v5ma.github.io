...
WALK="""async targets=>{
 ...
 await new Promise((resolve,reject)=>{let n=0,started=performance.now();const timer=setInterval(()=>{
  ...
  if(d<.10){for(const k of [...held])key(k,false);n++;started=performance.now();if(n===targets.length){clearInterval(timer);resolve();}}
  else if(c.paused||s.phase!=='playing'||performance.now()-started>85000){for(const k of [...held])key(k,false);clearInterval(timer);reject(Error('Blocked route '+JSON.stringify({target:targets[n],position:s.p,phase:s.phase})));}
 },2);});
}"""
...
with sync_playwright() as pw:
 ...
 b=pw.chromium.launch(**args);ctx=b.new_context(viewport={'width':1120,'height':800},device_scale_factor=.5 if MODE=='routes' else 1,service_workers='block');host=urlparse(BASE).hostname;ctx.route('**/*',lambda r:r.continue_() if urlparse(r.request.url).hostname==host or r.request.url.startswith(('blob:','data:')) else r.abort());p=ctx.new_page();p.set_default_timeout(95000);p.on('pageerror',lambda e:errors.append(str(e)))
 try:
  if MODE=='visual':
  ...
  if MODE=='visual':
   p.screenshot(path=str(OUT/'after-choir.png'));p.locator('a-scene canvas').focus();aim(p,-.72,.12);p.screenshot(path=str(OUT/'masonry-and-sculpture.png'))
   ...
   p.keyboard.press('KeyM');p.screenshot(path=str(OUT/'between-rooms-skywalk.png'));p.keyboard.press('KeyM');walk(p,[br['b'],[br['exit'][0],3.2,br['b'][2]],br['exit']]);check(snap(p)['player'][1]<.05,'The skywalk ends at another real stair and reconnects with the ground route')
   ...
   check(snap(p)['score']==score+200,'The high belfry reward is collected only after reaching its floor');aim(p,0,-.30);p.screenshot(path=str(OUT/'belfry-crown.png'));walk(p,[t['top'],t['entry'],[entry[0],3.2,room['z']-4.85],entry]);check(snap(p)['player'][1]<.05,'The entire two-level belfry route returns to the street without a reset')
