...
with sync_playwright() as p:
 ...
 def aim_row(i):
  # Orient only the emulated controller, using the actual transformed hit face.
  # Do not assign menu selection or any game/saved state.
  page.evaluate("""i=>{const g=Vesperfall.component,T=g.T,m=g.xrPanel.mesh;
   m.updateMatrixWorld(true);g.rig.updateMatrixWorld(true);
   const target=m.localToWorld(new T.Vector3(0,(.5-(195+i*75+30.5)/768)*m.geometry.parameters.height,0));
   const origin=new T.Vector3(...TestXR.state.hands.right).applyMatrix4(g.rig.matrixWorld);
   const q=new T.Quaternion().setFromUnitVectors(new T.Vector3(0,0,-1),target.sub(origin).normalize());
   q.premultiply(g.rig.getWorldQuaternion(new T.Quaternion()).invert());TestXR.orientation('right',q.toArray());
  }""",i)
 def menu(text):
  wait('Vesperfall.component.dominionControls.state.xrNeutral&&!Vesperfall.component.threshold.state.menuNeutral')
 ...
 try:
  ...
  wait('Vesperfall.component.dominionControls.state.xrNeutral&&Vesperfall.component.dominionControls.state.xrAxesReady')
  selection=page.evaluate('Vesperfall.component.menuSelection')
  page.evaluate("TestXR.axes('left',0,1)");wait('!Vesperfall.component.dominionControls.state.xrAxesReady')
  page.evaluate("TestXR.axes('left',0,0)");wait('Vesperfall.component.dominionControls.state.xrAxesReady')
  check(page.evaluate('(i)=>Vesperfall.component.menuSelection===(i+1)%Vesperfall.component.xrMenuRows.length',selection),'The real movement stick advances spatial-menu focus exactly once')
  ...
  try:snapshot=page.evaluate('window.Vesperfall?{phase:Vesperfall.component.threshold?.state.phase,walking:Vesperfall.component.threshold?.state.walking,screen:Vesperfall.component.dominionControls.state.xrScreen,rows:Vesperfall.component.xrMenuRows.map(r=>r[0])}:null')
  except Exception:snapshot=None
  (OUT/'failure.json').write_text(json.dumps({'error':str(e),'checks':checks,'errors':errors,'console':console,'state':snapshot},indent=2))
