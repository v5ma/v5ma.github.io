...
with sync_playwright() as pw:
 ...
 try:
  ...
  if MODE=='visual':
   ...
   p.evaluate("TestXR.button('right',1,true)");p.wait_for_function('Vesperfall.state.shield&&Vesperfall.component.arsenal.shield.visible');p.screenshot(path=str(OUT/'stereo-wardglass.png'));check(p.evaluate('Vesperfall.component.jewelglass.current.particles===96'),'XR effects use the smaller bounded pool')
   p.evaluate("TestXR.missing('right',true)");p.wait_for_function('!Vesperfall.state.shield');check(True,'The shader cannot leave invisible protection after tracking loss')
