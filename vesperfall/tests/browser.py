...
def shot(page):
 # Wait for the actual charge, not wall-clock performance of a software GPU.
 page.keyboard.down('Space');page.wait_for_function('Vesperfall.component.charge>.98',timeout=60000);page.keyboard.up('Space');page.wait_for_timeout(500)
...
with sync_playwright() as p:
 ...
 # Screenshots remain evidence; video encoding needlessly competes with the
 # software renderer and does not establish physical-headset performance.
 browser=p.chromium.launch(**opts);context=browser.new_context(viewport=({'width':800,'height':600} if MODE=='expedition' else {'width':1024,'height':720}),service_workers='block')
