...
INPUT="""async o=>{
 ...
 await new Promise((resolve,reject)=>{const start=performance.now(),timer=setInterval(()=>{
  ...
  if(done||s.phase!=='playing'||performance.now()-start>140000){for(const k of [...held])key(k,false);clearInterval(timer);done?resolve():reject(Error('Input did not finish '+JSON.stringify(o)));}
 },3);});
}"""
def trial(page,kind):
...
with sync_playwright() as pw:
 ...
 try:
  ...
  for id in route[1:]:page.evaluate(INPUT,{'walk':[rooms[id]['x'],rooms[id]['z']]})
  # Approach from below the stair rather than asking a diagonal path to tunnel.
  ...
  (OUT/'report.json').write_text(json.dumps({'passed':len(checks),'checks':checks,'errors':errors,'scope':'Real HTTP A-Frame/WebGL. Ordinary UI/keyboard inputs only; read-only observations guide aiming/navigation. Pixel ratio .65 reduces software-GPU cost; not physical headset certification.','state':snap(page)},indent=2))
