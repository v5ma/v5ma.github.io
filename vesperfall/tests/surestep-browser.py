...
with sync_playwright() as p:
    ...
    try:
        ...
        wait("[...Vesperfall.component.questHands.state.sources].some(([s,p])=>s.handedness==='right'&&!p.pinch.armed&&!p.pinch.closed)")
        page.evaluate("TestHands.missing('right','index-finger-tip',false)")
        wait("[...Vesperfall.component.questHands.state.sources].some(([s,p])=>s.handedness==='right'&&p.pinch.closed&&!p.pinch.armed)")
