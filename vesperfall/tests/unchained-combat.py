...
DRIVE="""async options=>{
 ...
 const visible=()=>Vesperfall.state.world.enemies.filter(e=>!e.dead&&e.required!==false&&C.len(C.sub(e.p,Vesperfall.state.head))<16&&!C.segmentBlocked(Vesperfall.state.world,Vesperfall.state.head,C.add(e.p,[0,.62,0])));
 await new Promise((resolve,reject)=>{const start=performance.now();let lastShot=Vesperfall.state.shots;const timer=setInterval(()=>{
  ...
  if(s.phase!=='playing'||c.paused||performance.now()-start>180000){for(const k of [...held])key(k,false);clearInterval(timer);reject(Error('Input stopped: '+JSON.stringify({phase:s.phase,p,options,time:s.time,guard:s.guard,shield:s.shield,bolts:s.bolts.length})));return;}
...
with sync_playwright() as pw:
 ...
 try:
  page.goto(BASE+'/vesperfall/',wait_until='domcontentloaded');page.wait_for_function('window.Vesperfall?.component.arsenal&&Vesperfall.component.rendererReady');page.locator('#jewel-settings summary').click();page.locator('#jewel-quality').select_option('classic');page.locator('#cathedral-shadows').uncheck();page.locator('#audio').uncheck();page.locator('#start').click();page.locator('a-scene canvas').focus();deadline=time.monotonic()+730;blocked=False
  ...
  while not snap(page)['portalReady'] and time.monotonic()<deadline:
   ...
   visible=page.evaluate('(()=>{const s=Vesperfall.state,C=VesperCore;return s.world.enemies.filter(e=>!e.dead&&e.required!==false&&C.len(C.sub(e.p,s.head))<16&&!C.segmentBlocked(s.world,s.head,C.add(e.p,[0,.62,0]))).sort((a,b)=>C.len(C.sub(a.p,s.p))-C.len(C.sub(b.p,s.p))).map(e=>e.id);})()')
   if visible:
    if not blocked:
    ...
    route=page.evaluate('(()=>{const s=Vesperfall.state,C=VesperCore,w=s.world,here=C.roomAt(w,s.p),e=w.enemies.filter(e=>!e.dead&&e.required!==false).sort((a,b)=>C.route(w,here,a.room).length-C.route(w,here,b.room).length)[0];return C.route(w,here,e.room).map(i=>[w.rooms[i].x,w.rooms[i].z]);})()')
    for target in route:
     page.evaluate(DRIVE,{'target':target,'combat':True})
     if page.evaluate('Vesperfall.state.world.enemies.some(e=>!e.dead&&e.required!==false&&VesperCore.len(VesperCore.sub(e.p,Vesperfall.state.head))<16&&!VesperCore.segmentBlocked(Vesperfall.state.world,Vesperfall.state.head,VesperCore.add(e.p,[0,.62,0])))'):break
   print('PROGRESS',json.dumps({k:snap(page)[k] for k in ['player','health','kills','shots','headshots','blocks']}),flush=True)
  check(snap(page)['portalReady'] and len([e for e in snap(page)['enemies'] if e.get('required',True) and e['dead']])==5,'Five real wardens fall to actual arrows and open the beacon')
  ...
  check(snap(page)['kills']==5 and len([e for e in snap(page)['enemies'] if not e['dead']])==21 and len([e for e in snap(page)['enemies'] if e.get('required',True) and not e['dead']])==5,'The blessing opens a new seeded sector while preserving run counters')
