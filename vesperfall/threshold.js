...
(function(root){'use strict';
 function install(g){
  ...
  const oldProcess=g.processXR.bind(g);g.processXR=function(dt,head){
   ...
   if(session?.visibilityState!=='visible'||!viewer||viewer.emulatedPosition){crossing.reset();state.walkingPinches.clear();state.moveReady=false;return;}
   ...
   // Do not retain pinch arming across disappearance or replacement of a hand.
   for(const source of state.walkingPinches.keys())if(!optical.includes(source))state.walkingPinches.delete(source);
   if(optical.length){
