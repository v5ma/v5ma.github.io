...
(function(root){'use strict';
 ...
 VesperArt.create=function(T,scene){
  ...
  const lathe=new T.LatheGeometry([[0,0],[.49,0],[.49,.12],[.40,.18],[.31,.24],[.25,.32],[.23,.42],[.23,.88],[.3,.91],[.39,.96],[.4,1],[0,1]].map(p=>new T.Vector2(...p)),16);sharedGeometry.add(lathe);
  class Batch{
   constructor(){this.parts=new Map();this.count=0;}
   add(geo,mat,x,y,z,sx=1,sy=1,sz=1,ry=0,rx=0,rz=0){let group=this.parts.get(mat);if(!group){group={positions:[],normals:[],uvs:[],indices:[]};this.parts.set(mat,group);}const matrix=new T.Matrix4().compose(new T.Vector3(x,y,z),new T.Quaternion().setFromEuler(new T.Euler(rx,ry,rz)),new T.Vector3(sx,sy,sz)),normalMatrix=new T.Matrix3().getNormalMatrix(matrix),pos=geo.getAttribute('position'),norm=geo.getAttribute('normal'),base=group.positions.length/3,p=new T.Vector3(),n=new T.Vector3();
   ...
   finish(parent){for(const [mat,p]of this.parts){const g=new T.BufferGeometry();g.setAttribute('position',new T.Float32BufferAttribute(p.positions,3));g.setAttribute('normal',new T.Float32BufferAttribute(p.normals,3));g.setAttribute('uv',new T.Float32BufferAttribute(p.uvs,2));g.setAttribute('uv1',g.getAttribute('uv').clone());g.setIndex(p.indices);g.computeBoundingSphere();const m=new T.Mesh(g,mat);m.name=mat.name||'Batched cathedral detail';m.castShadow=m.receiveShadow=true;parent.add(m);}return this.count;}
  }
  ...
  function world(model){
   ...
   for(const f of model.floors){if(f.type==='stair'){
    ...
    if(f.type==='bridge'){const along=f.w>f.d;for(const side of[-1,1]){const x=f.x+(along?0:side*(f.w/2-.16)),z=f.z+(along?side*(f.d/2-.16):0);b.box(pale,x,.64,z,along?f.w:.13,.15,along?.13:f.d);const n=Math.floor((along?f.w:f.d)/.65);for(let i=0;i<=n;i++)b.add(lathe,stone,x+(along?(i/n-.5)*(f.w-.3):0),0,z+(along?0:(i/n-.5)*(f.d-.3)),.22,.63,.22);}}
   }
   for(const s of model.solids){const p=s.min.map((v,i)=>(v+s.max[i])/2),d=s.max.map((v,i)=>v-s.min[i]);
    ...
    if(s.type==='balustrade'){const along=d[0]>d[2],n=Math.max(1,Math.ceil((along?d[0]:d[2])/.52));b.box(pale,p[0],s.max[1]-.055,p[2],d[0]+.08,.13,d[2]+.08);b.box(dark,p[0],s.min[1]+.08,p[2],d[0],.16,d[2]);for(let j=0;j<=n;j++)b.add(lathe,pale,p[0]+(along?(j/n-.5)*d[0]:0),s.min[1]+.12,p[2]+(along?0:(j/n-.5)*d[2]),.16,d[1]-.22,.16);continue;}
