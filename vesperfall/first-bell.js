...
(function(root){'use strict';
 ...
 const lessons=[
  ...
  ['Retrieve a crystal','Find the glowing lesson supply near you. In Goldwind VR, reach and grip, or point and hold grip to pull. Classic retains trigger pulling. On desktop, approach it.'],
 ...
 function install(g){
  ...
  const menu=document.createElement('section');menu.id='first-bell-menu';menu.innerHTML='<p class="eyebrow">FIRST BELL / LEARN, READ, COUNTER</p><label for="expedition-mode">Choose mission / available in VR and AR expedition</label><select id="expedition-mode"><option value="pilgrimage">Pilgrimage / Lantern Causeway + Ashen Archive</option><option value="archive">Ashen Archive / start here</option><option value="returning-bell">The Returning Bell / authored opening</option><option value="endless">Endless cloisters / original expedition</option><option value="oath">Bellkeeper\'s Oath / three connected trials</option></select><div class="first-bell-actions"><button id="first-bell-start">Learn physical archery</button><button id="oath-start">Take the Bellkeeper\'s Oath</button></div><p id="lesson-summary">Ten guided lessons. A scored route across Roseglass, Ivory Crown and Ember Crown. Existing saves remain protected.</p><div class="first-bell-actions"><button id="coach-help">Coach / current instruction</button><button id="coach-ready">Coach ready / continue</button><button id="coach-skip">Skip current lesson</button></div>';
  ...
  function controls(){const i=actualCoach()?.index||0;
   ...
   return g.xr&&g.game.weapon==='bow'&&g.goldwind?.enabled()?g.goldwind.hint(i):(g.xr?xr:ui.state.pad?pad:pc)[i];
  }
