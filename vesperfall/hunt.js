 ...
 function install(g){
  ...
  function update(){
   ...
   if(active){const remaining=s.world.enemies.filter(e=>!e.dead).length,room=s.world.rooms[VesperCore.roomAt(s.world,s.p)];
    ...
    const text=s.guardLock>0?'GUARD BROKEN · recover '+s.guardLock.toFixed(1)+'s':target?VesperEncounters.names[target.kind]+' · '+phase:g.training?(remaining?'SPARRING · guard, sidestep, then shoot':'TRIAL COMPLETE · P to choose another opponent'):(s.chapter?room.label:(room.family||'Cloister'))+' · '+(s.p[1]>2?'UPPER ROUTE':'GROUND ROUTE');
