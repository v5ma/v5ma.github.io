...
checks={'art.js':('3273498e46fcbd5bcbf541f0e64435dc6c07bb2a2147960dad6829353be55f2a','4b9ec93446349d8fd8236e6983a0bd6ba3968967a94b97e5955ba2d892afedbc'),'tests/spectral-browser.py':('7e44774b746c66e3e6905b0c955cb84d31bfcf68c9883607babff93ee1fa2715','781a9be3cbb94b26bf80808a3ffdb9aae41151d63983a5eb3802ab3341202e31')}
...
s=s.replace("o.id=n.id;o.g.visible=true;", "o.id=n.id;o.dir=n.dir;o.g.visible=true;")
s=s.replace('new T.TorusGeometry(r,tube,8,96)','new T.TorusGeometry(r,tube,r<.05?6:8,r<.05?32:96)')
s=s.replace('for(let i=active;i<notes.length;i++)notes[i].g.visible=false;','for(let i=active;i<notes.length;i++)notes[i].g.visible=false;\n   // Tiny rings do not need 96 segments. In Undertow XR, glyphs and facets take priority over per-note decoration.\n   if(immersive&&(s?.song.id||scene.components?.[\'prism-game\']?.track)===\'undertow\')for(const o of notes){o.halo.visible=false;o.sparks.visible=false;}')
f.write_text(s)
...
print('Verified two source-polish results, including bounded hilt tessellation and XR decoration budget.')
