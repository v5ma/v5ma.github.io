...
(()=>{'use strict';const C=window.TheologyPaulineCore;
 function enhance(p){
  if(p.slug!=='james-and-contested-succession')return;
  const body=document.querySelector('#article-body');if(!body||body.querySelector('#pauline-lab'))return;
  const anchor=[...body.querySelectorAll('h2')].find(h=>h.textContent==='Three different clocks, not a single seven-year story');if(!anchor)return;
  ...
  function render(changeAddress){
   const s=state();$('#pc-results').replaceChildren();$('#pc-error').hidden=true;
