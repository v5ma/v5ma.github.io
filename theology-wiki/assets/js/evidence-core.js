...
function selection(data,params){const ids=new Set(data.claims.map(c=>c.id));const requested=params.get('claim'),second=params.get('compare'),first=ids.has(requested)?requested:(ids.has('jt-knowledge')?'jt-knowledge':data.claims[0]?.id||'');return {first,second:ids.has(second)&&second!==first?second:'',query:(params.get('find')||'').slice(0,300),topic:[...new Set(data.claims.map(c=>c.topic))].includes(params.get('topic'))?params.get('topic'):'',status:[...new Set(data.claims.map(c=>c.status))].includes(params.get('status'))?params.get('status'):'',invalid:Boolean(requested&&!ids.has(requested)||second&&!ids.has(second))};}
