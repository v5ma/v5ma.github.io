...
function build(){
 ...
 add({slug:'connected-arguments',title:'The book argument map and connecting studies',category:'context',kind:'Navigator',summary:'Five bridge studies, seventeen chapter handoffs, three linked diagrams and eight explicit research questions.',updated:'2026-09-07'},Connected.markdown(F.parts));
...
${E.articles.length} developed articles accompany the archive. They present the inquiry in first-person, theory-centered prose. They are working authorial articles, not replacement transcripts.`;
 ...
 const navigator=(slug,title,summary,body)=>add({slug,title,summary,category:'context',kind:'Navigator',updated:slug==='computational-argument-map'?H.updated:F.updated},body);
...
function patchReaders(){
 ...
 const pinConnected=text=>text.replace(/(\.\/assets\/(js|css)\/(connected-tools\.js|connected\.css|research-tools\.js|products-tools\.js)\?v=)[^"'\s]+/g,(_,prefix,type,name)=>prefix+E.version+'&amp;asset='+sha(fs.readFileSync(path.join(ROOT,'assets',type,name))).slice(0,16));
