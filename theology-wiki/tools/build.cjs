...
const C=require('../assets/js/research-core.js'),E=require('../editorial/edition.cjs');
...
function build(){
 ...
 function add(p,body){
  ...
  const row={...p,path:p.path||'developed/'+p.slug+'.md',topic:E.categories.find(c=>c.id===p.category)?.title||'Theology',tags:[p.category,p.kind==='Developed article'?'developed':'navigation'],editionGenerated:p.slug!=='home',xrRoomId:'theology-'+p.slug,xrDeckId:'theology-reader'};
 ...
 for(const p of pages){
  ...
  const text=bodies.get(p.slug)||'';for(const term of C.tokens(text+' '+p.title+' '+p.summary)){if(term.length>60)continue;(postings[term]??=[]).push(pages.indexOf(p));}
 }
 ...
 emit('data/research.json',{version:E.version,categories:E.categories,paths:E.paths,sourceCount:manifest.length,developedCount:E.articles.length,sourceIntegrity:'354/354 matched existing SHA-256 manifest',backlinks});
 ...
 patchReaders();
 const report={version:E.version,pages:pages.length,sourceChats:manifest.length,verifiedHashes:chats.size,developedArticles:E.articles.length,topicCollections:E.categories.length,links:Object.values(graph).reduce((n,x)=>n+x.length,0),searchTerms:Object.keys(postings).length,sourceBytes:[...chats.values()].reduce((n,x)=>n+x.bytes,0)};
