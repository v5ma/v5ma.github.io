...
export function createScene(canvas){
 ...
 function render(state,view,dt){if(lastState!==state){lastState=state;lastEvent=0;cameraSet=false;}const t=state.t;pose(hero,state.player,t);for(const e of state.enemies){if(!enemies.has(e.id))enemies.set(e.id,actor(scene,mesh,e.type==='drifter'?0x5e7667:0x7e795c,true));pose(enemies.get(e.id),{...e,aim:e.state==='chase'&&(e.aimTime||0)>.1},t,true);}
  ...
  rain.position.y=-(t*9%16);const p=state.player,aim=view.aim,lookY=(p.stance==='prone'?.38:p.stance==='crouch'?1.05:1.5)+(p.vault?Math.sin(Math.PI*Math.min(1,p.vault.t/p.vault.duration))*.95:0);
 ...
 return {renderer,scene,camera,hero,enemies,render,aimDirection,project,resize,stats:()=>({calls:renderer.info.render.calls,triangles:renderer.info.render.triangles}),cameraState:()=>({x:camera.position.x,y:camera.position.y,z:camera.position.z,near:camera.near,heroVisible:hero.root.visible}),setQuality(low){renderer.setPixelRatio(low?1:Math.min(devicePixelRatio||1,1.6));renderer.shadowMap.enabled=!low;}};
}
