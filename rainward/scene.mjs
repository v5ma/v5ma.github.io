import {motionStats} from './grounded-motion.mjs';
...
export function createScene(canvas,{onXRStart=()=>{},onXREnd=()=>{}}={}){
 ...
 function render(state,view,dt){renderer.info.reset();if(lastState!==state){lastState=state;lastEvent=0;cameraSet=false;hero.motion=null;for(const a of enemies.values())a.motion=null;}const t=state.t;pose(hero,state.player,t,false,heightAt);scenery?.update(state,dt);taskArt.update(state);firstLight.update(state);for(const e of state.enemies){const monster=isMonster(e);if(!enemies.has(e.id))enemies.set(e.id,monster?creature(scene,mesh,e.type):actor(scene,mesh,e.type==='drifter'?0x5e7667:0x7e795c,true,e.type));const model=enemies.get(e.id);if(monster)poseCreature(model,e,t);else{pose(model,{...e,aim:e.state==='chase'&&(e.aimTime||0)>.1},t,true,heightAt);}}
