...
export function createScene(canvas,{sharedRenderer=null}={}){
 ...
 return {portalEnvironment:arEnvironment,renderAR(xrCamera,state){renderWaistAR(renderer,scene,xrCamera,arEnvironment,heightAt(state.player.x,state.player.z)-(state.player.swimDepth||0)+.92);},bindXR(adapter){xr=adapter;},xrSupported:()=>xr.supported(),enterXR:()=>xr.enter(),exitXR:()=>xr.exit(),xrStatus:()=>xr.stats(),setCinematic(value){cinemaEnabled=!!value;cinema.set(cinemaEnabled,requestedLow);},setMotion:value=>{living.setMotion(value);atmosphere.setMotion(value);},setRainworn(human,film){humanEnabled=!!human;filmEnabled=!!film;humans.set(humanEnabled,requestedLow);rainFilm.set(filmEnabled,requestedLow);},retryHumans:()=>humans.retry(),visualStatus:()=>({freightCut:freightCut.stats(),recut:recut.stats(),aquatic:chapter.id==='natatorium'?scenery?.stats?.()||null:null,rainworn:{humans:humans.stats(),film:rainFilm.stats()},firstLight:firstLight.stats(),characters:{skinned:hero.skin.isSkinnedMesh,bones:hero.bones.length,vertices:hero.skin.geometry.attributes.position.count,humanoids:1+[...enemies.values()].filter(e=>e.skin).length},tasks:taskArt.count,cinema:cinema.stats(),living:living.stats(),atmosphere:atmosphere.stats()}),setScanned:value=>scans.set(value),assetStatus:()=>scans.report(),dispose,renderer,scene,camera,hero,enemies,render,aimDirection,project,resize,stats:()=>({staticInstances:staticCulling.stats(),scannedMeshes:scans.report().texturedMeshes,calls:renderer.info.render.calls,triangles:renderer.info.render.triangles}),cameraState:()=>({x:camera.position.x,y:camera.position.y,z:camera.position.z,near:camera.near,heroVisible:hero.root.visible}),setQuality(low){requestedLow=!!low;humans.set(humanEnabled,xr.isActive()?true:low);rainFilm.set(filmEnabled,xr.isActive()?true:low);graphics.set(xr.isActive()?true:low);cinema.set(cinemaEnabled,low);}};
}