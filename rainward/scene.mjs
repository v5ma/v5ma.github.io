...
import {throwPosition} from './throw-path.mjs';
import {createCurrentworksEnvironment} from './currentworks-environment.mjs';
import {footContacts} from './foot-contact.mjs';
import {renderWaistAR} from './waist-ar.mjs';
import {createFreightCutArt} from './freight-cut-art.mjs';
import {createFloodgateRecutArt} from './floodgate-recut-art.mjs';
...
import {buildNatatorium} from './natatorium-art.mjs';
...
export function createScene(canvas,{sharedRenderer=null}={}){
 ...
 const environment=skyEnvironment(scene,renderer);scene.fog=new T.FogExp2(chapter.id==='terminus'?0x637985:chapter.id==='natatorium'?0x71817f:0x9aadb3,chapter.id==='terminus'?.014:chapter.id==='natatorium'?.011:.007);const baseFogColor=scene.fog.color.clone(),baseFogDensity=scene.fog.density,baseExposure=renderer.toneMappingExposure;
 ...
 const A=artkit(scene,scans),{mesh,geos}=A;const scenery=chapter.id==='natatorium'?buildNatatorium(scene,A):['meridian','breakwater','whiteout'].includes(chapter.id)?buildReclaimed(scene,A):chapter.id==='terminus'?buildTerminus(scene,A):chapter.id==='conservatory'?buildConservatory(scene,A):null;if(!scenery)buildDistrict(scene,A);const taskArt=createTaskArt(scene,A),firstLight=createFirstLightArt(scene,A,chapter),recut=createFloodgateRecutArt(scene,A,chapter),freightCut=createFreightCutArt(scene,A,chapter);
 const arEnvironment=new Set(scene.children.filter(o=>!o.isCamera&&!o.isLight));
 ...
 const listenArt=createListenArt(scene,enemies),throwGuide=createThrowGuide(scene);
 const living=createLivingMaterials(scene,chapter,heightAt),atmosphere=createAtmosphere(scene,chapter,heightAt);
 const currentworks=createCurrentworksEnvironment(scene,chapter);arEnvironment.add(currentworks.root);
 ...
 let requestedLow=false,cinemaEnabled=true,dead=false;let xr={isActive:()=>false,stats:()=>({active:false,mode:'immersive-first-person',hardwareVerified:false})};scans.start();
 function render(state,view,dt){renderer.info.reset();if(lastState!==state){lastState=state;lastEvent=0;cameraSet=false;}const t=state.t;pose(hero,state.player,t);hero.root.position.y+=heightAt(state.player.x,state.player.z)-(state.player.swimDepth||0);footContacts(hero,state.player,heightAt);scenery?.update(state,dt);taskArt.update(state);firstLight.update(state);recut.update(state);freightCut.update(state);for(const e of state.enemies){const monster=isMonster(e);if(!enemies.has(e.id))enemies.set(e.id,monster?creature(scene,mesh,e.type):actor(scene,mesh,e.type==='drifter'?0x5e7667:0x7e795c,true,e.type));const model=enemies.get(e.id);if(monster)poseCreature(model,e,t);else{pose(model,{...e,aim:e.state==='chase'&&(e.aimTime||0)>.1},t,true);model.root.position.y+=heightAt(e.x,e.z);footContacts(model,e,heightAt);}}
  ...
  for(const item of ITEMS){const mesh=itemMeshes.get(item.id);mesh.visible=!state.taken.has(item.id);const iw=chapter.water?.find(w=>Math.abs(item.x-w.x)<w.w/2&&Math.abs(item.z-w.z)<w.d/2);mesh.position.y=item.underwater?heightAt(item.x,item.z)-(iw?.depth||1)+.38:heightAt(item.x,item.z)+.42+Math.sin(t*1.4+item.x)*.035;}
  ...
  for(const p of state.projectiles){if(!bottles.has(p)){const bottle=mesh('cyl',[.08,.27,.08],0x96b5a0);scene.add(bottle);bottles.set(p,bottle);}const q=bottles.get(p),f=1-p.life/p.total;if(p.trajectory){const at=throwPosition(p.trajectory,f);q.position.set(at.x,at.y,at.z);}else q.position.set(p.x+(p.to.x-p.x)*f,heightAt(p.x,p.z)*(1-f)+heightAt(p.to.x,p.to.z)*f+1+Math.sin(f*Math.PI)*2,p.z+(p.to.z-p.z)*f);q.rotation.z=f*7;}
  ...
  if(chapter.id==='natatorium'&&p.submerged&&!(xr.isActive()&&xr.isDiorama?.())){scene.fog.color.setHex(0x24545b);scene.fog.density=.052;renderer.toneMappingExposure=.78;}else{scene.fog.color.copy(baseFogColor);scene.fog.density=baseFogDensity;renderer.toneMappingExposure=baseExposure;}sun.target.position.set(p.x,heightAt(p.x,p.z)+1,p.z);sun.position.set(p.x-28,heightAt(p.x,p.z)+48,p.z+20);sun.target.updateMatrixWorld();
  humans.update(state);rainFilm.update();listenArt.update(state);graphics.update();living.update(state);atmosphere.update(state);currentworks.update(state,{viewer:xr.isActive()?[p.x,heightAt(p.x,p.z)+1.6,p.z]:camera.position.toArray(),xr:xr.isActive(),ar:xr.isActive()&&xr.preferences?.().view?.endsWith('-ar')});
  if(xr.isActive()){hero.root.visible=!!xr.isDiorama?.();xr.update(state,view,dt);throwGuide.update(state,view.aim,xr.aimYaw());renderer.xr.updateCamera(xr.camera);const stereoCamera=renderer.xr.getCamera();if(xr.isDiorama?.()){scans.restoreInstances();staticCulling.restore();}else{scans.cull(stereoCamera,false);staticCulling.update(stereoCamera,false);}xr.render();}else{throwGuide.update(state,view.aim,view.yaw);scans.cull(camera);staticCulling.update(camera,renderer.shadowMap.enabled);cinema.render();}
 }
 function aimDirection(state){
  ...
  const hits=raycaster.intersectObjects([...blockMeshes.filter(m=>!m.userData.obstacle.disabled),...targets],true).filter(hit=>!miniature||xr.containsWorldPoint(hit.point,!!hit.object.userData.obstacle));
 ...
 function dispose(keepRenderer=false){dead=true;throwGuide.dispose();currentworks.dispose();listenArt.dispose();humans.dispose();if(!keepRenderer)renderer.setAnimationLoop(null);cinema.dispose();living.dispose();atmosphere.dispose();scans.dispose();graphics.dispose();const gs=new Set(),ms=new Set(),ts=new Set();[scene,...blockMeshes].forEach(root=>root.traverse(o=>{if(o.geometry)gs.add(o.geometry);for(const m of Array.isArray(o.material)?o.material:o.material?[o.material]:[]){ms.add(m);for(const v of Object.values(m))if(v?.isTexture)ts.add(v);}}));Object.values(geos).forEach(g=>gs.add(g));for(const m of A.mats.values()){ms.add(m);for(const v of Object.values(m))if(v?.isTexture)ts.add(v);}gs.forEach(g=>g.dispose());ms.forEach(m=>m.dispose());ts.forEach(t=>t.dispose());scenery?.dispose();environment.dispose();if(!keepRenderer)renderer.dispose();}
 return {prepareEnvironment:()=>currentworks.prepare(renderer,camera),needsEnvironmentPreparation:()=>currentworks.needsPreparation(),setCurrentworks:value=>currentworks.settings({enabled:!!value}),portalEnvironment:arEnvironment,renderAR(xrCamera,state){renderWaistAR(renderer,scene,xrCamera,arEnvironment,heightAt(state.player.x,state.player.z)-(state.player.swimDepth||0)+.92);},bindXR(adapter){xr=adapter;},xrSupported:()=>xr.supported(),enterXR:()=>xr.enter(),exitXR:()=>xr.exit(),xrStatus:()=>xr.stats(),setCinematic(value){cinemaEnabled=!!value;cinema.set(cinemaEnabled,requestedLow);},setMotion:value=>{currentworks.settings({quiet:!value});living.setMotion(value);atmosphere.setMotion(value);},setRainworn(human,film){humanEnabled=!!human;filmEnabled=!!film;humans.set(humanEnabled,requestedLow);rainFilm.set(filmEnabled,requestedLow);},retryHumans:()=>humans.retry(),visualStatus:()=>({throwGuide:throwGuide.stats(),currentworks:currentworks.stats(),freightCut:freightCut.stats(),recut:recut.stats(),aquatic:chapter.id==='natatorium'?scenery?.stats?.()||null:null,rainworn:{humans:humans.stats(),film:rainFilm.stats()},firstLight:firstLight.stats(),characters:{skinned:hero.skin.isSkinnedMesh,bones:hero.bones.length,vertices:hero.skin.geometry.attributes.position.count,humanoids:1+[...enemies.values()].filter(e=>e.skin).length},tasks:taskArt.count,cinema:cinema.stats(),living:living.stats(),atmosphere:atmosphere.stats()}),setScanned:value=>scans.set(value),assetStatus:()=>scans.report(),dispose,renderer,scene,camera,hero,enemies,render,aimDirection,project,resize,stats:()=>({staticInstances:staticCulling.stats(),scannedMeshes:scans.report().texturedMeshes,calls:renderer.info.render.calls,triangles:renderer.info.render.triangles}),cameraState:()=>({x:camera.position.x,y:camera.position.y,z:camera.position.z,near:camera.near,heroVisible:hero.root.visible}),setQuality(low){requestedLow=!!low;currentworks.settings({low:!!low});humans.set(humanEnabled,xr.isActive()?true:low);rainFilm.set(filmEnabled,xr.isActive()?true:low);graphics.set(xr.isActive()?true:low);cinema.set(cinemaEnabled,low);}};
}
