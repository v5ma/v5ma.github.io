...
const view={yaw:0,pitch:-.09,aim:false,snap:true,shoulder:1},touch={x:0,z:0,aim:false,fire:false,listen:false};let settings={mute:false,low:false,sensitivity:85};
...
$('start').onclick=()=>start(true);$('continue').onclick=()=>start(false);$('resume').onclick=()=>setMode('play');$('retry').onclick=()=>start(false);$('to-title').onclick=()=>{setMode('title');syncTitle();};$('result-retry').onclick=()=>start(false);$('result-new').onclick=()=>start(true);$('result-next').onclick=()=>{$('chapter-select').value='conservatory';start(true);};$('map-button').onclick=()=>act('map');$('pack-button').onclick=()=>act('pack');$('map-close').onclick=closePanel;$('pack-close').onclick=closePanel;$('help-button').onclick=()=>{if(mode==='title'){$('title').hidden=true;mode='pause';$('pause').hidden=false;}else act('pause');};
...
function tick(now){requestAnimationFrame(tick);if(!scene)return;const dt=Math.min(.05,(now-(last||now))/1000);last=now;
 ...
 scene.render(state,view,dt);view.snap=false;for(const event of state.events)if(event.seq>eventSeq){eventSeq=event.seq;sound(event);}if(++frame%3===0){ui.hud();stealth.update(state,view,mode);}
}
