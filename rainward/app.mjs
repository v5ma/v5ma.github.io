...
function start(fresh=false){const old=restore(read(SAVE));if(fresh&&old&&mode==='title'&&!confirm('Start a new expedition? Your current Rainward checkpoint will be replaced.'))return;state=!fresh&&old?old:createGame();eventSeq=0;view.yaw=0;view.pitch=-.09;view.snap=true;view.shoulder=1;if(fresh)write(SAVE,checkpoint(state));setMode('play');}
