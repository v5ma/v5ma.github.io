...
const view={yaw:0,pitch:-.09,aim:false,snap:true,shoulder:1},touch={x:0,z:0,aim:false,fire:false,listen:false};let settings={mute:false,low:false,sensitivity:85,scanned:true};
...
try{const d=JSON.parse(read(SETTINGS)||'null');if(d){settings.mute=!!d.mute;settings.low=!!d.low;settings.scanned=d.scanned!==false;settings.sensitivity=Math.max(30,Math.min(170,Number(d.sensitivity)||85));}}catch{}
...
for(const id of ['muted','low','sensitivity','scanned']){const el=$(id);if(id==='sensitivity')el.value=settings.sensitivity;else el.checked=settings[id==='muted'?'mute':id];el.oninput=()=>{settings[id==='muted'?'mute':id]=id==='sensitivity'?Number(el.value):el.checked;write(SETTINGS,JSON.stringify(settings));scene?.setQuality(settings.low);scene?.setScanned(settings.scanned);resize();audioStart();};}
