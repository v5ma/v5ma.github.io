...
export function checkpoint(s){syncDrops(s);return JSON.stringify({version:3,completedTasks:s.completedTasks||[],trackedTask:s.trackedTask||null,level:s.level||'district',puzzle:s.puzzle,drops:s.drops||[],defeated:s.enemies.filter(e=>e.hp<=0).map(e=>({id:e.id,x:e.x,z:e.z})),checkpoint:s.checkpoint,taken:[...s.taken],player:Object.fromEntries(['hp','mag','reserve','cloth','canister','bottles','medkit','smoke'].map(k=>[k,s.player[k]])),objectives:{...s.objectives},stats:{...s.stats}});}
export function restore(raw,activate=true){

 try{
  if(typeof raw!=='string'||raw.length>24000)return null;const d=JSON.parse(raw),level=d.version===1?'district':d.level,def=LEVELS[level];if(!def||![1,2,3].includes(d.version))return null;
