...
export function createGame(level='district',activate=true){const data=LEVELS[level];if(!data)throw Error('Unknown chapter');if(activate)useLevel(level);return {level,fieldNotes:[],guideRoute:level==='district'?'garden':null,completedTasks:[],trackedTask:null,drops:[],puzzle:data.puzzle?{wheels:[...data.puzzle.initial],solved:false,clueRead:false}:null,t:0,status:'playing',player:{...data.start,y:levelHeight(level,data.start.x,data.start.z),hp:100,stamina:100,stance:'stand',yaw:0,speed:0,aim:false,listen:false,noise:0,vx:0,vz:0,oxygen:100,waterMode:'dry',submerged:false,swimDepth:0,exhausted:false,invulnerable:0,dodge:0,vault:null,gun:"pistol",equipped:"pistol",arsenal:{pistol:{mag:6,reserve:12},rifle:{mag:0,reserve:0}},meleeWeapon:"fists",meleeDurability:0,melee:null,healing:null,mag:6,reserve:12,reload:0,shotCD:0,cloth:0,canister:0,bottles:0,medkit:0,smoke:0,craft:null},enemies:data.patrols.map(e=>({...e,x:e.points[0][0],z:e.points[0][1],hp:enemyProfile(e.type).hp,index:1,state:'patrol',awareness:0,target:null,path:[],repath:0,timer:0,attack:0,seen:false,lastNoise:0})),taken:new Set(),objectives:{cell:false,crank:false},smokes:[],projectiles:[],sounds:[],events:[],serial:0,stats:{melee:0,shots:0,alerts:0,takedowns:0,hits:0,bottles:0,escapes:0,seconds:0},checkpoint:data.shelters[0].id,hint:data.intro||(level==='district'?'Find the signal battery and gate spindle. The wet grass is your cover.':'Explore the archive and glasshouse. Read the inscription and open the north waterway.'),hintTime:5};}
...
export function checkpoint(s){syncDrops(s);return JSON.stringify({version:4,fieldNotes:s.fieldNotes||[],guideRoute:s.guideRoute||null,arsenal:arsenal(s.player),gun:s.player.gun||"pistol",equipped:s.player.equipped||"pistol",meleeWeapon:s.player.meleeWeapon||"fists",meleeDurability:s.player.meleeDurability||0,completedTasks:s.completedTasks||[],trackedTask:s.trackedTask||null,level:s.level||'district',puzzle:s.puzzle,drops:s.drops||[],defeated:s.enemies.filter(e=>e.hp<=0).map(e=>({id:e.id,x:e.x,z:e.z})),checkpoint:s.checkpoint,taken:[...s.taken],player:Object.fromEntries(['hp','mag','reserve','cloth','canister','bottles','medkit','smoke'].map(k=>[k,s.player[k]])),objectives:{...s.objectives},stats:{...s.stats}});}
export function restore(raw,activate=true){

 try{
  if(typeof raw!=='string'||raw.length>24000)return null;const d=JSON.parse(raw),level=d.version===1?'district':d.level,def=LEVELS[level];if(!def||![1,2,3,4].includes(d.version))return null;
  ...
  if(def.puzzle&&(!d.puzzle||!Array.isArray(d.puzzle.wheels)||d.puzzle.wheels.length!==def.puzzle.wheels.length||d.puzzle.wheels.some(v=>!Number.isInteger(v)||v<0||v>=def.puzzle.symbols.length)))return null;
  ...
  const completed=d.completedTasks||[],objectives={cell:def.items.some(i=>i.objective==='cell'&&d.taken.includes(i.id)),crank:def.items.some(i=>i.objective==='crank'&&d.taken.includes(i.id))};
  if(!validTaskSave(def,completed,objectives,{solved:def.puzzle&&puzzleSolved(def.puzzle,d.puzzle.wheels)}))return null;
  if(d.version===4){if(!['pistol','rifle'].includes(d.gun)||!['pistol','rifle','medkit','bottle','smoke'].includes(d.equipped)||!['fists','club','blade'].includes(d.meleeWeapon)||!Number.isInteger(d.meleeDurability)||d.meleeDurability<0||d.meleeDurability>8)return null;
