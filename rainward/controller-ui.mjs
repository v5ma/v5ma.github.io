...
export function createMenuNavigation(E){
 ...
 const items=r=>r?[...r.querySelectorAll(selector)].filter(el=>menuControlVisible(el)&&!(document.body.classList.contains('immersive-rainward')&&(el.id.startsWith('xr-')||el.tagName==='A'))):[];
 ...
 function move(sign){const r=enter(),all=items(r);if(!all.length)return;const i=all.indexOf(current(r,all));focus(all[(i+sign+all.length)%all.length]);E.sound?.('move');}
 ...
 function activate(){const r=enter(),all=items(r),el=current(r,all);if(!el)return;E.sound?.('select');if(E.activate?.(el))return;focus(el,false);if(el.tagName==='SELECT')adjust(1);else if(el.type!=='range')el.click();}
 ...
 function update(pad,dt){const r=enter();const hint=document.getElementById('controller-menu-hint');if(hint)hint.hidden=!pad.connected||!r;
  ...
  if(pad.back){E.sound?.('back');E.closePanel();return;}if(pad.actions?.includes('pause')){E.menuButton();return;}if(pad.actions?.includes('map')){if(!menuOnly)E.viewButton();return;}
 ...
 document.addEventListener('keydown',event=>{if(E.mode==='play'||E.mode==='xr')return;if(!root())return;const code=event.code;let fn=null;
  if((code==='Enter'||code==='Space')&&E.mode==='pack'&&document.activeElement?.id?.startsWith('craft-')){E.craftKey?.(true);}if(code==='Escape'||code==='KeyP')fn=()=>E.closePanel();else if(code==='Tab')fn=()=>move(event.shiftKey?-1:1);else if(code==='ArrowDown'||code==='ArrowUp')fn=()=>move(code==='ArrowDown'?1:-1);else if(code==='ArrowRight'||code==='ArrowLeft')fn=()=>adjust(code==='ArrowRight'?1:-1);else if(code==='Enter'||code==='Space')fn=activate;
 ...
 document.addEventListener('keyup',e=>{if(e.code==='Enter'||e.code==='Space')E.craftKey?.(false);});
