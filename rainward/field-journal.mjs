import {createPlaceBriefing} from './place-briefing.mjs';
...
export function createFieldJournal(getState){
 ...
 const heading=document.createElement('h3'),intro=document.createElement('p'),list=document.createElement('div');heading.textContent='PEOPLE, PLACES, AND A WAY THROUGH';intro.textContent='Required tasks open the way out. Optional tasks reveal the district, award supplies, or open useful routes. Select a task to mark it in the world. Progress is recorded at shelters.';panel.append(heading,intro,list);host.insertBefore(panel,document.getElementById('map'));let signature='';const routes=createPlaceBriefing(getState,panel);
 function update(){routes.update();const s=getState(),key=s.level+'/'+s.completedTasks.join()+'/'+s.trackedTask+'/'+JSON.stringify(s.objectives)+'/'+s.puzzle?.solved;if(key===signature)return;signature=key;const focusId=list.contains(document.activeElement)?document.activeElement.id:null;list.replaceChildren();
  ...
  for(const t of tasksFor(s)){const done=taskDone(s,t.id),ready=taskReady(s,t),row=document.createElement('article');row.className='field-task'+(done?' complete':'')+(s.trackedTask===t.id?' tracked':'');row.dataset.task=t.id;
   const title=document.createElement('strong');title.textContent=(done?'COMPLETE / ':t.required?'REQUIRED / ':'OPTIONAL / ')+t.title;const text=document.createElement('p');text.textContent=t.description;const detail=document.createElement('small');detail.textContent=done?(t.completionHint||'Task complete.')+' Progress is recorded at the next shelter.':ready?'Ready to complete. Approach the station and press E / Y.':'First: '+taskBlockReason(s,t)+'.';row.append(title,text,detail);
