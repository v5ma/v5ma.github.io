...
export function createFieldJournal(getState){
 ...
 function update(){const s=getState(),key=s.level+'/'+s.completedTasks.join()+'/'+s.trackedTask+'/'+JSON.stringify(s.objectives)+'/'+s.puzzle?.solved;if(key===signature)return;signature=key;list.replaceChildren();
  ...
  for(const t of tasksFor(s)){const done=taskDone(s,t.id),ready=taskReady(s,t),row=document.createElement('article');row.className='field-task'+(done?' complete':'')+(s.trackedTask===t.id?' tracked':'');row.dataset.task=t.id;
   ...
   const button=document.createElement('button');button.textContent=done?'COMPLETED':s.trackedTask===t.id?'STOP TRACKING':'TRACK THIS TASK';button.disabled=done;button.onclick=()=>{s.trackedTask=s.trackedTask===t.id?null:t.id;signature='';update();};row.append(button);list.append(row);
  }
 }
 return {update};
}
