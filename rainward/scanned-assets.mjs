import {compactVisibleInstances} from './scan-culling.mjs';
...
export function createScannedAssets(scene,renderer,chapter,{heightAt,onEnvironment=()=>{}}){
 let disposed=false,enabled=true,started=false,environment=null;const bindings=[],surfaces=new Map(),roots=[],instanceMatrices=new Map(),textures=new Set(),bitmaps=new Set(),clonedGeometries=new Set(),groups=[],replacements=[];
 ...
 const status={enabled:true,pending:true,total:8,done:0,errors:[],surfaces:0,modelVariants:0,instances:0,texturedMeshes:0,visibleInstances:0};
 ...
 function instances(variant,placements,label,normalize=false){if(!placements.length)return null;const mesh=new T.InstancedMesh(variant.geometry,variant.material,placements.length);mesh.name=label;mesh.userData.scannedVisual=true;mesh.userData.authoredUV=true;mesh.castShadow=!['Scanned fern clumps','Scanned edge boulder','Scanned canyon rock'].includes(label);mesh.receiveShadow=true;
  const temp=new T.Object3D(),matrices=[];for(const [i,p]of placements.entries()){temp.position.set(p.x,p.y??heightAt(p.x,p.z),p.z);temp.rotation.set(0,p.yaw||0,0);const s=p.scale||1;if(p.axes)temp.scale.set(p.axes.x/variant.size.x,p.axes.y/variant.size.y,p.axes.z/variant.size.z);else temp.scale.setScalar(s*(normalize?1/Math.max(variant.size.x,variant.size.z):1));temp.updateMatrix();mesh.setMatrixAt(i,temp.matrix);matrices.push(temp.matrix.clone());}instanceMatrices.set(mesh,matrices);mesh.instanceMatrix.needsUpdate=true;mesh.computeBoundingSphere();mesh.visible=enabled;scene.add(mesh);groups.push(mesh);status.instances+=placements.length;status.texturedMeshes++;return mesh;
 }
