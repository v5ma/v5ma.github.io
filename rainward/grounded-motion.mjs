 ...
 * Recovered from PR147; current integration decisions are in MERGE-RECONCILIATION.md.
 * Mesh, bind proportions and material code remain the published Field Desk version.
...
export class FootContact{
 constructor(){this.anchor=v();this.point=v();this.velocity=v();this.offset=v();this.offsetVelocity=v();this.input=v();this.elapsed=1;this.duration=.10;this.locked=false;this.ready=false;this.age=0;this.waitForLift=false;}
 reset(){this.ready=false;this.locked=false;this.age=0;this.waitForLift=false;}
 update(input,wantsContact,dt,forceRelease=false,transition=.10){
  const nextDuration=clamp(Number.isFinite(transition)?transition:.10,.02,.10);
  ...
  if(change){this.duration=nextDuration;this.locked=wantsContact;if(this.locked)this.anchor.copy(input);const next=this.locked?this.anchor:input;this.offset.copy(this.point).sub(next);this.offsetVelocity.copy(this.velocity).sub(this.locked?v():inputVelocity);this.offsetVelocity.clampLength(0,8);this.elapsed=0;this.age=0;}
  else this.elapsed+=dt;
  this.age+=dt;const duration=this.duration,t=clamp(this.elapsed/duration,0,1),h=2*t*t*t-3*t*t+1,hv=(t*t*t-2*t*t+t)*duration;
...
function modeOf(p){return p.hp<=0?'dead':p.waterMode==='swim'?'swim':p.vault?'vault':p.dodge>0?'dodge':p.stance==='prone'?'prone':p.stance==='crouch'?'crouch':'stand';}
export function applyGroundedMotion(a,p,time,dt,heightAt=()=>0){
 ...
 m.speed+=(clamp(instant,0,14)-m.speed)*(1-Math.exp(-dt*14));
 ...
 if(!grounded){m.feet.forEach(f=>f.reset());}
  ...
  target.y=support.height+.105+(contactWanted?0:Math.sin(Math.PI*air)**2*(crouch?.075:.10+.08*run));
  ...
  const wasLocked=f.locked;const transition=moving?Math.min(.10,stanceFraction*stride/Math.max(m.speed,.1)*.30):.10;const point=f.update(target,contactWanted,dt,tooFar,transition);if(!wasLocked&&f.locked)m.stats.plants++;
  ...
  if(f.locked){m.stats.locked++;if(f.elapsed>=f.duration)m.stats.maxPlantError=Math.max(m.stats.maxPlantError,error);}
 }
