...
export function createXRSight(){
 ...
 lens.renderOrder=10000;group.add(lens);group.position.set(0,.12,-.16);group.visible=false;let active=false,draws=0,last=-Infinity;
 function render(renderer,scene,rig,hero,origin,direction,t,enabled){
  active=!!enabled;group.visible=active;if(!active||t-last<1/30)return;last=t;camera.position.copy(origin);camera.lookAt(origin.clone().add(direction));camera.updateMatrixWorld();
  ...
  try{rig.visible=false;hero.visible=false;renderer.xr.enabled=false;renderer.autoClear=true;renderer.setRenderTarget(target);renderer.setViewport(0,0,256,256);renderer.setScissorTest(false);renderer.render(scene,camera);draws++;}
 ...
 return {group,render,stats:()=>({active,draws,size:256,headsetFovChanged:false}),dispose(){target.dispose();group.removeFromParent();lens.geometry.dispose();lens.material.dispose();}};
}
