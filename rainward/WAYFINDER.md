...
The map now distinguishes dotted garden routes, solid terrace approaches and the inside-unlocked yard return. A closed crossing is marked with an X rather than suggesting a route already open. The expandable CLINIC / TERRACE / RETURN ROUTE journal entry explains the elevation and the save-at-shelter rule. Its native summary is reachable with Xbox controls and through the XR spatial menu; READ TEXT exposes the explanation inside XR. No enemy positions are disclosed. The underlying routes, enemy statistics, objectives, supplies and detection remain unchanged.
