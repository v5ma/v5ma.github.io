...
export function toggleSubmerge(s){const p=s.player,w=deepWater(p);if(!w)return false;if(p.reload||p.craft||p.healing||p.melee)return false;p.submerged=!p.submerged;p.stance='stand';p.vx=p.vz=0;if(p.submerged){p.swimDepth=Math.min(w.depth-.35,1.7);emit(s,'submerge',{x:p.x,z:p.z});hint(s,'Underwater. Watch your air. A surfaces.');}else{p.swimDepth=WATER.surfaceDepth;emit(s,'surface',{x:p.x,z:p.z});hint(s,'Back at the surface.');}return true;}
export function surfaceWater(s){const p=s.player;if(p.waterMode!=='swim')return false;if(p.submerged){p.submerged=false;p.swimDepth=WATER.surfaceDepth;emit(s,'surface',{x:p.x,z:p.z});hint(s,'Surface reached.');}return true;}
export function updateAquatic(s,input,dt){const p=s.player,w=waterAt(p),deep=w?.swimmable&&w.depth>=.8,prior=p.waterMode||'dry';
 ...
 p.waterMode='swim';p.stance='stand';p.dodge=0;p.vault=null;p.aim=false;p.listen=false;if(prior!=='swim'){p.submerged=false;p.swimDepth=WATER.surfaceDepth;emit(s,'water-enter',{x:p.x,z:p.z,deep:true});hint(s,'Deep water. B dives. A surfaces.');}
