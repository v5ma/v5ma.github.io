...
export function updateAquatic(s,input,dt){const p=s.player,w=waterAt(p),deep=w?.swimmable&&w.depth>=.8,prior=p.waterMode||'dry';
 ...
 p.waterMode='swim';p.stance='stand';p.dodge=0;p.vault=null;p.aim=false;p.listen=false;if(prior!=='swim'){cancelAction(s,'Dry actions cancelled on entering deep water.');p.reload=0;p.reloadTotal=0;p.melee=null;p.submerged=false;p.swimDepth=WATER.surfaceDepth;emit(s,'water-enter',{x:p.x,z:p.z,deep:true});hint(s,'Deep water. B dives. A surfaces.');}
