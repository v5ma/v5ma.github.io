...
export function buildTerminus(scene,A){
 ...
 const places=createReclaimedPlacesArt(scene,A,CURRENT);
 ...
 return {update(s,dt){places.update(s);gates.forEach(g=>g.position.y+=((s.puzzle.solved?7:0)-g.position.y)*Math.min(1,dt*4));circuitMats.forEach((m,i)=>{m.color.setHex(s.puzzle.wheels[i]?0xcfd899:0x354440);m.emissive.setHex(s.puzzle.wheels[i]?0xe9c270:0x000000);m.emissiveIntensity=s.puzzle.wheels[i]?1.8:0;});},dispose(){tx.dispose();glass.dispose();}};
}
