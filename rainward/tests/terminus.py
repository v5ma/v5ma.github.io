...
with sync_playwright() as pw:
 ...
 try:
  page.goto(BASE+'/rainward/index.html',wait_until='domcontentloaded');wait(page,'window.Rainward');check(page.locator('#chapter-select option').count()==7,'Seven separate authored chapters are present')
  ...
  if MODE=='guidance':
   ...
   page.screenshot(path=str(OUT/'dispatch-gallery.png'));go(page,26,10);go(page,0,10);go(page,0,-32);go(page,0,-42);go(page,0,-64);check(page.evaluate('Math.hypot(Rainward.state.player.x,Rainward.state.player.z+65)<2.4'),'The no-kill route enters the physical tram interaction range before pressing E');page.keyboard.press('KeyE');wait(page,'Rainward.mode==="won"')
