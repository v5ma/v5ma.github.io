...
export function freightRecovery({startDelay=0,decisionDelay=.4,reactive=true}={}){
 ...
 try{
  ...
  go(-24,5,true);use(-26.1,5.8,true);go(-30,5.8);go(-31,11);go(-31,-15);go(-28,-24,true);go(-28,-42,true);go(-28,-46);go(-18,-46);record('quay preparation');go(0,-46,true);use(0,-43,true);record('extraction');
...
if(process.argv[1]?.endsWith('freight-quay-preflight.mjs')){const results=[];for(const decisionDelay of [.15,.4,.8])for(const startDelay of [0,5,10,15,20])results.push(freightRecovery({startDelay,decisionDelay}));console.log(JSON.stringify({scope:'Continuous model steering with original finite supplies, interior-lane breakaway, conservative western walking and northern quay crossing. No actor/health/clock writes; not browser or human timing evidence.',results},null,2));}
