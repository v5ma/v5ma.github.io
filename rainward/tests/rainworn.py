 ...
 try:
  ...
  for gender in ['female','male']:
   ...
   for stance in ['stand','crouch','prone']:
    start=p.evaluate('AssetTest.time');p.locator('#'+stance).click();p.wait_for_function('(start)=>AssetTest.time>=start+.5',arg=start);bounds=p.evaluate('AssetTest.bounds()');check(bounds['max'][1] < {'stand':1.84,'crouch':1.09,'prone':.46}[stance],gender+' '+stance+' uses the real fitted skin and collision-height envelope');p.screenshot(path=str(OUT/(gender+'-'+stance+'.png')))
   start=p.evaluate('AssetTest.time');p.locator('#stand').click();p.locator('#aim').click();p.wait_for_function('(start)=>AssetTest.time>=start+.5',arg=start);p.screenshot(path=str(OUT/(gender+'-aim.png')));p.locator('#back').click();p.wait_for_timeout(200);p.screenshot(path=str(OUT/(gender+'-back.png')))
