...
with sync_playwright() as pw:
 ...
 try:
  p.goto(BASE+'/rainward/tests/rainworn-harness.html');wait('window.AssetTest');check(p.evaluate('AssetTest.gltf.scene.children.length>0'),'Self-hosted adapted glTF decodes in the real browser')
  ...
  press(9,'Rainward.mode==="pause"');nav('detailedHumans');press(0);nav('low');press(0);press(1,'Rainward.mode==="play"');wait('Rainward.snapshot().visuals.rainworn.humans.active>0&&Rainward.snapshot().visuals.rainworn.film.active');frames(5);p.screenshot(path=str(OUT/'game-rainworn-full.png'))
  ...
  press(9,'Rainward.mode==="pause"');nav('rainFilm');press(0);press(1,'Rainward.mode==="play"');wait('Rainward.snapshot().visuals.rainworn.film.amount===0');frames(4);p.screenshot(path=str(OUT/'game-dry-full.png'))
  ...
  press(9,'Rainward.mode==="pause"');nav('rainFilm');press(0);nav('low');press(0);press(1,'Rainward.mode==="play"');wait('!Rainward.snapshot().visuals.rainworn.film.active');check(True,'Reduced Graphics bypasses rain-film shading while retaining the fitted hero')
  ...
  (OUT/'report.json').write_text(json.dumps({'passed':len(checks),'checks':checks,'errors':errors,'console':console,'scope':'Native HTTP/WebGL, decoded CC0 human assets, original keyboard/standard-pad actions, safe shelter fixture for visual comparisons, isolated network failure. Not physical controller, performance-tier or artistic approval.'},indent=2))
