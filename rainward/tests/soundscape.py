...
with sync_playwright() as pw:
 ...
 try:
  ...
  p.close();c=b.new_context(viewport={'width':1050,'height':740});c.add_init_script("localStorage.setItem('svgn.rainward.v1.freefield',JSON.stringify({freeStride:false,xrLayout:'legacy',pinnedXR:true,footsteps:100,waterVolume:100,score:'legacy'}));localStorage.setItem('svgn.rainward.v1.settings',JSON.stringify({low:true,scanned:false,cinematic:false,masterVolume:60,musicVolume:40}))");p=c.new_page();p.set_default_timeout(120000);p.on('pageerror',lambda e:errors.append(str(e)));p.goto(BASE+'/rainward/index.html',wait_until='domcontentloaded');p.wait_for_function('window.Rainward');p.locator('#start').click();p.wait_for_function('Rainward.snapshot().audio.context===\"running\"');p.wait_for_function('Rainward.snapshot().audio.level>.0001');check(True,'User activation produces real audible live output, not just an allocated audio context')
