...
with sync_playwright() as pw:
 ...
 try:
  ...
  (OUT/'report.json').write_text(json.dumps({'passed':len(checks),'checks':checks,'view':VIEW,'layer':LAYER,'errors':errors,'console':console,'scope':'Actual HTTP/WebGL game using artificial XR controller/head poses, distinct grip and ray frames, real offscreen base-layer framebuffer or projection-layer texture attachments. Captures are compositor images, not desktop mirror screenshots. Read-only steering with braking and synthetic defensive controller aiming use actual finite ammunition and damage; no game-state assignment, grants or enemy removal. Not physical Quest or real room passthrough approval.'},indent=2))
