...
with sync_playwright() as pw:
 ...
 def go(x,z):
  ...
  p.evaluate('''async({x,z})=>{const W=await import('./world.mjs'),route=W.findPath(Rainward.state.player,{x,z});route.push({x,z});await new Promise((resolve,reject)=>{let i=0;const start=performance.now(),stop=()=>{clearInterval(timer);questDevice.sources[0].gamepad.axes=[0,0,0,0];},timer=setInterval(()=>{const p=Rainward.state.player;if(Rainward.mode!=='play'||performance.now()-start>90000){stop();reject(Error('Travel stopped '+JSON.stringify({x:p.x,z:p.z,hp:p.hp,goal:route[i],mode:Rainward.mode})));return;}const q=route[i],dx=q.x-p.x,dz=q.z-p.z,len=Math.hypot(dx,dz);if(len<.35){if(++i===route.length){stop();resolve();}return;}const yaw=Rainward.view.yaw,s=Math.max(.3,Math.min(1,len)),axes=questDevice.sources[0].gamepad.axes;axes[2]=(Math.cos(yaw)*dx-Math.sin(yaw)*dz)/len*s;axes[3]=(Math.sin(yaw)*dx+Math.cos(yaw)*dz)/len*s;},20);});}''',{'x':x,'z':z});frames(3)
 def scope():
  aim();p.evaluate("questDevice.button('left',0,true)");wait('Rainward.snapshot().xr.sight.active&&Rainward.snapshot().xr.sight.sceneDraws>1');frames(4);r=p.evaluate('Rainward.snapshot().xr.sight');p.evaluate("questDevice.button('left',0,false)");frames(3);return r
 try:
  ...
  if not VIEW.startswith('diorama'):
   ...
   scope();capture('02-sight-first-chapter')
  ...
  if not VIEW.startswith('diorama'):
   second=scope();check(second['sceneDraws']>1 and not second['headsetFovChanged'],'Scope refreshes immediately in the next chapter with unchanged headset field of view')
  ...
  (OUT/'report.json').write_text(json.dumps({'passed':len(checks),'checks':checks,'view':VIEW,'layer':LAYER,'errors':errors,'console':console,'scope':'Actual HTTP/WebGL game using artificial XR controller/head poses, distinct grip and ray frames, real offscreen base-layer framebuffer or projection-layer texture attachments. Captures are compositor images, not desktop mirror screenshots. No game-state assignment, grants or enemy removal. Not physical Quest or real room passthrough approval.'},indent=2))
