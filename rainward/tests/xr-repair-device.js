...
(()=>{
 ...
 const original=navigator.xr.requestSession.bind(navigator.xr);navigator.xr.requestSession=async(...args)=>{const session=await original(...args),raf=session.requestAnimationFrame.bind(session);session.requestAnimationFrame=cb=>raf((t,frame)=>{
   const pose=frame.getPose.bind(frame);frame.getPose=space=>{const src=space?.owner;if(src&&space===src.gripSpace&&src.tracked){const a=65*Math.PI/360,q=src.gripOrientation||{x:Math.sin(a),y:0,z:0,w:Math.cos(a)};return {transform:transform(src.position,q),emulatedPosition:false};}return pose(space);};
   cb(t,frame);
