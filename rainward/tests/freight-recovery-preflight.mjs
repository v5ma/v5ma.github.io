 ...
 * Synthetic reactive healing is disclosed, including every action and failure. */
import * as M from '../model.mjs';
import * as W from '../world.mjs';
export function freightRecovery({startDelay=0,decisionDelay=.4,reactive=true}={}){
 const s=M.createGame(),trace=[],recoveryInputs=[];let lastHeal=-Infinity,allowRecovery=false;
 function tick(input={}){
 ...
 try{
  ...
  go(16.2,-14);go(16.2,-18);go(16.2,-22);go(16.2,-24);record('loading decision');
...
if(process.argv[1]?.endsWith('freight-recovery-preflight.mjs')){
 ...
 console.log(JSON.stringify({scope:'Fifteen sampled Classic model-action schedules with disclosed synthetic reactive healing and two planned, earned smokes. Not native browser/controller/hardware or unfamiliar-player approval.',results},null,2));
}
