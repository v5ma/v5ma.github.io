...
def goto(p,x,z,sprint=True):
 ...
 q=snapshot(p);print('ARRIVED',x,z,'health',q['player']['hp'],flush=True);(OUT/'navigation-progress.json').write_text(json.dumps(q,indent=2))
with sync_playwright() as pw:
 ...
 ctx.add_init_script("localStorage.setItem('svgn.rainward.v1.freefield',JSON.stringify({freeStride:false,xrLayout:'legacy',pinnedXR:true,footsteps:100,waterVolume:100,score:'legacy'}));localStorage.setItem('svgn.rainward.v1.settings',JSON.stringify({controlPreset:'classic',mute:true,low:"+'true'+",sensitivity:85}))")
 ...
 try:
  ...
  print('RENDER',snapshot(page)['renderer'],flush=True)
  ...
  if MODE=='ui':
   ...
   # The original north route crosses the quay twice. Prepare from the
   # existing clinic drawer, not a granted item or optional receiver reward.
   goto(page,-24,5);page.keyboard.press('KeyE');awaitstate('Rainward.state.taken.has("clinic-kit")')
   page.locator('#pack-button').click();page.locator('#craft-med').click();awaitstate('Rainward.state.player.medkit===2&&!Rainward.state.player.craft');page.locator('#pack-close').click()
   check(page.evaluate('Rainward.state.player.cloth===1&&Rainward.state.player.canister===0&&Rainward.state.player.smoke===1'),'Clinic preparation crafts a second medkit from existing finite supplies before the exposed return')
   ...
   check(page.evaluate('!Rainward.snapshot().visuals.freightCut.open&&Rainward.state.completedTasks.length===0'),'The complete legacy mission leaves both optional shutters sealed and every field task uncompleted')
   page.keyboard.press('KeyE');awaitstate('Rainward.mode==="won"');check(snapshot(page)['stats']['shots']==0 and snapshot(page)['stats']['takedowns']==0,'A complete live expedition can succeed without shooting or taking down an enemy');check(snapshot(page)['objectives']=={'cell':True,'crank':True},'Extraction requires both actual collected components');check(all(e['hp']>0 for e in snapshot(page)['enemies']),'All five original enemies remain alive on the unpowered mission');page.screenshot(path=str(OUT/'extracted.png'))
  ...
  (OUT/'report.json').write_text(json.dumps({'suite':MODE,'passed':len(checks),'checks':checks,'errors':errors,'state':snapshot(page),'scope':'Native HTTP/WebGL software renderer; mission and interface use the public Reduced Graphics preset; conservatory visual acceptance separately exercises full-quality captures. Gameplay uses normal keyboard/DOM events. Mission navigation uses read-only path guidance plus synthetic KeyH presses for earned medkits below 65 health. Preparation spends the original rations and clinic drawer; both optional shutters remain sealed. This is assisted software validation, not human pacing or reaction-time evidence. Physical GPU/controller/phone performance and enjoyment not certified.'},indent=2))
