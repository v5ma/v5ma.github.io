...
export function gardenMission(){
 ...
 const pause=()=>{for(let i=0;i<15;i++)tick();};
 ...
 use(-40,-3);mark('catalogue opens maintenance shutter');use(-41,-18);use(-40,-21);
 for(const [x,z]of GARDEN_RETURN)go(x,z);mark('used the optional return loop');
 for(const [x,z]of[[-18,-23],[19,-26],[26,-27],[29,4]])go(x,z);use(39,-19);M.smoke(s);
 for(const [x,z]of[[20,-6],[0,-27],[0,-37],[0,-61],[10,-66],[10,-72]])go(x,z);use(0,-72);mark('extraction');
 return {status:s.status,hp:s.player.hp,stats:s.stats,alive:s.enemies.filter(e=>e.hp>0).length,trace};
