...
test('Long acquired text remains complete and paginated above the toolbar',()=>{const f=setup();try{const text=('Complete acquired instructions. ').repeat(70);f.panel.showDocument({title:'Inscription',text});assert.equal(f.panel.document().text,text);assert.ok(f.panel.document().pages>1);assert.ok(f.panel.document().visibleLines.length<=14);f.click('desk-layout');assert.equal(f.panel.view(),'desk');assert.equal(f.panel.document(),null,'Hidden reading must not consume placement A input');f.panel.navigate({confirm:true},.016);assert.equal(f.height,-1);assert.equal(f.panel.view(),'desk');f.click('back');assert.equal(f.panel.view(),'text');assert.equal(f.panel.document().text,text);}finally{f.dispose();}});
