...
function setup(autoMap=true){
 ...
 const map={width:400,height:400},sheet={querySelectorAll:()=>native,querySelector:s=>s==='canvas#map'?map:s==='#map-legend'?{textContent:'Known routes'}:s==='#next-goal'?{textContent:'NEXT: Field clinic / 45 m / Required component'}:{textContent:'Pause'},innerText:'Native text'};
...
test('Placement hides the reading input target without discarding the acquired text',()=>{const f=setup();try{const text='An acquired inscription that must survive adjustments.';f.panel.showDocument({title:'Recorded clue',text});f.click('desk-layout');assert.equal(f.panel.document(),null,'Placement must not be intercepted by the document close shortcut');f.panel.navigate({nav:1},.016);f.panel.navigate({confirm:true},.016);assert.equal(f.height,1);f.panel.closeDesk();f.panel.collect();assert.equal(f.panel.view(),'text');assert.equal(f.panel.document().text,text);}finally{f.dispose();}});
test('The displayed map heading names the actual next objective, not only a generic instruction',()=>{const f=setup();try{f.mode('map');assert.ok(f.drawn.some(([text])=>text==='NEXT: Field clinic / 45 m / Required component'));}finally{f.dispose();}});

test('Placement footer never promises persistence independently of the actual layout write result',()=>{const f=setup();try{f.click('desk-layout');assert.ok(f.drawn.some(([text])=>text==='Position preferences / campaign saves unchanged'));assert.equal(f.drawn.some(([text])=>text==='Saved layout / no save-game changes'),false);}finally{f.dispose();}});
