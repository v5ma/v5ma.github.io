...
with sync_playwright() as p:
 ...
 c.add_init_script("localStorage.setItem('svgn.rainward.v2.chapter-checkpoints',"+json.dumps(bank)+");localStorage.setItem('svgn.rainward.v1.checkpoint',"+json.dumps(fixture)+");localStorage.setItem('svgn.rainward.v1.freefield',JSON.stringify({freeStride:false,xrLayout:'legacy',pinnedXR:true,footsteps:100,waterVolume:100,score:'legacy'}));localStorage.setItem('svgn.rainward.v1.settings',JSON.stringify({controlPreset:'classic',mute:true,low:true,scanned:false,cinematic:false,sensitivity:85}));window.pad={connected:true,mapping:'standard',index:0,id:'Xbox standard test',axes:[0,0,0,0],buttons:Array.from({length:17},()=>({pressed:false,value:0}))};window.padPolls=0;Object.defineProperty(navigator,'getGamepads',{value:()=>{padPolls++;return [pad];}});")
 ...
 try:
  ...
  check(page.evaluate('Rainward.snapshot().version')==json.loads(Path('rainward/release.json').read_text())['version'],'The upgraded HTTP application loads with all seven chapter options')
  check(page.locator('#chapter-select option').count()==7,'All seven expeditions remain selectable')
