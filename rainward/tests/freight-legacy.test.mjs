...
test('A saved drop beneath the extended pallet keeps its amounts and can be collected once',()=>{
 const s=M.createGame(),e=s.enemies.find(e=>e.id==='watch-3');
 Object.assign(e,{hp:0,x:17.8,z:-23,dropMade:true});s.drops=[{id:e.id,x:17.8,z:-23,items:{ammo:3,cloth:1}}];
 const restored=M.restore(M.checkpoint(s));assert.ok(restored);
 Object.assign(restored.player,{x:16.2,z:-23});assert.equal(M.interactable(restored)?.kind,'loot');
 const ammo=restored.player.reserve;assert.ok(M.interact(restored));assert.equal(restored.player.reserve,ammo+3);assert.equal(restored.player.cloth,1);
 const again=M.restore(M.checkpoint(restored));assert.ok(again);assert.deepEqual(again.drops[0].items,{ammo:0,cloth:0});
});
