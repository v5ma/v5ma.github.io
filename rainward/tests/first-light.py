...
with sync_playwright() as pw:
 ...
 c.add_init_script("if(!localStorage.getItem('first-light-seeded')){localStorage.setItem('svgn.rainward.v1.checkpoint',"+json.dumps(fixture)+");localStorage.setItem('first-light-seeded','1');localStorage.setItem('svgn.rainward.v1.freefield',JSON.stringify({freeStride:false,xrLayout:'legacy',pinnedXR:true,footsteps:100,waterVolume:100,score:'legacy'}));localStorage.setItem('svgn.rainward.v1.settings',JSON.stringify({mute:true,low:true,scanned:false,cinematic:false}));}window.pad={connected:true,mapping:'standard',id:'First Light Xbox-standard',index:0,axes:[0,0,0,0],buttons:Array.from({length:17},()=>({pressed:false,value:0}))};window.padPolls=0;Object.defineProperty(navigator,'getGamepads',{value:()=>{padPolls++;return [pad];}});")
 ...
 try:
  ...
  check(p.evaluate('Rainward.snapshot().version')==json.loads(Path('rainward/release.json').read_text())['version'],'The actual browser loads First Light inside the declared gameplay release')
  ...
  nav('read-garden-ledger');press(0)
  placeholder=p.locator('#note-garden-ledger p').inner_text()
  check('Find this record' in placeholder and 'beds used to catch' not in placeholder,'A controller-opened undiscovered record displays a clue instead of its story text')
  press(0)
  ...
  press(9,'Rainward.mode==="pause"');nav('low');box=p.locator('#low').bounding_box();p.locator('#low').uncheck();after_box=p.locator('#low').bounding_box()
  check(abs(box['x']-after_box['x'])<1 and abs(box['y']-after_box['y'])<1,'Switching from controller focus to mouse activation keeps the checkbox stationary')
  p.locator('#cinematic').check();press(1,'Rainward.mode==="play"');frames(8);p.screenshot(path=str(OUT/'garden-full-quality.png'))
