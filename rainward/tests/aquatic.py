...
with sync_playwright() as pw:
 ...
 c.add_init_script("localStorage.setItem('svgn.rainward.v1.checkpoint',"+json.dumps(fixture)+");localStorage.removeItem('svgn.rainward.v2.chapter-checkpoints');localStorage.setItem('svgn.rainward.v1.freefield',JSON.stringify({freeStride:false,xrLayout:'legacy',pinnedXR:true,footsteps:100,waterVolume:100,score:'legacy'}));localStorage.setItem('svgn.rainward.v1.settings',JSON.stringify({controlPreset:'survival',mute:true,low:true,scanned:false,cinematic:false,detailedHumans:false,rainFilm:true}));window.pad={connected:true,mapping:'standard',index:0,id:'Undertow Xbox-standard',axes:[0,0,0,0],buttons:Array.from({length:17},()=>({pressed:false,value:0}))};window.padPolls=0;Object.defineProperty(navigator,'getGamepads',{value:()=>{padPolls++;return [pad];}});")
