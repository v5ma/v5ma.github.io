...
with sync_playwright() as pw:
 ...
 p=c.new_page();p.set_default_timeout(90000);p.on('pageerror',lambda e:errors.append(str(e)));p.on('console',lambda m:console.append(m.text) if m.type=='error' else None);p.on('dialog',lambda d:(dialogs.append(d.type),d.dismiss()))
 ...
 try:
  ...
  check(p.evaluate('Rainward.snapshot().visuals.aquatic.activeLights')<=6,'The pool hall uses a bounded local-light budget')
  ...
  oxygen=p.evaluate('Rainward.state.player.oxygen');hold(1,'Rainward.state.player.submerged');p.wait_for_timeout(900);check(p.evaluate('Rainward.state.player.oxygen')<oxygen,'Holding B in the Survival layout dives and consumes air')
  check(p.locator('body').evaluate("e=>e.classList.contains('underwater')"),'Underwater visual treatment follows the real swim state')
  # Briefly inspect the same stationary underwater scene in full graphics, then return
  # to Reduced Graphics for traversal so acceptance does not depend on software-GPU speed.
  ...
  p.set_viewport_size({'width':1180,'height':780});press(0,'!Rainward.state.player.submerged');check(p.evaluate('Rainward.snapshot().visuals.aquatic.extraRenderTargets')==0,'Pool and caustic shaders add no full-screen render target in Reduced Graphics')
