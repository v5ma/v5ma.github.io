...
files=[ROOT/'index.html',*[p for p in GAME.iterdir() if p.is_file() and p.suffix in ['.mjs','.html','.css','.svg','.json']],*[p for p in (GAME/'vendor').rglob('*') if p.is_file()],*[p for p in (GAME/'assets').rglob('*') if p.is_file()]]
