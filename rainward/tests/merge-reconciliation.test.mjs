...
test('The selective motion recovery preserves every unchanged declared gameplay, body and licensed-asset hash after later XR UI work',()=>{
 ...
 const tools=JSON.parse(fs.readFileSync(path.join(root,'evidence/tactical-throws-20260923/render-revision.json'),'utf8'));
 assert.deepEqual(Object.keys(tools.files),['scene.mjs']);assert.equal(tools.files['scene.mjs'].before,current.files['scene.mjs'].after);assert.notEqual(tools.files['scene.mjs'].before,tools.files['scene.mjs'].after);
 ...
 for(const [name,sha] of Object.entries({...audit.protectedRuntimeSHA256,...audit.protectedAssetsSHA256})){
  ...
  assert.equal(crypto.createHash('sha256').update(fs.readFileSync(path.join(root,name))).digest('hex'),tools.files[name]?.after||current.files[name]?.after||(name===revision.file?revision.after:sha),name);
 }
});
