...
with sync_playwright() as pw:
 ...
 try:
  ...
  menu();select('last-clue');wait('!!Rainward.snapshot().xr.reading');check('No clue' in p.evaluate('Rainward.snapshot().xr.reading.text'),'Clue recall does not reveal an unacquired solution')
  reading=p.evaluate('Rainward.snapshot().xr.reading.text');click('desk-layout');wait('Rainward.snapshot().xr.panelView==="desk"')
  check(p.evaluate('Rainward.snapshot().xr.reading') is None,'Placement does not retain a hidden document-dismissal target')
  if KIND=='controllers':
   p.evaluate("questDevice.sources[1].orientation={x:0,y:1,z:0,w:0}");frames(4);p.evaluate("questDevice.pulse('right',4)");frames(4)
   check(p.evaluate('Rainward.snapshot().xr.panelView')=='desk' and p.evaluate('Rainward.mode')=='pause','Focused A adjusts the desk without dismissing the underlying reading panel')
  click('back');wait('Rainward.snapshot().xr.panelView==="text"');check(p.evaluate('Rainward.snapshot().xr.reading.text')==reading,'The entire reading content survives a placement adjustment');click('back');wait('Rainward.mode==="play"')
