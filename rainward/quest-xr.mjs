...
export function createQuestXR(E){
 ...
 const api={camera,rig,bind,detach,poll,update,enter,exit,reset,recenter,isActive:()=>active,isDiorama,changeView,containsWorldPoint:(point,environment=false)=>diorama.contains(point,environment),
  ...
  render(){const p=E.state().player;sight.render(renderer,scene,rig,renderContext.hero.root,new T.Vector3(p.x,heightAt(p.x,p.z)+HEIGHT[p.stance]*.82,p.z),new T.Vector3(...Object.values(renderContext.aimDirection(E.state()))),E.state().t,E.freefield?.scope!==false&&safe&&E.mode()==='play'&&p.aim&&p.waterMode!=='swim'&&['pistol','rifle'].includes(p.equipped));if(isDiorama())diorama.render(renderer,scene,camera,rig,renderContext.portalEnvironment);else if(viewMode==='first-person-ar')renderContext.renderAR(camera,E.state());else renderer.render(scene,camera);},
