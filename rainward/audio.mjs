...
export function createAudio(settings,getMode){
 let graph=null,context=null,state=null,view={yaw:0},chapter=null,step=0,next=0,timer=null,intensity=0,enabled=false,dead=false,resuming=false,variant=0,breathing=0,lastState=null,caption=null,captionEnd=0,uiAt=0;
 const footsteps=new Map();let triggers=0;
 const hidden=()=>typeof document!=='undefined'&&document.hidden;
 function clock(){if(!graph||!context||context.state!=='running'||hidden()||settings.mute)return;const now=context.currentTime;if(next<now-.5)next=now+.04;let budget=0;while(next<now+.18&&budget++<3){const seq=scoreStep(chapter||'district',step++);for(const n of seq.notes)graph.play(n.kind,{...n,when:next,bus:n.bus,priority:false,send:.18});next+=seq.seconds;}}
 function resetBrokenContext(){try{graph?.dispose();}catch{}graph=null;try{void context?.close?.().catch?.(()=>{});}catch{}context=null;clearInterval(timer);timer=null;resuming=false;}
 function start(){if(dead)return false;if(context&&!graph)resetBrokenContext();if(!context){try{const Constructor=globalThis.AudioContext||globalThis.webkitAudioContext;if(!Constructor)return false;const candidate=new Constructor({latencyHint:'interactive'});const candidateGraph=createSoundGraph(candidate,settings);context=candidate;graph=candidateGraph;timer=setInterval(clock,40);next=context.currentTime+.08;enabled=true;}catch{resetBrokenContext();return false;}}
  if(!graph||!context)return false;const stop=hidden()||settings.mute;if(stop){if(context.state==='running')void context.suspend().catch(()=>{});return true;}graph.configure({menu:!['play','pack'].includes(getMode()),listen:!!state?.player.listen,intensity});if(context.state==='suspended'&&!resuming){resuming=true;void context.resume().then(()=>{resuming=false;if(context)next=context.currentTime+.08;}).catch(()=>{resuming=false;});}return true;}
 const active=()=>!!(graph&&context&&context.state==='running'&&!hidden()&&!settings.mute);
 ...
 function cue(kind,point,opts={}){if(!active())return false;const space=point?spatial(point):{gain:1,pan:0,cutoff:15000};if(space.gain<.02)return false;return graph.play(kind,{duration:.5,...opts,pan:space.pan,cutoff:space.cutoff,gain:(opts.gain??.6)*space.gain,variant:variant++%4});}
 ...
 function sound(ev){triggers++;const point=ev.from||(Number.isFinite(ev.x)?ev:state?.enemies.find(e=>e.id===ev.id)||null),now=context?.currentTime||0;let type=ev.type;if(type==='sound'){if(ev.kind==='bottle')type='bottle';else return;}mark(type,point);if(!active())return;const at=(kind,delay,opts={})=>cue(kind,point,{when:now+delay,...opts});
 ...
 function update(s,v,dt){state=s;view=v;if(lastState!==s){lastState=s;chapter=s.level;step=0;intensity=0;breathing=0;footsteps.clear();caption=null;captionEnd=0;if(graph&&context){graph.clear();next=context.currentTime+.08;graph.loop(THEMES[chapter]?.air||'rain');graph.loop('wind',.25);}}intensity=smoothThreat(intensity,threatLevel(s),dt);if(graph)graph.configure({intensity,listen:!!s.player.listen,menu:!['play','pack'].includes(getMode())});if(hidden()&&context?.state==='running'){void context.suspend().catch(()=>{});return;}if(!active()||!['play','pack'].includes(getMode()))return;if(!graph.loopCount){graph.loop(THEMES[s.level]?.air||'rain');graph.loop('wind',.25);}for(const e of s.enemies){if(e.hp<=0||dist(e,s.player)>22)continue;const before=footsteps.get(e.id)||{at:s.t,x:e.x,z:e.z};const moving=Math.hypot(e.x-before.x,e.z-before.z)>.45;if(moving&&s.t-before.at>.38){cue('step-'+surfaceAt(e),e,{duration:.35,bus:'threat',gain:e.type==='brute'?.95:.54});footsteps.set(e.id,{at:s.t,x:e.x,z:e.z});}else if(!footsteps.has(e.id))footsteps.set(e.id,before);}if(s.player.hp<35&&s.t-breathing>1.3){breathing=s.t;cue('heartbeat',null,{duration:.5,gain:.30});}clock();}
 ...
 function dispose(){dead=true;clearInterval(timer);timer=null;try{graph?.dispose();}catch{}graph=null;try{void context?.close?.().catch?.(()=>{});}catch{}context=null;if(typeof window!=='undefined'){window.removeEventListener('pointerdown',gesture);window.removeEventListener('keydown',gesture);document.removeEventListener('visibilitychange',gesture);}}
