...
export function createFirstLightUI(E){const $=id=>document.getElementById(id),make=(tag,text)=>{const e=document.createElement(tag);if(text)e.textContent=text;return e;};
 ...
 const marker=make('div');marker.id='route-beacon';marker.setAttribute('aria-hidden','true');$('hud').append(marker);let previous=null,signature='',lastRouteAt=-1,path=[],target=null,adviceKey=null,adviceUntil=0;
 ...
 function update(){const s=E.state;if(previous!==s){previous=s;signature='';lastRouteAt=-1;path=[];adviceKey=null;}refresh();const play=E.mode==='play',help=E.settings.fieldGuidance!==false,info=play&&help?openingAdvice(s,E.settings.controlPreset,E.pad.connected):null;if(!info)adviceKey=null;if(info&&info.key!==adviceKey){adviceKey=info.key;adviceUntil=s.t+10;}advice.hidden=!info||s.t>adviceUntil;if(info){heading.textContent=info.title;text.textContent=info.body;}
