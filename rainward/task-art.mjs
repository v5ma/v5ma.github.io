...
export function createTaskArt(scene,A){
 const stations=[];
 for(const t of CURRENT.tasks||[]){if(t.customArt){stations.push({t,lamp:null});continue;}const group=new T.Group();group.position.set(t.x,heightAt(t.x,t.z),t.z-.7);const kind=t.kind;
 ...
 return {update(s){for(const {t,lamp}of stations){if(!lamp)continue;const done=s.completedTasks.includes(t.id);lamp.material.color.setHex(done?0x7bad7a:0xc49f60);lamp.material.emissive.setHex(done?0x335c39:0x80683f);}},count:stations.length};
}
