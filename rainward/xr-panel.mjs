...
export function createXRPanel(E){
 ...
 function collect(){
  ...
  if(mapView&&mapCanvas){const height=565,width=height*mapCanvas.width/mapCanvas.height;c.drawImage(mapCanvas,(1024-width)/2,214,width,height);c.fillStyle='#e5eeee';c.font='22px sans-serif';wrap(r.querySelector('#map-legend')?.textContent,75).slice(0,2).forEach((l,i)=>c.fillText(l,28,807+i*25));}if(reading){c.fillStyle='#e5eeee';c.font='23px sans-serif';(documentPages?documentPages[textPage]:lines.slice(textPage*19,textPage*19+19)).forEach((l,i)=>c.fillText(l,28,242+i*30));}
