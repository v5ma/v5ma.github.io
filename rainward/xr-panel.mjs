...
export function createXRPanel(E){
 ...
 let page=0,textPage=0,reading=false,mapView=false,lastMode='',lastSignature='',lastFocus=null,hoverId=null,rows=[],all=[],caption='',hold=null,documentText=null,drawnDocument=null;
 ...
 function collect(){
  ...
  const mapCanvas=r?.querySelector('canvas#map');const toolbar=[action(documentText?'CLOSE':'BACK','back',()=>{documentText=null;reading=false;E.back();}),action(documentText?'CLOSE':mapView?'CONTROLS':reading&&mapCanvas?'VIEW MAP':reading?'CONTROLS':'READ TEXT','read',()=>{if(documentText){documentText=null;reading=false;E.back();return;}if(mapView){mapView=false;reading=false;}else if(reading&&mapCanvas){reading=false;mapView=true;}else reading=!reading;textPage=0;}),action('RECENTER','recenter',()=>E.recenter()),action('EXIT XR','exit',()=>E.exit())];
  ...
  const signature=JSON.stringify([mode,caption,E.status(),rows.map(r=>[r.label,!!r.disabled]),page,textPage,reading,mapView,reading?(documentPages||lines):[],hold?.id,focus?.id,hoverId,progress?.percent]);
  ...
  c.fillStyle='#e5eeee';c.font='24px sans-serif';wrap(E.status(),73).slice(0,2).forEach((l,i)=>c.fillText(l,28,91+i*29));c.fillStyle='#e4d2a9';wrap(progress?.label||caption,71).slice(0,2).forEach((l,i)=>c.fillText(l,28,154+i*27));
  if(mapView&&mapCanvas){const height=565,width=height*mapCanvas.width/mapCanvas.height;c.drawImage(mapCanvas,(1024-width)/2,214,width,height);c.fillStyle='#e5eeee';c.font='22px sans-serif';wrap(r.querySelector('#map-legend')?.textContent,75).slice(0,2).forEach((l,i)=>c.fillText(l,28,807+i*25));}if(reading){c.fillStyle='#e5eeee';c.font=documentPages?'32px sans-serif':'23px sans-serif';(documentPages?documentPages[textPage]:lines.slice(textPage*19,textPage*19+19)).forEach((l,i)=>c.fillText(l,28,242+i*(documentPages?42:30)));}
  for(const row of rows){c.fillStyle=row.disabled?'#1d2a2d':hold?.id===row.id?'#456b62':hoverId===row.id?'#426362':row.element===focus?'#4a5a71':'#263b40';c.fillRect(row.x,row.y,row.w,row.h);c.lineWidth=hoverId===row.id?4:1;c.strokeStyle=hoverId===row.id?'#f7df9d':'#819995';c.strokeRect(row.x,row.y,row.w,row.h);c.fillStyle=row.disabled?'#97aaa9':'#ffffff';c.font=(row.w<300?'bold 22px':'25px')+' sans-serif';wrap((row.disabled?'UNAVAILABLE: ':row.held?'HOLD: ':'')+row.label,row.w<300?15:67).slice(0,2).forEach((l,i)=>c.fillText(l,row.x+14,row.y+29+i*25));}
