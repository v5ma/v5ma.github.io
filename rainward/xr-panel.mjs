...
export function createXRPanel(E){
 ...
 function collect(){
  ...
  c.fillStyle='#b6cac6';c.font='22px sans-serif';c.textAlign='center';if(!hasDesk)c.fillText((reading?textPage+1:page+1)+' / '+(reading?maxText:pages),512,880);else{c.font='16px sans-serif';c.fillText(deskView?'Saved layout / no save-game changes':(reading?textPage+1:page+1)+' / '+(reading?maxText:pages),512,1010);}c.textAlign='left';texture.needsUpdate=true;
 }
