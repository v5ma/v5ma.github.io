...
export function createXRPanel(E){
 ...
 let page=0,textPage=0,reading=false,lastMode='',lastSignature='',rows=[],all=[],caption='',hold=null;
 ...
 function collect(){
  const mode=E.mode();if(mode!==lastMode){lastMode=mode;page=0;textPage=0;reading=false;release();lastSignature='';}
  ...
  const pages=Math.max(1,Math.ceil(all.length/8));page=Math.min(page,pages-1);
  rows=all.slice(page*8,page*8+8).map((r,i)=>({...r,x:28,y:214+i*78,w:968,h:68}));
  const toolbar=[action('BACK','back',()=>E.back()),action(reading?'CONTROLS':'READ TEXT','read',()=>{reading=!reading;textPage=0;}),action('RECENTER','recenter',()=>E.recenter()),action('EXIT XR','exit',()=>E.exit())];
  ...
  if(reading)rows=rows.filter(r=>r.y>=838);
  ...
  const signature=JSON.stringify([mode,caption,E.status(),rows.map(r=>r.label),page,textPage,reading,reading?lines:[],hold?.id]);
  ...
  if(reading){c.fillStyle='#e5eeee';c.font='23px sans-serif';lines.slice(textPage*19,textPage*19+19).forEach((l,i)=>c.fillText(l,28,242+i*30));}
  for(const row of rows){c.fillStyle=hold?.id===row.id?'#456b62':'#263b40';c.fillRect(row.x,row.y,row.w,row.h);c.strokeStyle='#819995';c.strokeRect(row.x,row.y,row.w,row.h);c.fillStyle='#ffffff';c.font=(row.w<300?'bold 22px':'25px')+' sans-serif';wrap((row.held?'HOLD: ':'')+row.label,row.w<300?15:67).slice(0,2).forEach((l,i)=>c.fillText(l,row.x+14,row.y+29+i*25));}
 ...
 function select(row){if(!row)return;if(row.held){hold=row;E.hold(row,true);lastSignature='';}else change(()=>row.run());}
 return {mesh,collect,hit,select,release,held:()=>hold,page:()=>page,rows:()=>rows.map(({id,x,y,w,h,label})=>({id,x,y,w,h,label})),dispose(){release();mesh.removeFromParent();mesh.geometry.dispose();mesh.material.dispose();texture.dispose();}};
}
