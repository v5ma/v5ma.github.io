...
export function createXRPanel(E){
 ...
 let page=0,textPage=0,reading=false,mapView=false,lastMode='',lastSignature='',lastFocus=null,hoverId=null,rows=[],all=[],caption='',hold=null;
 ...
 function collect(){
  ...
  const signature=JSON.stringify([mode,caption,E.status(),rows.map(r=>[r.label,!!r.disabled]),page,textPage,reading,mapView,reading?lines:[],hold?.id,focus?.id,hoverId,progress?.percent]);
