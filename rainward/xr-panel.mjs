...
export function createXRPanel(E){
 ...
 let page=0,textPage=0,reading=false,mapView=false,lastMode='',lastSignature='',lastFocus=null,hoverId=null,rows=[],all=[],caption='',hold=null,documentText=null,drawnDocument=null,deskView=false,deskFocus=0,navSign=0,navTime=0;
 ...
 function collect(){
  ...
  if(hasDesk){if(deskView){rows=rows.filter(r=>r.y>=918);rows.unshift(...E.deskActions().map((a,i)=>({...a,x:i<8?28+(i%2)*492:28,y:214+(i<8?Math.floor(i/2):i-4)*78,w:i<8?476:968,h:68})));}rows.push({...action(deskView?'DONE / BACK TO PANEL':'DESK POSITION','desk-layout',()=>{deskView=!deskView;}),x:326,y:838,w:372,h:64});}
  const documentPages=documentText?readingPages(documentText.text,52,14):null,lines=wrap(r?.innerText||E.instructions(),64),maxText=documentPages?.length||Math.max(1,Math.ceil(lines.length/19));textPage=Math.min(textPage,maxText-1);drawnDocument=documentText?{title:documentText.title,text:documentText.text,page:textPage,pages:maxText,visibleLines:documentPages[textPage]}:null;
  const progress=E.progress?.();
  const signature=JSON.stringify([mode,deskView,caption,E.status(),rows.map(r=>[r.label,!!r.disabled]),page,textPage,reading,mapView,reading?(documentPages||lines):[],hold?.id,focus?.id,hoverId,progress?.percent]);
  ...
  c.fillStyle='#e5eeee';c.font='24px sans-serif';wrap(E.status(),73).slice(0,2).forEach((l,i)=>c.fillText(l,28,91+i*29));c.fillStyle='#e4d2a9';wrap(deskView?'Stable while you look around. Only these controls or Recall move the desk.':progress?.label||caption,71).slice(0,2).forEach((l,i)=>c.fillText(l,28,154+i*27));
  if(!deskView&&mapView&&mapCanvas){const height=565,width=height*mapCanvas.width/mapCanvas.height;c.drawImage(mapCanvas,(1024-width)/2,214,width,height);c.fillStyle='#e5eeee';c.font='22px sans-serif';wrap(r.querySelector('#map-legend')?.textContent,75).slice(0,2).forEach((l,i)=>c.fillText(l,28,807+i*25));}if(reading&&!deskView){c.fillStyle='#e5eeee';c.font=documentPages?'32px sans-serif':'23px sans-serif';(documentPages?documentPages[textPage]:lines.slice(textPage*19,textPage*19+19)).forEach((l,i)=>c.fillText(l,28,242+i*(documentPages?40:30)));}
  for(const row of rows){c.fillStyle=row.disabled?'#1d2a2d':hold?.id===row.id?'#456b62':(hoverId===row.id||deskView&&row.id===E.deskActions?.()[deskFocus]?.id)?'#426362':row.element===focus?'#4a5a71':'#263b40';c.fillRect(row.x,row.y,row.w,row.h);c.lineWidth=hoverId===row.id?4:1;c.strokeStyle=hoverId===row.id?'#f7df9d':'#819995';c.strokeRect(row.x,row.y,row.w,row.h);c.fillStyle=row.disabled?'#97aaa9':'#ffffff';c.font=(row.w<300?'bold 22px':'25px')+' sans-serif';wrap((row.disabled?'UNAVAILABLE: ':row.held?'HOLD: ':'')+row.label,row.w<300?15:67).slice(0,2).forEach((l,i)=>c.fillText(l,row.x+14,row.y+29+i*25));}
  if(progress){c.fillStyle='#294140';c.fillRect(28,196,968,8);c.fillStyle='#edcf84';c.fillRect(28,196,968*progress.percent/100,8);}
  c.fillStyle='#b6cac6';c.font='22px sans-serif';c.textAlign='center';if(!hasDesk)c.fillText((reading?textPage+1:page+1)+' / '+(reading?maxText:pages),512,880);else{c.font='16px sans-serif';c.fillText(deskView?'Position preferences / campaign saves unchanged':(reading?textPage+1:page+1)+' / '+(reading?maxText:pages),512,1010);}c.textAlign='left';texture.needsUpdate=true;
 }
 function navigate(sample,dt){
  if(!deskView)return false;const actions=E.deskActions?.()||[];if(!actions.length)return false;const value=Math.abs(sample.nav||0)>.55?sample.nav:sample.navX||0,sign=Math.abs(value)>.55?Math.sign(value):0;
  navTime-=Math.max(0,Math.min(.1,dt));if(sign&&(sign!==navSign||navTime<=0)){deskFocus=(deskFocus+sign+actions.length)%actions.length;navTime=sign!==navSign?.34:.14;lastSignature='';}navSign=sign;if(!sign)navTime=0;
  if(sample.confirm)select(actions[deskFocus]);return true;
 }
 ...
 function select(row,sourceId=null){if(!row||row.disabled||row.element?.disabled)return;row.element?.focus({preventScroll:true});lastFocus=document.activeElement;if(row.held){hold={...row,sourceId};E.hold(row,true);lastSignature='';}else change(()=>row.run());}
 return {mesh,collect,hit,select,release,navigate,closeDesk(){deskView=false;lastSignature='';},showDocument(value){collect();deskView=false;documentText={title:String(value.title||'Field message'),text:String(value.text||'')};reading=true;mapView=false;textPage=0;lastSignature='';collect();},clearDocument(){documentText=null;drawnDocument=null;reading=false;},document:()=>deskView?null:drawnDocument,setHover(id){hoverId=id;},hover:()=>hoverId,held:()=>hold,page:()=>page,view:()=>deskView?'desk':mapView?'map':reading?'text':'controls',rows:()=>rows.map(({id,x,y,w,h,label,disabled})=>({id,x,y,w,h,label,disabled:!!disabled})),dispose(){release();mesh.removeFromParent();mesh.geometry.dispose();mesh.material.dispose();texture.dispose();}};
}
