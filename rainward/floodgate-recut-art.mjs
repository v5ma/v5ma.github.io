...
export const TERRACE_SURFACE=Object.freeze({color:0x54615b,roughness:.96,clearcoat:.15,clearcoatRoughness:.75});
export function createFloodgateRecutArt(scene,A,chapter){
 if(chapter.id!=='district')return {update(){},stats:()=>null,dispose(){}};
 const cues=createClinicCues(scene),surfaces=[],floorMaterial=A.mat(TERRACE_SURFACE.color,'paving',{side:T.DoubleSide,roughness:TERRACE_SURFACE.roughness,clearcoat:TERRACE_SURFACE.clearcoat,clearcoatRoughness:TERRACE_SURFACE.clearcoatRoughness});
 for(const r of RECUT_SURFACES){
  ...
  const g=new T.BufferGeometry();g.setAttribute('position',new T.Float32BufferAttribute(positions,3));g.setIndex(indices);g.computeVertexNormals();const mesh=new T.Mesh(g,floorMaterial);mesh.name='Recut '+r.id;mesh.receiveShadow=true;scene.add(mesh);surfaces.push(mesh);
 }
 ...
 let open=false;
 return {update(s){open=(s.completedTasks||[]).includes(RETURN_TASK);gate.position.y=open?3.45:0;gate.scale.y=open?.08:1;closedLabel.visible=!open;openLabel.visible=open;amber.material.color.setHex(open?0x9dccad:0xe9bb71);},stats:()=>({revision:RECUT_REVISION,phase:'playable-graybox',rampSurfaces:surfaces.length,upperHeight:2.4,returnGateOpen:open,routeChevrons:cues.count,terraceSurface:'world-scaled-paving',terraceTexture:!!floorMaterial.map,gateLabel:open?'YARD RETURN OPEN':'RELEASE FROM INSIDE',finalArtApproved:false}),dispose(){}};
}
