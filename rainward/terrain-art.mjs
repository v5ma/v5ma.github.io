...
export function buildTerrain(scene,A){
 function sheet(width,color,tile,yOffset,type){
  ...
  const base=A.mat(color,type),material=base.clone();material.color.setHex(color);material.roughness=.86;worldTexturing(material,tile);
  ...
  A.scans?.bind(material,type);const mesh=new T.Mesh(geometry,material);mesh.name='Continuous conservatory ground';mesh.receiveShadow=true;scene.add(mesh);return mesh;
 }
 const ground=sheet(104,0x78806b,3,0,'ground'),walkway=sheet(13,0xaaa58c,2.4,.025,'paving');
