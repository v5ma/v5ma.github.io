...
export function clipBoom(target,desired,radius=.20,boxes=OBSTACLES){
 desired={...desired,y:Math.max(heightAt(desired.x,desired.z)+radius+.04,desired.y)};
 ...
 for(let t=0;t<=limit;t+=.08){const x=target.x+direction.x*t,z=target.z+direction.z*t,y=target.y+direction.y*t;if(y<heightAt(x,z)+radius){limit=Math.max(0,t-.08);break;}}
