...
import {LEVELS,dist,obstruction,heightAt,syncRouteGates,routeTransitionBlocked} from './world.mjs';
export const tasksFor=s=>LEVELS[s.level]?.tasks||[];
export const taskDone=(s,id)=>(s.completedTasks||[]).includes(id);
export function taskReady(s,t){return !routeTransitionBlocked(s,t.id)&&(t.requires||[]).every(r=>r==='puzzle'?!!s.puzzle?.solved:r.startsWith('task:')?taskDone(s,r.slice(5)):s.objectives[r]===true);}
...
export function taskBlockReason(s,t){if(routeTransitionBlocked(s,t.id))return 'clear the marked partition floor; someone is beneath it';return (t.requires||[]).filter(r=>r==='puzzle'?!s.puzzle?.solved:r.startsWith('task:')?!taskDone(s,r.slice(5)):!s.objectives[r]).map(r=>r==='puzzle'?'open the puzzle gate':r.startsWith('task:')?'complete '+tasksFor(s).find(v=>v.id===r.slice(5))?.title:'recover '+LEVELS[s.level].objectiveNames[r]).join('; ');}
export function completeTask(s,id){const t=tasksFor(s).find(t=>t.id===id);if(!t||s.status!=='playing'||s.player.craft||taskDone(s,id)||!taskReady(s,t)||taskTarget(s)?.id!==id)return false;s.completedTasks.push(id);syncRouteGates(s.completedTasks);
 for(const [key,amount]of Object.entries(t.reward||{})){const k=key==='ammo'?'reserve':key==='health'?'hp':key,cap=k==='reserve'?weapon(s.player).reserve:k==='hp'?100:12;s.player[k]=Math.min(cap,s.player[k]+amount);}if(s.trackedTask===id)s.trackedTask=null;return true;}
