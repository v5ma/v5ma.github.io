...
export const STAGE_METRES=Object.freeze({width:1.6,depth:1.2,height:1.65});
export const DIORAMA_DEFAULTS=Object.freeze({view:'first-person',shell:'both-open',scale:.04,size:1,follow:true});
export const XR_PREFS_KEY='svgn.rainward.v1.xr-view';
export function normalizeDiorama(value={}){
 const v=value&&typeof value==='object'?value:{};
 return {view:XR_VIEWS.includes(v.view)?v.view:'first-person',shell:DIORAMA_SHELLS.includes(v.shell)?v.shell:'both-open',scale:typeof v.scale==='number'&&Number.isFinite(v.scale)?Math.min(.08,Math.max(.02,v.scale)):.04,size:typeof v.size==='number'&&Number.isFinite(v.size)?Math.min(1.6,Math.max(.7,v.size)):1,follow:typeof v.follow==='boolean'?v.follow:true};
}
