...
import {normalizeDiorama,shellOpenings,STAGE_METRES,stageMetres} from './diorama-core.mjs';
...
export function createWorldPortal(){
 ...
 let anchor=null,heading=0,config=normalizeDiorama(),stage=stageMetres(),centre=new T.Vector3(),eye=new T.Vector3(),enabled=false;
 ...
 function update(rig,head,player,yaw,ground,dt,settings){
  config=normalizeDiorama(settings);stage=stageMetres(config.size);enabled=config.view.startsWith('diorama');if(!enabled)return;
  ...
  const rotation=new T.Quaternion().setFromAxisAngle(UP,heading-yaw),scale=new T.Vector3().setScalar(config.scale),target=anchor.clone().add(new T.Vector3(0,stage.height/2,0));
  ...
  shell.position.copy(anchor);shell.rotation.set(0,heading,0);shell.scale.setScalar(config.size);shell.updateMatrixWorld(true);materials.configure(anchor,heading,stage);
