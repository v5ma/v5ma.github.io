...
export function pose(a,p,time,enemy=false){const stance=p.stance||'stand',moving=enemy?(p.state==='patrol'?1:p.state==='chase'?2:1):p.speed;const rate=time*(moving>3?10:7),prone=stance==='prone',crouch=stance==='crouch';a.root.position.set(p.x,prone?.28:0,p.z);a.root.rotation.y=p.yaw;
