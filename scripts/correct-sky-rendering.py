...
# A tilted camera needs enough depth for the large backdrop, not the old 1000
# unit plane that clipped a diagonal black wedge out of the visible sky.
if 'const far=active()' not in s:
 s=s.replace('    const m=window.__merged;','    const m=window.__merged;\n    const far=active()||window.__delivery?.state.menu?10000:1000;\n    if(camera.far!==far){camera.far=far;camera.updateProjectionMatrix();}')
p.write_text(s)
