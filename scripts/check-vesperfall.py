...
files=['index.html','projects.css']+['vesperfall/'+n for n in ['index.html','style.css','core.js','input.js','art.js','app.js','cover.svg','roadmap.html','roadmap.json','README.md','release.json','vendor/aframe-1.8.0.min.js','vendor/AFRAME-LICENSE.txt','vendor/manifest.json']]
for name in files:
...
manifest={'version':'0.1.0','source':subprocess.check_output(['git','rev-parse','HEAD'],cwd=root,text=True).strip(),'files':{n:hashlib.sha256((root/n).read_bytes()).hexdigest() for n in files}}
