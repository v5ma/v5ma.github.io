...
async def main():
 report={'mode':MODE,'base':BASE,'checks':[],'errors':[],'consoleErrors':[],'success':False,'physicalDevicesTested':False,'inputOnly':True};start=time.time()
 ...
 async with async_playwright() as p:
  ...
  async def trigger():
   hands=await page.evaluate('!!__xrFixture.session.inputSources[0].hand');await page.evaluate('__xrFixture.pinch=.012' if hands else '__xrFixture.right.gamepad.buttons[0]={pressed:true,value:1}');await frames(3);await page.evaluate('__xrFixture.pinch=.06' if hands else '__xrFixture.right.gamepad.buttons[0]={pressed:false,value:0}');await frames(4)
  async def click(row,u=.5):
   point=await page.evaluate("""async([r,u])=>{const T=await import('./vendor/three.module.js'),p=NeighborhoodMissions.panel();return new T.Vector3(((r.x+r.w*u)/1024-.5)*p.width,(.5-(r.y+r.h/2)/1024)*p.height,.03).applyMatrix4(new T.Matrix4().fromArray(p.referenceMatrix)).toArray();}""",[row,u]);await aim(point);await trigger()
  ...
  try:
   await page.goto(BASE,wait_until='domcontentloaded');await wait('window.NeighborhoodMissions&&!document.querySelector("#start").disabled');await page.bring_to_front();assert await page.evaluate('NeighborhoodMissions.inspect().version')=='0.16.1'
   ...
   await page.evaluate('__xrFixture.tracking=false');await wait('NeighborhoodMissions.inspect().paused');assert not await page.evaluate('NeighborhoodMissions.inspect().xr.input.y');await page.evaluate('__xrFixture.tracking=true');await frames(12);await choose('ward-xr');await choose('xr-exit');await wait('!NeighborhoodMissions.inspect().xr.active');ok('Tracking loss clears movement; native Exit XR restores desktop access')
   ...
   try:report['final']=await page.evaluate('NeighborhoodMissions.inspect()');await page.screenshot(path=str(OUT/'failure.png'))
