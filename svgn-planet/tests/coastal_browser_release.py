...
async def main():
 ...
 async with async_playwright() as p:
  ...
  try:
   ...
   s=await state();assert s['version']=='0.8.0' and not s['failed'];ok('Controller starts Coastal Pulse')
   ...
   await page.evaluate("__pad.buttons[6]={pressed:true,touched:true,value:1}");await page.wait_for_function('SVGNPlanet.inspect().speed<.04',timeout=90000);await page.evaluate("__pad.buttons[6]={pressed:false,touched:false,value:0}");assert (await state())['speed']<.1;ok('LT braking is explicit and effective')
   ...
   await page.evaluate('__pad.connected=false');await page.wait_for_function('SVGNPlanet.inspect().paused',timeout=90000);assert (await state())['paused'];ok('Controller disconnect pauses the game')
