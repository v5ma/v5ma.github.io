...
with sync_playwright() as p:
 ...
 try:
  ...
  entry=page.locator('.project.planet a.primary-link')
  check(entry.count()==1 and '3D Paper Delivery' in entry.inner_text(),'The homepage identifies the repaired game and opens the existing app')
  entry.click();page.wait_for_function('window.SVGNPlanet&&SVGNPlanet.inspect().render');page.locator('#start').click()
  page.wait_for_function('SVGNPlanet.inspect().started&&SVGNPlanet.inspect().steps>3&&SVGNPlanet.inspect().render.triangles>1000');begin=time.monotonic()
  ...
  if not mobile:
   ...
   for i in range(1,8):
    ...
    walk(page,street(t));walk(page,street(t,side*4.2));page.keyboard.press('KeyE');page.wait_for_function('SVGNPlanet.inspect().deliveries.length==='+str(i+1));walk(page,street(t))
   ...
   walk(page,street(-9,-4.2));page.keyboard.press('KeyE');page.wait_for_function('SVGNPlanet.inspect().complete');check(snap(page)['complete'],'Returning to the depot completes the round')
   ...
   page.reload();page.wait_for_function('window.SVGNPlanet&&SVGNPlanet.inspect().render');page.locator('#start').click();page.wait_for_function('SVGNPlanet.inspect().steps>3');check(len(snap(page)['deliveries'])==8 and snap(page)['started'],'Saved completion resumes into a playable scene')
  else:
   ...
   page.screenshot(path=str(OUT/'mobile-riding.png'));page.locator('#touch-jump').tap();page.wait_for_function('SVGNPlanet.inspect().events.some(e=>e.type==="jump")');check(any(e['type']=='jump' for e in snap(page)['events']),'Touch hopping reaches the simulation')
   page.locator('#pause').tap();page.wait_for_function('SVGNPlanet.inspect().paused');old=snap(page)['steps'];page.wait_for_timeout(500);check(snap(page)['steps']==old,'Pause holds the simulation without leaving the game');page.locator('#resume').tap();page.wait_for_function('!SVGNPlanet.inspect().paused')
