...
async def main():
 ...
 async with async_playwright() as p:
  ...
  context=await browser.new_context(viewport={'width':960,'height':600},device_scale_factor=1)
  ...
  try:
   ...
   await page.wait_for_timeout(1800);await capture('street-start.png');state=await inspect()
   ...
   await press(13);assert await page.locator('#jobs-dialog').is_visible();await capture('jobs-controller.png');await press(0);assert not (await inspect())['paused'];assert (await inspect())['coastal']['active'];passed('D-pad down opens jobs and A accepts a contract')
   ...
   await focus('audio-muted');await press(0);assert (await inspect())['coastal']['audio']['mix']['muted'];await press(0);await focus('invert-x');await press(0);assert (await inspect())['coastal']['audio']['mix']['invertX'];await press(0);await capture('mixer-controller.png');await press(1);assert not (await inspect())['paused'];passed('Mute and independent camera inversion are controller-accessible')
   ...
   await press(8);assert await page.locator('#map-dialog').is_visible();await focus('district-select');await press(15);await press(15);await focus('transit');await press(0);await page.wait_for_timeout(3200);state=await inspect();assert not state['paused'];await capture('city-district.png');passed('Controller-only map, district selection, transit and return',state['render']['city'])
   state=await seed('local-repair');await press(2);assert await page.locator('#repair-dialog').is_visible();await capture('signal-repair.png');await press(1);assert not (await inspect())['paused'];assert (await inspect())['coastal']['active']['index']==0;passed('Repair dialog opens with X and B returns without losing progress')
   ...
   await page.set_viewport_size({'width':390,'height':844});await page.wait_for_timeout(900);await press(13);await capture('mobile-jobs.png');await press(1);assert not (await inspect())['paused'];passed('Small-screen contracts remain controller-navigable')
   ...
   try:report['final']=await inspect();await capture('failure.png')
