...
async def main():
 report={'version':'0.10.0','checks':[],'errors':[],'physicalHardwareTested':False,'performanceCertification':False,'base':BASE};start=time.time()
 async with async_playwright() as p:
  ...
  async def seed(complete=False):
   await page.evaluate("""async complete=>{const m=await import('./model.mjs?v=0.10.0'),h=await import('./homecoming.mjs?v=0.10.0');const s=m.initial();s.ride=true;s.n=m.street(-8,1.5);s.jobs.wallet=77;s.homecoming.project='workshop';s.homecoming.accepted=complete;if(complete)s.jobs.completed=h.CHAPTER.map(c=>c.job);window.__seed=JSON.stringify(m.saveData(s));localStorage.setItem(m.SAVE_KEY,__seed);} """,complete)
