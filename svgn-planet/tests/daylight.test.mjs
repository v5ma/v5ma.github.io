...
test('Daylight is a background draw through the actual inverse camera, not a close sphere or reference image',()=>{
 ...
 assert.ok(sky.mesh.material.uniforms.dip.value>0&&sky.mesh.material.uniforms.dip.value<.5);assert.equal(sky.inspect().rendered,0);sky.mesh.onAfterRender();assert.equal(sky.inspect().rendered,1);sky.update(camera,up,false);assert.equal(sky.mesh.visible,false);
});
...
test('Leafy canopies have deterministic bounded, finite geometry rather than a balloon per cluster',()=>{
 ...
 assert.equal(a.attributes.position.count,512*6);assert.deepEqual(a.attributes.position.array,b.attributes.position.array);assert.notDeepEqual(a.attributes.position.array,c.attributes.position.array);
