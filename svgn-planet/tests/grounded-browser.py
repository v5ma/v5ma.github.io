...
async def main():
 ...
 async with async_playwright() as p:
  ...
  try:
   ...
   await press(9);await page.evaluate('__pad.buttons[7]={pressed:true,value:1}');await press(1);await page.wait_for_timeout(400);assert not (await state())['controller']['boost'];await page.evaluate('__pad.buttons[7]={pressed:false,value:0}');await wait('NeighborhoodController.inspect().gameplayReady');ok('Held acceleration cannot leak from a menu into gameplay')
