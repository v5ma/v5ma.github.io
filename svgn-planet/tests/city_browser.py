...
with sync_playwright() as pw:
 ...
 try:
  ...
  go(p,175,24);hack(p,'camera');check(read(p)['city']['loop']>0,'The scout loops the yard camera without exposing its body')
  ...
  if read(p)['city']['crouch']:p.keyboard.press('KeyC')
  ...
  check(not any(e['type']=='safe-retry' for e in read(p)['city']['events']),'The investigation completes without an intercepted-body retry')
