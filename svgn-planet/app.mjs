import {mountAquaticDOM,createAquaticUI,poolPreferences} from './aquatics-ui.mjs';
...
buildCoastalDOM();mountHomecomingDOM();mountAquaticDOM();
const $=id=>document.getElementById(id),canvas=$('world'),audio=createCoastalAudio(),metrics=createFrameHealth();
const dialogs=['pause-dialog','map-dialog','help-dialog','jobs-dialog','repair-dialog','photo-dialog','homecoming-dialog','health-dialog','production-dialog','pool-board','pool-valve','pool-result'];
...
function clear(){if(typeof poolUI!=='undefined')poolUI.resetTouch();metrics.gap();touchBraking=false;keys.clear();stick=[0,0];jump=boost=false;acc=0;previousPose=null;$('stick').querySelector('i').style.transform='';}
...
function changeView(){if(s.aquatics.inside){s.aquatics.view=s.aquatics.view==='chase'?'first':'chase';renderDirty=true;return;}if(!view)return;renderDirty=true;mode=(mode+1)%3;view.setCamera(['street','adventure','overview'][mode]);$('view').textContent=['Street view','Adventure view','World overview'][mode];}
...
function action(code){
 ...
 if(code==='KeyE'){if(!poolUI.interact()&&!storyUI.interact()&&!pulseUI.interact(!nearest(s))){if(nearest(s))interact(s);else throwPaper(s);}persist();}
...
canvas.addEventListener('pointerdown',e=>{orbitDrag={id:e.pointerId,x:e.clientX};canvas.setPointerCapture(e.pointerId);});canvas.addEventListener('pointermove',e=>{if(orbitDrag?.id===e.pointerId){s.aquatics.inside?poolLook(s,-(e.clientX-orbitDrag.x)*.005,0):view?.orbitBy((e.clientX-orbitDrag.x)*.005);renderDirty=true;orbitDrag.x=e.clientX;}});for(const type of ['pointerup','pointercancel','lostpointercapture'])canvas.addEventListener(type,()=>orbitDrag=null);
...
function openMap(){if(!started||paused)return;if(s.aquatics.inside){poolUI.board();return;}drawMap();openDialog('map-dialog');$('map-close').focus();}$('atlas').onclick=openMap;$('map-close').onclick=resume;
...
function transit(id){if(!travelTo(s,id))return false;if(s.aquatics.inside){s.aquatics.inside=false;view.aquatic.release();}waypoint=id==='post'?null:id;view.recenter();view.setCamera('street');mode=0;$('view').textContent='Street view';renderDirty=true;persist();resume();return true;}
...
function ui(){
 ...
 $('save-status').textContent=storage?'Progress saved on this device':'Storage unavailable - play continues';$('toast').textContent=s.toast;$('toast').classList.toggle('visible',s.toastT>0);$('context').textContent=started&&near?(padState.connected?'X | ':'E | ')+near.name:'';pulseUI.update();storyUI.update();poolUI.update();
}
...
$('copy-diagnostics').onclick=async()=>{const text=JSON.stringify({version:VERSION,artVersion:ART_VERSION,started,paused,graphicsLost,events:diagnostics,render:view?.inspect(),coastal:pulseUI.inspect(),homecoming:storyUI.inspect(),aquatics:poolUI.inspect(),viewport:[innerWidth,innerHeight,devicePixelRatio]},null,2);$('diagnostics').hidden=false;$('diagnostics').value=text;try{await navigator.clipboard.writeText(text);}catch{}};
const poolUI=createAquaticUI({state:()=>s,view:()=>view,audio,open:openDialog,resume,persist,changed:()=>{renderDirty=true;clear();}});
...
function frame(now){
 ...
 try{
  if(started&&!paused){
   if(padState.lookX||padState.lookY){if(s.aquatics.inside)poolLook(s,padState.lookX*dt*2*preferences.sensitivity*(preferences.invertX?-1:1),padState.lookY*dt*1.2*preferences.sensitivity*(preferences.invertY?-1:1));else view.lookBy(padState.lookX*dt*2*preferences.sensitivity*(preferences.invertX?-1:1),padState.lookY*dt*1.2*preferences.sensitivity*(preferences.invertY?-1:1));renderDirty=true;}
   acc=Math.min(.20,acc+dt);while(acc>=1/60){previousPose=capturePose(s);if(s.aquatics.inside)stepPool(s,poolInput(),1/60);else step(s,input(),1/60);jump=false;acc-=1/60;}
  }else{acc=0;previousPose=null;}
  if((renderDirty||(started&&!paused))&&(view.fps>=60||now-lastRender>=1000/view.fps-.5)){
   const workStart=performance.now();view.update(paused?0:Math.min(.2,(now-lastRender)/1000),{...renderPose(previousPose,s,acc*60),poolAlpha:acc*60},{vehicle,poolSimple:poolPreferences.simple});if(started&&!paused)metrics.frame(now,s.time,performance.now()-workStart);else metrics.gap();lastRender=now;renderDirty=false;
  }
  audio.update(s,{active:started&&!paused,vehicle,brake:padState.brake||touchBraking||keys.has('ControlLeft')||keys.has('ControlRight')||keys.has('KeyB'),traffic:view.life?.traffic(),rain:s.aquatics.inside?0:view.atmosphere?.rainLevel||0,pool:s.aquatics});
