...
import {createXRTheater} from './xr-theater.mjs';
...
const vault=createSaveVault(()=>localStorage);let touchBraking=false,saved=vault.load(),storage=true,xr=null;
...
function persist(){s.vehicle=vehicle;const raw=JSON.stringify(saveData(s));if(raw===saveStamp)return;storage=vault.commit(raw);if(storage)saveStamp=raw;}
...
function input(){const x=Number(keys.has('KeyD')||keys.has('ArrowRight'))-Number(keys.has('KeyA')||keys.has('ArrowLeft'))+stick[0]+padState.x+xrInput.x,z=Number(keys.has('KeyW')||keys.has('ArrowUp'))-Number(keys.has('KeyS')||keys.has('ArrowDown'))+stick[1]+padState.y+xrInput.y,b=view.movementBasis(s),magnitude=Math.min(1,Math.hypot(x,z));return {direction:magnitude>.05?norm(add(mul(b.right,x),mul(b.forward,z))):null,throttle:magnitude,brake:xrInput.brake||padState.brake||touchBraking||keys.has('ControlLeft')||keys.has('ControlRight')||keys.has('KeyB'),boost:xrInput.boost||padState.boost||boost||keys.has('ShiftLeft')||keys.has('ShiftRight'),jump};}
function drawMap(){
