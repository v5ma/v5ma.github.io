...
let saved=null,storage=true;try{saved=readSave(localStorage.getItem(SAVE_KEY));if(!saved)saved=readSave(localStorage.getItem(LEGACY_SAVE_KEY));}catch{storage=false;}
...
function clear(){keys.clear();stick=[0,0];jump=boost=false;acc=0;previousPose=null;$('stick').querySelector('i').style.transform='';}
...
const touchBrake=document.createElement('button');touchBrake.id='touch-brake';touchBrake.textContent='BRAKE';$('touch-boost').after(touchBrake);let touchBraking=false;touchBrake.onpointerdown=e=>{touchBraking=true;e.currentTarget.setPointerCapture(e.pointerId);};for(const type of ['pointerup','pointercancel','lostpointercapture'])touchBrake.addEventListener(type,()=>touchBraking=false);
