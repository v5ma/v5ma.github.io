...
import {readTide,tideSave,stepWatercraft,tickWater} from './tidewater-core.mjs';
...
export function step(s,input={},dt=1/60){
 ...
 if(s.tide.boat){stepWatercraft(s,input,dt);tickWater(s,previous,dt);return;}
 ...
 if(!s.tide.active)tickJobs(s,previous,dt);tickWater(s,previous,dt);
}
export function travelTo(s,id){
 ...
 s.tide.boat=false;s.n=n;s.north=stop?tangent(add(stop.mail,mul(n,-1)),n):tangent([0,0,-1],n);s.facing=[...s.north];s.speed=0;s.lift=0;s.vy=0;s.energy=1;s.boosting=false;s.paper=null;
