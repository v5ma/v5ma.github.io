...
export function loadStreetArt({root,fallback,renderer,asphalt,sidewalks,land,roadPoints}){
 ...
 const work=Promise.all([new GLTFLoader().loadAsync(new URL('street-art.gltf',base).href),material('aerial_asphalt_01',.91,.42),material('concrete_pavement',.95,.45),material('rocky_terrain_02',1,.30),texture(low?'sky-mobile.jpg':'sky-desktop.jpg',true)]).then(([gltf,road,paving,lawn,skyMap])=>{
  ...
  // Triplanar grass has no polar UV singularity or stretched ground streaks.
  lawn.side=T.FrontSide;lawn.normalMap=null;lawn.roughnessMap=null;
  lawn.onBeforeCompile=shader=>{
   ...
   shader.fragmentShader=shader.fragmentShader.replace('#include <common>','#include <common>\nvarying vec3 terrainPosition; varying vec3 terrainNormal;').replace('#include <map_fragment>',`#ifdef USE_MAP
     vec3 weights=pow(abs(normalize(terrainNormal)),vec3(6.));weights/=weights.x+weights.y+weights.z;
     vec3 p=terrainPosition*.22;
     vec4 grass=texture2D(map,p.yz)*weights.x+texture2D(map,p.xz)*weights.y+texture2D(map,p.xy)*weights.z;
     diffuseColor*=grass;
    #endif`);
