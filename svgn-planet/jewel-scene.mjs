...
import {createMaterialLibrary,gemGeometry,prepareCrystal,bevelGeometry,resolveLook,LOOK_KEY} from './jewel-materials.mjs';
...
export function createJewelScene({renderer,scene,root,sea,sun,courier,traffic,stamps,boxes,network,touch=false}){
 ...
 let look=(new URLSearchParams(location.search).get('quality')==='low'? 'light':pref.preset),quiet=pref.quiet,rebuilds=0,seen=new Set(),initial=true,draws=0,glowEnabled=false,bindingsDirty=true;sea.material=water;
 ...
 stamps.forEach(({m},i)=>{prepareCrystal(m,diamond,library.gems[i%3]);m.name='Faceted postmark / '+i;});
 ...
 for(const [i,[t,x]] of nearPos.entries()){
  ...
  const exhibit=block(gemGeometry(.64),library.displayGems[i%3],[0,1.7,0]);landmarks.at(-1).exhibit=exhibit;
 ...
 function setLook(value,persist=true){look=resolveLook(value,touch);library.setLook(look);bindingsDirty=true;glowEnabled=look==='cinematic'&&!quiet;haloMat.uniforms.amount.value=look==='light'?.5:1;renderer.transmissionResolutionScale=.5;if(persist)savePref();}
 function setQuiet(value){quiet=!!value;glowEnabled=look==='cinematic'&&!quiet;savePref();}
 function assetMaterials(libraryRoot){bindingsDirty=true;libraryRoot.traverse(o=>{if(!o.isMesh||Array.isArray(o.material))return;if(o.material.name==='MI_Glass'){o.material=library.glass;windows.push(o);}});}
 function update(dt,s,camera,overview){
  if(bindingsDirty){library.applyTo(root);bindingsDirty=false;}
