...
export function world(){
 ...
 const sites=specs.map(v=>mainSite(...v)),homes=sites.filter(s=>!['post','garden'].includes(s.id));
 const bonusStops=[
  bonusSite('bonus-oak-1','Oak Books',30,-20,-1,'BOOKS'),bonusSite('bonus-oak-2','Juniper Bakery',58,-20,-1,'BAKERY'),bonusSite('bonus-oak-3','Westside Records',102,-20,-1,'RECORDS'),
  bonusSite('bonus-market-1','Market Arcade',26,20,1,'ARCADE'),bonusSite('bonus-market-2','Sunrise Camera',54,20,1,'CAMERA'),bonusSite('bonus-market-3','Bay Repair',82,20,1,'REPAIR'),bonusSite('bonus-market-4','Harbor Cafe',122,20,1,'CAFE'),
  bonusSite('bonus-ridge-1','Ridge Flowers',38,36,1,'FLOWERS'),bonusSite('bonus-ridge-2','Skyline Market',76,36,1,'MARKET'),bonusSite('bonus-ridge-3','Lookout News',110,36,1,'NEWS')
 ];
 const trees=[];
 for(let i=0;i<260;i++){const z=rand(i+991)*2-1,a=rand(i)*Math.PI*2,n=[Math.sqrt(1-z*z)*Math.sin(a),Math.sqrt(1-z*z)*Math.cos(a),z];if(nearRoad(n)||[...sites,...bonusStops].some(s=>distance(s.n,n)<5)||height(n)<-.3)continue;trees.push({id:'tree-'+i,n,size:1.15+rand(i+2)*1.55,style:i%5===0?'fir':'oak',seed:i});}
 ...
 for(const [r,x] of [['oak',-20],['market',20],['ridge',36]])for(let i=0;i<8;i++){const t=8+i*15,side=(i%2?1:-1),n=street(t,x+side*7.2);if(![...sites,...bonusStops].some(s=>distance(s.n,n)<4))trees.push({id:'street-tree-'+r+'-'+i,n,size:1.1+rand(i+700+x)*.42,style:'oak',seed:700+i+Math.round(x)});}
 const rocks=[[65,-46,21],[100,54,27],[140,48,28],[35,51,18],[220,-30,15]].map(([t,x,size],i)=>({n:street(t,x),size,id:'rock-'+i}));
 const stars=Array.from({length:24},(_,i)=>({id:'stamp-'+i,n:street(7+i*7.5,[0,-20,20,36][i%4])}));
 const stuntGates=[{id:'stunt-1',name:'Oak sprint gate',n:street(42,-20),minSpeed:14},{id:'stunt-2',name:'Market speed gate',n:street(90,20),minSpeed:14},{id:'stunt-3',name:'Ridge boost gate',n:street(114,36),minSpeed:16}];
 const mirror=homes.map((h,i)=>({...h,id:'scenery-main-'+i,type:'cabin',side:-h.side,x:-h.x,n:street(h.t,-h.side*9.3),mail:street(h.t,-h.side*5.1),color:['#d5c9b8','#e2c8a1','#bdd0c2'][i%3]}));
 const branchScenery=bonusStops.map((h,i)=>({...h,id:'scenery-branch-'+i,bonus:false,sign:null,type:'cabin',x:h.roadX-h.side*8.4,n:street(h.t,h.roadX-h.side*8.4),mail:street(h.t,h.roadX-h.side*4.1),floors:i%3===0?3:i%2===0?2:1,color:['#d8c7a8','#aac2bd','#c8b3a8','#b7c3d0'][i%4]}));
 const buildings=[...sites,...mirror,...bonusStops,...branchScenery];
 return {sites,homes,bonusStops,buildings,trees,rocks,stars,stuntGates,roads:ROADS,roadLabels:ROAD_LABELS};
}
