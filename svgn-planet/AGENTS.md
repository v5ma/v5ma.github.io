...
Current direction and recovery: read FUTURE-DIRECTION.md, HIGHLINE.md and tests/highline-evidence/status.json. Preserve the additive Highline case, 32 m save safety ceiling and supported tower routes. Keep future-direction notes beside the game at each implementation/verification checkpoint; do not rely on chat history as the only plan.

Current graphics/room checkpoint: read ENVIRONMENT-INTEGRATION.md and tests/environment-evidence/status.json. Pinned Currentworks copies are tracked in environment/currentworks/provenance.json. Keep the host r177 engine and composed portal hooks; never assume r184 module tests establish host GPU acceptance. Preserve cancellable preparation, pausable visual clocks, isolated preferences and the optional Unsent Call sequel. Fire/Islands and South Cable Exchange are not already integrated.
