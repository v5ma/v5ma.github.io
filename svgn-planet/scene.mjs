...
export function createScene(canvas){
 ...
 const sea=new T.Mesh(new T.IcosahedronGeometry(RADIUS-.20,5),new T.MeshStandardMaterial({color:'#48bfc1',roughness:.27,metalness:.12,flatShading:true}));root.add(sea);
 // Narrow circumplanet road: it actually wraps around, with no world boundary.
 const ring=Array.from({length:361},(_,i)=>at((i-180)/180*Math.PI*RADIUS,0));road(root,ring,1.7,'#69877b',.07);
 for(const [a,b] of [[[0,0],[-9,-5]],[[0,0],[9,-10]],[[0,0],[11,6]],[[0,0],[-11,12]],[[0,0],[0,7]]]){
  const p=[];for(let i=0;i<=60;i++){const t=i/60;p.push(at(a[0]+(b[0]-a[0])*t+.7*Math.sin(t*Math.PI),a[1]+(b[1]-a[1])*t));}road(root,p,1.3,'#e4c782',.05);
 }
 for(let x=-60;x<=60;x+=2.6){const g=anchor(root,at(x,0),.11);mesh(g,'box','#cce0b4',[0,0,0],[.6,.035,.07]);}
 const buildings=WORLD.sites.map(s=>({site:s,...building(root,s)})),boxes=WORLD.sites.map(s=>({site:s,...mailbox(root,s)}));bridge(root);
 for(const t of WORLD.trees)tree(root,t);
 ...
 for(let i=0;i<100;i++){const n=at((rand(i+11)-.5)*35,(rand(i+42)-.5)*30);if(height(n)<-.3||WORLD.sites.some(s=>distance(n,s.n)<2.5))continue;const g=anchor(root,n);for(let j=0;j<3;j++)mesh(g,'ball',['#f3d67a','#ecb3c3','#dce3b7'][i%3],[j*.14,.16+rand(i)*.15,rand(j+i)*.3],[.07,.09,.07]);}
 ...
 let far=1,orbit=0,camReady=false;const defaultDistance=74,up=new T.Vector3(),behind=new T.Vector3(),position=new T.Vector3(),north=new T.Vector3();
 ...
 function update(dt,s,{snap=false,menu=false}={}){
  ...
  courier.g.position.set(...point(s.n,s.lift+.04));courier.g.quaternion.setFromRotationMatrix(frame);courier.unicycle.visible=s.ride;courier.body.position.y=s.ride?.23:0;courier.wheel.rotation.x+=s.speed*dt*2;courier.legs.forEach((l,i)=>l.rotation.x=s.ride?.14:Math.sin(s.time*11+i*Math.PI)*Math.min(.65,s.speed*.14));
  ...
  const basis=movementBasis(s);up.copy(n);north.set(...basis.forward);behind.copy(north).negate();const widthComp=camera.aspect<1?1.16:1;
