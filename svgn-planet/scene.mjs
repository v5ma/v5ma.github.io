...
export function createScene(canvas){
 ...
 const scene=new T.Scene(),camera=new T.PerspectiveCamera(55,1,.1,450);scene.background=new T.Color('#87c9df');scene.fog=new T.Fog('#b8d7d7',48,145);
 ...
 road(root,route(0),6.4,'#596969',.16);for(const side of[-1,1]){road(root,route(side*4.22),1.78,'#d4cbb5',.2);road(root,route(side*3.26),.18,'#eee5cf',.23);}
 road(root,Array.from({length:361},(_,i)=>at(i/360*Math.PI*2*RADIUS,0)),2.4,'#c6b98d',.13);
 ...
 for(const [i,s] of WORLD.sites.entries()){if(s.type!=='garden')home(stat,s,i);groundShadow(stat,s.n,3.2,3);const ps=Array.from({length:17},(_,i)=>street(s.t,s.side*(4.7+i*.11)));road(root,ps,1.35,'#c7c1ad',.24);}
 ...
 for(const r of WORLD.rocks){const g=anchor(stat,r.n);mesh(g,'ball','#839182',[0,r.size*.35,0],[r.size*.75,r.size*.7,r.size*.55]);mesh(g,'cone','#c9ccb6',[0,r.size*.8,0],[r.size*.43,r.size*.8,r.size*.4]);}
 ...
 const clouds=new T.Group();root.add(clouds);for(let i=0;i<26;i++){const g=anchor(clouds,street(-40+i*9,(rand(i+1)-.5)*90),19+rand(i)*7);for(let j=0;j<5;j++){const m=mesh(g,'ball',j%2?'#f5f0d8':'#e4eddf',[(j-2)*1.6,Math.sin(j)*.55,rand(j+i)*1.1],[2.3,1.2+rand(j),1.7]);m.castShadow=m.receiveShadow=false;}}batchStatic(clouds);clouds.traverse(o=>{o.castShadow=false;o.receiveShadow=false;});
 ...
 function setCamera(kind){mode=({close:'street',planet:'adventure',wide:'overview'})[kind]||kind;if(!CAMERA_PRESETS[mode])mode='street';camera.fov=CAMERA_PRESETS[mode].fov;camera.updateProjectionMatrix();ready=false;}
 ...
 function update(dt,s,{snap=false,vehicle='unicycle'}={}){
  ...
  if(mode==='overview'){desired.copy(n).multiplyScalar(RADIUS*(camera.aspect<1?4.2:3.05)).addScaledVector(forward,-RADIUS*.9);aim.set(0,0,0);scene.fog=null;}else{scene.fog=new T.Fog('#b8d7d7',48,145);desired.copy(aim).addScaledVector(up,preset.height).addScaledVector(forward,-preset.distance);const ray=new T.Ray(aim.clone(),desired.clone().sub(aim).normalize()),hit=new T.Vector3();let clearance=desired.distanceTo(aim);for(const h of WORLD.sites){if(h.type==='garden')continue;const sphere=new T.Sphere(new T.Vector3(...point(h.n,1.5)),2.7);if(ray.intersectSphere(sphere,hit)){const d=aim.distanceTo(hit)-.4;if(d>2&&d<clearance)clearance=d;}}desired.copy(aim).addScaledVector(ray.direction,clearance);}
