...
export const CONSOLE_DEFAULTS=Object.freeze({v:1,mount:'floor',height:.95,distance:.9,size:.72,hud:'wrist',motion:true,triggerDrive:true,driveHand:'primary',floorMap:true,mapSize:1});
const ranges={height:[.35,1.5],distance:[.55,1.5],size:[.35,1.6]};
export function parseConsolePrefs(raw){
 ...
 const floorMap=p.floorMap===undefined?true:p.floorMap,mapSize=p.mapSize===undefined?1:p.mapSize;
 if(typeof floorMap!=='boolean'||!Number.isFinite(mapSize)||mapSize<.6||mapSize>2)throw Error('Invalid floor map preferences; original data retained.');
 ...
 return Object.fromEntries(Object.keys(CONSOLE_DEFAULTS).map(k=>[k,k==='driveHand'?driveHand:k==='floorMap'?floorMap:k==='mapSize'?mapSize:p[k]]));
}
