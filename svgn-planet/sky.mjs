...
export function createSky(scene) {
 const uniforms={up:{value:new T.Vector3(0,1,0)},rays:{value:new T.Matrix4()},eye:{value:new T.Vector3()},
 ...
 fragmentShader:`precision highp float;varying vec2 screen;uniform mat4 rays;uniform vec3 eye,up,zenith,horizon,white,shade;
 ...
 void main(){vec4 world=rays*vec4(screen,1.,1.);vec3 d=normalize(world.xyz/world.w-eye);float h=dot(d,up);
 vec3 blue=mix(horizon,zenith,smoothstep(-.14,.48,h));
 vec3 p=d*(5.5/(max(.24,h+.4)))+vec3(13,2,7);
 float n=noise(p)*.59+noise(p*2.03)*.28+noise(p*4.1)*.13;
 float cover=smoothstep(.50,.68,n)*smoothstep(-.08,.14,h);
 ...
 return {update(camera,normal,visible){mesh.visible=visible;uniforms.up.value.copy(normal);uniforms.eye.value.copy(camera.position);uniforms.rays.value.multiplyMatrices(camera.matrixWorld,camera.projectionMatrixInverse);},
