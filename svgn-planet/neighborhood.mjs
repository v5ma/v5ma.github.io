...
export function home(parent,s,i){const g=new T.Group();parent.add(g);faceSurface(g,s.n,norm(add(s.mail,mul(s.n,-1))));
 ...
 mesh(g,'box','#aaa38d',[0,.15,0],[4.65,.3,4.3]);mesh(g,'box',s.color,[0,1.6,0],[4.3,2.85,3.9]);const gg=new T.BufferGeometry();gg.setAttribute('position',new T.Float32BufferAttribute([-2.15,2.95,1.95,2.15,2.95,1.95,0,3.93,1.95,2.15,2.95,-1.95,-2.15,2.95,-1.95,0,3.93,-1.95],3));gg.computeVertexNormals();g.add(new T.Mesh(gg,new T.MeshStandardMaterial({color:s.color,roughness:.9})));for(const side of[-1,1])for(const z of[-.9,.9]){mesh(g,'box',trim,[side*2.17,1.65,z],[.08,1.17,.95]);mesh(g,'box',dark,[side*2.22,1.65,z],[.03,.95,.73]);mesh(g,'box',trim,[side*2.25,1.65,z],[.025,.045,.76]);mesh(g,'box',trim,[side*2.25,1.65,z],[.025,.97,.05]);}
...
export function avenueTree(parent,t){const g=anchor(parent,t.n),k=t.size;mesh(g,'cylinder','#8b7760',[0,1.45*k,0],[.15*k,2.9*k,.15*k]);for(let j=0;j<3;j++)mesh(g,'cylinder','#8b7760',[(j-1)*.3*k,2.2*k,0],[.085*k,1.6*k,.085*k],[0,0,(j-1)*-.45]);for(let j=0;j<23;j++){const a=j*2.4,r=j===0?0:(.5+rand(j+t.seed)*.68)*k;mesh(g,'ball',['#4c8f60','#73a964','#99b96f','#608f56'][j%4],[Math.cos(a)*r,(2.55+rand(j+t.seed)*1.1)*k,Math.sin(a)*r],[.53*k,.54*k,.58*k]);}return g;}
