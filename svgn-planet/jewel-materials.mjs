...
export function createMaterialLibrary(renderer,scene){
 ...
 function setLook(value){look=resolveLook(value);scene.environment=look==='light'?null:env?.texture||null;for(const m of [...glass,...gem]){const crystal=gem.has(m),refract=look==='cinematic';m.transmission=refract?(crystal?.78:.97):0;m.dispersion=refract&&crystal?.22:0;m.opacity=refract?1:crystal?1:.24;m.transparent=!refract&&!crystal;m.depthWrite=crystal;m.iridescence=crystal&&look!=='light'?.38:0;m.needsUpdate=true;}for(const m of materials)m.envMap=env?.texture||null;}
 function rebuild(){const old=env;const generator=new T.PMREMGenerator(renderer);env=generator.fromEquirectangular(source);generator.dispose();env.texture.name='SVGN / original HDR reflection environment';scene.environment=look==='light'?null:env.texture;scene.environmentIntensity=.35;for(const m of materials){m.envMap=env.texture;m.needsUpdate=true;}old?.dispose();generation++;}
