...
export const padState={connected:false,x:0,y:0,lookX:0,lookY:0,boost:false,brake:false,guard:false,glide:false,id:''};
...
function clear(){for(const key of['x','y','lookX','lookY'])padState[key]=0;padState.boost=padState.brake=padState.guard=padState.glide=false;}
...
function chooseFocus(root){const keep=[...root.querySelectorAll('[data-cancel]')].find(visible);if(keep)return keep;if(root.id==='pause-dialog'&&visible($('confirm-reset')))return $('cancel-reset');return root.querySelector('[data-pad-default]:not(:disabled)')||root.querySelector('.primary:not(:disabled)')||items(root)[0];}
...
function frame(now){
 ...
 if(root){
  ...
  const field=controlContext.ward&&controlContext.combat,cape=controlContext.ward&&controlContext.canGlide&&buttons[4]&&buttons[5];
  padState.guard=field&&(pad.buttons[6]?.value||0)>.2;padState.glide=cape;
  if(field){padState.boost=!!buttons[10];if(edge(7))emit((pad.buttons[6]?.value||0)>.2?'tool':'strike');}
  const actions={0:'hop',2:'interact',3:'ride',4:cape?null:field?'tool':'throw',5:cape?null:'camera',8:'map',9:'pause',10:field?null:'bell',11:field?'tool-cycle':'recenter',12:'map',13:'jobs',14:controlContext.ward?'scan':'previous-district',15:controlContext.ward?'strike':'next-district'};
  for(const [i,name]of Object.entries(actions))if(name&&edge(Number(i)))emit(name);
 }
