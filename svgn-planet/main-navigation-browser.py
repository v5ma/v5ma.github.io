...
async def main():
 ...
 async with async_playwright() as p:
  ...
  try:
   ...
   await press(9);await choose('ward-restore');assert await page.locator('#ward-confirm').is_hidden();assert 'No backup' in await page.locator('#toast').inner_text();ok('Missing district backup is reported without presenting a destructive confirmation')
