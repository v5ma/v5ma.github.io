...
async def main():
 ...
 async with async_playwright() as p:
  ...
  try:
   ...
   await press(9);await choose('ward-save');await choose('ward-restore');await frames();rows=await page.evaluate('NeighborhoodMissions.panel().rows');assert rows[0]['id']=='ward-keep';assert {r.get('id') for r in rows if r.get('id')}=={'ward-keep','ward-replace'};saved=await page.evaluate('localStorage.getItem("svgn.lantern-ward.v1")');await choose('ward-keep');assert await page.evaluate('localStorage.getItem("svgn.lantern-ward.v1")')==saved;ok('District restore isolates cancel-first choices and keeping progress changes no saved bytes')
   ...
   await page.evaluate('__xrFixture.session.end()');await wait('!NeighborhoodMissions.inspect().xr.active');assert not report['errors'] and not report['consoleErrors'];report['success']=True
