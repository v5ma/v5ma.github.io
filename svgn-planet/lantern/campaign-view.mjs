...
import {CAMPAIGN_CASES,campaignState,campaignRuntime,campaignTarget,campaignCanGlide} from './campaign.mjs';
export function createCampaignView({world,box,cyl,label}){
 ...
 return {update(s){const c=campaignState(s),r=campaignRuntime(s),active=!!c.active;for(let i=0;i<enemies.length;i++){const v=enemies[i],e=r.enemies[i];v.g.visible=!!e&&e.hp>0;v.cue.visible=!!e&&e.hp>0&&e.phase==='windup';if(e){v.g.position.set(e.x,e.y,e.z);v.g.rotation.y=e.yaw;v.cone.visible=c.active==='predator'&&e.awareness<1;v.cone.material.opacity=.04+.08*Math.min(1,e.awareness);v.cue.position.set(e.x,e.y+2,e.z);}}
 ...
 },inspect:()=>({rooms:stations.length,perches:perches.length,enemySlots:enemies.length,capeRig:true,target:campaignTarget?true:false})};
}
