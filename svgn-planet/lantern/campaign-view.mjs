...
export function createCampaignView({world,box,cyl,label}){
 ...
 return {update(s){const c=campaignState(s),r=campaignRuntime(s),active=!!c.active;for(const item of systems){const visible=active&&r.focus&&item.node.cases.includes(c.active);item.marker.visible=item.text.visible=visible;item.marker.material.color.setHex(r.systemsUsed.includes(item.node.id)?0x61747b:0xffd27a);}for(let i=0;i<enemies.length;i++){const v=enemies[i],e=r.enemies[i];v.g.visible=!!e&&e.hp>0;v.cue.visible=!!e&&e.hp>0&&e.phase==='windup';if(e){v.g.position.set(e.x,e.y,e.z);v.g.rotation.y=e.yaw;v.cone.visible=(c.active==='predator'||c.active==='highline')&&e.awareness<1;v.cone.material.opacity=.04+.08*Math.min(1,e.awareness);v.cue.position.set(e.x,e.y+2,e.z);}}
