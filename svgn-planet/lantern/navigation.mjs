...
export function navigation(s,yaw=0){const t=storyTarget(s);if(!t)return {label:'Meet a resident marked ! or open Missions.',distance:0,angle:0,level:'',target:null};const guide=routeGuide(s,t),cue=guide.cue||t,dx=cue.x-s.x,dz=cue.z-s.z;return {label:t.label,distance:Math.hypot(t.x-s.x,t.z-s.z,t.y-s.y),angle:Math.atan2(dx,-dz)+yaw,level:t.y>6||s.y>6?Math.round(s.y*10)/10+' m / target '+Math.round(t.y*10)/10+' m':t.y>s.y+1?'UPSTAIRS':t.y<s.y-1?'LOWER LEVEL':'THIS LEVEL',target:{...t},guide};}
