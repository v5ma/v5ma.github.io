...
export const CAMPAIGN_CASES=Object.freeze([
 ...
 {id:'highline',title:'Highline: The Rooftop Signal',reward:240,summary:'A new vertical adventure available immediately. Climb the Print Exchange and Radio Tower, choose between two high crossings, restore the repeater, and bring Sal\'s recording home.',steps:[
  point('highline-brief','Meet Sal in the old loading loft / borrow the traversal kit',12,4.4,0),
  point('highline-archive','Read Ada\'s upper archive / Print Exchange 10.8 m',-12.5,10.8,-5.05),
...
const patrols={
 highline:[
  {id:'highline-1',role:'Watcher',y:10.8,home:[-1,-5.05],route:[[-2,-5.05],[1,-5.05]]},
  {id:'highline-2',role:'Watcher',y:10.8,home:[6,-5.05],route:[[4,-5.05],[7,-5.05]]}
 ],
 predator:[
...
function makeEnemy(e){return {...e,x:e.home[0],y:e.y??(e.id.startsWith('final')?4.4:0),z:e.home[1],yaw:0,hp:e.role==='Brute'?5:3,phase:'patrol',timer:0,routeIndex:0,awareness:0,stun:0,distract:0,distractPoint:null};}
function runtimeFor(id){return {caseId:id,enemies:(id==='highline'?[]:patrols[id]||[]).map(makeEnemy),highlineAwake:false,smoke:0,smokeCooldown:0,pulseCooldown:0,strikeCooldown:0,lunge:null,glideSeconds:0,glideLanded:false,takedowns:0,dropTakedowns:0,counters:0,bestCombo:0,combo:0,alertPeak:0,selected:'grapple',holsterDraws:0,guard:false,guardWindow:0,focus:false,systemsUsed:[],systemUses:0};}
export function campaignRuntime(s){const c=campaignState(s);let r=sessions.get(s);if(!r||r.caseId!==c.active){r=runtimeFor(c.active);if(c.active==='highline'&&highlineRestored(s))r.enemies=[];sessions.set(s,r);}if(c.active==='highline'&&!r.highlineAwake&&(c.progress.highline||0)>=2&&!highlineRestored(s)){r.enemies=patrols.highline.map(makeEnemy);r.highlineAwake=true;}return r;}
const distance=(a,b)=>Math.hypot(a.x-b.x,a.y-b.y,a.z-b.z);
const eye=s=>({x:s.x,y:s.y+1.3,z:s.z});
const visible=(s,e,api)=>api.lineClear(s,eye(s),{x:e.x,y:e.y+1,z:e.z});
function enemyStepClear(s,e,x,z,api){const floor=api.support(s,x,z,e.y+.1);return !!floor&&Math.abs(api.floorHeight(floor,z)-e.y)<.4&&!api.blocked(s,x,e.y,z,.3,e.id);}
...
function useSystem(s,m,r,node,api){
 ...
 if(node.effect==='distract'){
  affected=r.enemies.filter(e=>e.hp>0&&Math.abs(e.y-node.y)<1.4&&distance(e,node)<=node.radius).sort((a,b)=>distance(a,node.target)-distance(b,node.target)).slice(0,2);
...
export function campaignAction(s,name,api,ray){const c=campaignState(s),m=CAMPAIGN_CASES.find(x=>x.id===c.active),kitUnlocked=highlineKit(s)||watchState(s).stage===4||c.completed.length>0;const r=campaignRuntime(s);
 ...
 if(!m)return false;
 if(m.id==='highline'&&!highlineKit(s)&&['tool','strike','pulse','smoke','tool-cycle'].includes(name)){api.say(s,'Meet Sal at the old loading loft to borrow the Highline kit.');return true;}
 if(name==='tool'&&r.selected==='grapple'&&kitUnlocked)return watchAction(s,'grapple',api,ray);
 ...
 if(name==='interact'&&(m.id==='predator'||m.id==='finale'||m.id==='highline')){const candidates=r.enemies.filter(e=>{const h=Math.hypot(s.x-e.x,s.z-e.z),drop=s.y>e.y+2&&h<2.5,closeGround=Math.abs(s.y-e.y)<1.4&&h<1.7;return e.hp>0&&(drop||closeGround)&&visible(s,e,api);});const e=candidates.sort((a,b)=>distance(s,a)-distance(s,b))[0];if(e&&(behind(s,e)||s.y>e.y+2)){e.hp=0;e.phase='disabled';if(s.y>e.y+2)r.dropTakedowns++;else r.takedowns++;r.combo++;api.say(s,s.y>e.y+2?'Drop takedown. Stay above the remaining patrols.':'Silent takedown. Patrol gap created.');return true;}}
 if(name==='strike'&&(m.id==='freeflow'||m.id==='finale'||m.id==='highline')){if(r.strikeCooldown||r.lunge)return true;const e=r.enemies.filter(e=>e.hp>0&&distance(s,e)<5.2&&Math.abs(s.y-e.y)<1.2&&visible(s,e,api)&&aimScore(s,e,ray)>.15).sort((a,b)=>distance(s,a)-distance(s,b))[0];if(!e){api.say(s,'No target in the freeflow lane. Reposition instead of swinging at empty space.');return true;}const d=distance(s,e);r.strikeCooldown=.34;if(d>2.25){const L=Math.hypot(e.x-s.x,e.z-s.z),to={x:e.x-(e.x-s.x)/(L||1)*1.25,y:e.y,z:e.z-(e.z-s.z)/(L||1)*1.25};if(!api.lineClear(s,eye(s),{x:to.x,y:to.y+1,z:to.z})){api.say(s,'No safe lunge lane. Use the route or close normally.');return true;}r.lunge={from:{x:s.x,y:s.y,z:s.z},to,target:e,t:0};api.say(s,'Freeflow lunge.');return true;}strikeEnemy(s,e,r,api);return true;}
 return false;
}
export function campaignBlocks(s,x,y,z,ignoreActor=null){const r=campaignRuntime(s);return r.enemies.some(e=>e.id!==ignoreActor&&e.hp>0&&Math.abs(y-e.y)<1.6&&Math.hypot(x-e.x,z-e.z)<.58);}
