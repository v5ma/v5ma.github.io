...
async def main():
 ...
 async with async_playwright() as p:
  ...
  try:
   await page.goto(BASE,wait_until='domcontentloaded');await wait('window.LanternWard&&!document.querySelector("#start").disabled');await page.bring_to_front();await press(0);await wait('LanternWard.inspect().controllerReady');assert (await state())['version']=='0.14.0';ok('Default entry boots the current game with Xbox')
   ...
   if SUITE=='city':
    ...
    await page.evaluate('__xrFixture.left.gamepad.buttons[3]={pressed:true,value:1}');await wait('LanternWard.inspect().state.speed>5');await page.evaluate('__xrFixture.left.gamepad.buttons[3]={pressed:false,value:0}');await wait('LanternWard.inspect().state.speed===0');ok('Tracked-controller speed release stops without a dismount or wall')
