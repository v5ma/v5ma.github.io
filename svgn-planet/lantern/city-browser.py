 ...
 async with async_playwright() as p:
  ...
  try:
   ...
   if SUITE=='city':
    ...
    await press(9);await page.click('#vr-diorama');await wait('LanternWard.inspect().xr.frames>8');await choose('Resume');await wait('!LanternWard.inspect().paused');s=await state();assert s['view']['portal']['centerError']<1e-6;assert s['view']['portal']['opaqueEnclosurePlanes']==0;assert not s['xr']['actionPanelVisible'];anchor=s['view']['portal']['anchor'];ok('Diorama is a centered perspective aperture without opaque enclosure planes or controller HUD panels')
