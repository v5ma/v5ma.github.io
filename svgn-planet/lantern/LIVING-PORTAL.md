...
The first real stereo screenshots exposed two solid cyan tracked-grip placeholder boxes (.045 x .05 x .11 metres) rendered with filled faces in front of the miniature. These appeared as large rectangles over the miniature even after the other fixes. They are replaced by small 4.4-centimetre wire grip outlines with depth testing enabled and no filled faces. The old renderer also constructed opaque double-sided left, right and rear enclosure planes. The old XR UI also copied the current headset position and full orientation into its panel group every frame. These are confirmed source-level causes of view-dependent rectangles. Remove the opaque enclosure entirely, make foreground frame faces automatically transparent per eye, leave only faint far glass and a thin edge frame, and dock the hand panel at a fixed yaw-only position. The pause panel is placed once on opening rather than following every head tilt. Controller gameplay hides it. The physical report does not establish which of these planes was most objectionable; actual headset review remains required.
...
The first candidate passed its 11 XR lifecycle checks and six isolated stereo pixel cases, yet screenshot review still showed the filled controller boxes covering the view. That visual failure was not accepted as fixed merely because numerical tests passed. The final regression inspects actual grip object types as well as head/portal transforms, and retains before/after screenshots. The old grip material already used depth testing; removing filled faces, not toggling that existing setting, is the relevant fix.
