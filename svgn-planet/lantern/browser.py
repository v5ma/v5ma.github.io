 ...
 async with async_playwright() as p:
  ...
  try:
   ...
   if ROUTE!='xr':
    ...
    await page.evaluate('__xrFixture.useHands()');await wait('LanternWard.inspect().paused');await xrframes();await choose('Resume',True);await wait('!LanternWard.inspect().paused');ok('Joint-pinch UI resumes without a controller');assert (await state())['xr']['actionPanelVisible'];await shot('xr-hand-actions');before=(await state())['state']['distance'];await choose('Forward (hold)',True);assert (await state())['state']['distance']>before;assert (await state())['xr']['input']['y']==0;await wait('LanternWard.inspect().state.speed<.05');ok('Hand-only motion uses real input and brakes on pinch release')
