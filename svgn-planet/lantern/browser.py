  ...
  try:
   ...
   if ROUTE!='xr':
    ...
    async def choose(label,hand=False):
     # Native UI is paginated. Move through its own Next control rather than
     # assuming every settings row is simultaneously visible.
     for _ in range(12):
      labels=await page.evaluate('LanternWard.panel().rows.map(r=>r.label)')
      if label in labels:return await click_visible(label,hand)
      nxt=next((x for x in labels if x.startswith('Next ')),None)
      if not nxt:break
      await click_visible(nxt,hand)
     raise AssertionError('Missing XR row after paging: '+label)
    ...
    await choose('Resume');await xrframes();assert not (await state())['xr']['actionPanelVisible'];await page.evaluate('__xrFixture.viewerPitch=-.45');await xrframes();await shot('xr-world');await page.evaluate('__xrFixture.viewerPitch=0');await xrframes();ok('Controller play keeps the miniature clear; native head tilt reveals its depth');await press(9);await wait('LanternWard.inspect().paused');await press(0);await wait('!LanternWard.inspect().paused');await press(9);await wait('LanternWard.inspect().paused');ok('Xbox operates native XR panels without DOM navigation');
    ...
    await page.evaluate('__xrFixture.useHands()');await wait('LanternWard.inspect().paused');await xrframes();await choose('Controls',True);await choose('Hand action panel: off',True);await choose('Back to Menu',True);await choose('Resume',True);await wait('!LanternWard.inspect().paused');ok('Joint-pinch UI can explicitly enable optional seated hand controls');assert (await state())['xr']['handActionsOptIn'];await shot('xr-hand-actions');before=(await state())['state']['distance'];await choose('Forward (hold)',True);assert (await state())['state']['distance']>before;assert (await state())['xr']['input']['y']==0;await wait('LanternWard.inspect().state.speed<.05');ok('Hand-only motion uses real input and brakes on pinch release')
