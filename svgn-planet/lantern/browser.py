 ...
 async with async_playwright() as p:
  ...
  try:
   ...
   if ROUTE!='xr':
    ...
    await page.reload(wait_until='domcontentloaded');await wait('window.LanternWard&&!document.querySelector("#start").disabled');await press(0);s=(await state())['state'];assert s['gate'] and s['claimed'] and s['credits']==600;ok('Route outcome and shortcut persist on ordinary reload')
    if ROUTE=='canal':
    ...
    await page.evaluate('__pad.connected=false');await wait('LanternWard.inspect().paused');ok('Controller disconnect pauses')
   else:
    ...
    await page.evaluate('__xrFixture.useHands()');await wait('LanternWard.inspect().paused');await xrframes();await choose('Resume',True);await wait('!LanternWard.inspect().paused');ok('Joint-pinch UI resumes without a controller')
    await page.evaluate('__xrFixture.tracking=false');await wait('LanternWard.inspect().paused');assert (await state())['xr']['input'].get('y',0)==0;await page.evaluate('__xrFixture.tracking=true');await xrframes();ok('Hand tracking loss clears held motion and pauses');await page.evaluate('__xrFixture.session.end()');await wait('!LanternWard.inspect().xr.active');await page.click('#ar-diorama');await wait('LanternWard.inspect().xr.active');await xrframes();assert (await state())['xr']['kind']=='diorama-ar';assert (await state())['xr']['environmentBlendMode']=='alpha-blend';ok('AR requests a separate alpha-blended session, not a VR fallback');await shot('xr-ar');await page.evaluate('__xrFixture.session.end()');await wait('!LanternWard.inspect().xr.active');ok('XR exit returns to desktop safely')
