...
import {campaignState,campaignRuntime,campaignCanGlide} from './campaign.mjs';
import {holsterZone,capeGesture} from './xr-embodiment.mjs';
...
export function createXR(view,hooks){
 ...
 let motionStrikes=0,menuGestures=0,holsterDraws=0,capeFrames=0;
 ...
 const caster=new T.Raycaster();let rows=[],focused=0,panelSignature='',input={x:0,y:0,boost:false,brake:false,guard:false,glide:false},lastSnap=false;
 function clear(){input={x:0,y:0,boost:false,brake:false,guard:false,glide:false};for(const s of slots){s.lastRelative=null;s.ready=false;s.prev=[];s.pinch=false;s.lastHand=null;s.lastHead=null;s.motionArmed=true;s.menuHeld=0;s.gestureUsed=false;}hooks.clear();}
 ...
 function actions(){
  if(hooks.paused()&&section==='help')return [
   ...
   ['Click the turning stick to cycle grapple, pulse, smoke and cape.'],
   ['Body holsters: chest grapple, forearm pulse, outer hip smoke. Grip to draw.'],
   ['Cape glide: with the cape unlocked, hold both grips low and spread your hands.'],
  ...
  if(hooks.paused()&&section==='tools')return [['Scanner',()=>field('scan')],['Grapple',()=>field('holster-grapple')],['Pulse',()=>field('holster-pulse')],['Smoke',()=>field('holster-smoke')],['Cape rig',()=>field('holster-cape')],['Change Watch approach',()=>field('watch-route')],['Change campaign approach',()=>field('campaign-route')],['Recover at watch desk',()=>field('watch-recover')],['Back to Menu',()=>{section='';page=0;lastPaint=0;}]];
 ...
 function update(now,frame,state){
  input={x:0,y:0,boost:false,brake:false,guard:false,glide:false};const delta=lastNow?(now-lastNow)/1000:0;lastNow=now;if(!session||!frame)return input;frames++;
  ...
  tracked=0;let consumed=false,hands=false,missing=false,snap=0;const bodyHands=[null,null],bodyGrips=[false,false];
  for(let i=0;i<2;i++){
   ...
   bodyHands[i]=relative;bodyGrips[i]=!!b[1];
   ...
   if(!paused&&slot.ready&&!src.hand&&!row&&!consumed){
    const role=sourceRoles(src.handedness,prefs),zone=(campaignState(state).active||watchState(state).stage===4)?holsterZone(relative):null;
    ...
    if(prefs.profile==='courier'){
     ...
     if(role.primary){
      ...
      if(edge(1)&&zone){hooks.action('holster-'+zone);holsterDraws++;consumed=true;}else if(edge(1)&&relative[1]>-.34)hooks.action('interact');
      if(edge(0)){hooks.action(aiming?'tool':(watchState(state).tracking||campaignState(state).active)?'strike':'interact',ray);slot.motionArmed=false;}
      ...
      if(first&&prefs.motionPunch&&!aiming&&(watchState(state).tracking||campaignState(state).active)&&handDelta>headDelta+.01&&motionStrike(slot.lastRelative,relative,delta,b[1],slot.motionArmed)){hooks.action('strike',ray);slot.motionArmed=false;motionStrikes++;}
     }else{
      if(edge(1)&&zone){hooks.action('holster-'+zone);holsterDraws++;consumed=true;}input.brake=b[1]&&!zone;input.guard=b[1]&&!zone&&(guardPose(relative,true)||!prefs.motionPunch);
  ...
  if(!paused&&campaignCanGlide(state)&&capeGesture(bodyHands,bodyGrips)){input.glide=true;input.brake=false;input.guard=false;capeFrames++;}
  ...
  if(hooks.paused()||view.curtain.visible)input={x:0,y:0,boost:false,brake:true,guard:false,glide:false};
  return input;
 }
