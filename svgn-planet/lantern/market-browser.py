...
async def main():
 report={'version':'0.15.0','checks':[],'routes':{},'errors':[],'consoleErrors':[],'physicalHardwareTested':False,'humanComprehensionTested':False,'inputOnly':True}
 start=time.time()
 async with async_playwright() as p:
  ...
  async def tour(choice):
   ...
   try:
    await page.goto(BASE,wait_until='domcontentloaded');await wait('window.LanternWard&&!document.querySelector("#start").disabled');await page.bring_to_front();await press(0);await wait('LanternWard.inspect().controllerReady');assert (await state())['version']=='0.15.0'
