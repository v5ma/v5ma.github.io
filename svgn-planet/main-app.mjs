import {createSaveVault,mountSaveRecovery} from './save-vault.mjs';
import {createMainHub} from './main-hub.mjs';
import {xrInput} from './xr-input.mjs';
import {mountWaterDOM,createWaterUI} from './tidewater-ui.mjs';
...
import {createMainScene as createScene} from './main-scene.mjs';
buildCoastalDOM();mountHomecomingDOM();mountWaterDOM();
const $=id=>document.getElementById(id),canvas=$('world'),audio=createCoastalAudio(),metrics=createFrameHealth();
const dialogs=['pause-dialog','map-dialog','help-dialog','jobs-dialog','repair-dialog','photo-dialog','homecoming-dialog','health-dialog','production-dialog','water-dialog','water-pump-dialog','water-results-dialog','save-dialog'];
const vault=createSaveVault(()=>localStorage);let touchBraking=false,saved=vault.load(),storage=true,xr=null,hub=null;
...
let previousBoost=false,mainReleaseBrake=0;const diagnostics=[];let waypoint=null,lastReward=0;
...
function persist(){if(hub?.wardActive)return hub.persist();s.vehicle=vehicle;const raw=JSON.stringify(saveData(s));if(raw===saveStamp)return;storage=vault.commit(raw);if(storage)saveStamp=raw;return storage;}
...
function pause(){if(hub?.wardActive){hub.wardMenu();return;}if(!started||paused||graphicsLost||failed)return;paused=true;clear();persist();audio.setPlaying(false);$('pause-dialog').showModal();$('resume').focus();}
...
mountSaveRecovery({vault,state:()=>s,serialize:saveData,restore(data){s=initial(data);vehicle=s.vehicle;$('vehicle').value=$('vehicle-pause').value=vehicle;saveStamp='';clear();view?.recenter();renderDirty=true;},open:openDialog,resume,persist});
...
$('accept-reset').onclick=()=>{if(!vault.commit(JSON.stringify(saveData(initial())),{explicit:true})){s.toast=vault.inspect().status;s.toastT=6;return;}s=initial();s.ride=true;saveStamp='';persist();$('confirm-reset').hidden=true;resume();};
...
function action(code){
 if(hub?.wardActive){if(started&&!paused)hub.command(code);return;}
 if(!started||paused||failed||graphicsLost)return;
 if(code==='KeyE'){if(!waterUI.interaction()&&!storyUI.interact()&&!pulseUI.interact(!nearest(s))){if(nearest(s))interact(s);else throwPaper(s);}persist();}
 if(code==='KeyQ')throwPaper(s);if(code==='KeyF'){if(!waterUI.craft())switchRide(s);persist();}if(code==='Space')jump=true;
...
const codes=['KeyL','KeyZ','KeyX','KeyU','KeyT','KeyR','KeyW','KeyA','KeyS','KeyD','ArrowUp','ArrowDown','ArrowLeft','ArrowRight','Space','ShiftLeft','ShiftRight','ControlLeft','ControlRight','KeyB','KeyE','KeyQ','KeyF','KeyV','KeyM','KeyH','KeyC','KeyJ','KeyG','KeyK'];
...
function input(){const rawBoost=xrInput.boost||padState.boost||boost||keys.has('ShiftLeft')||keys.has('ShiftRight');if(previousBoost&&!rawBoost)mainReleaseBrake=20;const released=mainReleaseBrake>0;if(released)mainReleaseBrake--;previousBoost=rawBoost;const x=Number(keys.has('KeyD')||keys.has('ArrowRight'))-Number(keys.has('KeyA')||keys.has('ArrowLeft'))+stick[0]+padState.x+xrInput.x,z=Number(keys.has('KeyW')||keys.has('ArrowUp'))-Number(keys.has('KeyS')||keys.has('ArrowDown'))+stick[1]+padState.y+xrInput.y,b=view.movementBasis(s),magnitude=Math.min(1,Math.hypot(x,z));return {direction:magnitude>.05?norm(add(mul(b.right,x),mul(b.forward,z))):null,throttle:magnitude,brake:released||(!rawBoost&&magnitude<.05)||xrInput.brake||padState.brake||touchBraking||keys.has('ControlLeft')||keys.has('ControlRight')||keys.has('KeyB'),boost:xrInput.boost||padState.boost||boost||keys.has('ShiftLeft')||keys.has('ShiftRight'),jump};}
function drawMap(){
...
function ui(){
 ...
 const t=waterTarget(s)||storyTarget(s)||jobTarget(s)||(waypoint?cityStops.find(d=>d.id===waypoint)||target(s):target(s)),near=nearest(s);
...
$('copy-diagnostics').onclick=async()=>{const text=JSON.stringify({version:VERSION,artVersion:ART_VERSION,started,paused,graphicsLost,events:diagnostics,render:view?.inspect(),coastal:pulseUI.inspect(),homecoming:storyUI.inspect(),tidewater:waterUI.inspect(),viewport:[innerWidth,innerHeight,devicePixelRatio]},null,2);$('diagnostics').hidden=false;$('diagnostics').value=text;try{await navigator.clipboard.writeText(text);}catch{}};
const waterUI=createWaterUI({state:()=>s,view:()=>view,audio,open:openDialog,resume,persist,resetView(){clear();view?.recenter();view?.setCamera('street');mode=0;renderDirty=true;}});
...
function boot(){try{view=createScene(canvas);hub=createMainHub({view,canvas,state:()=>s,keys:()=>keys,stick:()=>stick,boost:()=>boost,braking:()=>touchBraking||keys.has('ControlLeft')||keys.has('KeyB'),
 pause,resume,openMap,clear,cityMap:()=>{drawMap();return $('map');},playing:()=>started&&!paused&&!failed&&!graphicsLost,setPaused:v=>{paused=v;clear();},persistCity:persist,ensureStarted:()=>{if(!started)$('start').click();},changed:()=>{renderDirty=true;last=0;},
 command(name){const codes={interact:'KeyE',throw:'KeyQ',ride:'KeyF',hop:'Space',bell:'KeyG',pause:'KeyP',map:'KeyM',jobs:'KeyJ',camera:'KeyV'};if(codes[name])action(codes[name]);}});xr=hub.xr;$('material-look').value=view.inspect().jewel.look;$('quiet-effects').checked=view.inspect().jewel.quiet;record('renderer-ready');view.artReady.then(()=>{$('start').disabled=false;$('start').textContent=saved?'Continue your neighborhood':'Start riding';renderDirty=true;record('art-ready',view.inspect().art.status);});}catch(e){showFailure(String(e.message||e));}}
...
function frame(now,xrFrame){
 ...
 try{
  ...
  if((renderDirty||(started&&!paused))&&(view.fps>=60||now-lastRender>=1000/view.fps-.5)){
   const workStart=performance.now();const draw=()=>view.update(paused?0:Math.min(.2,(now-lastRender)/1000),renderPose(previousPose,s,acc*60),{vehicle});xr?xr.flatRender(draw):draw();if(started&&!paused)metrics.frame(now,s.time,performance.now()-workStart);else metrics.gap();lastRender=now;renderDirty=false;
  }
  xr?.present();
  audio.update(s,{active:started&&!paused,vehicle,brake:xrInput.brake||padState.brake||touchBraking||keys.has('ControlLeft')||keys.has('ControlRight')||keys.has('KeyB'),traffic:view.life?.traffic(),rain:view.atmosphere?.rainLevel||0});
  if(now-lastUI>120){ui();lastUI=now;}if(now-lastSave>5000){idleSave();lastSave=now;}
 }catch(e){showFailure(String(e.message||e));}
}
function startRenderLoop(){if(view)view.renderer.setAnimationLoop(frame);else requestAnimationFrame(startRenderLoop);}startRenderLoop();
