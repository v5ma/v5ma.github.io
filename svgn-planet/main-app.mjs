...
function input(){const rawBoost=xrInput.boost||padState.boost||boost||keys.has('ShiftLeft')||keys.has('ShiftRight');if(previousBoost&&!rawBoost)mainReleaseBrake=20;const released=mainReleaseBrake>0;if(released)mainReleaseBrake--;previousBoost=rawBoost;const x=Number(keys.has('KeyD')||keys.has('ArrowRight'))-Number(keys.has('KeyA')||keys.has('ArrowLeft'))+stick[0]+padState.x+xrInput.x,z=Number(keys.has('KeyW')||keys.has('ArrowUp'))-Number(keys.has('KeyS')||keys.has('ArrowDown'))+stick[1]+padState.y+xrInput.y,b=view.movementBasis(s),magnitude=Math.min(1,Math.hypot(x,z));return {direction:magnitude>.05?norm(add(mul(b.right,x),mul(b.forward,z))):null,throttle:magnitude,brake:released||xrInput.brake||padState.brake||touchBraking||keys.has('ControlLeft')||keys.has('ControlRight')||keys.has('KeyB'),boost:xrInput.boost||padState.boost||boost||keys.has('ShiftLeft')||keys.has('ShiftRight'),jump};}
function drawMap(){
