...
test('Main movement releases cruise on neutral input and never imports the theater',()=>{const app=readFileSync(new URL('./main-app.mjs',import.meta.url),'utf8');assert.ok(app.includes('(!rawBoost&&magnitude<.05)'));assert.ok(!app.includes("from './xr-theater.mjs'"));});
