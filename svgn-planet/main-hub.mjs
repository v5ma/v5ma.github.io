import {drawCityFieldMap,cityMissionTarget} from './field-map.mjs';
...
export function createMainHub(hooks){
 ...
 <dialog id="xr-controls-dialog"><h2>Direct XR controls and field desk</h2><button id="xr-controls-back" data-pad-default data-pad-back>Back / resume</button><p>On foot, Action uses left stick movement and held stick click sprint. Riding defaults to left trigger speed and right trigger brake; release speed to stop. Change Vehicle speed below to retain the original profile. Right grip interacts; right trigger strikes or uses the aimed tool. Left trigger aims; left grip guards or brakes. A hops, B mounts, X scans, Y opens menus. Turning-stick click cycles tools. Hands: low pinch moves, ordinary pinch interacts, raised held pinch opens Menu. Release to stop.</p><label for="desk-vehicle-speed">Vehicle speed</label><select id="desk-vehicle-speed"><option value="left-trigger">Left trigger speed / right trigger brake</option><option value="right-trigger">Right trigger speed / left trigger brake</option><option value="profile">Keep saved Action/Courier profile</option></select><label for="desk-hud">Compact mission display</label><select id="desk-hud"><option value="wrist">Wrist map and objective</option><option value="pedestal">Near-feet map and objective</option><option value="off">Hidden</option></select><label for="desk-height">Desk height above floor</label><input id="desk-height" type="range" min="0.45" max="1.8" step="0.05"><label for="desk-distance">Desk distance</label><input id="desk-distance" type="range" min="0.65" max="2" step="0.05"><label for="desk-size">Desk scale</label><input id="desk-size" type="range" min="0.65" max="1.3" step="0.05"><label><input id="desk-reduced" type="checkbox">Instant desk opening (reduced motion)</label><button id="desk-place">Place desk here</button><label for="xr-profile">On-foot profile</label><select id="xr-profile"><option value="action">Action</option><option value="courier">Courier</option></select><label for="xr-dominant">Dominant hand</label><select id="xr-dominant"><option value="right">Right</option><option value="left">Left</option></select><label><input type="checkbox" id="xr-swap">Swap sticks</label><label for="xr-snap">Snap turn</label><select id="xr-snap"><option value="30">30 degrees</option><option value="45">45 degrees</option></select><label><input type="checkbox" id="xr-motion">Motion strikes</label></dialog>`;
 ...
 const xr=createUnifiedXR({renderer,storage:store,clear:hooks.clear,playing:hooks.playing,pause:()=>wardActive?wardMenu():hooks.pause(),resume:()=>wardActive?close():hooks.resume(),message,changed:hooks.changed,
  ...
  status:()=>wardActive?{district:'LANTERN WARD',goal:missionGoal(ward),detail:Math.ceil(navigation(ward,yaw).distance)+' m / '+navigation(ward,yaw).level,notice:ward.messageTime>0?ward.message:''}:{district:'MAIN NEIGHBORHOODS',goal:cityMissionTarget(hooks.state(),hooks.waypoint?.()).name,detail:$('objective-text').textContent,notice:hooks.state().toastT>0?hooks.state().toast:''},
  drawFieldMap:canvas=>wardActive?drawMap(canvas,ward,true):drawCityFieldMap(canvas,hooks.state(),hooks.waypoint?.()),
 ...
 for(const [id,key]of [['desk-vehicle-speed','vehicleSpeed'],['desk-hud','hud'],['desk-height','height'],['desk-distance','distance'],['desk-size','size'],['desk-reduced','reducedMotion']]){const el=$(id),v=xr.deskSettings[key];if(el.type==='checkbox')el.checked=v;else el.value=v;el.onchange=()=>xr.deskPreference(key,el.type==='checkbox'?el.checked:el.type==='range'?Number(el.value):el.value);}
 $('desk-place').onclick=()=>xr.placeDesk();
