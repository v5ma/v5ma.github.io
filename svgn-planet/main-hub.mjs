...
const VERSION='0.19.0',HUB_KEY='svgn.neighborhood-hub.v1';
export function createMainHub(hooks){
 ...
 function ensureWard(){if(ward)return;const r=load(store);ward=r.state;blocked=r.blocked;wardView=createDistrictView(hooks.canvas,renderer);applyEnvironment();wardSpatial=spatialView(wardView,'ward');}
 async function switchDistrict(next,mission){
  ...
  try{if(next==='lantern'){ensureWard();if(!wardReady){open('ward-loading');await wardView.prepareEnvironment();if(travelToken!==travelGeneration)return false;wardReady=true;}error='';wardActive=true;controlContext.ward=true;if(mission)message(trackStory(ward,mission));}else{wardActive=false;controlContext.ward=controlContext.combat=controlContext.canGlide=false;}
   document.body.classList.toggle('in-lantern',wardActive);$('ward-map-shortcut').hidden=!wardActive;$('district-name').textContent=wardActive?'LANTERN WARD':'MAIN NEIGHBORHOODS';$('view').textContent=wardActive?'Recenter camera':'Street view';$('view').title=wardActive?'Drag the scene or use the right stick to look; C or this button recenters; right-stick click cycles tools during on-foot action':'Change camera: RB or V';$('touch-interact').textContent=wardActive?'Interact':'Deliver';transfers++;last=0;saveTime=0;refreshAdventureEntry();
 ...
 <dialog id="ward-loading"><h2>Preparing neighborhood scenery</h2><p>Preparing the canal, foliage and rooftop archive. Your progress is unchanged.</p><button id="ward-loading-cancel" data-pad-default data-pad-back>Cancel travel</button></dialog>
 ...
 const artControls=installEnvironmentControls({container:$('ward-menu'),storage:store,apply:applyEnvironment,message});
 function applyEnvironment(){const prefs=artControls.preferences;if($('quiet-effects')?.checked)prefs.quiet=true;if($('quality')?.value==='low')prefs.quality='light';wardView?.setEnvironmentPreferences(prefs);}
 for(const id of ['quiet-effects','quality'])$(id)?.addEventListener('change',applyEnvironment);
 $('ward-loading-cancel').onclick=()=>{travelGeneration++;close();message('Travel cancelled. Your neighborhood progress is retained.');};
 $('ward-loading').addEventListener('cancel',e=>{e.preventDefault();$('ward-loading-cancel').click();});
