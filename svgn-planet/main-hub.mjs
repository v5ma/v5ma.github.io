...
export function createMainHub(hooks){
 ...
 for(const id of ['ward-menu','ward-map-dialog','xr-mode-dialog','xr-controls-dialog'])$(id).addEventListener('cancel',e=>{e.preventDefault();if(pending)$('ward-keep').click();else wardActive?close():hooks.resume();});
 ...
 function tickWard(now,frame){
  const dt=Math.min(.05,last?(now-last)/1000:1/60);last=now;frames++;xr.update(now,frame);wardSpatial.restore();
  if(hooks.playing()){
