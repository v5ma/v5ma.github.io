...
with sync_playwright() as p:
 ...
 try:
  ...
  page.keyboard.down('KeyD');page.keyboard.down('KeyC');page.wait_for_function('player.x>=550',timeout=60000)
  page.keyboard.down('Space');page.wait_for_function('__ground.state.upper.size>0',timeout=30000);page.keyboard.up('Space')
  ...
  page.wait_for_function('player.onGround&&!player.track&&player.x>950',timeout=30000)
  check(state(page)['tries']==1,'Leaving the upper ramp lands safely on the lower road')
  page.keyboard.up('KeyD');page.keyboard.up('KeyC');page.wait_for_function('Math.abs(player.vx)<.1',timeout=10000)
  ...
  check(state(page)['deliveries']==before['deliveries'],'Checkpoint retry retains completed deliveries')
