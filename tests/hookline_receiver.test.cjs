 ...
 * The fast sample below came from failing CI 34050802733. */
...
function launch(speed,receiver=rails[3]){
 ...
 let caught=false,won=false,face=null,air=0,landed=false;
 for(let tick=0;tick<360;tick++){
  if(p.trackCD>0)p.trackCD--;
  if(p.track)K.ride(p,{right:true});
  else{
   const old={x:p.x,y:p.y};K.flight(p,{right:true});p._airTicks++;air++;
   // The real movement resolves the depot floor before checking a rail catch.
   // A low artificial catch line on top of the floor is therefore not enough.
   if(p.vy>=0&&p.x+p.w>4680&&p.x<5256&&p.y+p.h>=2304){p.y=2304-p.h-.01;p.vy=0;landed=true;}
   const hit=K.catchRail(p,old,[receiver],'loop-2');if(hit){caught=true;face=hit.face;}
  }
  if(p.x+p.w>=5076&&p.x<=5112&&p.y+p.h>=2268&&p.y<=2304){won=caught;break;}
  if(p.y>2900||p.x>5350)break;
 }
 return {caught,won,face,air,landed};
}
test('The observed high-speed failure now catches the real receiver before reaching the goal',()=>{
 const speed=Math.hypot(16.036379505722408,16.288609540936154),result=launch(speed);
 assert.ok(result.caught&&result.won);assert.equal(result.face,1);assert.ok(result.air>30);
...
test('The rolling receiver accepts 25 launch speeds while preserving a real goal crossing',()=>{
 for(let speed=18;speed<=24;speed+=.25){const r=launch(speed);assert.ok(r.caught&&r.won,'exit speed '+speed);assert.equal(r.face,1);}
});
test('35 earlier whip-landing states keep their complete carried-state receiving connection',()=>{
 for(let startS=1050;startS<=1450;startS+=100)for(let speed=20;speed<=26;speed++){
  const p={w:26,h:30,x:0,y:0,vx:0,vy:0,speed,track:rails[2],trackS:startS,_railFace:1,trackCD:0,dir:1,roll:0,onGround:true};K.pose(p,p.track,startS);let braking=false,from=null,caught=false;
