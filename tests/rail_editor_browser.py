...
with sync_playwright() as pw:
 ...
 try:
  ...
  if MODE=='editor':
   ...
   check(not page.evaluate('document.documentElement.scrollWidth>innerWidth'),'The game header and rendered stage fit the desktop viewport')
   page.screenshot(path=str(OUT/'unicycle-start.png'))
   page.keyboard.down('KeyD');page.wait_for_function('player.x>=550',timeout=180000);page.keyboard.down('Space');page.wait_for_function('player.track?.sky.id==="grip-0"&&player.nitro===1',timeout=60000)
   # Send the real boost key at the observed contact, before extra diagnostic
   # round-trips can consume the entire short ramp on a slow software GPU.
   page.keyboard.down('KeyX');page.keyboard.up('Space');page.wait_for_function('player.nitroT>0',timeout=30000);page.keyboard.up('KeyX')
   check(page.evaluate('RailGripCore.history.some(h=>h.id==="grip-0"&&h.face===1)'),'Ordinary jump catches the top after collecting a real nitro pickup')
   ...
   page.screenshot(path=str(OUT/'underside-grip.png'));(OUT/'grip-history.json').write_text(json.dumps(page.evaluate('RailGripCore.history'),indent=2))
   page.wait_for_function('RailGripCore.history.some(e=>e.id==="grip-2"&&e.face===1)',timeout=120000)
   history=page.evaluate('RailGripCore.history');(OUT/'grip-history.json').write_text(json.dumps(history,indent=2));check([h['face'] for h in history[:3]]==[1,-1,1],'Uninterrupted normal-input route catches top, underside, then top')
