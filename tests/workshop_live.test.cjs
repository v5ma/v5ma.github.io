...
for(let i=4;i<=6;i++)test('Featured world '+i+' retains every curve, terrain tile, peg, neighbor and original soundtrack',()=>{
 ...
 assert.deepEqual(a,b);assert.deepEqual(Array.from(W.decode(out).cells),Array.from(d.cells));assert.equal(doc.paths.length,d.ct.length);assert.equal(doc.paths.length,i===4?15:i===5?54:62);if(i===4)assert.equal(doc.extra.gp.flowRoutes.version,3);assert.deepEqual(a.gp.skyNetwork,b.gp.skyNetwork);assert.deepEqual(a.gp.cast,b.gp.cast);assert.equal(a.m,b.m);
});
