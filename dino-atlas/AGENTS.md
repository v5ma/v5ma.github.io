...
## Current field clarity contract / 2026-09-22

Read FIELD-CLARITY.md and FIELD-CLARITY-HANDOFF.md before changing UI or old guidance text. The user explicitly superseded the shallow fixed-size aperture: default height is 2 m, adjustable from 0.6 to 4 m; width is independently bounded from 0.8 to 4.8 m. Preserve the stationary character-centered full-depth portal and all three legal opening states. Menu scale and floor-display scale/height are independent personal settings, not world transforms.

The floor guide shows an actual live map, current selected mission/step, and HERE with the correct control or a range/speed explanation. Temporary notification/radio messages use a 2-second wall-clock lifetime with the last twelve retained in Help; the current goal and HERE do not expire. Do not restore the old face-locked board or remove the map when closing the full-map dialog. Use ranger-guidance.js to describe the current device and Active/Legacy profile rather than stale trigger-driving strings. Run field-feedback-browser.py for both SPATIAL_SCENE=classic and tidegate, along with the retained spatial, entry, portal and Express journeys.

## Story direction and durable notes

Read FUTURE-DIRECTION.md, LIVING-RESERVE.md, LIVING-RESERVE-HANDOFF.md and the newest verification/living-reserve receipts before story work. The user explicitly asks for notes beside the game throughout development. Update the future-direction brief and current handoff after meaningful changes; record exact source, completed verification, revealing failures and the next bounded task. Never leave the only record in chat or mark proposed chapters as shipped.

The approved origin is an AI Singularity that can design creatures by recombining DNA, producing living dinosaur-like animals matched to known fossil data above the 99th percentile in Atlas' fictional benchmark. Do not substitute ancient-blood cloning, a 99% original-DNA claim or an evil-AI plot. Keep fossil-fit, synthesized behavior and field observations distinct. The story is about ranger knowledge, colleagues, actual animal needs, useful routes and ignored field reports.

First Light is the optional seven-step opening in full Classic Reserve, not a completed campaign. Preserve its visible story entry, Mara/Ivo/Leena conversations and replay journal, real observation, restored gate and physical return. Its separate dino-atlas.living-reserve.v1 key is future-version-safe; suspend rather than reset other activities, never discard secured cargo, and combine chapter gate power with original campaign power. Run tests/living-reserve-browser.py both normally and with LIVING_XR=1 alongside retained entry/UI/control coverage. These use synthetic input and explicit XR mocks, not physical headset approval. The next proposed content is a Tidegate missing-team chapter; implement against current service-loop geometry rather than recreating delivered systems.
