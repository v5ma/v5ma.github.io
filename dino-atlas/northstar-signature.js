...
export function buildNorthstarSignature(scene,physics){
 ...
 const consoles=[console(CIRCUIT_POINTS.start,'A / CANOPY CIRCUIT'),console(CIRCUIT_POINTS.seed,'A / SEED RELAY'),console(CIRCUIT_POINTS.roof,'A / WIND SENSOR'),console(CIRCUIT_POINTS.observatory,'A / CANOPY RESET'),console(CIRCUIT_POINTS.report,'A / FILE REPORT')];
 ...
 return {root,consoles,stageRing,pointLights,service:{x:NORTHSTAR.x+x2,z:NORTHSTAR.z,top:NORTHSTAR.h+1.4},setStage(stage,active){const c=consoles[Math.max(0,Math.min(4,stage))];stageRing.visible=!!active;if(c)stageRing.position.set(c.p.x-NORTHSTAR.x,.12+(c.p.y>4?c.p.y:0),c.p.z-NORTHSTAR.z);consoles.forEach((o,i)=>{o.lamp.material.color.setHex(active&&i===stage?0xffdb85:0x7bb3a1);});},update(time,reduced=false){for(const [i,l] of pointLights.entries())l.intensity=reduced?8:9+Math.sin(time*1.8+i)*3;for(const o of consoles)o.lamp.rotation.y+=reduced?0:.02;}};
}
...
export class NorthstarSignature{
 ...
 update(dt,time){this.world.update(time,this.ctx.settings.reduced);if(dt<=0||!this.s.active)return;this.s.elapsed+=dt;const p=this.ctx.fleet.position;if(this.s.stage===2&&this.s.lastRoute==='none'&&p.y>NORTHSTAR.h-1&&distance(p,{x:NORTHSTAR.x-12,z:NORTHSTAR.z-9})<7){this.s.lastRoute='lift';this.save();}}
