...
export const SPATIAL_BUILD='ranger-spatial-console-20260920.1';
...
export function consoleSettings(v={}){
 return {version:1,height:finite(v?.height,1.15,.55,1.8),distance:finite(v?.distance,1.25,.8,2.2),scale:finite(v?.scale,.8,.55,1.2),wrist:v?.wrist!==false};
}
...
export class SpatialConsole{
 ...
 positionPanel(){
  ...
  x.panel.rotation.set(-.10,p.yaw,0,'YXZ');x.panel.updateWorldMatrix(true,false);
 ...
 paintDock(){const c=this.dock.paint;c.fillStyle='#173c35';c.fillRect(0,0,512,192);c.strokeStyle='#edcf86';c.lineWidth=8;c.strokeRect(4,4,504,184);c.fillStyle='#fff2ca';c.font='bold 33px sans-serif';c.textAlign='center';c.fillText('MENU',128,107);c.fillText('FIELD',384,107);this.dock.texture.needsUpdate=true;}
 paintStatus(){
  ...
  const status=f.mode==='helicopter'?Math.round(f.position.y)+' m altitude / right stick up-down':Math.round(Math.abs(f.actor.speed||0)*3.6)+' km/h / '+(x.travel?.activeLayout?'LS: move / LT: aim / RT: tool':'Legacy controls / field for help');
  lines(c,status,22,130,720,32,1);lines(c,text('goal-compass')||text('mission-title'),22,170,720,32,2);
  c.fillStyle='#f4d48c';c.font='bold 26px sans-serif';lines(c,text('interact-label')||'Grip: interact / B: menu',22,242,720,30,1);
  c.fillStyle='#355e50';c.fillRect(12,280,366,92);c.fillRect(390,280,366,92);c.fillStyle='#fff2cd';c.textAlign='center';c.font='bold 32px sans-serif';c.fillText('MENU / B',194,339);c.fillText('FIELD CONTROLS',573,339);this.wrist.texture.needsUpdate=true;
 }
 hit(entry){
  const ray=this.xr.rayFor(entry);if(!ray||this.xr.invisible)return null;
  if(this.xr.panel.visible){this.rail.mesh.updateWorldMatrix(true,false);this.xr.raycaster.set(ray.origin,ray.direction);const h=this.xr.raycaster.intersectObject(this.rail.mesh,false)[0];if(h?.uv){entry.uiDistance=h.distance;return this.tabs[Math.min(5,Math.floor(h.uv.x*6))];}}
  for(const [surface,threshold] of [[this.wrist,.27],[this.dock,1]]){
   if(!surface.mesh.visible)continue;surface.mesh.updateWorldMatrix(true,false);this.xr.raycaster.set(ray.origin,ray.direction);const hit=this.xr.raycaster.intersectObject(surface.mesh,false)[0];
   if(hit?.uv&&hit.uv.y<threshold){entry.uiDistance=hit.distance;return this.shortcuts[hit.uv.x<.5?0:1];}
  }return null;
 }
 decorate(){
 ...
 afterInput(){
  const x=this.xr;for(const b of this.buttonPool){const hot=x.panel.visible&&x.controllers.some(e=>e.hit?.label===b.tile?.label);b.base.material.color.setHex(hot?0xffd784:0x749c85);b.group.position.z=hot?.019:.008;}
  x.controllers.forEach((e,i)=>{const d=this.dots[i];d.visible=!!e.hit&&!!e.ray.visible&&!x.invisible;if(d.visible)d.position.set(0,0,-Math.max(.01,e.uiDistance||1));});
 }
