...
import {NorthstarSignature,buildNorthstarSignature,NORTHSTAR_BUILD} from './northstar-signature.js?v=crew1';
import {ReserveCrew,CREW_BUILD} from './reserve-crew.js?v=crew1';
...
export async function boot(){
 ...
 let ranch=null,director=null,herds=null,northstar=null,crew=null;
 ...
 function save(){crew?.save();northstar?.save();herds?.save();ranch?.save();director?.save();fleet.capture();settings.camera=cameraMode;const a=saveRanger(storage,campaign),b=saveFrontier(storage,state);if(!a||!b)toast('This browser cannot save progress. Keep the tab open or enable local storage.');}
 ...
 function interact(){if(isPaused()||!candidate)return;if(candidate.kind==='crew'){crew.interact(candidate);return;}if(candidate.kind==='northstar'){northstar.interact();return;}if(candidate.kind==='herds'){herds.interact();return;}if(candidate.kind==='director'){director.interact(candidate);return;}if(candidate.kind==='ranch'){ranch.interact(candidate);return;}if(Math.abs(fleet.actor.speed)>3){toast('Slow down before interacting.');return;}const c=candidate;
 ...
 function updateCandidate(){const p=fleet.position;candidate=null;const key=MISSIONS[campaign.stage].key;
  ...
  candidate=northstar?.candidate()||director?.candidate()||herds?.candidate()||crew?.candidate()||ranch?.candidate()||candidate;
 ...
 function drawMap(cv,full=false){const ctx=cv.getContext('2d'),s=cv.width,p=fleet.position,span=full?1110:145,k=s/span,cx=full?0:p.x,cz=full?0:p.z,to=(x,z)=>[s/2+(x-cx)*k,s/2+(z-cz)*k];ctx.clearRect(0,0,s,s);ctx.save();if(!full){ctx.beginPath();ctx.arc(s/2,s/2,s/2-1,0,7);ctx.clip();}ctx.fillStyle='#24493e';ctx.fillRect(0,0,s,s);ctx.fillStyle='#516b4c';ctx.beginPath();ctx.arc(...to(0,0),LAND_RADIUS*k,0,7);ctx.fill();
  ...
  ranch?.drawMap(ctx,to,k,full);director?.drawMap(ctx,to,k,full);northstar?.drawMap(ctx,to,k,full);crew?.drawMap(ctx,to,k,full);ctx.strokeStyle='#c4af7e';ctx.lineWidth=full?2:3;for(const r of [...ROADS,...TRAILS,...EXTRA_ROADS]){ctx.beginPath();r.forEach(([x,z],i)=>i?ctx.lineTo(...to(x,z)):ctx.moveTo(...to(x,z)));ctx.stroke();}
 ...
 function frame(now){requestAnimationFrame(frame);const realDt=Math.min(Math.max((now-last)/1000,0),.1);last=now;
  ...
  syncModels(dt);herds?.update(dt,time);ranch?.update(dt,time);park.update(dt,time,fleet.actor,settings.night,settings.reduced);director?.update(dt,time);northstar?.update(dt,time);crew?.update(dt,time);audio.update(fleet.actor.speed,!paused&&fleet.mode!=='foot');
 ...
 northstar=new NorthstarSignature({scene,physics,R,fleet,state,storage,audio,input,signatureWorld,director,herds,ranch,settings,notify:toast,radio,save,close,show:id=>{backTarget=null;show(id);},info});
 crew=new ReserveCrew({scene,camera,renderer,physics,R,fleet,state,storage,audio,input,northstar,director,herds,ranch,settings,park,signatureWorld,personModel,notify:toast,radio,save,close,show:id=>{backTarget=null;show(id);},info,started:()=>started});
 window.__dinoNorthstar={get state(){return northstar.snapshot();}};
 window.__dinoCrew={get state(){return crew.snapshot();}};
