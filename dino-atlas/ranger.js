...
export async function boot(){
 ...
 function save(){ranch?.save();director?.save();fleet.capture();settings.camera=cameraMode;const a=saveRanger(storage,campaign),b=saveFrontier(storage,state);if(!a||!b)toast('This browser cannot save progress. Keep the tab open or enable local storage.');}
 ...
 function interact(){if(isPaused()||!candidate)return;if(candidate.kind==='director'){director.interact(candidate);return;}if(candidate.kind==='ranch'){ranch.interact(candidate);return;}if(Math.abs(fleet.actor.speed)>3){toast('Slow down before interacting.');return;}const c=candidate;
 ...
 function updateCandidate(){const p=fleet.position;candidate=null;const key=MISSIONS[campaign.stage].key;
  ...
  candidate=director?.candidate()||ranch?.candidate()||candidate;
 ...
 function frame(now){requestAnimationFrame(frame);const realDt=Math.min(Math.max((now-last)/1000,0),.1);last=now;
  ...
  syncModels(dt);ranch?.update(dt,time);park.update(dt,time,fleet.actor,settings.night,settings.reduced);director?.update(dt,time);audio.update(fleet.actor.speed,!paused&&fleet.mode!=='foot');
