...
export function sanitizeEconomy(v){
 ...
 // Keep the exact existing mission IDs: dropping them allowed duplicate rewards after reload.
 if(Array.isArray(v.rewardLedger))s.rewardLedger=[...new Set(v.rewardLedger.filter(x=>typeof x==='string'&&(x.startsWith('ranch:')||['aaa:storm-response','aaa:living-herds','aaa:pelagic-recovery'].includes(x))))].slice(0,64);
