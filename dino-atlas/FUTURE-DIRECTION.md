...
The user requested the reusable Prism environment library alongside the planned game updates. Source commit 4df8736d4db11cfe796895d07d60794f31a81745 adds pinned Currentworks Water0.1.0 and Trees0.1.3 to full Classic Reserve using Dino's existing r177 renderer. It upgrades three original water areas and twelve selected original central tree positions; it does not replace the game engine, physics, saves, controls or portal. Keep the original ocean, pool, canal, mission routes and Classic fallback. Reduced Motion and Low/XR quality limits remain. Fire and the newly developing upstream Grass module are not yet integrated into Dino.

Read CURRENTWORKS.md and CURRENTWORKS-HANDOFF.md for the exact implementation and latest verification state. The complete305-test source suite and66 syntax checks passed; the downloaded source artifact matched all83 runtime hashes and the locally edited runtime/test/workflow files. Native source and public checks are recorded separately in the feature handoff and verification/currentworks/. Do not treat the historical First Light public evidence below as verification of these new graphics. Local browser navigation is administratively blocked; use the established hosted workflow, not a policy bypass.

The accompanying First Light polish suppresses unrelated Ranch/Coast dispatch copy while First Light owns guidance, and mounted-rig advertising only while on foot. It retains current goal, HERE, map, equipment, ammunition and action shortcuts. Continue evaluating real player comprehension; this narrow presentation change does not establish that onboarding is finished. Graphics should strengthen place identity and readable paths, not overwhelm the current task or introduce decorative obstacles without collision.
...
Read AGENTS.md, HANDOFF.md, CURRENTWORKS-HANDOFF.md, LIVING-RESERVE-HANDOFF.md, LIVING-RESERVE.md, INTEGRATION-AUDIT.md and the newest verification receipts. Inspect actual current master, exact edited blobs and the established workflow. Direct master writes only; no new PRs, staging branches, transport payloads or temporary source-integration workflows. Reconcile concurrent siblings without force. Do not re-merge historical branches or replace current saves, reward sanitizers or renderers with old packages.
...
The completed First Light recovery verified the source and public archives independently, matched all 77 runtime hashes, and reran 297 model/physics/source tests with zero failures or skips. Public story checks passed screen16 and XR-mock18; the retained public UI/portal checks bring the total to152. The first public attempt failed on five not-yet-updated files before gameplay; its unchanged retry passed. The separate release job returned HTTP403 at archival creation. That opening website publication succeeded; the intended GitHub Release entry was not claimed created. The publication receipt preserves all these historical distinctions. Read the Currentworks handoff for the later graphical runtime, not this earlier baseline.
...
After that bounded review, implement the missing-team chapter using the existing Tidegate geography, useful route choices, actual wildlife conditions and a safe separate chapter-state contract. Further fire or grass integration must use justified reserve locations, compatible material/portal hooks, reversible fallback and measured performance rather than silently inheriting newer Prism assumptions. Keep ongoing notes beside the game after each meaningful checkpoint, including what changed, what passed, what failed and what remains only proposed.
