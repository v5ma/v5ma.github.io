...
export class AAADirector {
 ...
 installUI(){
  ...
  <dialog id="aaa-roadmap-dialog" aria-labelledby="aaa-roadmap-title"><p class="eyebrow">DEVELOPMENT / QUALITY GATES</p><h2 id="aaa-roadmap-title">The path to a finished game.</h2><button class="primary" data-aaa-close>Back to play / B</button><p>Dino Atlas is an expanding playable sandbox. Storm Response connects its vehicles and interiors into an authored mission. This is not a claim that the full game has reached AAA production quality.</p><p><b>Current gate:</b> prove a coherent mission with clear instructions, reachable destinations, controller-only interactions, reliable saves and no repeated reward exploits.</p><p><b>Living Herds:</b> travel-driven gait, articulated heads/tails, feeding/resting, warning-before-charge and a five-step field study are implemented. Grouped responses and bounded wildlife cues are testable; physical-controller and art-review gates remain open. <b>Next:</b> a signature interior and measured real-device performance. Feature quantity alone does not close these gates.</p><p><b>Later:</b> complete planned content, balance the economy, add accessibility and remapping, run long-session tests, then check release and rollback readiness.</p><p>The version-controlled checklist is stored beside the game in <code>AAA-ROADMAP.md</code>. Every milestone has acceptance criteria; hardware testing is tracked separately from automated testing.</p><p><a href="./AAA-ROADMAP.md" target="_blank" rel="noopener">Read the full development checklist in a new tab</a></p><p class="pad-help">D-pad navigates. Right stick scrolls. B closes this panel without leaving the game.</p></dialog>`);
