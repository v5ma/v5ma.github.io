...
document.querySelector('#reset-dialog').addEventListener('close',e=>{
  if(e.target.returnValue!=='reset')return;progress=emptyProgress();try{storage?.removeItem(STORAGE_KEY);}catch{}render();notify('Dino Atlas progress erased. Other browser data was not changed.');
});
