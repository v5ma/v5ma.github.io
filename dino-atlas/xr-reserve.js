...
export class ReserveXR{
 constructor(ctx){
  ...
  this.console=new SpatialConsole(this);
  this.checkSupport();
 }
 ...
 draw(root){
  ...
  if(root){
   ...
   actions.push({label:this.travel.cruise.active?'Express OFF':'Express '+this.travel.settings.multiplier+'x',run:()=>this.travel.toggle()},{label:'Turn left',run:()=>this.snap(Math.PI/6)},{label:'Turn right',run:()=>this.snap(-Math.PI/6)},{label:'Hide field controls',run:()=>this.console?.hideField()},{label:'Leave VR',run:()=>this.enter()});
  ...
  this.texture.needsUpdate=true;this.console?.decorate();
 }
 update(dt){
  ...
  if(this.console)this.console.update(dt,root);else{
