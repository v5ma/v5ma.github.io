...
Before browser CI, 133 local Node tests passed: all 124 prior tests plus nine grounding/control regression groups. The new groups cover degenerate/unreachable IK, fixed segment lengths, scale-correct feet across 13 procedural body families and four scales, planted turning, ranger movement/airborne release, old-save isolation, tracked controller mode mapping and neutral/edge gating. The local native browser is blocked by environment policy, so rendered tests run in GitHub Actions instead.
...
No prior save namespace is cleared or migrated. Journal, recorder, frontier, ranch, economy, storm, herd study, optics, audio and pool mission state models remain unchanged. Only the separate controls preference is new. Revert the Grounded Reserve source commit to restore the previous runtime; the new preference key can remain inert. Do not reset the whole site's local storage.
