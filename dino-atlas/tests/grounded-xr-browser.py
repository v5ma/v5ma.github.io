...
try:
 with sync_playwright() as pw:
  ...
  try:
   ...
   # Unsupported/rejected entry must not break the current game or camera.
   ...
   # Explicit lifecycle mock: no hardware, stereo projection, latency or hand pose claim.
   page.evaluate("""async()=>{
    ...
    for(const [i,s] of [[0,__left],[1,__right]]){const e=x.controllers[i];e.ray.visible=true;e.ray.position.set(i? .25:-.25,1.3,-.25);e.ray.dispatchEvent({type:'connected',data:s});}
   }""")
   ...
   page.evaluate("__rayTile(t=>t.label?.includes('Direct D-pad tools'))")
   check(not page.evaluate('__dinoGrounded.state.quickTools'),'Tracked ray trigger changes mirrored production checkbox')
   page.evaluate("__right.gamepad.buttons[5].value=1");page.wait_for_timeout(350);page.evaluate("__right.gamepad.buttons[5].value=0");wait('!__dinoRanger.state.paused');page.wait_for_timeout(500)
   ...
   wait('__dinoRanger.state.paused');page.wait_for_timeout(500)
   check(page.evaluate('__dinoGrounded.state.hands')==1,'Switching tracked controller to mocked hand is recognized and pauses')
   page.evaluate("__rayTile(t=>t.label==='Back / B',__hand)");wait('!__dinoRanger.state.paused');page.wait_for_timeout(600)
   ...
   page.evaluate("__session.visibilityState='visible';__session.dispatchEvent(new Event('visibilitychange'))");page.wait_for_timeout(300);snap('03-xr-menu-mock.png')
