...
test('signature world keeps the helipad clear and provides physical support for the exterior service ascent',()=>{const old=globalThis.document;globalThis.document={createElement:()=>({width:1024,height:100,getContext:()=>({fillRect(){},strokeRect(){},fillText(){},measureText:()=>({width:40})})})};const physics=new ParkPhysics();try{buildRanchWorld(new T.Scene(),physics);const w=buildNorthstarSignature(new T.Scene(),physics);physics.world.step();assert.equal(w.consoles.length,4);const ray=(x,y,z)=>physics.world.castRay(new R.Ray({x,y,z},{x:0,y:-1,z:0}),50,true);assert.ok(ray(NORTHSTAR.x+NORTHSTAR.hx+4.2,5,NORTHSTAR.z+NORTHSTAR.hz));assert.ok(ray(NORTHSTAR.x+NORTHSTAR.hx+8.2,40,NORTHSTAR.z));assert.ok(ray(NORTHSTAR.x+NORTHSTAR.hx+8.2,45,NORTHSTAR.z));const center=physics.world.castRay(new R.Ray({x:NORTHSTAR.x+2,y:NORTHSTAR.h+12,z:NORTHSTAR.z+1},{x:0,y:-1,z:0}),30,true);assert.ok(center,'existing helipad roof remains supported');}finally{physics.world.free();globalThis.document=old;}});
