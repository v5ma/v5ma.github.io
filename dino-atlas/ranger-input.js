// Standard Xbox-layout Gamepad API input. Poll this even when the simulation is paused.
...
export class RangerInput{
 ...
 clear(){this.keys.clear();this.touch.clear();this.mouseAim=false;this.mouseFire=false;this.neutral=true;}
 ...
 activate(){const root=this.modal(),els=focusable(root);if(!els.includes(document.activeElement)){els[0]?.focus();return;}const e=document.activeElement;if(e.tagName==='SELECT')this.adjust(1);else if(e.type==='range')this.adjust(1);else e.click();}
 ...
 sample(dt){
  let pads=[];try{pads=Array.from(navigator.getGamepads?.()||[]).filter(p=>p?.connected!==false&&p);}catch{}
  const p=pads.find(p=>p.index===this.pad?.index)||pads[0]||null;if(!p&&this.pad){this.previous=[];this.clear();if(this.device==='gamepad')this.onDisconnect?.();}this.pad=p;
  const root=this.modal(),buttons=p?Array.from({length:17},(_,i)=>value(p,i)>.35):[],active=buttons.some(Boolean)||Math.abs(deadzone(p?.axes?.[0]))>.1||Math.abs(deadzone(p?.axes?.[1]))>.1||Math.abs(deadzone(p?.axes?.[2]))>.1||Math.abs(deadzone(p?.axes?.[3]))>.1;
  if(root!==this.context){this.context=root;this.keys.clear();this.touch.clear();this.mouseFire=false;this.mouseAim=false;this.neutral=!!active;this.repeatDirection=0;if(root){const els=focusable(root);if(!root.contains(document.activeElement))els[0]?.focus();}}
  const edges=buttons.map((v,i)=>v&&!this.previous[i]);this.previous=buttons;
  if(this.neutral){if(!active)this.neutral=false;return zero();}
  if(p&&active)this.setDevice('gamepad');
  if(root){
   if(edges[1]||edges[9])this.action('back');else if(edges[8])this.action('map');else if(edges[0])this.activate();
   else if(edges[4])this.action('tabPrev');else if(edges[5])this.action('tabNext');
   const y=deadzone(p?.axes?.[1]),x=deadzone(p?.axes?.[0]),dir=buttons[12]||y<-.55?-1:buttons[13]||y>.55?1:0,side=buttons[14]||x<-.65?-1:buttons[15]||x>.65?1:0,token=dir||side*2;
   ...
   const scroll=deadzone(p?.axes?.[3]);if(Math.abs(scroll)>.1)root.scrollTop+=scroll*dt*500;
   return zero();
  }
  const actions=[[0,'interact'],[2,'reload'],[3,'board'],[5,'nextTool'],[8,'map'],[9,'menu'],[11,'camera'],[13,'horn'],[14,'prevTool'],[15,'nextTool']];
  for(const [i,a] of actions)if(edges[i]){this.action(a);if(this.modal())return zero();}
  const mode=this.mode(),v=p?padMotion(p,mode):zero(),has=(...codes)=>codes.some(k=>this.keys.has(k)),t=this.touch;
  ...
  v.climb=mode==='helicopter'?(Number(has('KeyZ','Space')||t.has('rise'))-Number(has('KeyX')||t.has('lower'))||v.climb):0;
  v.aim||=this.mouseAim||has('ControlLeft')||t.has('aim');v.fire||=(this.mouseFire||t.has('fire'));
  v.brake||=(mode!=='helicopter'&&has('Space'))||t.has('brake');v.boost||=has('ShiftLeft','ShiftRight')||t.has('boost');v.jump||=has('KeyJ')||(mode==='foot'&&has('Space'))||t.has('jump');
  if(v.aim){v.throttle=0;v.climb=0;}return v;
 }
}
