...
export function installTacticsUI(api){
 ...
 const pause=button('Field rig / powers & research','field-from-pause',()=>open('kit'));$('pause-dialog').append(pause);
 ...
 function stock(){const s=api.state(),t=s.tactics;body.replaceChildren();$('field-message').textContent='';$('field-title').textContent=page==='hack'?'Rewire the security junction':page==='relay'?'Prepare. Defend. Recover.':'The second hand changes the fight.';
  ...
  if(page==='hack'){
   const circuit=document.createElement('div');circuit.id='field-circuit';circuit.dataset.navGrid='3';const path=connectedCircuit(t.circuit).path;
   ...
   if(t.learned)body.append(button('Close and survey aimed target','field-survey-menu',()=>{api.closeAll();requestAnimationFrame(()=>api.action('survey'));}));
