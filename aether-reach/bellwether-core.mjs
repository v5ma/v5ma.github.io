...
export function createBellwether({emit,clearLine,groundAt,occupied,drop}){
 ...
 function handle(s,id){if(s.won||nearby(s)?.id!==id)return false;const b=s.bellwether;s.expedition.tracked=BELL_TASK.id;
  ...
  if(id==='bell-test'&&b.stage===2){if(!BELL_TARGETS.every((v,i)=>v===b.dials[i])){say(s,['Supply does not reach the workshop. Set supply to 2.','Supply reaches the workshop; return is interrupted. Set return to 1.','Supply and return connect; balance is unstable. Set balance to 3.'][bellCircuitFeedback(s).firstFault]+' The maintenance card remains available.');return false;}advance(s,3,'TAVI: Arcade power is stable. The west service shutter is open. Climb the maintenance galleries for an overlook, or take the familiar south Theatre ladder. Reconnect the rooftop receiver.');return true;}
