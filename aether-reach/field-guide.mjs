...
import {fieldGuidance,localMapPoint,noticeAlpha,readingPages} from './field-guide-core.mjs';
...
export function createFieldGuide({rig,api}){
 ...
 const map=surface('Live floor map and next action',1.28,512),notice=surface('Two-second floor message',1.28,192);notice.mesh.position.z=.5;
 ...
 function update({enabled,head,config,now}){
  ...
  const data=api.guide?.()||{};source=data.notice;info=fieldGuidance(api.state(),data.near,api.useLabel?.()||'Right grip');
