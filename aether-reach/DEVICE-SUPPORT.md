# Field Guide 0.15.1 device repair

Look down for a room-floor live map, current objective and nearby interaction. Touch right grip uses; the active Xbox use binding is shown instead when a standard pad is active. Touch Y / Xbox Menu opens help/history and separate menu/map/window sizing on the pause menu's first page. Messages fade completely after two wall-clock seconds and remain in session history. Map size and seated reading height are adjustable independently of the diorama. Aperture height defaults to 2.5 times the old height, retaining the footprint; its multiplier and overall size are adjustable. First-person AR remains a player-eye window, not a room-filling scene. No controller remap, hand-only gameplay, physical approval or private hub integration is implied. See FIELD-GUIDE.md and the latest release receipt.
