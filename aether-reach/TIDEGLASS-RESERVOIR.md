...
The final candidate is reconciled with the current master before browser validation. Legacy browser checks now wait for real dialog/rail conditions instead of relying on fixed software-renderer timing; those changes preserve the same gameplay assertions and do not write game state.
