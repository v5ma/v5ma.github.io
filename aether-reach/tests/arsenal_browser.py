...
with sync_playwright() as pw:
 ...
 b=pw.chromium.launch(**kw);ctx=b.new_context(viewport={'width':1440,'height':900},service_workers='block');host=urlparse(BASE).hostname;ctx.route('**/*',lambda r:r.continue_() if urlparse(r.request.url).hostname==host or r.request.url.startswith(('blob:','data:')) else r.abort());page=ctx.new_page();page.set_default_timeout(90000);page.on('pageerror',lambda e:errors.append(str(e)));page.on('dialog',lambda d:d.accept())
 try:
  page.goto(BASE+'/aether-reach/index.html',wait_until='domcontentloaded');page.wait_for_function('window.AetherReach');page.locator('#start').click();page.wait_for_function('AetherReach.snapshot().playing');check(page.evaluate('AetherReach.version==="0.3.0"'),'The upgraded real application is running')
  ...
  if MODE=='arsenal':
   ...
   page.locator('#shop-dialog form button').click();page.locator('#world').focus();page.keyboard.press('KeyZ');page.wait_for_function('AetherReach.snapshot().fov<25');check(page.locator('#scope-view').is_visible(),'A real camera-FOV magnification appears through the circular sniper optic')
   ...
   page.locator('#shop-dialog form button').click();page.reload(wait_until='domcontentloaded');page.wait_for_function('window.AetherReach');page.locator('#continue').click();page.wait_for_function('AetherReach.snapshot().playing');check(snap(page)['weapon']=='sniper' and snap(page)['tune']['sniper']['damage']==1 and snap(page)['credits']==10,'Reload and Continue preserve purchased kit, credits and upgrade without repaying drops')
   ...
   page.locator('#shop-dialog form button').click();page.locator('#world').focus();page.keyboard.press('Digit2');page.wait_for_function('AetherReach.snapshot().weapon==="carbine"');page.keyboard.down('KeyF');page.wait_for_timeout(450);page.keyboard.up('KeyF');check(snap(page)['ammo']<22,'Automatic carbine has its own firing cadence and magazine');page.screenshot(path=str(OUT/'tempest-carbine.png'))
   page.keyboard.press('Digit4');page.wait_for_function('AetherReach.snapshot().weapon==="scatter"');page.wait_for_timeout(300);page.keyboard.press('KeyF');check(snap(page)['ammo']==5,'Scattergun consumes one shell for a distinct six-pellet shot');page.screenshot(path=str(OUT/'foundry-scattergun.png'))
   page.set_viewport_size({'width':390,'height':844});page.wait_for_function('innerWidth===390');check(not page.evaluate('document.documentElement.scrollWidth>innerWidth'),'The new buy/loadout HUD fits a narrow viewport');page.screenshot(path=str(OUT/'arsenal-mobile.png'))
  else:
   ...
   page.keyboard.down('ArrowRight');page.wait_for_function('AetherReach.snapshot().target?.id==="gale-loop"');page.keyboard.up('ArrowRight');page.keyboard.up('KeyW');after=snap(page)
   ...
   page.screenshot(path=str(OUT/'free-look-transfer.png'));page.keyboard.press('Space');page.wait_for_function('!AetherReach.snapshot().rail');page.wait_for_timeout(230);page.keyboard.press('KeyE',delay=100);page.wait_for_function('AetherReach.snapshot().rail?.id==="gale-loop"');check(snap(page)['stats']['transfers']==1,'A real jump and aimed catch changes onto the new rail without a scripted position assignment')
  ...
  (OUT/(MODE+'-failure.json')).write_text(json.dumps({'error':str(e),'checks':checks,'errors':errors,'snapshot':s},indent=2));page.screenshot(path=str(OUT/(MODE+'-failure.png')));raise
