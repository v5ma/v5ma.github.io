...
with sync_playwright() as pw:
 ...
 b=pw.chromium.launch(**kw);ctx=b.new_context(viewport={'width':960,'height':640},device_scale_factor=.5,service_workers='block');host=urlparse(BASE).hostname;ctx.route('**/*',lambda r:r.continue_() if urlparse(r.request.url).hostname==host or r.request.url.startswith(('blob:','data:')) else r.abort());page=ctx.new_page();page.set_default_timeout(90000);page.on('pageerror',lambda e:errors.append(str(e)));page.on('dialog',lambda d:d.accept());page.add_init_script("window.combatInputLog=[];window.addEventListener('keydown',e=>{if(['KeyZ','KeyE','Space','Digit2'].includes(e.code)){combatInputLog.push({code:e.code,repeat:e.repeat,focus:e.target.tagName,state:window.AetherReach?.snapshot()});if(combatInputLog.length>20)combatInputLog.shift();}},true)")
 try:
  page.goto(BASE+'/aether-reach/index.html',wait_until='domcontentloaded');page.wait_for_function('window.AetherReach');balanced_graphics(page,"low");page.locator('#start').click();page.wait_for_function('AetherReach.snapshot().playing');check(page.evaluate('AetherReach.version')==json.loads((ROOT/'aether-reach/release.json').read_text())['version'],'The upgraded real application is running')
  ...
  if MODE=='arsenal':
   ...
   # Issue the intended jump/catch during the moving window, before a slow screenshot.
   page.keyboard.press('Space');page.wait_for_function('!AetherReach.snapshot().rail');page.keyboard.press('KeyE',delay=100);page.wait_for_function('AetherReach.snapshot().rail?.id==="gale-loop"');check(snap(page)['stats']['transfers']==1,'A real jump and aimed catch changes onto the new rail without a scripted position assignment')
