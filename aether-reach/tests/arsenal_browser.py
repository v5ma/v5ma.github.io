...
with sync_playwright() as pw:
 ...
 try:
  ...
  if MODE=='arsenal':
   ...
   page.locator('#shop-dialog form button').click();page.wait_for_function('!AetherReach.snapshot().paused');page.locator('#world').focus();page.keyboard.press('Digit2');page.wait_for_function('AetherReach.snapshot().weapon==="carbine"');page.keyboard.down('KeyF');page.wait_for_timeout(450);page.keyboard.up('KeyF');check(snap(page)['ammo']<22,'Automatic carbine has its own firing cadence and magazine');page.screenshot(path=str(OUT/'tempest-carbine.png'))
