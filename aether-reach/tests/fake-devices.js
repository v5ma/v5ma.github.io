...
(()=>{
 ...
 function transform(px,py,pz,yaw=0,roll=0,pitch=0){const sy=Math.sin(yaw/2),cy=Math.cos(yaw/2),sr=Math.sin(roll/2),cr=Math.cos(roll/2),sx=Math.sin(pitch/2),cx=Math.cos(pitch/2),x=cy*sx*cr+sy*cx*sr,y=sy*cx*cr-cy*sx*sr,z=cy*cx*sr-sy*sx*cr,w=cy*cx*cr+sy*sx*sr;return {position:{x:px,y:py,z:pz},orientation:{x,y,z,w},matrix:new Float32Array([1-2*(y*y+z*z),2*(x*y+z*w),2*(x*z-y*w),0,2*(x*y-z*w),1-2*(x*x+z*z),2*(y*z+x*w),0,2*(x*z+y*w),2*(y*z-x*w),1-2*(x*x+y*y),0,px,py,pz,1])};}
 ...
 class Session extends EventTarget{
  ...
  requestAnimationFrame(fn){if(this.ended)return 0;return window.requestAnimationFrame(t=>{
   ...
   const h=transform(devices.headX,devices.headY??1.65,devices.headZ||0,-devices.headYaw,devices.headRoll||0,devices.headPitch||0);const views=['left','right'].map((eye,i)=>({eye,projectionMatrix:projection,transform:transform(devices.headX+(i?.032:-.032),devices.headY??1.65,devices.headZ||0,-devices.headYaw,devices.headRoll||0,devices.headPitch||0)}));
