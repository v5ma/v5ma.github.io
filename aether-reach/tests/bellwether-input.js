...
window.BlackoutDriver=(()=>{
 ...
 async function clear(prefix,timeout=180000){
  // The older chapter replay assumed every boarder could be shot from the receiver.
  // Crosswind intentionally blocks that angle. Use the actual southern roof and
  // gallery-side flank; never change a guard, screen or mission state for the test.
  const initial=snap(),live=id=>initial.enemies.some(e=>e.id===id&&e.hp>0);
  if(prefix.length===1&&prefix[0]==='bell-blackout-roof'&&initial.bellwetherArt?.windbreak?.mode===0&&live('bell-blackout-roof-0')&&live('bell-blackout-roof-1')){
   await clear(['bell-blackout-roof-1'],timeout);
   for(const [x,z] of [[-111,-3],[-104,-4],[-102,-6]])await walk(x,z);
   await clear(['bell-blackout-roof-0'],timeout);
   for(const [x,z] of [[-104,-4],[-111,-3],[-111,-6]])await walk(x,z);
   return snap();
  }
  const deadline=performance.now()+timeout;TestPad.button(6,true);let idle=0;
