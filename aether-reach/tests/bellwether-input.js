...
window.BlackoutDriver=(()=>{
 ...
 async function walk(x,z,timeout=160000,tolerance=.38){TestPad.button(6,false);const deadline=performance.now()+timeout;while(performance.now()<deadline){const s=snap(),p=s.position;if(s.paused)throw Error('Paused during route');const dx=x-p.x,dz=z-p.z,d=Math.hypot(dx,dz);if(d<tolerance){await neutral();return s;}const turn=angle(Math.atan2(dx,-dz)-p.yaw);const [rx,ry]=stick(turn*.9,p.pitch*.9);TestPad.axes([0,Math.abs(turn)<.15?-Math.min(1,d):0,rx,ry]);await frame();}await neutral();throw Error('Blocked route to '+x+','+z+' at '+JSON.stringify(snap().position));}
