...
with sync_playwright() as pw:
 ...
 try:
  ...
  drive((-7,4));tap(2);p.wait_for_selector('#record-dialog[open]');p.wait_for_function("AetherReach.snapshot().records.includes('quay-letter')");go('#record-dialog button');tap(0);p.wait_for_selector('#record-dialog[open]',state='hidden');tap(8);p.wait_for_selector('#map-dialog[open]');go('#journal button:not(:disabled)');tap(0);check(snap()['devices']['menu']=='record-dialog','Controller opens an earned archive record from inside the atlas');tap(1);check(snap()['devices']['menu']=='map-dialog' and snap()['paused'],'Closing a record returns to its atlas parent without advancing combat');tap(1)
