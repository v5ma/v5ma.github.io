...
with sync_playwright() as pw:
 ...
 try:
  ...
  enter('diorama-ar');d=snap()['devices']['presentation'];check(p.evaluate('TestXR.devices.requestedMode')=='immersive-ar' and d['active'],'Portal AR requests immersive-ar explicitly');check(d['workspace']['active'] and not d['workspace']['attachedToWindow'] and not d['uiHeadLocked'],'Diorama status uses the separate room/controller workspace, not the head or box')
