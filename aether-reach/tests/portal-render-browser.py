...
with sync_playwright() as pw:
 ...
 result=p.evaluate('portalResult');(OUT/'portal-render.json').write_text(json.dumps(result,indent=2));p.screenshot(path=str(OUT/'portal-render.png'));b.close();assert not result.get('error'),result;assert result['passed']==7,result
 print('PASS seven ray-aperture GPU fixtures, including scaled stereo viewports, rolled head, custom shader and sprite',flush=True)
