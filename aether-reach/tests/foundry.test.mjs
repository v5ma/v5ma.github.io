...
test('Arena title signs stay above standing eye height at the console',()=>{const previous=globalThis.document;globalThis.document={createElement:()=>({width:512,height:128,getContext:()=>({fillRect(){},strokeRect(){},fillText(){}})})};try{const scene=new T.Scene(),art=installFoundryArt({scene,cover:COMBAT_COVER,districts:DISTRICTS});scene.updateMatrixWorld(true);for(const c of ARENA_CONSOLES){const sign=scene.getObjectByName('sign:'+c.name.replace(' combat trial','').toUpperCase());assert.ok(sign);assert.ok(new T.Box3().setFromObject(sign).min.y>c.y+2.5);}art.dispose();}finally{if(previous===undefined)delete globalThis.document;else globalThis.document=previous;}});
