import {tideProgress} from './tideglass-world.mjs';
...
export function installExpeditionUI(api){
 ...
 function progress(s,t){const e=s.expedition,has=id=>e.flags.includes(id);if(has(t.flag))return 'Completed';if(t.id.startsWith('tideglass-'))return tideProgress(s,t.id);if(t.id===BELL_TASK.id)return bellProgress(s);
 ...
 let last='';function update(){button.hidden=!api.playing();hud.hidden=!api.playing()||api.paused();if(hud.hidden)return;const s=api.state(),e=s.expedition,t=TASKS.find(t=>t.id===e.tracked),goal=expeditionGoal(s),completed=TASKS.filter(t=>e.flags.includes(t.flag)).length;
  ...
  if(e.tracked.startsWith('tideglass-'))detail=tideProgress(s,e.tracked)+' / '+detail;
