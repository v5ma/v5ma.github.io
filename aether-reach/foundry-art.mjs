...
export function installFoundryArt({scene,cover,districts}){
 ...
 const consoleLights=[];
 for(const c of ARENA_CONSOLES){const g=new T.Group();g.name=c.id;g.position.set(c.x,c.y,c.z);root.add(g);add('cylinder','metal',[0,.43,0],[.35,.86,.35],g);const panel=add('box','dark',[0,1,0],[1.1,.45,.55],g);panel.rotation.x=-.18;const lamp=add('sphere','glow',[0,1.2,0],[.11,.11,.11],g);consoleLights.push({id:c.id,mesh:lamp});sign(c.name.replace(' combat trial','').toUpperCase(),c.x,c.y+2.2,c.z+.12,4.6);}
