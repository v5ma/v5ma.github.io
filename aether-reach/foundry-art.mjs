...
export function installFoundryArt({scene,cover,districts}){
 ...
 function update(s,{menu=false,reduced=false}={}){const k=s.skirmish;if(!k)return;dynamicCount=0;const sky=scene.userData.skyglass;kit.materials.ghost.uniforms.clock.value=reduced?0:s.time;kit.materials.ghost.uniforms.motion.value=!reduced&&sky?.enabled!==false&&sky?.mode!=='low'?1:0;const c=k.companion;tavi.visible=!menu&&!scene.userData.castHas?.('tavi')&&Math.hypot(c.x-s.p.x,c.y-s.p.y,c.z-s.p.z)<100;tavi.position.set(c.x,c.y,c.z);tavi.rotation.y=c.heading||0;flightRing.visible=!!c.flying;const moving=last&&Math.hypot(c.x-last.x,c.z-last.z)>.005,phase=reduced?0:Math.sin(s.time*9)*(moving?.38:.025);for(const{leg,arm,side}of limbs){leg.rotation.x=side*phase;arm.rotation.x=k.offer?-.95:-side*phase*.65;}last={x:c.x,z:c.z};stats.companionVisible=tavi.visible;
