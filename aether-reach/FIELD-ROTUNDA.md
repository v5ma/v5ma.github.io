...
Workspace height, size, distance and rotation edits now use the room origin captured on opening. Turning or leaning while editing an option cannot silently recall the panel to a new position. Explicit Recall or closing/reopening remains the deliberate recenter path. Reset workspace placement preserves the chosen controller-side status, reduced-motion setting and guided-aim setting; it resets geometry only.

A review of the original raised-menu screenshot found that the page's accessible HTML fallback covered the underlying WebGL image. Those captures are not XR visual proof. The native/public rotunda journey now captures the actual stereo canvas immediately after the normal XR render, without hiding the menu, changing gameplay or substituting an isolated scene. Both the original page capture and the canvas evidence are retained. Three added model fixtures and the extended ordinary-input hand journey cover stable editing and non-destructive placement reset.
