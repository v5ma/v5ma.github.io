...
runs=get(f'repos/{repo}/actions/runs?head_sha={head}&event=pull_request&status=success&per_page=100')['workflow_runs']
run=next(r for r in runs if r['path']=='.github/workflows/aether-grounded.yml' and r['head_sha']==head and r['conclusion']=='success')
...
with zipfile.ZipFile(out/f'Aether-Reach-v{version}-native-grounded-evidence.zip','w',zipfile.ZIP_DEFLATED) as archive:
 for suite,name in reports.items():
  ...
  subprocess.run(['gh','run','download',str(run['id']),'--name','aether-grounded-'+suite,'--dir',str(folder)],check=True)
...
print('Archived all nine exact-source grounded review suites from',run['id'])
