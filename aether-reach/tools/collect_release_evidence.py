...
out=Path('/tmp/aether-release');out.mkdir(exist_ok=True);version=json.loads(Path('aether-reach/release.json').read_text())['version']
...
if Path('aether-reach/tactics-core.mjs').exists():subprocess.run(['python','aether-reach/tools/collect_tactical_evidence.py'],check=True)
if Path('aether-reach/quay-art.mjs').exists():subprocess.run(['python','aether-reach/tools/collect_art_evidence.py'],check=True)
