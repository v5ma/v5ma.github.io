...
export {GLIDE};
import {CITY_DISTRICTS,CITY_BRIDGES,CITY_BUILDINGS,ROOMS,CITY_FURNITURE,roomWalls,closedGates,stairContains,cityFloor} from './city-world.mjs';
import {initCity,cleanCity,maxHealth,cityNearby,cityInteract,cityChoice,cityTick} from './city-state.mjs';
import {makeSkiff,boardSkiff,parkSkiff,vehicleStep} from './skiff.mjs';
...
export const DISTRICTS=[
 ...
 {id:'spire',name:'The Meridian Spire',x:45,y:20,z:-119,w:38,d:36,theme:'spire'},...CITY_DISTRICTS
];
export const BRIDGES=[
 ...
 {id:'north-walk',a:[-13,12,-90],b:[29,20,-116],width:5},...CITY_BRIDGES
];
export const RELAYS=[
...
export const RAILS=[
 ...
 rail('commons-loop','Lantern Courier Loop','skyyard','commons',[[-33,5.1,32],[-46,17,17],[-35,25,-1],[-21,22,16],[0,12,26],[-3,3.1,38]]),
...
export const BUILDINGS=[
 ...
 {id:'spire-tower',x:44,z:-132,y:20,w:10,d:8,h:33}, {id:'observatory',x:57,z:-112,y:20,w:7,d:7,h:16},...CITY_BUILDINGS
];
// Relays and notes sit outside solid buildings, intentionally reachable on foot.
RELAYS[0].x=64;RELAYS[1].x=-28;
export const SOLIDS=[...BUILDINGS.map(b=>{const room=ROOMS.find(r=>r.building===b.id);return {x1:b.x-b.w/2,x2:b.x+b.w/2,y1:room?b.y+room.height+.25:b.y,y2:b.y+b.h+4,z1:b.z-b.d/2,z2:b.z+b.d/2};}),...ROOMS.flatMap(roomWalls),...CITY_FURNITURE];
export const stateSolids=s=>[...SOLIDS,...closedGates(s?.city)];
...
export function groundAt(x,z,under=Infinity){let y=-Infinity,id=null;
 for(const p of DISTRICTS)if(Math.abs(x-p.x)<=p.w/2&&Math.abs(z-p.z)<=p.d/2&&p.y<=under&&p.y>y&&!(p.id==='harbor'&&stairContains(x,z))){y=p.y;id=p.id;}
 ...
 const roomFloor=cityFloor(x,z,under);if(roomFloor.y>y){y=roomFloor.y;id=roomFloor.id;}return {y,id};
}
...
export function clearLine(a,b,state=null){const solids=stateSolids(state),l=distance(a,b);if(l<1e-8)return !solids.some(s=>a.x>s.x1&&a.x<s.x2&&a.y>s.y1&&a.y<s.y2&&a.z>s.z1&&a.z<s.z2);const d={x:(b.x-a.x)/l,y:(b.y-a.y)/l,z:(b.z-a.z)/l};return !solids.some(s=>rayBox(a,d,s,l)!==null);}
...
export function railTarget(s){if(s.p.vehicle)return null;const p=s.p,head={x:p.x,y:p.y+1.6,z:p.z},view=forward(p.yaw,p.pitch);let best=null;
 for(const r of RAILS){if(r.id===p.rail?.id||!p.grounded&&s.time-p.airSince<.85&&r.id===p.lastRail)continue;const target=nearestRailOn(head,r,p.grounded?4.3:6.3);if(!target)continue;const v={x:target.point.x-head.x,y:target.point.y-head.y,z:target.point.z-head.z},dot=(v.x*view.x+v.y*view.y+v.z*view.z)/(target.distance||1);if(!p.grounded&&dot<-.12)continue;if(!clearLine(head,target.point,s))continue;const dest={x:target.point.x,y:target.point.y-2.65,z:target.point.z};if(occupied(dest.x,dest.y,dest.z,s)||!clearLine({...head,y:head.y-.6},{...dest,y:dest.y+1},s))continue;const score=target.distance-(p.grounded?0:dot*2.8);if(!best||score<best.score)best={...target,score,dot};}return best;
}
...
export function nearby(s){const p=s.p;if(p.vehicle)return cityNearby(s);const head={x:p.x,y:p.y+1.6,z:p.z};if(p.rail){const target=railTarget(s);if(target&&clearLine(head,target.point))return {type:'hook',target,label:'SPACE then E · Transfer to '+target.rail.name};return {type:'rail',label:'SPACE release · C reverse · S brake'};}
 ...
 const city=cityNearby(s,(a,b)=>clearLine(a,b,s));if(city)return city;const target=railTarget(s);if(target&&clearLine(head,target.point,s))return {type:'hook',target,label:'E · Hook '+target.rail.name};return null;}
export function interact(s){const n=nearby(s),p=s.p;
 ...
 if(!n)return false;
 if(n.type==='city-vehicle')return p.vehicle?parkSkiff(s):boardSkiff(s);
 if(n.type.startsWith('city-'))return cityInteract(s,n);
 if(n.type==='loot')return collect(s,n.id);
 if(n.type==='relay'){s.kit.credits=Math.min(99999,s.kit.credits+90);s.relays.add(n.id);s.checkpoint=n.id;p.health=maxHealth(s);p.shield=60+s.kit.shield*20;p.ammo=weaponStats(s).mag;emit(s,'relay',{id:n.id});emit(s,'save');return true;}
...
export function reverseRail(s){if(s.p.vehicle){s.vehicleDescend=.4;return;}if(!s.p.rail)return;s.p.rail.dir*=-1;s.p.speed=Math.max(7,s.p.speed*.72);s.stats.reversals++;emit(s,'reverse');}
...
function occupied(x,y,z,s=null){return stateSolids(s).some(b=>x+.38>b.x1&&x-.38<b.x2&&y+1.8>b.y1&&y<b.y2&&z+.38>b.z1&&z-.38<b.z2);}
export function step(s,input,dt){
 if(s.won)return;dt=clamp(dt,0,.025);s.time+=dt;const p=s.p;cityTick(s,dt);
 ...
 if(p.vehicle){vehicleStep(s,input,dt,{groundAt,blocked:(x,y,z)=>occupied(x,y,z,s)});}else if(p.rail){const r=RAILS.find(r=>r.id===p.rail.id);p.speed=clamp(p.speed+(input.back?-16:input.boost?15:p.speed>19?-3:4)*dt,3,input.boost||p.speed>19?28:19);const old=p.rail.s;p.rail.s=clamp(old+p.speed*p.rail.dir*dt,0,r.length);s.stats.railDistance+=Math.abs(old-p.rail.s);const q=pointOnRail(r,p.rail.s);if(p.latch){p.latch.t+=dt;const f=Math.min(1,p.latch.t/.18);p.x=p.latch.x+(q.x-p.latch.x)*f;p.y=p.latch.y+(q.y-2.65-p.latch.y)*f;p.z=p.latch.z+(q.z-p.latch.z)*f;if(f===1)p.latch=null;}else{p.x=q.x;p.y=q.y-2.65;p.z=q.z;}
  ...
  const targetX=(f.x*iz+rx*ix)/len*speed,targetZ=(f.z*iz+rz*ix)/len*speed,blend=1-Math.exp(-dt*(p.grounded?14:2.2));if(p.gliding){const charge=p.glideCharge;glideVelocity(p,input,dt);p.glideCharge+=Math.max(0,charge-p.glideCharge)*(s.city.skills.aviation*.08);}else{p.vx+=(targetX-p.vx)*blend;p.vz+=(targetZ-p.vz)*blend;}
  ...
  if(Number.isFinite(floor.y)&&((was&&Math.abs(floor.y-oldY)<=.55)||(oldY>=floor.y&&p.y<=floor.y))){p.y=floor.y;p.vy=0;p.grounded=true;}
 ...
 for(let i=0;i<s.drones.length;i++){
  ...
  const range=b.kind==='sentry'?52:30,seen=distance(p,b)<range&&clearLine(b,{x:p.x,y:p.y+1.3,z:p.z},s);b.telegraph=seen&&b.attack<.8?Math.max(0,1-b.attack/.8):0;
...
export function jump(s){if(s.p.vehicle){s.vehicleRise=.4;return true;}if(s.p.rail){detach(s,true);return true;}if(s.p.grounded){s.p.vy=7;s.p.grounded=false;s.p.y+=.05;return true;}return false;}

// Room-scale translation is collision checked independently of joystick motion.
export function roomMove(s,dx,dz){if(!Number.isFinite(dx)||!Number.isFinite(dz)||Math.hypot(dx,dz)>.5||s.p.rail||s.p.vehicle)return false;const p=s.p;if(!occupied(p.x+dx,p.y,p.z,s))p.x+=dx;if(!occupied(p.x,p.y,p.z+dz,s))p.z+=dz;return true;}
...
export function toggleGlide(s){const p=s.p;if(s.won)return false;if(p.gliding)return foldGlide(s);const floor=groundAt(p.x,p.z,p.y);if(p.grounded||p.rail||p.vehicle||p.glideCharge<8||occupied(p.x,p.y,p.z)||(Number.isFinite(floor.y)&&p.y-floor.y<1.2))return false;p.gliding=true;s.stats.glides++;emit(s,'glide-open');return true;}

export function chooseCity(s,id,choice){return cityChoice(s,id,choice,(a,b)=>clearLine(a,b,s));}
