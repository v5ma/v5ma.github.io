...
export function createDiorama(view){
 ...
 function update(s,dt){if(!active)return;
  ...
  if(preview){for(const child of camera.children)if(!child.userData.xrUI)hide(child,true);const q=config.yaw;camera.position.set(x-Math.sin(q)*46,y+(config.opening==='front'?12:config.opening==='top'?78:43),z+Math.cos(q)*46);camera.lookAt(x,y+6,z);}
