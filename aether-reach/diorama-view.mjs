...
import {PortalMaterials,shellMaterial,enterPortal} from './portal-aperture.mjs';
...
export function createDiorama(view){
 ...
 const faces={floor:[[0,-1,0],[0,0,0]],top:[[0,1,0],[0,35,0]],left:[[-1,0,0],[-30,0,0]],right:[[1,0,0],[30,0,0]],back:[[0,0,-1],[0,0,-24]],front:[[0,0,1],[0,0,24]]};
 const gold=new T.MeshBasicMaterial({color:0xc8a363});
 const plane=new T.PlaneGeometry(1,1),walls={};
 for(const name of ['floor','left','right','back','top','front']){const [n,p]=faces[name],m=new T.Mesh(plane,shellMaterial(new T.Vector3(...n),new T.Vector3(...p),portal.uniforms));m.renderOrder=800;m.name='Diorama '+name;root.add(m);walls[name]=m;}
 ...
 function center(s,head=null){const p=s.p;focus={x:p.x,y:p.y,z:p.z};heading=p.yaw;if(head){const n=Math.hypot(head.forward?.x??0,head.forward?.z??-1)||1;anchor={x:head.x+(head.forward?.x??0)/n*1.45,y:config.height,z:head.z+(head.forward?.z??-1)/n*1.45};}else anchor.y=config.height;}
 ...
 function scan(){
  const open=openingState(config.opening);scene.traverse(o=>{if(o.userData.roomShell)hide(o,!!open[o.userData.roomShell+'Open']);});
  portal.collect(scene,new Set([rig,camera,root]));
 }
 function syncRig(target){if(!active||preview)return;rig=target;const pos=worldPoint({x:0,y:0,z:0},focus,anchor,viewConfig());rig.position.set(pos.x,pos.y,pos.z);rig.rotation.set(0,-viewConfig().yaw,0);rig.scale.setScalar(1/config.scale);rig.updateMatrixWorld(true);}
 function update(s,dt,{followAim=preview}={}){if(!active)return;
  const p=s.p;focus={x:p.x,y:p.y,z:p.z};if(followAim)heading=p.yaw;
  scene.fog=null;scene.background=null;renderer.setClearColor(config.mode==='diorama-ar'&&!preview?0x000000:0x0d202b,config.mode==='diorama-ar'&&!preview?0:1);portal.active=true;
  const {x,y,z}=focus,low=y-3,q=viewConfig().yaw;
  root.position.set(x,low,z);root.rotation.y=-q;root.updateMatrixWorld(true);portal.configure(root.matrixWorld);inverse.copy(root.matrixWorld).invert();walls.floor.position.set(0,0,0);walls.floor.rotation.x=-Math.PI/2;walls.floor.scale.set(60,48,1);
 ...
 return {set,configure,center,syncRig,update,aim,orbit(delta){heading+=delta;},get viewYaw(){return viewConfig().yaw},get active(){return active},get preview(){return preview},get config(){return {...config}},stats:()=>({active,preview,...config,...openingState(config.opening),focus:{...focus},anchor:{...anchor},widthMeters:60*config.scale,avatar:actorStatus,avatarError:actorError,avatarHeight:1.76,avatarPose:actor?.pose||null,clippedMaterials:portal.entries.size,portalMaterials:portal.entries.size,portalBuild:"aether-portal-20260917.1",fullDepth:true,playerCentered:true,viewYaw:viewConfig().yaw,roomAnchored:true,headMovesCharacter:false})};
}
