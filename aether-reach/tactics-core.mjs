...
export function createTactics({solids,clearLine,rayBox,raySphere,forward,emit,defeated,hurt}){
 ...
 function start(s){if(s.won||!near(s,RECOVERY,2.7)||s.tactics.encounter.phase==='active')return false;s.drones=s.drones.filter(b=>!b.tactical);s.tactics.projectiles=[];s.tactics.encounter={phase:'active',time:0,hull:RECOVERY.hull,wave:0,kills:0,turretCooldown:0,turretCharge:14};emit(s,'tactical-start');return true;}
 ...
 function step(s,input,dt){const t=s.tactics;if(!t)return;t.cooldown=Math.max(0,t.cooldown-dt);t.scanCooldown=Math.max(0,t.scanCooldown-dt);t.flash=Math.max(0,t.flash-dt);
  ...
  if(t.hacked){e.turretCooldown-=dt;const enemy=alive.sort((a,b)=>dist(a,RECOVERY)-dist(b,RECOVERY)).find(b=>dist(TURRET,b)<23&&clearLine(TURRET,b));if(enemy&&e.turretCooldown<=0&&e.turretCharge>0){e.turretCooldown=.8;e.turretCharge--;damage(s,enemy,t.module==='engineer'?25.2:18,'turret');t.metrics.turretHits++;emit(s,'tactical-turret',{o:{...TURRET},end:{x:enemy.x,y:enemy.y,z:enemy.z}});}}
