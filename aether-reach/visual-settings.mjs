...
export function installVisualSettings(view){
 ...
 let characters=saved.characters!==false,skyglass=saved.skyglass!==false,environment=saved.environment!==false;
 function persist(){try{localStorage.setItem(key,JSON.stringify({version:1,mode,characters,skyglass,environment}));}catch{}}
 view.setVisualMode(mode);view.setCastEnabled(characters);view.setSkyglassEnabled(skyglass);view.setEnvironmentEnabled(environment);
 ...
 for(const [id,text,value,change] of [['visual-currentworks','Currentworks water, trees and mesh clouds',environment,v=>{environment=v;view.setEnvironmentEnabled(v);}], ['visual-characters','Animated character models',characters,v=>{characters=v;view.setCastEnabled(v);}],['visual-skyglass','Skyglass rifts and moving cloud shade',skyglass,v=>{skyglass=v;view.setSkyglassEnabled(v);}]] ){
 ...
 const note=document.createElement('p');note.className='fine';note.textContent='Currentworks adds rippled water, planted trees and solid mesh clouds using the existing renderer. Turn it off to restore the original decoration. Nearby people use licensed animated models, with simpler silhouettes at distance or while loading. Light and immersive VR limit the animated cast and use static rift outlines; moving cloud shade is disabled. Reduced motion freezes decorative shader motion. Hardware performance remains unverified.';document.querySelector('#settings-dialog form').before(note);
}
