 ...
 * Shared/instanced materials and meshes; lower-detail mobile payloads.
 * A missing asset retains its corresponding original visual fallback. */
import * as T from './vendor/three.module.js';
import {recolorFacade,cloudSky} from './quay-detail.mjs';
import {GLTFLoader} from './vendor/loaders/GLTFLoader.js';
import {RGBELoader} from './vendor/loaders/RGBELoader.js';
import {QUAY_BUILDINGS,QUAY_LAMPS,QUAY_PLANTERS,facadePlan,roofPlan} from './quay-layout.mjs';
export function installQuayArt({scene,renderer,quality,sky,hemi,sun,decks,fallbacks}){
 const profile=quality==='low'||/OculusBrowser|Quest/i.test(navigator.userAgent)?'mobile':'desktop',state={revision:'quay-art-1',profile,loaded:[],errors:[],settled:false,models:0,instances:0,paleMeshes:0};
 ...
 const materials=new Map();const url=file=>new URL('./art/'+file,import.meta.url).href;
 ...
 jobs.push(attempt('modular architecture',async()=>{
  const library=prepare(await loader.loadAsync(url('architecture-'+profile+'.glb'))),templates=new Map(library.children.map(o=>[o.name,o]));let theme='brick';
  function piece(group,name,x,y,z,sx=1,sy=1,sz=1,yaw=0){const original=templates.get(name);if(!original)throw Error('Missing architecture module: '+name);const copy=original.clone(true);copy.position.set(x,y,z);copy.scale.set(sx,sy,sz);copy.rotation.y=yaw;state.paleMeshes+=recolorFacade(copy,theme);group.add(copy);return copy;}
  const group=new T.Group();group.name='Licensed modular Quay facades';
  for(const b of QUAY_BUILDINGS){theme=b.theme;
   // Existing solid footprints remain authoritative; these doors are closed.
   box(group,b.x,4.4,b.z,b.w-.58,8.8,b.d-.58,mat(b.theme==='brick'?'#815b44':'#d7ccb3'));
   for(const slot of facadePlan(b)){piece(group,slot.module,slot.x,slot.y,slot.z,slot.sx,1,1,slot.yaw);if(slot.door){const door=piece(group,'Door_1',slot.x,0,slot.z,.95,1,1,slot.yaw);door.translateX(.48);}}
   for(const face of ['north','east','south','west']){
    ...
    for(let i=0;i<n;i++){const along=-width/2+span*(i+.5),x=horizontal?b.x+along:b.x+(face==='east'?b.w/2:-b.w/2),z=horizontal?b.z+(face==='north'?-b.d/2:b.d/2):b.z+along,nx=-Math.sin(yaw),nz=-Math.cos(yaw);piece(group,'Cornice_Trim_Center',x-nx*.42,9,z-nz*.42,span/2,.7,1,yaw);}
   }
   // Roof fronts are +Z, opposite facade fronts. Close all four corner seams.
   for(const roof of roofPlan(b))piece(group,roof.module,roof.x,roof.y,roof.z,roof.sx,1,1,roof.yaw);
   box(group,b.x,12.67,b.z,Math.max(.5,b.w-4.28),.12,Math.max(.5,b.d-4.28),mat('#506967',.45,.6));
 ...
 jobs.push(attempt('photographed paving',async()=>{
  ...
  decks[0].material=new T.MeshStandardMaterial({map:color,normalMap:normal,normalScale:new T.Vector2(.6,.6),roughnessMap:arm,aoMap:arm,aoMapIntensity:.55,roughness:.93,metalness:0,color:'#e9debf'});
 ...
 jobs.push(attempt('cut stone materials',async()=>{
  ...
  const stone=new T.MeshStandardMaterial({map:color,normalMap:normal,normalScale:new T.Vector2(.35,.35),roughnessMap:rough,color:'#e3d1aa'});box(root,0,-1.95,0,33.7,1.15,33.7,stone);
 }));
 jobs.push(attempt('cast-iron lamp models',async()=>{
  const model=prepare(await loader.loadAsync(url('lamp-'+profile+'.glb'))),group=new T.Group();group.name='Poly Haven ornate lamps';
 ...
 jobs.push(attempt('modeled foliage',async()=>{
  const model=prepare(await loader.loadAsync(url('planter-'+profile+'.glb'))),group=new T.Group();group.name='Poly Haven terracotta foliage';
