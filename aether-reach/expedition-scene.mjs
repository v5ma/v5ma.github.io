...
export function expeditionScene(T,{scene,part,box,beam,label,material,movingPart,quality}){
 ...
 function update(s,dt,reduced){const e=s.expedition;if(!e)return;const flags=e.flags;
  ...
  for(const b of s.drones){if(scene.userData.castHas?.(b.id)||!b.humanoid||b.hp<=0||Math.hypot(b.x-s.p.x,b.z-s.p.z)>125)continue;person(b,s.time);visibleActors++;}
  if(!scene.userData.castHas?.('surveyor')&&Math.hypot(e.escort.x-s.p.x,e.escort.z-s.p.z)<125){person({...e.escort,y:e.escort.y+1.05},s.time,true);visibleActors++;}
