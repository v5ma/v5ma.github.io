...
import {focusedTask,goalDirection} from './objective-focus.mjs';
import {expeditionGoal} from './expedition-core.mjs';
export function goalGuide(s){
 const task=focusedTask(s),g=task?expeditionGoal(s,task.id):null;if(!g||!['x','y','z'].every(k=>Number.isFinite(g[k])))return null;
 const height=g.y-s.p.y,distance=Math.hypot(g.x-s.p.x,height,g.z-s.p.z);
 return {suggested:task.id!==s.expedition.tracked,taskId:task.id,taskName:task.name,direction:goalDirection(s.p,g),id:g.id||task.id,name:g.name||'Tracked destination',x:g.x,y:g.y,z:g.z,distance:Math.round(distance),height:Math.round(height),level:Math.abs(height)<2?'same level':height>0?'above you':'below you',bearing:Math.atan2(g.x-s.p.x,s.p.z-g.z)};
}
export function drawGoalGuide(g,s,X,Z,w,h){
 ...
 g.save();g.fillStyle='#102633';g.fillRect(65,7,w-78,45);g.fillStyle='#ffe595';g.font='bold 16px Arial';g.textAlign='left';g.fillText('NEXT: '+goal.name,76,27,w-98);g.font='12px Arial';g.fillStyle='#e2eee5';g.fillText(goal.direction+' / '+goal.distance+' m / '+goal.level+(Math.abs(goal.height)>=2?' ('+Math.abs(goal.height)+' m)':'')+' / dashed line = direction, not a walkable route',76,44,w-98);g.restore();return goal;
}
