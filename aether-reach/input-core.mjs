import {PAD_DEFAULTS,curveStick} from './controller-profile.mjs';
/* Deterministic device-to-action normalization. No DOM, timers or game state. */
export const BUTTONS=PAD_DEFAULTS;
...
export class InputSampler{
 ...
 read(pad,key='desktop',xr=false,options={}){
  ...
  const mapping=xr?{fire:0,interact:1,jump:4,reload:5,boost:3}:options.bindings||BUTTONS;
  ...
  const move=xr?stick(pad.axes?.[2]??pad.axes?.[0],pad.axes?.[3]??pad.axes?.[1]):stick(pad.axes?.[0],pad.axes?.[1],options.moveDeadzone??.18);
  const look=xr?[0,0]:curveStick(stick(pad.axes?.[2],pad.axes?.[3],options.lookDeadzone??.18),options.lookCurve??1);
