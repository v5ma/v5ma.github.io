import {installWindbreak} from './bellwether-windbreak-scene.mjs';
...
export function installBellwetherScene({scene,groundAt,clearLine}){
 const windbreak=installWindbreak(scene,clearLine),refit=installBellwetherRefit(scene),kit=createFoundryKit(scene),{root,add,batch,sign}=kit;root.name='bellwether-blackout';
 ...
 function update(s,{playing=true,reduced=false,dt=0}={}){const b=s.bellwether;if(!b)return;refit.update(s,dt,reduced);windbreak.update(s);restored=b.stage>=5;
 ...
 return {update,dispose(){windbreak.dispose();refit.dispose();kit.dispose();},stats:()=>({windbreak:windbreak.stats(),refit:refit.stats(),restored,lamps:lamps.length,dials:dials.length,markerVisible:marker.visible})};
}
