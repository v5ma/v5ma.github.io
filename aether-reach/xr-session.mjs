...
import {WINDOW_CONTROLS,windowControls,stepWindowLook,windowAim,currentWindowAim} from './window-controls.mjs';
...
import {spatialPage} from './spatial-menu.mjs';
export function createXR(view,api){
 ...
 // The simulation can advance several steps per rendered XR frame. A window
 // weapon belongs to the current avatar, never its previous frame's position.
 function currentAim(value){return currentWindowAim(api.state(),value,diorama.active);}
 function activate(item){if(!item)return;rotunda.press(item);if(item.kind==='summon'){api.pause();api.clear();}else if(item.kind==='exit')end();else api.spatial(item.kind,item.element);selected++;pinches.reset();lastPaint=0;}
 function frame(frame,dt){
  ...
  }else if(!menu&&!api.standardInput?.()){(diorama.cameraWindow?stepWindowLook:stepDioramaLook)(p,controls.look,dt,{guided:rotunda.config.guidedAim,fine:controls.held.aim,...api.lookSettings?.()});} // Both stick axes aim; head and pointing rays cannot overwrite them.
  ...
  for(const {source,pose}of controllerPoses){worldRay(pose);const output={origin:{x:originVector.x,y:originVector.y,z:originVector.z},direction:{x:vector.x,y:vector.y,z:vector.z}};
   if(source.handedness==='right'){aim=diorama.active&&!menu?windowAim(api.state()):output;if(menu){hover=rotunda.hit(raycaster,true);if(hover?.element)api.spatial('focus',hover.element);if(controls.edges.confirm){activate(hover);controls.edges.confirm=false;controls.edges.fire=false;}}}
  ...
  for(const source of handSources){const hand=pinches.sample(source,frame,space);if(!hand?.tracked)continue;trackedHands++;const beam=handRays.find(h=>h.side===source.handedness)?.line;if(beam){const p=hand.ray.transform.position,q=hand.ray.transform.orientation;beam.position.set(p.x,p.y,p.z);beam.quaternion.set(q.x,q.y,q.z,q.w);beam.visible=true;}
   ...
   worldRay(hand.ray);const target=rotunda.hit(raycaster,!!menu);
   if(target)hover=target;if(!pinchUsed&&hand.pressed&&target){activate(target);pinchUsed=true;}
  }
