...
export function makeView(canvas,quality='balanced'){
 ...
 function label(text,x,y,z,w=8,h=3,bg='#244f5b',fg='#ffebbb',rotation=0){const c=document.createElement('canvas');c.width=768;c.height=Math.round(768*h/w);const g=c.getContext('2d');g.fillStyle=bg;g.fillRect(0,0,c.width,c.height);g.strokeStyle=fg;g.lineWidth=4;g.strokeRect(10,10,c.width-20,c.height-20);g.textAlign='center';g.textBaseline='middle';const lines=text.split('\n'),fs=Math.min(85,c.height/(lines.length+1));g.font='600 '+fs+'px Georgia';lines.forEach((line,i)=>g.fillText(line,384,c.height/2+(i-(lines.length-1)/2)*fs*1.16,695));const tex=new T.CanvasTexture(c);tex.colorSpace=T.SRGBColorSpace;tex.anisotropy=4;const mesh=new T.Mesh(new T.PlaneGeometry(w,h),new T.MeshStandardMaterial({map:tex,roughness:.8,side:T.DoubleSide}));mesh.position.set(x,y,z);mesh.rotation.y=rotation;scene.add(mesh);return mesh;}
 ...
 for(let n=0;n<DISTRICTS.length;n++){
  const d=DISTRICTS[n];const floor=new T.Mesh(new T.BoxGeometry(d.w,.9,d.d),new T.MeshStandardMaterial({map:paving,color:'#fff8e3',roughness:.94}));floor.position.set(d.x,d.y-.45,d.z);floor.receiveShadow=true;floor.castShadow=true;scene.add(floor);decks.push(floor);
 ...
 for(const d of BUILDINGS){
  box(d.x,d.y+d.h/2,d.z,d.w,d.h,d.d,palette.stone);box(d.x,d.y+.6,d.z,d.w+.4,1.2,d.d+.4,palette.edge);
 ...
 for(const d of ['g1','f1','s1']){const group=new T.Group();scene.add(group);const body=movingPart('sphere',material(palette.metal,'metal'),[0,0,0],[.85,.66,.7],group);movingPart('torus',railMat,[0,0,0],[1.05,.85,1.05],group);const eye=movingPart('sphere',new T.MeshBasicMaterial({color:'#ff9d70'}),[0,.06,.68],[.22,.22,.12],group);const wings=[];for(const sign of[-1,1]){movingPart('box',railMat,[sign*1,0,0],[.9,.12,.25],group);const rotor=movingPart('torus',material(palette.metal,'metal'),[sign*1.35,0,0],[.6,.6,.6],group);rotor.rotation.x=Math.PI/2;wings.push(rotor);}bots.set(d,{group,eye,wings});}
