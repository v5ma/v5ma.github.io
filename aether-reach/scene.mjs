...
import {ROOMS,STAIR,CELLAR} from './city-world.mjs';
import {buildCityScene} from './city-scene.mjs';
export {T};
...
export function makeView(canvas,quality='balanced'){
 ...
 const sky=new T.Mesh(new T.SphereGeometry(850,32,20),new T.ShaderMaterial({side:T.BackSide,depthWrite:false,uniforms:{top:{value:new T.Color('#598fae')},bottom:{value:new T.Color('#f4e6c6')}},vertexShader:'varying vec3 p;void main(){p=position;gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.);}',fragmentShader:'uniform vec3 top;uniform vec3 bottom;varying vec3 p;void main(){float t=clamp(normalize(p).y*.72+.24,0.,1.);gl_FragColor=vec4(mix(bottom,top,t),1.);\n#include <tonemapping_fragment>\n#include <colorspace_fragment>\n}'}));scene.add(sky);
 const geometries={box:new T.BoxGeometry(1,1,1),sphere:new T.SphereGeometry(1,quality==='low'?14:24,quality==='low'?10:16),rock:new T.DodecahedronGeometry(1,0),cylinder:new T.CylinderGeometry(1,1,1,quality==='low'?16:28),cone:new T.ConeGeometry(1,1,quality==='low'?8:16),torus:new T.TorusGeometry(1,.1,quality==='low'?6:10,quality==='low'?24:48)};
 ...
 const texCanvas=document.createElement('canvas');texCanvas.width=texCanvas.height=256;const tc=texCanvas.getContext('2d');tc.fillStyle='#e5d8b9';tc.fillRect(0,0,256,256);for(let i=0;i<600;i++){tc.fillStyle=i%2?'#ffffff12':'#66523d0b';tc.fillRect(rand(i)*256,rand(i+8)*256,1+rand(i+3)*4,1);}tc.strokeStyle='#ad9e8255';tc.lineWidth=2;for(let i=0;i<=256;i+=64){tc.beginPath();tc.moveTo(i,0);tc.lineTo(i,256);tc.moveTo(0,i);tc.lineTo(256,i);tc.stroke();}const paving=new T.CanvasTexture(texCanvas);paving.wrapS=paving.wrapT=T.RepeatWrapping;paving.repeat.set(4,4);paving.colorSpace=T.SRGBColorSpace;paving.anisotropy=Math.min(8,renderer.capabilities.getMaxAnisotropy());
 function rectangles(d,cut){
  const x1=d.x-d.w/2,x2=d.x+d.w/2,z1=d.z-d.d/2,z2=d.z+d.d/2;
  return [[x1,cut.x1,z1,z2],[cut.x2,x2,z1,z2],[cut.x1,cut.x2,z1,cut.z1],[cut.x1,cut.x2,cut.z2,z2]].filter(([a,b,c,e])=>b>a&&e>c);
 }
 const decks=[];
 for(let n=0;n<DISTRICTS.length;n++){
  const d=DISTRICTS[n];const floor=new T.Mesh(new T.BoxGeometry(d.w,.9,d.d),new T.MeshStandardMaterial({map:paving,color:({harbor:'#fff0ce',park:'#dae9d7',garden:'#c8e0c9',foundry:'#b5a4a0',spire:'#becfdf',commons:'#f1e4c5',yard:'#bccfca'})[d.theme],roughness:.94}));floor.position.set(d.x,d.y-.45,d.z);floor.receiveShadow=true;floor.castShadow=true;if(d.id==='harbor'){
   for(const [a,b,c,e] of rectangles(d,STAIR)){const m=new T.Mesh(new T.BoxGeometry(b-a,.9,e-c),floor.material);m.position.set((a+b)/2,d.y-.45,(c+e)/2);m.receiveShadow=true;scene.add(m);}
   floor.geometry.dispose();
  }else{scene.add(floor);decks.push(floor);}
  if(d.id==='harbor'){for(const [a,b,c,e] of rectangles(d,CELLAR)){box((a+b)/2,d.y-1.3,(c+e)/2,b-a,.65,e-c,palette.gold);box((a+b)/2,d.y-2.2,(c+e)/2,b-a,1.2,e-c,palette.stone);}}
  else{box(d.x,d.y-1.3,d.z,d.w+1,.65,d.d+1,palette.gold);box(d.x,d.y-2.2,d.z,d.w-1,1.2,d.d-1,palette.stone);}
  part('rock',d.x,d.y-9-(d.id==='harbor'?8:0),d.z,d.w*.58,11,d.d*.52,'#789297',0,n*.7);part('rock',d.x+3,d.y-12-(d.id==='harbor'?8:0),d.z-3,d.w*.36,13,d.d*.34,'#a0ac9c',.12,n*.4);
  for(let i=0;i<10;i++){const a=i/10*Math.PI*2,x=d.x+Math.cos(a)*d.w*.4,z=d.z+Math.sin(a)*d.d*.41;part('rock',x,d.y-5-rand(i+n)*5-(d.id==='harbor'?8:0),z,3+rand(i)*4,7,4,i%2?'#9daba0':'#b4bda7',.2,n+i);}
 ...
 for(const d of BUILDINGS){
  const room=ROOMS.find(r=>r.building===d.id),base=room?room.height+.25:0;
  box(d.x,d.y+base+(d.h-base)/2,d.z,d.w,d.h-base,d.d,d.id==='enginehall'?'#895f4f':d.id==='works-house'?'#b37559':d.id==='spire-tower'?'#939ebb':d.id==='observatory'?'#6e89a2':d.id==='greenhouse'?'#bedacf':palette.stone);if(!room)box(d.x,d.y+.6,d.z,d.w+.4,1.2,d.d+.4,palette.edge);
  ...
  for(let floor=room?1:0;floor<Math.floor(d.h/3);floor++)for(let col=-1;col<=1;col++){
 ...
 const cityArt=buildCityScene(T,scene,camera,quality);
