...
import {installCast} from './cast-scene.mjs';
import {installCloudShade} from './skyglass-shaders.mjs';
import {expeditionScene} from './expedition-scene.mjs';
import {installLuminousArt} from './luminous-art.mjs';
...
export function makeView(canvas,quality='balanced'){
 ...
 const hemi=new T.HemisphereLight('#e2f6ff','#819591',1.6);scene.add(hemi);const sun=new T.DirectionalLight('#fff0cb',3.2);sun.position.set(-60,120,50);sun.target.position.set(15,6,-60);sun.castShadow=true;sun.shadow.mapSize.set(1024,1024);Object.assign(sun.shadow.camera,{left:-42,right:42,top:42,bottom:-42,near:.5,far:240});sun.shadow.bias=-.00012;sun.shadow.normalBias=.045;scene.add(sun,sun.target);
 const sky=new T.Mesh(new T.SphereGeometry(850,32,20),new T.ShaderMaterial({side:T.BackSide,depthWrite:false,uniforms:{top:{value:new T.Color('#598fae')},bottom:{value:new T.Color('#f4e6c6')}},vertexShader:'varying vec3 p;void main(){p=position;gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.);}',fragmentShader:'uniform vec3 top;uniform vec3 bottom;varying vec3 p;void main(){float t=clamp(normalize(p).y*.72+.24,0.,1.);gl_FragColor=vec4(mix(bottom,top,t),1.);}'}));sky.name="Aether sky";scene.add(sky);
 ...
 const fallbacks={buildings:new T.Group(),lamps:new T.Group(),foliage:new T.Group(),cwFoliage:new T.Group()};for(const g of Object.values(fallbacks))scene.add(g);let artGroup=null;
 const buckets=new Map(),materials=new Map();const material=(color,kind='paint')=>{const k=color+kind;if(!materials.has(k))materials.set(k,kind==='metal'?new T.MeshPhysicalMaterial({color,roughness:.23,metalness:.9,clearcoat:.6,clearcoatRoughness:.18,envMapIntensity:1.35}):new T.MeshStandardMaterial({color,roughness:.83,metalness:0,emissive:kind==='glow'?color:'#000000',emissiveIntensity:kind==='glow'?1:0}));return materials.get(k);};
 ...
 for(let n=0;n<DISTRICTS.length;n++){
  const d=DISTRICTS[n];const floor=new T.Mesh(new T.BoxGeometry(d.w,.9,d.d),new T.MeshStandardMaterial({map:paving,color:({harbor:'#fff0ce',park:'#dae9d7',garden:'#c8e0c9',foundry:'#b5a4a0',spire:'#becfdf',market:'#e9cab1',academy:'#e7dabb',archive:'#d6d8bc',observatory:'#c8dce2',port:'#d4d7bf'})[d.theme],roughness:.94}));floor.position.set(d.x,d.y-.45,d.z);floor.receiveShadow=true;floor.castShadow=true;scene.add(floor);decks.push(floor);
  ...
  for(let i=0;i<6;i++){const x=d.x+(i%2?1:-1)*(d.w*.4),z=d.z+(i/6-.5)*d.d*.65;box(x,d.y+.22,z,2.8,.5,2.8,palette.metal);box(x,d.y+.5,z,2.6,.15,2.6,'#718b49');const foliageGroup=artGroup;if(['garden','bellmarket'].includes(d.id))artGroup='cwFoliage';part('cylinder',x,d.y+1.65,z,.16,2.4,.16,'#7c6850');for(let j=0;j<3;j++)part('sphere',x+(j-1)*.6,d.y+3.2+(.4*(j%2)),z,1.25,1.55,1.2,['#738f53','#8b9b58','#6b9c82'][n%3]);artGroup=foliageGroup;}
  artGroup=null;
 ...
 for(let i=0;i<60;i++){const m=new T.Sprite(new T.SpriteMaterial({map:cloudTex,color:i%3?'#f8ffff':'#dfedf1',transparent:true,depthWrite:false,opacity:.83}));const x=(rand(i+120)-.5)*850,z=(rand(i+320)-.5)*850-70,y=-24-rand(i+79)*60;m.position.set(x,y,z);const w=40+rand(i)*110;m.scale.set(w,w*.5,1);m.userData.portalBackdrop=true;scene.add(m);clouds.push(m);}
 ...
 const expeditionArt=expeditionScene(T,{scene,part,box,beam,label,material,movingPart,quality});
 ...
 const currentworks=createCurrentworks({scene,patches:tacticalArt.patches,foliageFallback:fallbacks.cwFoliage});
 ...
 const feedback=createShotFeedback(scene);
 ...
 function effect(e){currentworks.effect(e);feedback.shot(e);combatArt.effect(e);tacticalArt.effect(e);if(e.type==='shot'){const geo=new T.BufferGeometry().setFromPoints([new T.Vector3(e.o.x,e.o.y,e.o.z),new T.Vector3(e.end.x,e.end.y,e.end.z)]),line=new T.Line(geo,new T.LineBasicMaterial({color:e.hit?'#fff3af':'#83e5d5',transparent:true,opacity:1}));scene.add(line);sparks.push({mesh:line,t:.10});lastShot=performance.now();}if(e.type==='pulse'){const mesh=new T.Mesh(new T.SphereGeometry(1,18,10),new T.MeshBasicMaterial({color:'#73e5d9',transparent:true,opacity:.25,wireframe:true}));camera.getWorldPosition(mesh.position);scene.add(mesh);sparks.push({mesh,t:.6,pulse:true});}}
 function update(state,dt,menu=false,reduced=false){feedback.update(dt);const xr=renderer.xr.isPresenting,mode=luminous.stats().requested;currentworks.update(state,{mode,xr,reduced,eye:xr?state.p:camera.position});cast.update(state,dt,{menu,reduced,mode,xr});shade.update(state.time,{reduced,quality:mode,immersive:xr});Object.assign(scene.userData.skyglass,{enabled:skyglassEnabled,mode:xr?'low':mode,clock:reduced?0:state.time,motion:reduced?0:1});expeditionArt.update(state,dt,reduced);luminous.update(state,dt,reduced,renderer.xr.isPresenting);const c=renderer.xr.isPresenting?state.p:camera.position;const sx=Math.round(c.x/8)*8,sz=Math.round(c.z/8)*8;sun.position.set(sx-60,c.y+110,sz+50);sun.target.position.set(sx,c.y,sz);tacticalArt.update(state,dt,menu,renderer.xr.isPresenting);const t=state.time;
 ...
 return {renderer,scene,camera,resize,update,effect,prepareEnvironment:()=>currentworks.prepare(renderer,camera),setEnvironmentEnabled:v=>currentworks.setEnabled(v),setVisualMode:mode=>luminous.setMode(mode),setCastEnabled:v=>cast.setEnabled(v),setSkyglassEnabled:v=>{skyglassEnabled=!!v;shade.setEnabled(v);scene.userData.skyglass.enabled=!!v;},render:()=>renderer.render(scene,camera),stats:()=>({currentworks:currentworks.stats(),shotFeedback:feedback.stats(),drawCalls:renderer.info.render.calls,triangles:renderer.info.render.triangles,geometries:renderer.info.memory.geometries,visual:luminous.stats(),cast:cast.stats(),skyglass:shade.stats(),art:quayArt.stats(),expedition:expeditionArt.stats(),foundry:scene.userData.foundryStats?.()||null})};
}
