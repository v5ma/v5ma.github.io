import {installAuroraCast} from './aurora-cast.mjs';
...
export function makeView(canvas,quality='balanced'){
 ...
 const cast=installAuroraCast({scene});
 ...
 function effect(e){cast.effect(e);combatArt.effect(e);tacticalArt.effect(e);if(e.type==='shot'){const geo=new T.BufferGeometry().setFromPoints([new T.Vector3(e.o.x,e.o.y,e.o.z),new T.Vector3(e.end.x,e.end.y,e.end.z)]),line=new T.Line(geo,new T.LineBasicMaterial({color:e.hit?'#fff3af':'#83e5d5',transparent:true,opacity:1}));scene.add(line);sparks.push({mesh:line,t:.10});lastShot=performance.now();}if(e.type==='pulse'){const mesh=new T.Mesh(new T.SphereGeometry(1,18,10),new T.MeshBasicMaterial({color:'#73e5d9',transparent:true,opacity:.25,wireframe:true}));camera.getWorldPosition(mesh.position);scene.add(mesh);sparks.push({mesh,t:.6,pulse:true});}}
 function update(state,dt,menu=false,reduced=false){cast.update(state,dt,{menu,reduced,mode:luminous.stats().requested,xr:renderer.xr.isPresenting});expeditionArt.update(state,dt,reduced);luminous.update(state,dt,reduced,renderer.xr.isPresenting);const c=renderer.xr.isPresenting?state.p:camera.position;const sx=Math.round(c.x/8)*8,sz=Math.round(c.z/8)*8;sun.position.set(sx-60,c.y+110,sz+50);sun.target.position.set(sx,c.y,sz);tacticalArt.update(state,dt,menu,renderer.xr.isPresenting);const t=state.time;
 ...
 return {renderer,scene,camera,resize,update,effect,setAuroraOptions:cast.setOptions,setVisualMode:mode=>luminous.setMode(mode),render:()=>renderer.render(scene,camera),stats:()=>({drawCalls:renderer.info.render.calls,triangles:renderer.info.render.triangles,geometries:renderer.info.memory.geometries,visual:luminous.stats(),cast:cast.stats(),art:quayArt.stats(),expedition:expeditionArt.stats(),foundry:scene.userData.foundryStats?.()||null})};
}
