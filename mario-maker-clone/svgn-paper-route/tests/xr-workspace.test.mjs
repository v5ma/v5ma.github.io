...
test('Physical Xbox activation shares text and confirmation routing with tracked rays',()=>{const s=readFileSync(new URL('../flight-deck.js',import.meta.url),'utf8');assert(s.includes('SkyCycleXR?.activate?.(el)'));const x=readFileSync(new URL('../xr-play.js',import.meta.url),'utf8');assert(x.includes('back,activate,show'));assert(x.includes("kind:'alert'"));assert(x.includes("'New line'"));});
