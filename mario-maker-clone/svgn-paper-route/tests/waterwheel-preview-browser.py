...
with sync_playwright() as pw:
 ...
 ctx.add_init_script("localStorage.setItem('sprocket_muted','1');if(!localStorage.getItem('ww-preserved')){localStorage.setItem('ww-preserved','original');localStorage.setItem('svgn_delivery_records_v1',JSON.stringify({'canal-choices':{medal:'silver',time:99,score:400},'first-neighborhood':{medal:'bronze',time:150,score:100}}));localStorage.setItem('svgn.skycycle.mastery.v1',JSON.stringify({'canal-choices':{badges:['finish','clean'],finishes:3,best:80}}));localStorage.setItem('svgn.skycycle.sunrise.v1',JSON.stringify({marketPilot:true,finishes:3}));}")
 ...
 try:
  ...
  ledger=page.evaluate('JSON.stringify(SkyCycleFlightDeck.records)')
  ...
  page.wait_for_function('document.getElementById("ww-preview-result-note")?.textContent.includes("playtest results only")');check(True,'The real finish screen clearly distinguishes preview results from campaign awards')
  ...
  check(page.evaluate('JSON.stringify(SkyCycleFlightDeck.records)')==ledger and page.evaluate('!Object.hasOwn(SkyCycleFlightDeck.records,"canal-choices-r2")'),'Actual career ledger preserves all prior records and contains no preview revision award')
