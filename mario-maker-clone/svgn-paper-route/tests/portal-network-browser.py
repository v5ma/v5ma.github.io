...
with sync_playwright() as pw:
 ...
 try:
  ...
  capture('xr-hand-pause');choose('Sound & music');page.wait_for_function('document.getElementById("score-dialog").open');frames()
  choose('Effect intensity: soft plus');check(page.evaluate('SkyCycleSensory.settings.transients==="full"'),'Hand UI changes effect intensity through the actual select control')
  choose('Optional notices: balanced plus');check(page.evaluate('SkyCycleSensory.settings.notices==="quiet"'),'Hand UI adjusts optional notification frequency')
  choose('Water ambience: 35 plus');check(page.evaluate('Math.abs(SkyCycleSensory.settings.ambience-.36)<1e-9'),'Hand UI adjusts the water ambience slider without a mouse')
  capture('xr-hand-comfort');choose('Back');check(page.evaluate('__delivery.paused && !document.getElementById("score-dialog").open'),'Hand back closes only sound settings and leaves the paused parent')
  choose('Back to the route');page.wait_for_function('!__delivery.paused');frames()
