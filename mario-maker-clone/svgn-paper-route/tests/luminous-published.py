...
OUT=Path('/tmp/sky-cycle-luminous');OUT.mkdir(parents=True,exist_ok=True)
FILES=['luminous-core.mjs','luminous-materials.js','prismatic-renderer.js','prismatic.css','sunrise.js','release-status.js','release.json','sw.js']
...
try:
 for attempt in range(50):
  ...
  for f in FILES:
   try:
    req=urllib.request.Request('https://v5ma.github.io/mario-maker-clone/svgn-paper-route/'+f+'?luminous='+str(time.time_ns()),headers={'Cache-Control':'no-cache'})
