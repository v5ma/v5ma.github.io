...
with sync_playwright() as pw:
 ...
 def capture(name):
  data=page.evaluate('xrEmulator.image()');(OUT/(name+'.png')).write_bytes(base64.b64decode(data.split(',')[1]));samples.append(page.evaluate('({x:player.x,y:player.y,deliveries,tries,depth:SkyCycleDepth.diagnostics,environment:SkyCycleEnvironment.stats,point:SkyCycleDepth.sample(player.x+13),pixels:{colors:xrEmulator.lastCapture.colors,opaque:xrEmulator.lastCapture.opaque},xr:SkyCycleXR.diagnostics})'))
 def unobstructed(name):
  select('Resume play');page.wait_for_function('!__delivery.paused&&!SkyCycleXR.diagnostics.uiVisible');frames();capture(name);press(5);page.wait_for_function('__delivery.paused')
 def ride(target):
 ...
 try:
  page.goto(BASE+'tests/currentworks-render.html',wait_until='domcontentloaded');page.wait_for_function('window.fixture?.passed||window.fixture?.error');fixture=page.evaluate('fixture');check(fixture['passed'],'Currentworks real GPU fixture draws library geometry with the pinned node renderer: '+str(fixture.get('error')))
  ...
  page.goto(BASE+'?xr=1',wait_until='domcontentloaded');page.bring_to_front();page.wait_for_function('window.SkyCycleDepth&&window.SkyCycleEnvironment&&window.SkyCycleXR&&window.SkyCycleFlightDeck&&window.__gpuReady')
  ...
  page.locator('#ww-preview-open').click();page.locator('#ww-preview-curve').click();page.wait_for_function('RouteWorkshop.testing&&player.onGround&&SkyCycleDepth.diagnostics.active&&SkyCycleEnvironment.stats.active')
  ...
  check(page.evaluate('SkyCycleEnvironment.stats.clouds.clouds===6&&SkyCycleEnvironment.stats.islands.islands===4&&SkyCycleEnvironment.stats.materials===6'),'The original preview now loads actual Cloudlets, Islands and Toon resources')
  ...
  ride(1170);check(page.evaluate('SkyCycleDepth.sample(player.x+13).z')<-85,'The ordinary road reaches the receding side of the curved market');unobstructed(KIND+'-away')
  ...
  page.evaluate('''()=>{const s=xrEmulator.session,base=s.frame.bind(s);s.frame=function(){const f=base(),pose=f.getViewerPose;f.getViewerPose=(...args)=>{const p=pose(...args);for(const v of [p,...p.views]){v.transform.matrix[12]+=.18;v.transform.matrix[13]+=.07;}return p;};return f;};}''');frames();unobstructed(KIND+'-lean')
  ...
  select('Resume play');ride(2250);check(page.evaluate('SkyCycleDepth.sample(player.x+13).z')>80,'The same continuous road returns toward the viewer');unobstructed(KIND+'-toward')
  frozen=page.evaluate('SkyCycleEnvironment.stats.time');frames(8);check(page.evaluate('SkyCycleEnvironment.stats.time')==frozen,'Pausing freezes library motion on the original simulation clock')
  select('Materials & FX');select('Animate material shimmer');select('Resume play');frames(8)
  check(page.evaluate('SkyCycleEnvironment.stats.clouds.quiet'),'The existing reduced-decoration control freezes library motion without a new menu')
  press(5);select('Materials & FX');select('Animate material shimmer');select('Resume play');frames();press(5)
  ...
  check(page.evaluate('!SkyCycleEnvironment.stats.active'),'2D fallback releases the garden resources rather than leaving hidden scenery')
  ...
  check(page.evaluate('SkyCycleEnvironment.stats.active&&SkyCycleEnvironment.stats.builds>=2'),'Returning to 3D rebuilds one clean library layer')
  ...
  check(page.evaluate('!SkyCycleEnvironment.stats.active'),'Returning to the editor frees the library layer and keeps the document')
