...
test('Controller focus reveals the page and exposes a native exit control',()=>{assert(xr.includes('function focusControl(el)'));assert(xr.includes('page=Math.floor(i/6)'));assert(xr.includes('id="xr-stage-exit"'));assert(xr.includes("document.addEventListener('focusin'"));});
