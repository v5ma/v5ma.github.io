...
(function(){'use strict';
 ...
 const buffers=new Map(),effectsVoices=new Set();let lastEffect=-Infinity,lastEffectKey='',rejectedEffects=0;
 function clearEffects(){for(const v of effectsVoices){try{v.o.stop();v.o.disconnect();v.g.disconnect();}catch{}}effectsVoices.clear();}
 ...
 function setGains(){if(graph){const t=graph.a.currentTime;graph.music.gain.setTargetAtTime(prefs.music,t,.06);graph.effects.gain.setTargetAtTime(prefs.effects,t,.025);graph.master.gain.setTargetAtTime(muted?0:1,t,.025);}if(muted||prefs.effects===0)clearEffects();window.dispatchEvent(new Event('skycycle:mix'));}
 ...
 window.beep=function(f,dur,type='sine',vol=.1,slide=0){
  if(muted||destroyed||document.hidden||prefs.effects===0||![f,dur,vol,slide].every(Number.isFinite)||dur<=0||vol<=0)return;
  let voice=null;try{
   const {a,effects}=graphFor();if(a.state!=='running')return;
   const t=a.currentTime,key=[f,type,slide].join(':');
   if(effectsVoices.size>=8||(key===lastEffectKey&&t-lastEffect<.045)){rejectedEffects++;return;}
   lastEffect=t;lastEffectKey=key;dur=Math.max(.03,Math.min(1.5,dur));
   const o=a.createOscillator(),g=a.createGain();voice={o,g};effectsVoices.add(voice);
   o.type=type==='triangle'?'triangle':'sine';o.frequency.setValueAtTime(Math.max(45,Math.min(f,1800)),t);
   if(slide)o.frequency.exponentialRampToValueAtTime(Math.max(45,Math.min(f+slide,2100)),t+dur);
   g.gain.setValueAtTime(0,t);g.gain.linearRampToValueAtTime(Math.min(.15,vol)*(window.SkyCycleSensory?.intensity??.65),t+.012);
   g.gain.exponentialRampToValueAtTime(.0001,t+dur);o.connect(g);g.connect(effects);
   o.onended=()=>{o.disconnect();g.disconnect();effectsVoices.delete(voice);};o.start(t);o.stop(t+dur+.025);
  }catch{if(voice){try{voice.o.stop();voice.o.disconnect();voice.g.disconnect();}catch{}effectsVoices.delete(voice);}}
 };
 ...
 document.addEventListener('visibilitychange',()=>{if(document.hidden){clearEffects();if(graph)graph.a.suspend().catch(()=>{});}});
 function boot(){
  ...
  const d=document.createElement('dialog');d.id='score-dialog';d.setAttribute('aria-labelledby','score-title');d.innerHTML='<h2 id="score-title">Sound & music</h2><p>Original instrumental arrangements. No external music service.</p><label for="score-music">Music <input aria-label="Music" id="score-music" type="range" min="0" max="100" step="1"></label><label for="score-effects">Effects <input aria-label="Effects" id="score-effects" type="range" min="0" max="100" step="1"></label><label><input aria-label="Mute all sound" id="score-mute" type="checkbox"> Mute all sound</label><p id="score-status" role="status"></p><form method="dialog"><button>Done</button></form>';document.body.append(d);let resume=false;
  ...
  window.__score={prefs,stats,get desired(){return desired},get source(){return source},get context(){return graph?.a},get error(){return lastError},get effectsBus(){return graph?.effects},clearEffects,get effectVoices(){return effectsVoices.size},get rejectedEffects(){return rejectedEffects},version:'instrumental-2-quiet-water'};
 }
 if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot,{once:true});else boot();
 window.addEventListener('pagehide',()=>{clearEffects();stopMusic();worker?.terminate();worker=null;destroyed=true;});
