...
(() => {
  function boot(){
    ...
    window.tick=function(){
      ...
      if(state.paused||state.menu||(document.hidden&&!window.SkyCycleXR?.inputVisible)){accumulator=0;fit();render();return;}
    ...
    function draw2D(now){
      ...
      if(mode==='play'&&window.__ground?.meta?.waterwheel?.preview)window.Waterwheel2D?.draw(g2,cam.x,cam.y,W/z,H/z);
    ...
    window.addEventListener('blur',clearKeys);document.addEventListener('visibilitychange',()=>{if(window.SkyCycleXR?.inputVisible)return;clearKeys();if(document.hidden&&mode==='play'&&!won&&!state.menu&&!state.paused)act('pause');});
