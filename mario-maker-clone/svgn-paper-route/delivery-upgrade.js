  ...
  function boot(){
    ...
    window.win=function(){
      ...
      if(r&&levelCode()===state.code){
        const id=r.recordId||r.id,old=state.records[id],rank={bronze:1,silver:2,gold:3};
        state.records[id]={medal:old&&rank[old.medal]>rank[medal]?old.medal:medal,time:Math.min(Number(old?.time)||Infinity,seconds),score:Math.max(Number(old?.score)||0,score)};saveRecords();
      }
