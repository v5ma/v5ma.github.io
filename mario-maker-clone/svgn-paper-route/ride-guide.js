...
(function(){
 ...
 function toggle(){enabled=!enabled;retryAfter=0;button.setAttribute('aria-pressed',String(enabled));if(!enabled)dispose();canvas.hidden=note.hidden=!active();cv.focus({preventScroll:true});}
 ...
 function snapshot(){
  ...
  doc.paths=tracks.map(t=>({points:t.pts.map(p=>[...p]),meta:t.sky?{...t.sky}:null,anchors:null}));
