...
export function mappedPad(sources,{panel=false,suppressed=new Set(),held=new Set(),enabled=true,allowRecovery=false}={}){
 ...
 if(allowRecovery)for(const source of sources)if(!source.hand&&source.handedness==='right'&&source.gamepad?.mapping==='xr-standard'&&pressed(source.gamepad,5))set(9);
 ...
 for(const source of sources){const p=source.gamepad;if(source.hand||p?.mapping!=='xr-standard')continue;
  ...
  if(left){if(!panel||Math.hypot(p.axes[2]||0,p.axes[3]||0)>=Math.hypot(out.axes[0],out.axes[1])){out.axes[0]=Number.isFinite(p.axes[2])?p.axes[2]:0;out.axes[1]=panel&&Number.isFinite(p.axes[3])?p.axes[3]:0;}
   ...
   if(pressed(p,4))set(0);if(pressed(p,5))set(9);
...
export function sourcesNeutral(sources,{panel=false}={}){return sources.every(s=>!s.gamepad||((s.gamepad.buttons||[]).every((b,i)=>panel&&s.gamepad.mapping==='xr-standard'&&i===1||!b.pressed&&!(b.value>.2))&&(s.gamepad.axes||[]).every(v=>Math.abs(v||0)<.25)));}
