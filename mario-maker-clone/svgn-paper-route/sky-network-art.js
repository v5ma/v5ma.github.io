...
globalThis.SkyNetworkArt=(()=>{
 ...
 function populate({course,m,root,kit,metal,terrain,greenery,far,sign,paths}){
  ...
  for(const {pts,sky:tag}of paths){
   const f=sectionFrames(pts),v=(a,n,z)=>[a.x+a.nx*n*a.miter,a.y+a.ny*n*a.miter,z],gold=(tag.authoredFlow&&tag.id.startsWith('b')||tag.id?.startsWith('ww-')&&tag.tier===1)?'#85d5c7':tag.tier===3?'#eac576':'#d9ba63';
  ...
  for(let i=0;i<course.gp.skyNetwork.sectors.length;i++){
   ...
   if(!course.gp.flowRoutes&&!course.gp.waterwheel){far.box(x,-ground+220,z,12,440,16,'#637b7c');far.rod([x,-ground+360,z],[x+240,y-70,z],5,'#7a938d');}
   if(i%2===0&&!course.gp.waterwheel){kit.rock(terrain,x+170,y-130,-240,88,130,i+2);kit.grass(greenery,x+170,y-128,-240,88,.55);kit.tree(greenery,x+170,y-122,-240,.55,i);}
