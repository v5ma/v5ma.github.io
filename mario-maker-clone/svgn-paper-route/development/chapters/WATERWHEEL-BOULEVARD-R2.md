...
The lower canal collector and mill-court return are initial recovery geometry, not proof of every missed transfer. Their IDs are `ww-collector` and `ww-court-return`. Main line qualification does not qualify these alternates. The preview has no new grapple peg or movement upgrade; the full approved optional whip connection remains a later authoring gate, with an alternate non-whip continuation required.
...
- [ ] Test checkpoints/retries, muted readability, slower/coasting approaches and reverse re-entry cases.
- [ ] Implement revision-aware campaign settlement/display and rollback tests before replacing the old default geometry.
