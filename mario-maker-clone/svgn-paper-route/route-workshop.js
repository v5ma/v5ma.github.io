...
(function(){'use strict';
 function boot(){
  ...
  function action(a){try{
   if(a==='undo'||a==='redo'){const ids=new Set(selected().map(p=>p.meta?.id).filter(Boolean));const indexes=new Set(S.selected);S.doc=W.decode(S.history[a]());S.selected=new Set(S.doc.paths.map((p,i)=>p.meta?.id?ids.has(p.meta.id)?i:-1:indexes.has(i)?i:-1).filter(i=>i>=0));S.object=null;S.dirty=current()!==S.saved;refresh();return;}
  ...
  function draw(){if(!S.active||!S.doc||!S.dirtyFrame)return;S.dirtyFrame=false;ctx.setTransform(dpr,0,0,dpr,0,0);ctx.fillStyle='#112a38';ctx.fillRect(0,0,width,height);ctx.save();ctx.scale(S.view.zoom,S.view.zoom);ctx.translate(-S.view.x,-S.view.y);const z=S.view.zoom;
   ...
   for(let ty=Math.max(0,Math.floor(S.view.y/36));ty<Math.min(S.doc.h,Math.ceil((S.view.y+height/z)/36));ty++)for(let tx=Math.max(0,Math.floor(S.view.x/36));tx<Math.min(S.doc.w,Math.ceil((S.view.x+width/z)/36));tx++){const id=S.doc.cells[ty*S.doc.w+tx];if(!id)continue;const x=tx*36,y=ty*36;ctx.fillStyle=[1,2].includes(id)?'#4d6d7c':id===15?'#68d2ab':id===8?'#ecd184':id===60?'#79e4ed':id===4?'#e58b82':'#be9d6a';ctx.fillRect(x+1,y+1,34,34);if(z>.7){try{drawTileRaw(ctx,id,x,y,0,false,null);}catch{}}if(z>.35){ctx.fillStyle='#112936';ctx.textAlign='center';ctx.font='bold 10px system-ui';ctx.fillText(id===15?'START':id===8?'GOAL':id===60?'PEG':id===63?'MAIL':id===7?'--':id===16?'BOT':String(id),x+18,y+22);}}
