...
(function(){'use strict';
 function boot(){
  ...
  const $=id=>document.getElementById(id),canvas=$('maker-canvas'),ctx=canvas.getContext('2d'),mini=$('maker-minimap'),mg=mini.getContext('2d');let width=800,height=600,dpr=1,pointer=null,curves=null;
  ...
  function action(a){try{
   ...
   if(a==='reverse'){selected().forEach(p=>{if(!BezierCore.reverse(p))p.points.reverse();p.anchors=null;if(p.meta){p.meta.kind='open';p.meta.begin=0;p.meta.end=1;}});changed('Riding side reversed.');}
  ...
  canvas.addEventListener('pointerdown',e=>{if(!S.active||S.drag)return;e.preventDefault();canvas.focus();canvas.setPointerCapture(e.pointerId);const p=world(e);pointer=p;const before=current();S.dirtyFrame=true;
   ...
   if(S.tool==='piece'){try{const q=W.tagNew(S.doc,W.piece(S.piece,snap(p[0]),snap(p[1])));if(S.doc.paths.length>=W.LIMITS.tracks)throw Error('The level already contains 192 tracks.');S.doc.paths.push(q);S.object=null;S.selected=new Set([S.doc.paths.length-1]);if(!e.shiftKey)S.tool='select';changed('Roadway selected. Drag it, rotate it, or use shape handles. Shift-click placement repeats the piece.');}catch(err){message(err.message);}return;}
   if(S.tool==='neighbor'){
   ...
   if(S.handles&&S.selected.size===1&&!S.doc.paths[[...S.selected][0]].bezier){const i=[...S.selected][0],q=S.doc.paths[i];for(let j=0;j<5;j++){const k=Math.round(j*(q.points.length-1)/4),a=q.points[k];if(Math.hypot(a[0]-p[0],a[1]-p[1])<12/S.view.zoom){S.drag={type:'handle',before,i,k,p,base:W.clone(q)};return;}}}
  ...
  canvas.addEventListener('pointermove',e=>{if(!S.active)return;const p=world(e);pointer=p;S.dirtyFrame=true;if(curves?.pointerMove(e,p))return;const d=S.drag;if(!d)return;
  ...
  function end(cancel=false){if(curves?.pointerEnd(cancel))return;const d=S.drag;if(!d)return;S.drag=null;S.dirtyFrame=true;if(cancel&&d.before){S.doc=W.decode(d.before);refresh();return;}
  ...
  function draw(){if(!S.active||!S.doc||!S.dirtyFrame)return;S.dirtyFrame=false;ctx.setTransform(dpr,0,0,dpr,0,0);ctx.fillStyle='#112a38';ctx.fillRect(0,0,width,height);ctx.save();ctx.scale(S.view.zoom,S.view.zoom);ctx.translate(-S.view.x,-S.view.y);const z=S.view.zoom;
   ...
   if(S.drag?.type==='draw')line(ctx,S.drag.points,z);if(S.tool==='piece'&&pointer){ctx.globalAlpha=.45;line(ctx,W.piece(S.piece,snap(pointer[0]),snap(pointer[1])).points,z);ctx.globalAlpha=1;}curves?.draw();ctx.restore();
  ...
  curves=BezierEditor.attach({S,canvas,ctx,W,changed,refresh,message,transform,fit});
  window.RouteWorkshop={get curves(){return curves},get active(){return S.active},get testing(){return S.testing},state:S,open,close,draw,handleKey,action,returnToDraft,refresh,changed};
