...
(function(){'use strict';
 function boot(){
  ...
  for(const c of host.querySelectorAll('[data-tile-thumb]')){try{drawTile(c.getContext('2d'),Number(c.dataset.tileThumb),4,4,0,false,null);}catch{}}
  ...
  function changed(label){
   ...
   clearTimeout(autosave);autosave=setTimeout(()=>{if(S.dirty&&!safeStore.set(recoveryKey,JSON.stringify({name:S.doc.name,code:current(),time:Date.now()})))message('Recovery storage is full. Export your draft to keep it.');},600);
  }
  ...
  function open(code){
   ...
   if(code){draftId=null;load(code);}
    ...
    if(recover?.code){load(recover.code);message('Recovered the last workshop draft. Your campaign saves are unchanged.');}
    else load(S.currentCode||GroundCampaign.encode(GroundCampaign.make(0,T)));
   }
   refresh();canvas.focus({preventScroll:true});draw();
  }
  function close(){clearTimeout(autosave);if(S.dirty)safeStore.set(recoveryKey,JSON.stringify({name:S.doc.name,code:current(),time:Date.now()}));S.active=false;host.hidden=true;host.inert=true;document.body.classList.remove('maker-open');resetKeys();}
  ...
  function action(a){try{
   if(a==='undo'||a==='redo'){S.doc=W.decode(S.history[a]());clearSelection();S.dirty=current()!==S.saved;refresh();return;}
  ...
  function draw(){if(!S.active||!S.doc||!S.dirtyFrame)return;S.dirtyFrame=false;ctx.setTransform(dpr,0,0,dpr,0,0);ctx.fillStyle='#112a38';ctx.fillRect(0,0,width,height);ctx.save();ctx.scale(S.view.zoom,S.view.zoom);ctx.translate(-S.view.x,-S.view.y);const z=S.view.zoom;
   ...
   for(let ty=Math.max(0,Math.floor(S.view.y/36));ty<Math.min(S.doc.h,Math.ceil((S.view.y+height/z)/36));ty++)for(let tx=Math.max(0,Math.floor(S.view.x/36));tx<Math.min(S.doc.w,Math.ceil((S.view.x+width/z)/36));tx++){const id=S.doc.cells[ty*S.doc.w+tx];if(!id)continue;const x=tx*36,y=ty*36;ctx.fillStyle=[1,2].includes(id)?'#4d6d7c':id===15?'#68d2ab':id===8?'#ecd184':id===60?'#79e4ed':id===4?'#e58b82':'#be9d6a';ctx.fillRect(x+1,y+1,34,34);if(z>.7){try{drawTile(ctx,id,x,y,0,false,null);}catch{}}if(z>.35){ctx.fillStyle='#112936';ctx.textAlign='center';ctx.font='bold 10px system-ui';ctx.fillText(id===15?'START':id===8?'GOAL':id===60?'PEG':id===63?'MAIL':id===7?'--':id===16?'BOT':String(id),x+18,y+22);}}
