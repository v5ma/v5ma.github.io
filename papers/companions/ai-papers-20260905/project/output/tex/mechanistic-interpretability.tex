...
\hypersetup{pdftitle={AI Mechanistic Interpretability and Self-Aware Networks: From Neural Decoding to Causal Self-Regulation},pdfauthor={Micah Blumberg}}
\title{\bfseries AI Mechanistic Interpretability and Self-Aware Networks:\\[0.3em]\large From Neural Decoding to Causal Self-Regulation}
\author{%
...
\end{abstract}
\noindent\textbf{Keywords:} AI mechanistic interpretability; Self-Aware Networks; causal intervention; neurofeedback; hybrid state; representation learning; GPT-2; Qwen; semantic preservation.

\section{Interpreting a representation requires its receiving computation}
...
The present paper develops that question through a sequence of explicitly bounded experiments. Its central modern-model contribution is a native hybrid-state study and a subsequent attempt to learn the correction without access to a corrected test state. The favorable state-transfer result is preserved alongside the failed learned operation. The companion \emph{AI Core Alignment Across Scales: Human-Guided Co-Regulation in Self-Aware Artificial Networks} uses shared data to examine authorization and recovery. Those data are not independent replications, and this paper does not own the companion's authority architecture.

Neither paper identifies token position with neural phase, residual vectors with brainwaves, attention weights with cortical connections, or expert routing with a thalamus. A proposed functional correspondence needs a concrete implementation on each substrate. The phrase \emph{Self-Aware Networks} names the motivating theory; it does not make the present model experiments evidence of subjective experience.

\section{The source-recovered question}

My May 2012 note distinguishes decoding brain signals into commands from transforming measured signals into light or sound that the receiving brain could learn to use [\hyperref[ref:1]{1}]. A neighboring retrospective source acknowledges uncertainty about how that interpretation would work [\hyperref[ref:2]{2}]. These qualifications belong in the genealogy.

The 2017 Android Jones/Neural Lace article develops a closed-loop idea: measure the current pattern and return a difference relative to a desired pattern [\hyperref[ref:3]{3}]. A later transcript asks how to determine that an optimization was genuinely good or better, rather than merely changed [\hyperref[ref:4]{4}]. Its machine transcript is not an audio-verified quotation, and its assistant-written header is not historical author testimony.

In a February 2025 working document, I explicitly named a proposed “Bio \& Mechanistic Interpretability” project [\hyperref[ref:5]{5}]. An Oscillatory Cortical Algorithm draft discusses biological decoding alongside artificial-network interpretation [\hyperref[ref:6]{6}]. The first occurrence recovered in local Git of the explicit 2025 title has a February 17, 2025 commit timestamp. This is not independently verified public custody.
...
\section{Existing achievements and the remaining distinction}

Kay and colleagues demonstrate a substantive neural-decoding achievement: identifying viewed natural images from measured human brain activity [\hyperref[ref:7]{7}]. Decoding is not thereby a demonstration of writing those experiences into the brain. The same distinction matters for artificial systems. Causal abstraction formalizes relationships between computational levels [\hyperref[ref:8]{8}], and circuit tracing uses interventions and computational graphs rather than relying only on verbal labels [\hyperref[ref:9]{9}]. These are strong existing methodologies, not passive baselines to be dismissed.

Gurnee and colleagues study representations available for verbal report, modulation and flexible downstream use [\hyperref[ref:10]{10}]. Their functional workspace question is distinct from a claim about subjective experience. Ji-An and colleagues use in-context neurofeedback to investigate reporting and control of activation patterns, with dependence on the chosen direction and examples [\hyperref[ref:11]{11}]. Aoki and colleagues pose a more restrictive privileged-access test and report no reliable control under that setting [\hyperref[ref:12]{12}]. Their difference motivates controlling what the model and an external observer can infer; it does not establish that one study tested every interpretation of the other.

ReFT already learns interventions on frozen language-model representations [\hyperref[ref:18]{18}]. RAVEL explicitly evaluates intended changes, isolation of unrelated attributes and generalization [\hyperref[ref:17]{17}]. The experiments here are not full reproductions or superiority tests of those methods. Their residual contribution is the conjunction of native hybrid-state support, a query-independent learned correction, whole generated answers, relational and collateral endpoints, and explicit authorization/restoration boundaries. Familiar algebra or a new collection of plots cannot establish novelty by itself.

Within the SAN corpus, \emph{The External Cortex Loop} concerns a human measurement-and-feedback protocol, \emph{The Network That Learns It Is a Network} concerns explicit network self-knowledge, and \emph{Reciprocal AI-Neuroscience Translation} develops the broader translation method. Their full-text ownership comparison is retained in the companion source record. The current object is narrower: whether a proposed explanation or correction predicts the intended operation in the original artificial receiving computation.

\section{Mathematical distinctions and experimental obligations}

\subsection{Decoding, causal use and prediction}
...
\subsection{A feedback loop need not identify endogenous control}
...
\section{The evidence that shaped the native-model test}

\subsection{Controlled learned networks and a preserved leakage repair}
...
\subsection{GPT-2: causal effect, learned correction and a baseline floor}

The pinned quantized GPT-2 small graph supplies actual causal attention, value/output routes, residual updates, feed-forward computation and retained keys/values [\hyperref[ref:14]{14},\hyperref[ref:19]{19},\hyperref[ref:21]{21}]. The static assay obtains 240 vocabulary readouts from 48 prompts. A selected whole-residual patch restores seven of 16 outputs, with 11 clean and zero corrupted outputs. Mean donor-versus-other logit contrast moves from -2.6633 to 1.3495, compared with 2.6633 clean. Matched norm and wrong-location controls do not restore the selected outputs. A contrast restricted to two names is not full-vocabulary answer accuracy, and this coarse-hook assay is not Wang and colleagues' complete circuit replication [\hyperref[ref:20]{20}].
...
\section{Mapping and intervening in a native hybrid decoder}

\subsection{Exact model and state}

The contemporary application executes the text-only decoder from Qwen3.5-0.8B, using pinned upstream and quantized ONNX-conversion revisions [\hyperref[ref:22]{22},\hyperref[ref:23]{23}]. The 24-layer graph combines 18 gated-delta layers with six full-attention layers. It has hidden width 1,024, 18 convolution-state tensors, 18 recurrent-state tensors and 12 key/value tensors. The retained states are part of the original executed graph, not a learned surrogate of its output. Gated-delta recurrence is established architecture [\hyperref[ref:24]{24}]; the related DeltaNet-2 formulation [\hyperref[ref:25]{25}] is not substituted for the executed model.
...
\caption{State and query boundaries in the native Qwen hybrid application. A single modified prefix is branched into independent queries. Selective transplantation retains the unselected original state groups; complete transplantation supplies the counterfactual groups together.}
\end{figure}

\subsection{Development failure and the frozen transfer result}
...
\begin{figure}[p]
\centering
\includegraphics[width=\textwidth,height=0.78\textheight,keepaspectratio]{../../figures/qwen35-hybrid-09-v2/hybrid-transfer-results.png}
\caption{Native hybrid-state transfer outcomes. Complete donor transfer succeeds in all eight direction-paired groups; attention-only transfer fails one. All conditions preserve the tested color answers. These are counterfactual-state controls, not learned corrections.}
\end{figure}
...
\subsection{Continuation, stopping and restoration}
...
\section{Learning the correction without a test donor}

\subsection{Dataset, fitting and intervention support}
...
\subsection{Results: state improvement without the requested relation}
...
\begin{table}[htbp]
\centering
\caption{Requested reversal and collateral preservation on 16 held-out prefixes. A complete group requires all six answers across opposite directions. The original condition's ordinary answers are correct; its role zeros indicate that it does not perform the requested reversal.}
...
\caption{Frozen learned-correction results on the native hybrid decoder. The two learned maps preserve the tested color but fail relational reconstruction. Oracle success is a counterfactual-state result, not learned repair. Counts are shared with the Core Alignment companion.}
\end{figure}
...
\subsection{Actual control sequence and reconstruction}
...
\section{Mathematical interpretation of the learned failure}

\subsection{Zero mean is not absence of structured information}
...
\subsection{A unique fit is not a semantic proof}
...
\subsection{The output boundary supplies a relevant sufficient condition}
...
\subsection{Formal and executable scope}
...
\appendix
\section{Exact execution and evidence boundaries}

Executed GPT-2 conversion, Xenova/gpt2: \texttt{bf2c7f02\allowbreak{}e0b826c6\allowbreak{}0d03af34\allowbreak{}1171bde2\allowbreak{}0893da66}.

Qwen upstream model revision: \texttt{2fc06364\allowbreak{}715b967f\allowbreak{}1860aea9\allowbreak{}cf387788\allowbreak{}75588b17}.

Executed Qwen ONNX conversion: \texttt{fafab72d\allowbreak{}87a9e6be\allowbreak{}3925b38c\allowbreak{}af48286d\allowbreak{}2838f2d0}.
...
\item\label{ref:1}\begingroup\interlinepenalty=10000 Blumberg, M. Neo Mind Cycle note, internally dated May 18, 2012. Unpublished research note; exact source and chronology in the companion genealogy ledger.\par\endgroup
\item\label{ref:2}\begingroup\interlinepenalty=10000 Blumberg, M. Retrospective Neo Mind Cycle note. Recalled year is not a verified creation date; source context retained in the companion genealogy ledger.\par\endgroup
\item\label{ref:3}\begingroup\interlinepenalty=10000 Blumberg, M. (2017). The brain as a special kind of hard drive. \href{https://medium.com/silicon-valley-global-news/the-brain-as-a-special-kind-of-hard-drive-4bd3a99dba4c}{Article}.\par\endgroup
\item\label{ref:4}\begingroup\interlinepenalty=10000 Blumberg, M. Machine-transcribed discussion of Neo Mind Cycle evaluation. Audio verification pending; transcript status and source identity in the companion ledger.\par\endgroup
\item\label{ref:5}\begingroup\interlinepenalty=10000 Blumberg, M. (2025). Working document proposing Bio \& Mechanistic Interpretability. Unpublished draft; fixation and publication dates are distinguished in the companion ledger.\par\endgroup
\item\label{ref:6}\begingroup\interlinepenalty=10000 Blumberg, M. Oscillatory Cortical Algorithm working draft, section 7.6. Exact historical version in the companion ledger; not asserted to be the latest published version.\par\endgroup
\item\label{ref:7}\begingroup\interlinepenalty=10000 Kay, K. N., et al. (2008). Identifying natural images from human brain activity. Nature. \href{https://www.nature.com/articles/nature06713}{Primary paper}.\par\endgroup
\item\label{ref:8}\begingroup\interlinepenalty=10000 Geiger, A., et al. (2025). Causal abstraction: A theoretical foundation for mechanistic interpretability. JMLR, 26(83), 1-64. \href{https://jmlr.org/papers/v26/23-0058.html}{Primary paper}.\par\endgroup
\item\label{ref:9}\begingroup\interlinepenalty=10000 Anthropic (2025). Circuit tracing: Revealing computational graphs in language models. \href{https://transformer-circuits.pub/2025/attribution-graphs/methods.html}{Primary methods}.\par\endgroup
\item\label{ref:10}\begingroup\interlinepenalty=10000 Gurnee, W., et al. (2026). Verbalizable representations form a global workspace in language models. \href{https://transformer-circuits.pub/2026/workspace/index.html}{Primary report}.\par\endgroup
\item\label{ref:11}\begingroup\interlinepenalty=10000 Ji-An, L., et al. (2025). Language models are capable of metacognitive monitoring and control of their internal activations. \href{https://arxiv.org/html/2505.13763v2}{Version 2}.\par\endgroup
\item\label{ref:12}\begingroup\interlinepenalty=10000 Aoki, K., et al. (2026). In-context neurofeedback: Can LLMs control their internal representations through privileged access? \href{https://arxiv.org/html/2609.00904v1}{Version 1}.\par\endgroup
\item\label{ref:13}\begingroup\interlinepenalty=10000 Sentence Transformers. all-MiniLM-L6-v2 model card. \href{https://huggingface.co/sentence-transformers/all-MiniLM-L6-v2}{Model documentation}.\par\endgroup
\item\label{ref:14}\begingroup\interlinepenalty=10000 OpenAI. GPT-2 reference model implementation. \href{https://raw.githubusercontent.com/openai/gpt-2/master/src/model.py}{Primary code}.\par\endgroup
\item\label{ref:15}\begingroup\interlinepenalty=10000 DeepSeek-AI (2026). DeepSeek-V4: Towards Highly Efficient Million-Token Context Intelligence. \href{https://arxiv.org/html/2606.19348v1}{Technical report}. Architecture mapping only; unresolved displayed-date metadata is not used for exact-priority claims.\par\endgroup
\item\label{ref:16}\begingroup\interlinepenalty=10000 Sutton, R. S., and Barto, A. G. (2018). Reinforcement Learning: An Introduction, second edition. MIT Press. \href{https://mitpress.mit.edu/9780262039246/reinforcement-learning/}{Publisher}.\par\endgroup
\item\label{ref:17}\begingroup\interlinepenalty=10000 Huang, J., Wu, Z., Potts, C., Geva, M., and Geiger, A. (2024). RAVEL: Evaluating Interpretability Methods on Disentangling Language Model Representations. ACL, 8669-8687. \href{https://aclanthology.org/2024.acl-long.470/}{Primary paper}.\par\endgroup
\item\label{ref:18}\begingroup\interlinepenalty=10000 Wu, Z., et al. (2024). ReFT: Representation Finetuning for Language Models. \href{https://arxiv.org/abs/2404.03592}{Primary paper}.\par\endgroup
\item\label{ref:19}\begingroup\interlinepenalty=10000 Xenova. GPT-2 ONNX conversion. \href{https://huggingface.co/Xenova/gpt2/tree/bf2c7f02e0b826c60d03af341171bde20893da66}{Executed revision}.\par\endgroup
\item\label{ref:20}\begingroup\interlinepenalty=10000 Wang, K., Variengien, A., Conmy, A., Shlegeris, B., and Steinhardt, J. (2022). Interpretability in the Wild: a Circuit for Indirect Object Identification in GPT-2 small. \href{https://arxiv.org/abs/2211.00593}{Primary paper}.\par\endgroup
\item\label{ref:21}\begingroup\interlinepenalty=10000 Hugging Face. How caching works. Transformers documentation. \href{https://huggingface.co/docs/transformers/main/en/cache_explanation}{Implementation explanation}. Mutable documentation inspected September 4, 2026; pinned local adapters define execution.\par\endgroup
\item\label{ref:22}\begingroup\interlinepenalty=10000 Qwen Team (2026). Qwen3.5-0.8B. \href{https://huggingface.co/Qwen/Qwen3.5-0.8B/tree/2fc06364715b967f1860aea9cf38778875588b17}{Pinned upstream model}.\par\endgroup
\item\label{ref:23}\begingroup\interlinepenalty=10000 ONNX Community. Qwen3.5-0.8B-ONNX-OPT. \href{https://huggingface.co/onnx-community/Qwen3.5-0.8B-ONNX-OPT/tree/fafab72d87a9e6be3925b38caf48286d2838f2d0}{Executed quantized conversion}.\par\endgroup
\item\label{ref:24}\begingroup\interlinepenalty=10000 Yang, S., Kautz, J., and Hatamizadeh, A. (2024). Gated Delta Networks: Improving Mamba2 with Delta Rule. \href{https://arxiv.org/html/2412.06464v1}{Version 1}.\par\endgroup
\item\label{ref:25}\begingroup\interlinepenalty=10000 Hatamizadeh, A., Choi, Y., and Kautz, J. (2026). Gated DeltaNet-2: Decoupling Erase and Write in Linear Attention. \href{https://arxiv.org/html/2605.22791v1}{Version 1}.\par\endgroup
