...
September 5, 2026 · Draft 9 · Native contemporary hybrid-state transfer and restoration; development manuscript, not a final preprint

### Abstract

How can limited human guidance remain consequential inside a system whose computation exceeds what its author can inspect? I develop a receiver-relative Self Aware Networks (SAN) formulation separating authority, intended meaning, useful behavior and correction of the receiving state. Working applications test mediated execution, learned proposals, provenance, GPT-2 correction learning and continuing model state. Their negative results remain explicit, including an inadequate GPT-2 semantic baseline. A new experiment executes a small 2026 Qwen3.5 hybrid model with attention, recurrent memory and convolution buffers. Its unedited baseline answers 48/48 new relation-and-color questions under a prospectively declared capitalization rule. Complete counterfactual state transfer reverses 32/32 role answers; attention-only transfer reverses 31/32 while preserving color, but fails one complete role pair. A complementary intervention retains identical attention memory yet changes one recipient answer. An actual signed-request workflow distinguishes stopping edits, restoring attention state and restoring complete state through twelve fresh answer inferences. Across 300 condition/workflow answers, separate-code native reconstruction and 76 cumulative tests support bounded execution claims. Thirty-one earlier Lean statements delimit authority, identification and evaluation; no formal proof certifies neural semantics. These results make state-aware correction testable on a contemporary architecture without demonstrating learned human-intent interpretation, reduced oversight or frontier-model alignment. Independent review, stronger matched-information learning and the full release process remain open.

Keywords: AI core alignment; human oversight; transformers; authorization; continual adaptation; Self Aware Networks.

Reading the staged evidence: the earlier results and their then-open limitations are retained as development history. Section 6.11 gives the current contemporary-model extension. A numbered draft is a substantive iteration, not certification that every research or release requirement is complete.
...
### 6.11 A contemporary hybrid receiver requires a complete restoration contract

The GPT-2 continuation diagnostic established persistence but not adequate task understanding. I therefore moved the next application to a small 2026 post-trained model, Qwen3.5-0.8B, using its pinned native quantized text graph [21,22]. Its continuing computation contains eighteen recurrent layers and six full-attention layers. The selected interface exposes eighteen convolution buffers, eighteen recurrent-memory tensors and twelve attention key/value tensors. An instruction to “restore the cache” is consequently incomplete unless it identifies which stores are included.

This distinction is not a new discovery of recurrent memory. Gated DeltaNet already combines adaptive decay, associative state updates and attention-based hybrids [23]; later work explicitly separates erasing from writing [24]. SAN motivates asking whether a legitimate correction remains effective through this organized receiving system. The implemented mechanisms and their inventors retain their own credit. Inference changes the recurrent state, but this experiment updates no trained base parameters and learns no correction policy.

Before intervention testing, a development-only baseline answered all eight person-role questions correctly. Its four capitalized color answers failed the prospectively specified case-sensitive comparison, producing the original score 8/12 and failing that gate. The 12/12 capitalization-insensitive diagnostic was post-hoc. I preserve both the failure and the decision it motivated: the new follow-up declares capitalization-insensitive scoring before inference, retains exact-case scores and uses new names, settings and colors. This is an explicit development revision, not a retrospectively repaired success claim.

The frozen follow-up crosses two settings and two name pairs, both role directions and two colors. Each of sixteen stories supplies three questions. The unedited model answers all forty-eight correctly under the declared rule; the complete opposite-role prefix control supplies all forty-eight correct counterfactual answers. Every condition uses the same question tokens, full-vocabulary greedy decoding and at most four answer tokens. Questions fork from a retained story state; a previously generated name is never fed into another question.

![Native hybrid state and the command-controlled continuation pathway](../../figures/qwen35-hybrid-09-v2/hybrid-state-architecture.png)

Figure 9. The model produces three kinds of state. Declared group replacement selects original or counterfactual values before the question is processed. A separate typed, authenticated workflow selects stop, partial restoration or complete restoration and then performs fresh inference. It does not teach the model to interpret authorization.

Six conditions retain original state, replace the complete state, or replace convolution, recurrent, attention KV, or convolution-plus-recurrent state. Complete donors reverse all thirty-two role answers. Attention-only donors reverse thirty-one; recurrent-only donors reverse two; convolution-plus-recurrent donors reverse one; convolution-only donors reverse none. All six conditions retain the correct color on sixteen questions. More demanding paired evaluation passes all six role-and-color answers in eight of eight groups for complete donors and seven of eight for attention-only donors. Neither count measures a learned controller's success: the donor receives the entire correct counterfactual story.

The authority application uses one of the sixteen roots. A signed swap is followed by a signed stop, a forged complete-restore request, a signed attention-only restore, a signed complete restore and an old signed replay. Four requests are accepted and two rejected. Each accepted request precedes three genuinely new question/answer inferences, twelve in total. Stop retains the changed state. Attention-only restoration retains donor convolution and recurrent values. Complete restoration selects the known original state. All twelve readouts reproduce their independently tabulated same-state controls, including identical first-step full-vocabulary logits. The public signing fixture and command meanings are supplied, not learned or production-hardened.

Let the model state be \(s=(C,R,K)\), with convolution, recurrent and attention components. The three operations are

\[
\operatorname{Stop}(C,R,K)=(C,R,K),\qquad
\operatorname{Restore}_{K}(s;s_0)=(C,R,K_0),\qquad
\operatorname{Restore}_{\mathrm{all}}(s;s_0)=s_0.
\tag{12}
\]

The first operation means that no further edit is applied; ordinary subsequent model computation still evolves the state. Equation (12) does not say that stopping freezes the network. Complete restoration gives the ordinary deterministic-continuation guarantee under identical future inputs and settings. Partial restoration gives no such general guarantee. A measured counterexample occurs in the garden/Emma–Liam/green case: original attention state is identical, but retaining counterfactual non-attention state changes the recipient answer from Liam to Emma. The state mixture is a declared intervention and need not be naturally reachable without editing. It nevertheless refutes a universal restoration guarantee over the tested intervention domain.

![Individual role transfer and complete role-and-color agreement](../../figures/qwen35-hybrid-09-v2/hybrid-transfer-results.png)

Figure 10. The original-state rows are scored against requested counterfactual roles, not against their own source facts; their unedited source-fact accuracy is 48/48. Complete-group scoring exposes the attention-only exception. These are four crossed descriptive strata, not independently sampled populations or an estimate of frontier-model alignment.

The study records 288 condition answers and twelve fresh post-request answers, requiring 776 native decoder calls including prefix and further generation steps. It checks 4,608 tensor-origin records and six authority events. Sixteen separate-code reconstruction batches match every recorded first-step logit vector by hash and every generated sequence; seventy-six application tests pass. The original runs take 244.36 seconds in total, serially, with maximum process commit below 0.59 GB. No base-weight training, independent review, original DeepSeek execution or human-oversight benefit is asserted.

The resulting alignment requirement is concrete: authorization must select the intended operation on the relevant receiving state, and useful consequences must still be measured. Attention-memory restoration is not automatically whole-system restoration, even when a few answers appear unchanged. The experiment does not solve natural-language scope, unavailable historical state, long-horizon learning or the larger human–machine capability gap. It makes those requirements testable on a contemporary architecture rather than leaving them as an anatomical analogy.
...
21. Qwen Team (2026). Qwen3.5-0.8B model card and configuration, revision 2fc06364715b967f1860aea9cf38778875588b17. [Pinned publisher source](https://huggingface.co/Qwen/Qwen3.5-0.8B/tree/2fc06364715b967f1860aea9cf38778875588b17).
22. ONNX Community. Qwen3.5-0.8B-ONNX-OPT, revision fafab72d87a9e6be3925b38caf48286d2838f2d0. [Executed quantized conversion](https://huggingface.co/onnx-community/Qwen3.5-0.8B-ONNX-OPT/tree/fafab72d87a9e6be3925b38caf48286d2838f2d0). Exact local graph, weight, tokenizer and runtime hashes accompany the experiment.
23. Yang, S., Kautz, J., and Hatamizadeh, A. (2024). Gated Delta Networks: Improving Mamba2 with Delta Rule. [Version 1 methods](https://arxiv.org/html/2412.06464v1). Established recurrent-state, gating and hybrid mechanisms; not reproduced training.
24. Hatamizadeh, A., Choi, Y., and Kautz, J. (2026). Gated DeltaNet-2: Decoupling Erase and Write in Linear Attention. [Version 1 preprint](https://arxiv.org/html/2605.22791v1). Related formulation and gate-ablation excerpts inspected; this is not the architecture executed here.
...
Draft 9 adds the pinned Qwen3.5 text-only application, a preserved development scoring failure, a prospectively frozen new-family state-transfer assay, actual post-command inference and two screen-reviewed figures. All 300 follow-up answers and 32 prefix outputs are reconstructed through separate code, with 76 cumulative tests passing. The 31 previously accepted Lean statements remain unchanged; the new state argument is not a new compiled theorem. The two papers share this experiment rather than counting it as independent replication. Independent source/science/statistical/code/proof review, stronger matched-information learned control, approved-format PDF, author readback and release authorization remain open.
