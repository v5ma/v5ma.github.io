...
September 4, 2026 · Draft 6 · Learned GPT-2 correction and paired-role audit; development manuscript, not a final preprint

### Abstract

How can a limited human contribution remain consequential inside a system capable of far more computation than that person can inspect? I develop a receiver-relative Self Aware Networks (SAN) formulation that separates a commitment's authority, referent, behavioral influence and subsequent correction. A versioned virtual-action application tests propagation, revocation and delegation; learned-proposer and document-origin extensions show that correct enforcement can coexist with missing competence or a mistaken referent. Conditional Lean proofs specify where attenuation and complete mediation apply, without certifying human-intent interpretation. An executed GPT-2 extension adds both original-model interventions and an online-trained correction layer. On sixteen new cases, internal feedback yields four correct recipient outputs, task feedback five and supervised fitting nine. A post-hoc paired-role check finds that the active learners never get both opposite roles correct in any of eight held-out families; their successful individual answers occur within same-name collapse. A second boundary authenticates feedback before it can change learned parameters. Accepting forged feedback increases prohibited proposals from eight to eleven while correct outputs remain four; the separate trusted monitor blocks every prohibited virtual execution. Parameter reload, reset and actual revocation of edit permission distinguish stored learning from current authority. Forty-two application tests and separate-code model reconstruction support these bounded experiments. Five additional finite Lean statements establish why aggregate accuracy can improve without preserving a required distinction. These results motivate joint measurement of usefulness, semantic distinctions and authority, not a claim of solved learned alignment, reduced human oversight or a new security primitive. Generalization across greater intelligence, natural human instructions and contemporary DeepSeek models remains to be tested.
...
The most useful next test is not merely a larger success bar. It must independently vary who the instruction concerns, which operation is intended and which contextual details should be irrelevant, then test paired distinctions and clarification under bounded supervision. The present active layer supplies a working learned-state and authorization testbed. It has not established reliable human-intent generalization, reduced human oversight, or contemporary DeepSeek alignment.
