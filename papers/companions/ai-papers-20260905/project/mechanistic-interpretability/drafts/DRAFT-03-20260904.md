...
The passive assay has implemented live, reversed, random, yoked, visible-only and constant-score inputs. The still-uncompleted active-control stage must compare live internal-state feedback, matched replay or yoked feedback, visible-information-only feedback, and no feedback under equal budgets, with a useful task outcome independent of the returned score. A matched external controller is essential for privileged-access claims. Context, external-memory, and weight resets must be distinguished to locate any retained improvement.
