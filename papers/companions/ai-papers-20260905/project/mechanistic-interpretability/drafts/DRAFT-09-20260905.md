...
Decoding a state, identifying its causal use and learning to regulate it are distinct achievements. I connect Self Aware Networks (SAN) and the Neo Mind Cycle's measurement–feedback–learning question to controlled original-model interventions, crediting established interpretability and representation-editing methods. Earlier applications include small-network prediction, passive-feedback controls, learned GPT-2 correction and cache-carried effects; their failures and baseline limitations remain visible. The new study executes a small 2026 Qwen3.5 hybrid model, whose state spans attention memory, recurrent memory and convolution buffers. A frozen follow-up has an adequate 48/48 unedited relation-and-color baseline. Complete counterfactual state transfer changes all 32 role answers as intended; attention-only transfer changes 31, preserves color and passes seven of eight complete role-and-color groups. Other state groups alter scores and occasionally answers without recovering coherent reversal. A concrete equal-attention-state/different-answer pair bounds what an attention-only explanation can guarantee. Four accepted signed commands also generate twelve new post-request answers, separating stopping from partial and complete restoration. Separate-code reconstruction checks all 300 condition/workflow answers; 76 tests pass. Thirty-one previously checked Lean statements constrain identification, authority and evaluation, not neural or biological truth. This is a measured modern-architecture extension, not unique semantic localization, privileged introspection, learned endogenous feedback or original DeepSeek replication.
...
Reading the staged evidence: the earlier results and their then-open limitations are retained as development history. Section 6.10 gives the current contemporary-model extension. A numbered draft is a substantive iteration, not certification that every research or release requirement is complete.
...
### 6.9 What persists when the receiving state is actually carried forward

The separate-prefix limitation in section 6.8 motivates a different experiment. Instead of inserting the same edit at each question, I apply it once at the recipient prompt's final residual state and retain the actual decoder key/value cache. Two fixed continuations then fork from that root: “ someone. The giver was” and “ someone. The color of the book was.” The name predicted at the root is not fed back. Neither a generated answer nor the desired recipient is thereby smuggled into the later input. No additional residual patch is applied during either continuation.

All thirty-two recipient prompts from the previous eight families are retained, including both giver roles and book colors. They were already exposed in the earlier assay, so this is diagnostic follow-up rather than another untouched evaluation set. The protocol, complete cases, arms and exact continuation tokenization were frozen before execution; none was subsequently filtered or reworded. There is no new correction-layer fitting or base-model weight change. The two branches share one root state but are not a single uninterrupted generated conversation, a free dialogue or an intervention on a fully identified world representation.

Autoregressive caching is an established implementation mechanism, not a new SAN finding [14,21]. In the inspected GPT-2 graph, each block computes the current token's attention keys and values before that block's later residual output. Consequently, an edit after block 8 cannot retroactively alter that token's keys and values in layers 0–8. It can affect layers 9–11. Earlier tokens' cached entries also remain unchanged. This architectural account was checked against the actual pinned quantized graph, not inferred from a generic transformer analogy.

The deterministic continuation claim is elementary. With fixed weights, inference settings, tokens, masks and position handling, write the decoder transition as

\[
(z_j,K_j)=F_\theta(x_j,K_{j-1}),\qquad
K_0=\widetilde K_0
\ \Longrightarrow\
(z_j,K_j)=(\widetilde z_j,\widetilde K_j)
\quad\text{for all fixed continuation steps }j.
\tag{15}
\]

The base step uses equal initial cache and equal first input. Equal outputs and next cache supply the induction hypothesis for the following step. This argument assumes a deterministic transition and that all relevant non-cache inputs and state are fixed; it is not a universal cross-runtime floating-point guarantee. It also does not claim that an opaque cache is semantically interpretable. It is ordinary conditional mathematics, not a new Lean-compiled statement or a new caching algorithm.

Ten arms compare no edit, the four frozen learned matrices, reflection, a residual donor, a complete-prefix donor, and two selective restorations of the internal-feedback edit. The complete-prefix donor supplies the cache computed from the opposite-giver source prompt, including richer history. One restoration replaces layers 9–11 with the unedited cache; the other replaces layers 0–8. The recorded initial recipient output remains the previously produced edited output in both restoration arms. Replacing a subsequent state does not retroactively replace that already produced readout.

All 4,608 cache-support records conform to the predicted route. Restoring the upper layers makes the root cache exactly equal to the original. Restoring the lower layers, already unchanged, leaves it exactly equal to the internally edited cache. Every corresponding continuation agrees with those controls: upper restoration reproduces all original output logits; lower restoration reproduces the internal-edited logits. The complete-prefix donor also exactly matches continuation from the separately computed opposite-giver case. These are substantial implementation checks, but they establish a known route and exact state restoration under supplied reference access—not semantic decoding, learned recovery of an unknown state, or broad self-regulation.

For the internal edit, the mean maximum absolute vocabulary-logit change relative to unedited continuation is 0.4210 at the giver readout and 0.5537 at the color readout. It changes no giver top token and two color top tokens. Color correctness is twenty-one of thirty-two without editing, twenty under internal editing and twenty-one under task editing. These are distinct quantities: a changed distribution, a changed native answer and a correct fact are not interchangeable. The color totals are not an improvement curve from section 6.8, whose wording and intervention history differ.

![Persistent cache influence and the baseline competence limitation](../../figures/gpt2-continuation-08/cache-persistence-and-baseline.png)

Figure 8. Upper: internal-edit influence carried through subsequent computation, with selective cache-restoration controls. Lower: unedited competence on the original facts. The zero natural-giver baseline limits interpretation of a joint semantic score. These are paired constructed cases in one checkpoint, not independent model replications or a test of human awareness.

The semantic probe has a decisive adequacy defect. The unedited model gives the natural giver correctly in zero of thirty-two cases: twenty-nine top outputs are outside the two candidate names, and three are the other person's name. Those three errors coincide with the nominal swapped-giver target even though no intervention occurred. Treating them as successful controlled role reversal would mistake baseline error for causal success. The complete reversed-prefix control also produces zero correct later-giver answers for its intended swapped history.

Every arm therefore receives zero on the complete six-readout criterion: initial recipient, later giver and later color across both role arrangements. This floor cannot distinguish an inadequate intervention from an inadequate model/prompt pairing. It is not evidence that SAN's full receiver-relative mechanism fails, nor does it erase the measured numerical persistence. Baseline adequacy is explicitly recorded as a post-hoc diagnostic; no competence threshold was secretly added to the frozen protocol. All native predictions, target alternatives and failed cases remain in the evidence package.

This distinction changes the next experiment. Before freezing new evaluation cases, a development-only stage must establish that the chosen model and question format can answer the original relations and attributes. Untouched families must then test whether an intervention preserves that competence while producing the intended relational change. A contemporary instruction-capable model is directly relevant here: an older completion model with a zero baseline cannot substitute for today's systems merely because both use transformer components. Adequate baselines and stronger matched-information methods matter more than increasing the number of similarly uninformative GPT-2 probes.

The first family also executes four signed-request workflows. Each newly authorizes and computes the root edit, accepts an instruction to stop future edits, rejects a forged restoration request, accepts restoration of the original source-conditioned cache and rejects an old signed replay. Both continuations run freshly after each accepted stop or restore request. The twenty actual store events therefore include sixteen new post-request model readouts rather than selecting previously measured answers. Stopping further edits preserves the changed cache; explicitly restoring the original state restores the original continuation. These are different requested operations, not an assumed requirement that prospective revocation erase all past influence. No cache restoration can retract an answer already shown or an external action already taken.

Eight serial one-thread batches took 234.08 seconds in total, with 228 actual root readouts and 656 actual continuation readouts: 884 measured outputs over 5,252 incremental decoder steps. Tabulated primary root rows include reuse and are not independent inferences. Separate-code checks reconstruct all targets, cache support, event decisions and native readouts with zero discrepancy in compared logits and saved states. Sixty-one cumulative application tests pass. The audit shares the validated adapter and a frozen prior formula; it is not an independent implementation, independent scientific review or a new full-pipeline byte-identical replay.

The broader research question survives this stricter account intact. A learned receiver may make feedback useful; the present implementation demonstrates only some necessary computational and authorization distinctions. Persistent influence must be evaluated alongside competent behavior, legitimate scope, informative comparisons and continued human correction. No one of those properties establishes the others.

### 6.10 From GPT-2 cache to a contemporary hybrid state

A baseline-inadequate probe cannot discriminate semantic control. The next application therefore uses a small 2026 post-trained Qwen3.5-0.8B model through a pinned native quantized ONNX text graph [22,23]. This is actual model execution, not a diagram substituting for an experiment. The selected model has dense feed-forward layers and a hybrid sequence-processing stack; it is not DeepSeek, a distilled R1 model or a mixture-of-experts test. Its interface has eighteen convolution buffers, eighteen recurrent-memory tensors and twelve attention KV tensors.

The architecture belongs to established work. Gated DeltaNet provides adaptive recurrent updates and hybridization with attention [24]. Its fast-weight view distinguishes evolving inference state from fixed learned base parameters. Gated DeltaNet-2 further separates erase and write decisions [25], but that newer formulation is not what the selected checkpoint implements. Neither an existing recurrence nor a tensor-group transplant should be attributed to SAN as a new algorithm. SAN's receiver-relative question instead asks which state and route make a particular difference consequential.

Development exposed an evaluation detail before the new intervention study: all eight name answers were exact, while four correct color words were capitalized. The original case-sensitive score was 8/12 and its gate failed. A post-hoc case-insensitive diagnostic gave 12/12. The separately frozen follow-up therefore declares case-folded answer comparison in advance and keeps exact-case scores as a secondary outcome. No punctuation removal, aliases, restricted-name decoding, selected-case omission or rewrite of the original results is allowed.

The new fixture uses Emma/Liam and Nora/Owen in library and garden settings, with green and yellow books and both giver directions. These are sixteen stories in four crossed descriptive strata. Each story is followed by giver, recipient and color questions. Greedy decoding uses all 248,320 output logits and at most four tokens, stopping on either configured EOS. The original model answers all forty-eight questions correctly under the declared comparison. Full counterfactual prefixes also support all forty-eight corresponding answers. Thus this specific task, unlike the earlier natural-giver continuation, has an adequate unedited baseline.

I place the intervention boundary after the story and before the question. Each original and reversed-role donor prefix has equal token length. The native graph computes both states. Six conditions then retain original state or transplant all, convolution, recurrent, attention KV, or convolution-plus-recurrent tensors from the donor. Every condition receives the identical question continuation. All three questions are separate branches of one retained root; generated answers do not become new evidence in another branch.

![The measured hybrid-state intervention boundary](../../figures/qwen35-hybrid-09-v2/hybrid-state-architecture.png)

Figure 9. Original native model interfaces, not a reimplemented token mixer, supply all three state groups. Group selection occurs before the question. The separate command workflow uses the same state boundary and new subsequent model calls. Restoring attention entries alone leaves other state stores in place.

Complete-state donors reverse all thirty-two giver/recipient answers. Attention-only donors reverse thirty-one; recurrent-only donors reverse two; convolution-plus-recurrent donors reverse one; convolution-only donors reverse none. Color remains correct on all sixteen questions in every arm. A complete group requires both role directions and their unchanged colors: complete donors pass eight of eight groups, attention-only donors seven of eight, and the remaining conditions none. These paired outcomes prevent two isolated recurrent-arm changes from being mistaken for coherent relational transfer.

The attention-only exception is informative. For the green-book garden case with Liam giving to Emma, the transfer produces Emma for both giver and recipient rather than Emma and Liam. The complementary intervention—donor convolution and recurrent state with original attention—also has an exception in the opposite direction: its recipient becomes Emma rather than Liam. State groups interact. The stronger claim that the relation has been uniquely located in attention memory does not follow from thirty-one successes.

Even conditions that preserve the decoded word change the full vocabulary scores. Every non-original condition has a different first-step logit hash on all forty-eight questions. Convolution-only transfer preserves all source-fact answers while changing mean maximum logit differences by about 0.11 for giver, 0.09 for recipient and 0.08 for color. A stable top word is therefore weaker than identical computation. Conversely, a numerical difference alone does not establish a meaningful or useful changed answer.

Let \(F(s,q)\) denote a declared continuation readout from complete state \(s\) and future token sequence \(q\). For a state projection \(\pi\) and a specified continuation set \(Q\), sufficiency requires

\[
\forall s,t,\quad
\pi(s)=\pi(t)\ \Longrightarrow\
\bigl(\forall q\in Q,\ F(s,q)=F(t,q)\bigr).
\tag{16}
\]

This is ordinary behavioral sufficiency, not a new consciousness theorem. If the condition holds, a readout on a projection value can be defined by choosing any representative state: equality makes the result independent of that choice. Conversely, if the readout factors through the projection, equal projections give equal readouts. A single equal-projection/different-readout pair disproves such factorization on a domain containing that pair. The original-versus-convolution-plus-recurrent intervention supplies a measured KV-projection counterexample for the garden recipient question. Its mixed state is included in the declared intervention domain; this does not prove a statement about every naturally reached state. The argument adds no new Lean statement or compiler-verified claim about neural inference.

![Paired behavioral tests of native hybrid-state transfer](../../figures/qwen35-hybrid-09-v2/hybrid-transfer-results.png)

Figure 10. Complete donors have privileged counterfactual information. Partial groups differ in dimension, position and function, and mixed states may be off-manifold. These bars characterize this intervention family; they are not a group-size-normalized importance ranking, learned-edit benchmark or population confidence interval.

A separate first-root workflow executes four accepted signed changes and rejects one forged update and one old replay. Twelve fresh answer inferences follow accepted swap, stop, attention-only restore and complete-restore commands. Their full first-step logit hashes and generated sequences match the corresponding previously measured state controls. On that particular root, partial restoration recovers the three original words but not their complete logit vectors; complete restoration recovers both. The broader transfer data, rather than this single workflow alone, reveal the recipient exception. Authenticated command handling is externally implemented; no endogenous intent interpretation or learned neurofeedback benefit is inferred.

All 288 condition answers, twelve post-request answers, 4,608 tensor-origin rows and six authority events are retained. Sixteen separate-code native audits reconstruct every answer and initial prefix state, while seventy-six cumulative application tests pass. The reconstruction uses a different interchange/generation loop but shares the native adapter, model and authoring process, so it is not independent review or upstream full-precision equivalence. The study requires no new model training and saves no repeated full caches. It adds a baseline-competent contemporary hybrid measurement to the existing GPT-2 experiments without claiming transfer to original DeepSeek or a solution to general self-regulation.
...
Current update: section 6.10 now supplies the baseline-competent contemporary hybrid assay proposed after section 6.9. The remaining modern-model obligation is stronger matched-information learned control and wider transfer, not repeating the already completed intake. The following earlier-stage requirements remain part of the cumulative research record.
...
21. Hugging Face. Transformers documentation: Caching. [Primary implementation explanation](https://huggingface.co/docs/transformers/main/en/cache_explanation). Explanatory page inspected September 4, 2026; mutable documentation, not a pinned checkpoint or new algorithm.

22. Qwen Team (2026). Qwen3.5-0.8B model card and configuration, revision 2fc06364715b967f1860aea9cf38778875588b17. [Pinned publisher source](https://huggingface.co/Qwen/Qwen3.5-0.8B/tree/2fc06364715b967f1860aea9cf38778875588b17).
...
24. Yang, S., Kautz, J., and Hatamizadeh, A. (2024). Gated Delta Networks: Improving Mamba2 with Delta Rule. [Version 1 methods](https://arxiv.org/html/2412.06464v1). Established recurrent-state, gating and hybrid mechanisms; not reproduced training.
25. Hatamizadeh, A., Choi, Y., and Kautz, J. (2026). Gated DeltaNet-2: Decoupling Erase and Write in Linear Attention. [Version 1 preprint](https://arxiv.org/html/2605.22791v1). Related formulation and gate-ablation excerpts inspected; this is not the architecture executed here.
...
Draft 8 adds actual cache-carried continuations, selective restoration and fresh post-request inference on previously exposed cases. It preserves the inadequate natural-giver baseline instead of treating joint-score zeros as an intervention-specific semantic verdict. Sixty-one application tests, eight native-audit receipts and one screen-reviewed data figure support this bounded stage. No new training or Lean statement was added; the accepted total remains 31. Independent review, baseline-competent contemporary-model evaluation, approved-format PDF and author publication readback remain open. The next protocol must validate task competence on development-only material before freezing untouched semantic tests.
