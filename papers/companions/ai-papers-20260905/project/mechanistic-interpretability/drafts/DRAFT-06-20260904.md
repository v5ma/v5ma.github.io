...
Decoding a state, identifying its causal use and teaching a system to regulate it are different achievements. I develop a Self Aware Networks (SAN) research program connecting these distinctions to the Neo Mind Cycle's measurement–feedback–learning loop, with dated source recovery and explicit credit to earlier decoding and interpretability methods. Initial small-network experiments expose donor leakage and show a standard full-state Jacobian outperforming the proposed restricted estimator. Formal countermodels distinguish observation from intervention and stimulus-driven state change from privileged control. A pretrained GPT-2 application then executes original-model residual interventions and learns a constrained correction layer under internal, task, yoked and constant feedback. Eight training cases fit shared preprocessing and twenty adaptive coefficients; sixteen new cases in eight paired families supply native-vocabulary evaluation. Internal and yoked feedback each yield four correct answers, task feedback five and supervised fitting nine. A post-hoc diagnostic reveals that every active learner's success arises from the same name being returned for both opposite roles: none recovers both roles in any held-out family. Five new Lean-checked finite statements explain how accuracy can improve without this distinction being restored. Authenticated feedback, parameter reload, reset and actual edit revocation are also tested; accepting forged feedback increases prohibited proposals despite unchanged accuracy. Separate-code reconstruction and forty-two application tests support the implementation record, not independent scientific review. The result is a concrete transformer measurement and failure-control study, not a new representation-editing family, demonstrated privileged introspection, or successful general SAN self-regulation. Modern instruction-tuned and DeepSeek replication remain open.
...
The passive assay has implemented live, reversed, random, yoked, visible-only and constant-score inputs. The active calibration stage now compares internal-state, task, yoked and constant feedback and a matched external controller, with useful behavior measured separately. The GPT-2 correction-layer stage now tests active parameter updates, but fails the stronger paired-role criterion. The next stage must match observation as well as query budgets, distinguish context/KV, external-memory and parameter resets, and determine whether any improvement transfers beyond the task that supplied the feedback.
