...
from probes import ROOT,np,standardize,initialize,forward,loss_grad,fit_mlp,ridge,ridge_predict,relation,component_metrics,ReadoutDeadline
...
def main():
    ...
    try:
        fit_mlp(sx,sy,sx,sy,2718,dict(small,maximum_seconds_per_fit=-1))
        retained=False
    except ReadoutDeadline as exc:
        retained=exc.receipt['completed_epochs']==0 and len(exc.arrays)==12
    ck('deadline_retains_parameters_and_optimizer',retained)
    a,b=io.BytesIO(),io.BytesIO(); np.savez(a,**best); np.savez(b,**best)
    ck('deterministic_parameter_archive',a.getvalue()==b.getvalue())
    out=ROOT/'TESTS-02.json'
