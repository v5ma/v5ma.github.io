...
class ReadoutDeadline(TimeoutError):
    def __init__(self,p,m,v,logs,seed):
        super().__init__('Readout exceeded its fixed resource cap; parameters retained')
        self.arrays={**{'parameter_'+k:a.copy() for k,a in p.items()},
                     **{'m_'+k:a.copy() for k,a in m.items()},
                     **{'v_'+k:a.copy() for k,a in v.items()}}
        self.receipt={'seed':seed,'completed_epochs':len(logs),'logs':logs,'complete':False}
...
def fit_mlp(x,y,dx,dy,seed,cfg):
    ...
    for epoch in range(1,cfg['epochs']+1):
        if time.perf_counter()-began>cfg['maximum_seconds_per_fit']:
            raise ReadoutDeadline(p,m,v,logs,seed)
