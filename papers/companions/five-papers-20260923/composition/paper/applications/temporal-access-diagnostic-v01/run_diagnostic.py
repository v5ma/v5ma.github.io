...
from probes import ROOT,DONOR,np,standardize,fit_mlp,ridge,ridge_predict,forward,relation,component_metrics,ReadoutDeadline
...
def study_view(name,features,targets,cfg,archive,parameters,rows,clock,control=False):
    ...
    for seed in cfg['probe_seeds']:
        ...
        except ReadoutDeadline as exc:
            exc.receipt['view']=name
            raise
...
def run(out_name):
    ...
    tests=json.loads((ROOT/'TESTS-02.json').read_text(encoding='utf-8'))
    ...
    source_hashes={n:sha(ROOT/n) for n in (*SOURCES,'TESTS-02.json')}
    out.mkdir()
    ...
    PARTIAL_STATE.update(out=out,rows=rows,archive=archive,parameters=parameters)
...
if __name__=='__main__':
    ...
    try:
        ...
        if PARTIAL_STATE:
            out=PARTIAL_STATE['out']
            arrays=dict(PARTIAL_STATE['parameters'])
            if isinstance(exc,ReadoutDeadline):
                arrays.update({'interrupted__'+k:v for k,v in exc.arrays.items()})
            np.savez(out/'partial-parameters.npz',**arrays)
            np.savez(out/'partial-predictions.npz',**PARTIAL_STATE['archive'])
            save(out/'DEADLINE-PARTIAL.json',{'complete':False,'error':str(exc),
                 'completed_rows':PARTIAL_STATE['rows'],
                 'interrupted':exc.receipt if isinstance(exc,ReadoutDeadline) else None})
        raise
