...
def main(version):
    ...
    ck('query2_and_query3_equal_task_counts',all(np.bincount(meta[(meta[:,0]==si)&(meta[:,1]==ci)&(meta[:,5]==s),6],minlength=8).tolist()==[128]*8 for si in range(3) for ci in range(len(CONDITIONS)) for s in (1,2)))
    ...
    for start in range(0,len(meta),3):
        ...
        allowed=cfg['first_roles'];position=(block+second)%len(allowed);first=allowed[position]
        if first==second:first=allowed[(position+1)%len(allowed)]
        assert meta[start:start+3,6].tolist()==[first,second,(second+cfg['third_role_offset'])%8]
    ...
    ck('every_role_sequence_recomputed_without_target',trace_rows==len(meta))
    summaries=[]
