...
On Windows use a new folder on D:, for example `D:/research-checks/paper-112-run1`. The launcher refuses to write within the frozen companion or overwrite an existing run. It caps numerical libraries to one CPU thread and uses below-normal priority on Windows.
