...
def main():
    ...
    parser.add_argument("--output-name", required=True, help="New direct child directory; never overwrite a recorded run")
    args=parser.parse_args()
    allowed="abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-_"
    assert 1<=len(args.output_name)<=64 and all(ch in allowed for ch in args.output_name), "Use a simple directory name"
    assert args.output_name.lower() not in {"con","prn","aux","nul",*(f"com{i}" for i in range(1,10)),*(f"lpt{i}" for i in range(1,10))}, "Reserved directory name"
    started=time.perf_counter()
    priority=low_priority()
    plan_path=BASE/"ANALYSIS-PLAN.json"
    plan=json.loads(plan_path.read_text(encoding="utf-8"))
    out=BASE/args.output_name
    assert out.resolve().parent==BASE.resolve(), "Output must remain in this application"
